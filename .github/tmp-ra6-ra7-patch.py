...
# RA6: close the mixed-partial symmetry argument instead of treating it as known.
text = RA6.read_text(encoding='utf-8')
new_ra6_marker = '### 混合偏微分の交換を確認する'
if new_ra6_marker not in text:
    start_marker = '二階偏微分が近傍で連続なら、混合偏微分は交換できるためHessianは対称になります。このとき\n'
    end_marker = '高階も同様に、$D^{k-1}f$ を作用素値写像とみなし、そのFréchet微分として $D^kf$ を定義します。'
    assert text.count(start_marker) == 1, text.count(start_marker)
    assert text.count(end_marker) == 1, text.count(end_marker)
    start = text.index(start_marker)
    end = text.index(end_marker, start)
    insert = r'''二階偏微分が近傍で連続するとき、Hessian が対称になる根拠もここで閉じます。
...
# RA7: make the shear step self-contained at the Jordan-content level.
text = RA7.read_text(encoding='utf-8')
new_shear_marker = 'ここは「断面の長さが同じ」と言うだけではなく、Jordan内容の定義まで戻って確認します。'
if new_shear_marker not in text:
    old = '3. **shear**：$x_i\\mapsto x_i+c x_j$、他の座標は固定する変換では、$x_j$ と残りの座標を固定した断面の $x_i$ 方向の長さは平行移動するだけで変わりません。矩形の有限和について反復積分すれば体積は不変です。\n\nJordan可測集合 $E$ は、境界を総体積任意小の矩形で覆うことで、内側・外側の有限矩形和に挟めます。上の三種類の写像ではその誤差矩形の総体積も固定定数倍にしかならないため、矩形和で成立した体積倍率を分割誤差を0へ送って $E$ へ移せます。\n\n'
    assert text.count(old) == 1, text.count(old)
    new = r'''3. **shear**：$S(x)_i=x_i+c x_j$、他の座標は固定する変換を考えます。ここは「断面の長さが同じ」と言うだけではなく、Jordan内容の定義まで戻って確認します。
...
# RA7: keep the eight problems, but bring every solution to the RA6 explanation standard.
if '**詳細解答**' not in text[text.index('## 10. 演習A'):text.index('## 13. 章末チェック')]:
    ex_start = text.index('## 10. 演習A')
    ex_end = text.index('## 13. 章末チェック', ex_start)
    exercises = r'''## 10. 演習A
...
'''
    text = text[:ex_start] + exercises + text[ex_end:]
