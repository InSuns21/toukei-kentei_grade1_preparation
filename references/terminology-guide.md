...
| 正規閉包 | normal closure | 有限拡大 $E/F$ を含む有限正規拡大のうち、固定した代数閉包内で包含関係に関して最小のもの |
...
| 可解群 | solvable group | 導来列が有限回で自明群へ到達する群 |
| 単純群 | simple group | 非自明で、非自明な真の正規部分群を持たない群 |
| 根号拡大 | radical extension | 各段階で $alpha^n$ が直前の体に属する元 $alpha$ を有限回添加して得る体拡大 |
| 根号による可解性 | solvability by radicals | 多項式の全ての根が基礎体から有限回の根号添加で得られるある体に含まれる性質 |
...
| 分散減少法 | variance reduction | 推定対象の期待値を保ちながら、相関・補助変数・層・提案分布などを設計して Monte Carlo 推定量の分散を小さくする方法の総称 |
| 対称変量法 | antithetic variates | 同じ周辺分布を持つ負に相関した標本を組にして分散を減らす方法。本文では antithetic を主表記にしない |
| 制御変量法 | control variates | 期待値が既知で推定対象と相関する補助変数を用い、既知平均からのずれを差し引く分散減少法 |
