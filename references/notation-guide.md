...
- 独立は事象なら $A\perp B$、確率変数なら $X\perp Y$、条件付き独立は $A\perp B\mid C$ または $X\perp Y\mid Z$ と書く。ここで $\perp$ は確率論上の独立を表す記号で、ベクトルの直交を表す記号として使っているわけではない。文献で $\perp\!\!\!\perp$ と書く流儀もあるが、本教材では $\perp$ に統一する。
