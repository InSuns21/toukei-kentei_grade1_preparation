...
# Compact-image theorem moved from the old C lecture to current C1.
s = b1.read_text()
s = s.replace(
...
s = p.read_text()
s, n1 = re.subn(
    r'さらに \[F0-02B\]\(\.\./F0_02B_分離超平面定理_Farkas_SVM/index\.md\) では、SVMを別の幾何からも見ます。',
    'さらに [F0-02B1](../F0_02B1_SVM_凸包_最大マージン/index.md) では、SVMを別の幾何からも見ます。',
    s,
    count=1,
)
if n1 != 1:
    raise SystemExit(f'02A SVM bridge replacements={n1}')
s, n2 = re.subn(
    r'Farkasの補題の証明、分離超平面定理、SVMの凸包・最近点対・最大マージンまでさらに遡る場合は \[F0-02B 分離超平面定理・Farkasの補題\]\(\.\./F0_02B_分離超平面定理_Farkas_SVM/index\.md\) へ進んでください。',
    'Farkasの補題の証明・分離超平面定理まで遡る場合は [F0-02B 分離超平面定理・Farkasの補題](../F0_02B_分離超平面定理_Farkas_SVM/index.md) へ、SVMの凸包・最近点対・最大マージンを見る場合は [F0-02B1 SVM・凸包・最大マージン](../F0_02B1_SVM_凸包_最大マージン/index.md) へ進んでください。',
    s,
    count=1,
)
if n2 != 1:
    raise SystemExit(f'02A final route replacements={n2}')
p.write_text(s)

# Facade: authoritative numbering for the finite optimization / functional analysis route.
