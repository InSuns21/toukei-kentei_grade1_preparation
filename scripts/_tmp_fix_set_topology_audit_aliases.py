...
def patch(dirname, replacements):
    ...
    for old, new in replacements:
        if old not in text:
            raise SystemExit(f'missing pattern in {dirname}: {old[:100]!r}')
        text = text.replace(old, new, 1)
    p.write_text(text, encoding='utf-8')

patch('F0_00A_集合_写像_上限下限', [
...
patch('F0_00A1_上界_下界_supremum_infimum', [
...
patch('F0_00A1D_順序_全順序_最小最大_整列', [
    ...
    ('半順序の反対称律から $m=m\'$. 従って最小元は高々一つ。',
     '反対称律から $m=m\'$. 従って最小元は高々一つ。'),
])

patch('F0_00B0_点列_部分列_十分大きい添字', [
...
patch('F0_00C_連続写像_コンパクト性_最大最小', [
    ...
    ('$x\\in\\overline A$ を取る。距離空間では閉包の点列特徴付けにより、ある列 $a_n\\in A$ が存在して $a_n\\to x$。連続性から\n$$\nf(a_n)\\to f(x).\n$$\n各 $f(a_n)\\in f(A)$ なので、再び閉包の点列特徴付けから $f(x)\\in\\overline{f(A)}$。従って包含が成り立つ。',
     '$x\\in\\overline A$ を取る。各 $n$ について $B(x,1/n)$ は $x$ の近傍だから、閉包の定義より\n$$\na_n\\in A\\cap B(x,1/n)\n$$\nとなる点を一つ取れる。すると $a_n\\to x$。連続性から\n$$\nf(a_n)\\to f(x).\n$$\n任意の $f(x)$ の近傍には十分大きい $n$ で $f(a_n)$ が入り、しかも $f(a_n)\\in f(A)$ である。従ってすべての $f(x)$ の近傍が $f(A)$ と交わり、閉包の定義から $f(x)\\in\\overline{f(A)}$。よって包含が成り立つ。'),
...
patch('F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel', [
