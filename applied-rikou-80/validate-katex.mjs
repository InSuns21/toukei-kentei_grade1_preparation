...
const forbidden = [
  ...
  [/\\(?:boldsymbol|mathbf|mathbb)\s*(?:\{1\}|1)\s*\\\{/g, String.raw`指示関数の条件は添字で書く（例: \boldsymbol{1}_{\{A\}}）`],
];
...
for (const file of files) {
  ...
  for (const [pattern, label] of forbidden) {
    ...
    if (match) {
      errors.push(`${relative(file)}:${lineAt(searchable, match.index)} 禁止記法 ${label}`);
    }
  }
...
if (errors.length > 0) {
  console.error(`applied-rikou-80 の数式検証で ${errors.length} 件の問題が見つかりました:`);
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log(`${files.length} 個の applied-rikou-80 Markdown ファイルを KaTeX strict で検証しました。`);
