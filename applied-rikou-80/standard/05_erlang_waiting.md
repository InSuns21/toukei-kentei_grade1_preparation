...
4. 率 $\lambda$ のポアソン過程の第3到着時刻との関係を説明せよ。
5. $\lambda=2,t=1$ で $P(S_3\le1)$ をポアソン確率で表せ。

## 詳細解答

### 1. $S_3$ の密度

「独立な指数分布の和はガンマ分布」とだけ置くと、この設問で求められている密度導出を飛ばしてしまう。ここでは畳み込みから確認する。
...
これは形状母数3、率母数 $\lambda$ のガンマ分布、整数形状なのでErlang分布の密度である。

### 2. 生存確率 $P(S_3>t)$

第4問のポアソン過程との対応を使うと計算が短い。率 $\lambda$ のポアソン過程で到着間隔が独立な率 $\lambda$ の指数分布なら、
...
また $S_3$ は率 $\lambda$ のポアソン過程の第3到着時刻と同分布なので
