...
理論ACF/PACFの打ち切りは母集団の性質であり、標本ACF/PACFには標本変動がある。そのためグラフだけで次数を断定せず、ACF/PACFで候補を作り、AIC等でモデル比較し、残差ACFやLjung–Box型診断などで残差が白色雑音的か確認する。
...
根 $1/2,2$ のうち **可逆条件** $|\theta|<1$ から $\theta=1/2$。有限標本では標本ACF/PACFは厳密に打ち切れないのでAIC・残差診断を併用する。
