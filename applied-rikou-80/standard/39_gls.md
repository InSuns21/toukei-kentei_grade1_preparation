...
したがって誤差共分散が $\sigma^2I$ でなくても不偏性は残る。一方、通常形のガウス・マルコフ定理が最小分散性を保証するには誤差共分散が $\sigma^2I$ であることが必要である。本問では $\operatorname{Var}(\varepsilon)=\sigma^2\Omega$ なので、一般には通常最小二乗推定量が最良線形不偏推定量とは限らない。
