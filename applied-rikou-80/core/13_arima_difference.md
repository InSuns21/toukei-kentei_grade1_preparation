...
自己回帰側に因子 $1-B$、すなわち単位根1を持つ。差分を累積するとショックの効果が水準に永久に残るため、$X_t$ の分散は時点とともに増加し一定にならない。
...
$\Delta X_t=\varepsilon_t+0.4\varepsilon_{t-1}$ より 自己回帰和分移動平均$(0,1,1)$。$1-B$ が単位根を与えるため $X_t$ は非定常。
