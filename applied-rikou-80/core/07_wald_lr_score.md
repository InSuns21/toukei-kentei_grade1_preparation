# Core 07 最尤推定量の漸近正規性・Wald/尤度比/Score検定
...
1. 最尤推定量 $\hat p$ と1標本当たりの Fisher 情報量 $I_1(p)$ を求めよ。
...
### 2. Wald：中心極限定理とSlutskyの条件

$H_0$ の下では $X_i$ は独立同分布 Bernoulli$(p_0)$、
...
また大数の法則で $\hat p\to_p p_0$。$p_0$ は $(0,1)$ の内部点なので $p_0(1-p_0)>0$、従って **Slutskyの定理**で分母をplug-inでき、
...
これは上の中心極限定理で標準化した量の二乗なので $S_c\Rightarrow\chi^2_1$。

尤度比については **Wilksの定理**を使う。必要条件は、真値が母数空間内部、モデルが識別可能かつ十分滑らか、情報量が有限正であること。本問は
...
有限標本ではWaldは $\hat p$、Scoreは $p_0$ で曲率を評価し、尤度比は尤度差を見るため一致しない。

$H_0$ 下で中心極限定理から $\hat p-p_0=O_p(n^{-1/2})$。スコア方程式を $p_0$ 周りでTaylor展開すると
...
$\hat p=S/n$, $I_1(p)=1/[p(1-p)]$。$H_0$ では独立同分布有限正分散なので **中心極限定理**、大数の法則と $0<p_0<1$ から **Slutsky**を使え、
...
- (1) 最尤推定量・情報量: 4点
- (2) Wald（中心極限定理・Slutsky条件）: 4点
- (3) Score・尤度比・Wilks条件: 6点
- (4) 正しい厳密式: 4点
- (5) 局所2次近似: 2点

25分経過時は「中心極限定理/Slutskyの条件」「Wilksの内部点・正則性」を一行ずつ残す。
