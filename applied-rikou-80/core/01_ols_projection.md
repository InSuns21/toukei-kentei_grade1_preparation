...
- 難度: B
- 目安時間: 25分
- 位置付け: 行列によるOLSを、理工学午後問題で使う「推定・診断・予測」の道具として扱う
...
3. $\mathrm{SSE}=e^\top e$ について

$$
\mathrm{SSE}=y^\top y-\widehat\beta^\top X^\top y
$$

を用いて $\mathrm{SSE}$ と不偏分散推定量
...
午後問題で重要なのは、$H$ の冪等性を証明すること自体より、**OLSが説明変数で説明できる成分 $Hy$ と、説明できず直交方向へ残る成分 $e$ にデータを分けている**と理解することである。
...
$$
\mathrm{SSE}=30-(1,2,-1)(6,8,-4)^\top=4,
$$
