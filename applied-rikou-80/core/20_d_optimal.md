...
単回帰の D 最適 exact design は端点 $-1,+1$ に点を置き、その個数をできるだけ均等にする。端点 $+1$ の個数を $n_+$、$-1$ の個数を $n_-$ とすると
...
従来の「常に $n^2$ を達成する」という記述は、奇数 $n$ では正しくない。
...
一般には端点 $-1,+1$ へできるだけ均等に配置する。端点の個数を $n_-,n_+$ とすると

$$
\det(X^\top X)=4n_-n_+.
$$

したがって、偶数 $n$ では最大値 $n^2$、奇数 $n$ では最大値 $n^2-1$。
