...
たとえば `core/07_wald_lr_score.md` は「微分すると」「尤度は」等の語を多く含むため監査スコアが高いが、現状はすでに尤度、対数尤度、最尤推定量、フィッシャー情報量、ワルド・スコア・尤度比統計量、Taylor展開まで詳細化されている。この種の誤検知は人手確認で除外する。
...
- 係数解釈はリンク関数を逆変換して示す。
- 過分散では「分散=平均」というポアソン仮定とのずれを明示する。
