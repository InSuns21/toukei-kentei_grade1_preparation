...
といった工程間異質性があると、条件付きではポアソンでも周辺では過分散になる。
...
場合にも、観測分散はポアソン仮定より大きくなり得る。

したがって「過分散が見えたから必ずガンマ混合」とは限らず、工程異質性なのか依存構造・ゼロ過剰・exposure誤指定など別のモデル誤特定なのかを確認する必要がある。
...
通常のポアソン管理図は標準偏差を $\sqrt\mu$ とするが、実際は $\sqrt{\mu+\tau^2}$ なので管理限界を狭く設定し、偽警報を増やす。
