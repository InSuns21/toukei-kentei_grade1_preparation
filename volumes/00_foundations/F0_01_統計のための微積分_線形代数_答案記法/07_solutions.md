...
例外として $x_1=\cdots=x_n=0$ なら $M=0$ です。このとき $L(\theta)=\theta^{-n}$ $(\theta>0)$ であり、$\theta\downarrow0$ とすると尤度は上限なく増加します。0はパラメータ空間に含まれないため最尤推定量は存在しません。ただし、この例外標本が生じる確率は0なので、それ以外の全標本では$\widehat\theta=M$が定義されます。
