...
## P2-THM-05 元の分布による期待値計算と線形性

確率変数$g(X)$の分布を先に求めず、$X$の確率質量関数または密度で直接期待値を計算する次の公式を使います。この公式を英語圏ではlaw of the unconscious statisticianと呼ぶことがありますが、本教材では略語を用いません。
...
第1式は各条件付き分布について前節の期待値計算公式を使い、分割上で足せば得ます。第2式では$\mu_i=E[X\mid H_i]$, $\mu=E[X]$とおき、$H_i$の下で
$$
X-\mu=(X-\mu_i)+(\mu_i-\mu)
$$
...
確率母関数では、対応するモーメントが有限なら
$$
G_X'(1-)=E[X],\qquad G_X''(1-)=E[X(X-1)].
$$
モーメント母関数が0の開近傍で有限なら、その近傍で微分でき
$$
M_X^{(r)}(0)=E[X^r].
$$
$X,Y$ が独立で各母関数が有限なら
...
確率母関数は$0\leq s<1$でべき級数を項別微分でき、

$$
G_X'(s)=\sum_{k=1}^{\infty}kP(X=k)s^{k-1}.
$$

$s\uparrow1$では各項が非負のまま増加します。この場合、非負項級数の部分和と$s$の極限を交換できるので、$G_X'(1-)=\sum_k kP(X=k)=E[X]$です。二階微分も同じ理由で$k(k-1)P(X=k)s^{k-2}$の極限と和を交換できます。ここで必要なのは、この非負項級数に対する限定的な極限交換だけです。

モーメント母関数が0の開近傍で有限なら、その近傍内に$\pm\delta$を取って$M_X(\delta),M_X(-\delta)<\infty$とできます。$|t|\leq\delta/2$では、各固定次数$r$に対し定数$C_r$が存在して

$$
|X|^re^{tX}\leq C_r\{e^{\delta X}+e^{-\delta X}\}
$$

となります。右辺は期待値が有限で、$t$に依存しない上界です。このような共通上界があるときは積分と微分を交換できるため、$M_X^{(r)}(t)=E[X^re^{tX}]$、特に$t=0$でモーメント公式を得ます。一般的な積分交換定理の学習は本章の前提にしません。
