...
- 乱塊法の処置平方和：$SS_{\mathrm{tr}}=b\sum_i(\overline Y_{i\cdot}-\overline Y_{\cdot\cdot})^2$。
...
- 釣合い $2^k$ 計画の効果平方和：各組合せをr回反復すると $SS_A=r2^{k-2}\{\operatorname{effect}(A)\}^2$。
...
- 等群サイズnの一元変量模型：$E[MS_E]=\sigma^2$、$E[MS_B]=\sigma^2+n\tau^2$、$\widehat\tau^2=(MS_B-MS_E)/n$。
- BIBD（釣合い型不完備ブロック計画）：処置数v、ブロック数b、反復数r、ブロックサイズk、処置対の共出現数$\lambda$ は $vr=bk$、$\lambda(v-1)=r(k-1)$ を満たす。調整処置差の分散は $2k\sigma^2/(\lambda v)$。
- 分割法：一次因子は一次単位誤差、二次因子と一次×二次交互作用は二次単位誤差を分母として検定する。
