...
title: データ型と検査量から管理図を選択する
category: applied-engineering
subcategory: engineering-quality
topic: control-chart-selection
type: recognition
difficulty: 1
priority: S
hashtags: [管理図, プロセス管理, 管理図選択]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 管理図 }]
---
## 問題
次の4状況に使う管理図を答えよ。(a) 長さと群内範囲、(b) 毎群100個中の不適合品数、(c) 群サイズが異なる不適合品数、(d) 面積が異なる検査単位の不適合数。
## 記号・用語
不適合品は単位全体の合否、不適合は1単位中に複数生じ得る欠点を数える。
## 使用公式・定理
計量値の中心とばらつきには $\overline X$--$R$、不適合品数には標本サイズ一定なら $np$、可変なら $p$、不適合数には検査単位一定なら $c$、可変なら $u$ 管理図を使う。
## 一手／方針
まず計量値か計数値かを分け、計数値なら不適合品か不適合か、さらに検査量が一定かを判定する。
## 答え
(a) $\overline X$--$R$、(b) $np$、(c) $p$、(d) $u$ 管理図。
## 計算例
毎群の面積が一定で傷の個数を数えるなら $c$ 管理図へ変わる。
## 注意
不適合品率を使えば標本サイズ一定でも $p$ 管理図を使える。$np$ 管理図は個数表示が必要なときに便利である。
<!-- CARD -->
