...
## 答え
$$\alpha_{22}=\frac{0.2-0.6^2}{1-0.6^2}=\frac{-0.16}{0.64}=-0.25.$$
## 計算例
AR$(1)$ なら $\rho_2=\rho_1^2$ なので $\alpha_{22}=0$ となる。
## 注意
...
## 答え
$$1-0.5z-0.2z^2=0\iff z=\frac{-0.5\pm\sqrt{1.05}}{0.4}.$$
根は約 $1.31,-3.81$ で、絶対値はともに1を超える。したがって因果的である。
## 計算例
この条件は同じAR$(2)$モデルの弱定常性条件にも対応する。
## 注意
...
## 答え
$\bar t=2$、$\bar X=5$ だから
$$\widehat b=\frac{(-1)(-2)+0+1(2)}{1+0+1}=2,\qquad \widehat a=5-2(2)=1.$$
推定トレンドは $1+2t$ であり、残差系列は $(0,0,0)$ である。
## 計算例
新しい観測が $(3,6,7)$ なら同じトレンドを引いた残差は $(0,1,0)$ となる。
## 注意
...
## 答え
$\bar x=2$、偏差は $(-1,0,1,0)$ だから
$$\widehat\rho_1=\frac{0(-1)+1(0)+0(1)}{(-1)^2+0^2+1^2+0^2}=0.$$
## 計算例
分子は隣接する偏差の積、分母は全偏差平方和である。
## 注意
...
## 答え
$$|1-\phi e^{-i\lambda}|^2=(1-\phi e^{-i\lambda})(1-\phi e^{i\lambda})=1+\phi^2-2\phi\cos\lambda,$$
したがって
$$f(\lambda)=\frac{\sigma^2}{2\pi(1+\phi^2-2\phi\cos\lambda)}.$$
## 計算例
$\phi=0.5$、$\sigma^2=1$ なら $f(0)=1/[2\pi(1-0.5)^2]=2/\pi$。
## 注意
...
difficulty: 2
priority: A
hashtags: [ARIMAモデル, ARMAモデル, 定常平均]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: ARIMAモデル }]
---
## 問題
...
difficulty: 3
priority: A
hashtags: [ARIMAモデル, 後退作用素, ARMAモデル]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: ARIMAモデル }]
---
## 問題
