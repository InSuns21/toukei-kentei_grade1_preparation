...
## 記号・用語
- $h,\ell$：層を表す添字（$1,\ldots,H$）
- $N_h$：第 $h$ 層の母集団サイズ
- $W_h=N_h/\sum_{\ell=1}^H N_\ell$：第 $h$ 層の母集団比率
- $S_h$：第 $h$ 層における調査変数の母標準偏差
- $n_h$：第 $h$ 層へ配分する標本数

## 使用公式・定理
**この欄の役割：解答で使う定義・公式・定理と、その適用条件**

有限母集団補正のうち配分に依存しない項を除くと、最小化対象は
$$V=\sum_{h=1}^H\frac{W_h^2S_h^2}{n_h},\qquad
\sum_{h=1}^Hn_h=n.$$
Lagrange関数
$$L=\sum_{h=1}^H\frac{W_h^2S_h^2}{n_h}
+\lambda\left(\sum_{h=1}^Hn_h-n\right)$$
を $n_h$ で偏微分すると
$$\frac{\partial L}{\partial n_h}
=-\frac{W_h^2S_h^2}{n_h^2}+\lambda=0.$$
したがって $n_h=W_hS_h/\sqrt\lambda$、すなわち $n_h\propto W_hS_h\propto N_hS_h$ である。

## 答え
比例定数を制約 $\sum_hn_h=n$ へ代入すると
...
## 記号・用語
- 包含確率：有限母集団の要素が標本に含まれる、標本設計上の確率
