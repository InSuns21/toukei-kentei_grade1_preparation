...
## 使用公式・定理
$E[R_iY_i/\pi_i\mid Y_i,X_i]=Y_i$（正しい観測確率モデルの下）。
## 計算例
観測されにくい最初の値へ重み2を与える。
## 注意
極端に小さい観測確率は分散を大きくする。
...
Eステップで完全データ対数尤度（またはそこに現れる十分統計量）の条件付き期待値を計算し、MステップでQを最大化する。
## 使用公式・定理
完全データ対数尤度の条件付き期待値を反復的に最大化する。
## 計算例
欠測カテゴリ指標なら、その指標を事後所属確率で置き換える。
## 注意
EMは観測データ尤度の局所最大点へ収束し得るため初期値を確認する。
<!-- CARD -->
