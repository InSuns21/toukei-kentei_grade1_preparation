...
## 注意
積は対数変換で和になるので、対数正規やガンマの用途で再生性を導くのに使う。
...
## 答え
- 二項分布は $np$ が有限一定なら $\operatorname{Binomial}(n,p)\approx\operatorname{Poisson}(np)$ へ近似される。
- 二項分布・ポアソン分布は、$n$ が大きく平均・分散が大きい条件で正規分布へ近似される。
