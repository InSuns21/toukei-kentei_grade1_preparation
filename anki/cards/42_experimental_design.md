...
title: フィッシャーの3原則を答える
category: applied-common
subcategory: applied-design
topic: fisher-principles
type: recognition
difficulty: 2
priority: B
hashtags: [フィッシャーの3原則, 無作為化, 反復]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 無作為化・反復・局所管理 }]
---

## 問題
実験計画におけるフィッシャーの3原則を挙げ、それぞれの目的を一言で述べよ。

## 使用公式・定理
**この欄の役割：解答で使う定義・公式・定理と、その適用条件**

処置比較の標準誤差は反復数増加で小さくなり、ブロック化は説明可能な変動を誤差から分離する。

## 答え
無作為化は系統的偏りを避けて誤差の確率評価を正当化する。反復は誤差分散を推定して精度を上げる。局所管理は既知の異質性で実験単位を群分けし、誤差を小さくする。

## 計算例
圃場を位置でブロック化し、各ブロック内で品種を無作為に割り付け、複数ブロックで反復する。

## 注意
反復は同じ測定を繰り返すことではなく、独立な実験単位へ処置を割り付けること。
<!-- CARD -->
...
## 問題
横軸を因子A、線を因子Bの各水準とする交互作用プロットで、平行・非平行・交差する線をどう解釈するか。
...
## 答え
線が平行ならAの単純効果がB水準によらず、交互作用はない。非平行なら交互作用がある。交差は効果の向きがB水準で逆転する交互作用を示す。

## 計算例
線間の垂直距離がA水準で変わるならB効果がAに依存する。

## 注意
プロットは診断であり、標本変動を考慮した検定・区間も確認する。
<!-- CARD -->
...
difficulty: 4
priority: B
hashtags: [2水準要因計画, コントラスト, 平方和]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 2水準要因計画 }]
---
...
title: 2水準要因の効果をコントラストから求める
category: applied-common
subcategory: applied-design
topic: two-level-effect
type: formula
difficulty: 3
priority: B
hashtags: [2水準要因計画, コントラスト, 主効果]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 2水準要因計画 }]
---

## 問題
$2^k$ 完全要因計画で各処置組合せの平均を $\bar y_j$、効果列の符号を $x_j\in\{-1,+1\}$ とする。効果推定値を書け。

## 使用公式・定理
**この欄の役割：解答で使う定義・公式・定理と、その適用条件**

高水準側平均−低水準側平均。各側には $2^{k-1}$ 点がある。
...
difficulty: 4
priority: B
hashtags: [一部実施要因計画, resolution, alias]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 部分実施要因計画 }]
---
...
difficulty: 3
priority: B
hashtags: [直交表, 直交性, 2水準要因計画]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 直交表の基本 }]
---

## 問題
2水準直交表の任意の1列と任意の2列の組合せについて必要な均衡条件を述べよ。

## 使用公式・定理
**この欄の役割：解答で使う定義・公式・定理と、その適用条件**

切片列と効果列、異なる主効果列が直交する。

## 答え
各列で−1と+1が同数現れ、任意の2列では $(--),(-+),(+-),(++)$ が同数ずつ現れる。したがって各列の和と異なる列同士の内積が0。

## 計算例
4行なら各列に±が2回、2列の4組合せが各1回。

## 注意
列へ交互作用を割り当てるとalias関係を確認する必要がある。
<!-- CARD -->
