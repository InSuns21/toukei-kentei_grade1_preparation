...
title: 重要効果を守るブロック生成子を選ぶ
category: applied-engineering
subcategory: engineering-design
topic: block-generator-selection
type: calc_step
difficulty: 3
priority: B
hashtags: [交絡法, ブロック化, 2水準要因計画]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 交絡法 }]
---
## 問題
$2^4$ 計画を2ブロックへ分ける。AB交互作用が重要なとき、ブロック生成子としてABとABCDのどちらを選ぶべきか。
## 記号・用語
ブロック生成子に選んだ効果はブロック差と完全に交絡し、その計画では推定できない。
## 使用公式・定理
重要でない高次交互作用をブロック生成子へ選ぶ。
## 一手／方針
候補ごとに失われる効果を読む。
## 答え
ABを選ぶと重要なABが失われる。ABCDを選べば失われるのは4因子交互作用なのでABを推定できる。したがってABCDを選ぶ。
## 計算例
ABCD列の符号で8実施ずつ2ブロックへ分ける。
## 注意
ブロック生成子とその定義群が他の重要効果を巻き込まないかも確認する。
<!-- CARD -->
...
difficulty: 2
priority: B
hashtags: [交絡因子, 無作為化, 実験の計画と実施]
frequency: { past_exam: 0, textbook: 0, independent_problems: 0, source_confirmations: 0 }
sources: [{ type: official_syllabus, topic: 交絡因子 }]
---
## 問題
処置Aを午前、処置Bを午後だけ実施した実験で時間帯が応答へ影響する。問題点と改善策を答えよ。
## 記号・用語
交絡因子は処置割付と関連し、かつ応答へ影響するため処置効果と区別できない因子である。
## 使用公式・定理
各時間帯内でA、Bを無作為に割り付ければ、時間帯をブロック因子として処置効果から分離できる。
## 一手／方針
処置と時間帯が完全に一致しているかを確認する。
## 答え
現計画では処置差と時間帯差が完全に交絡する。午前・午後の各ブロック内に両処置を配置し、その中で無作為化する。
## 計算例
各時間帯でB−Aを作って平均すれば、加法的な時間帯効果は差から消える。
## 注意
実験計画の「効果の交絡」と、観察研究の交絡因子は関連するが、定義する対象を明示する。
<!-- CARD -->
