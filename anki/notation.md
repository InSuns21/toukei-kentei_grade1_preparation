...
共通記法の正本は `../references/notation-guide.md` であり、この節では定義を再掲しない。Ankiの生成・検証系が参照する代表キーは $\phi(z)$、$\Phi(z)$、$z_\alpha$、$I_1(\theta)$、$\boldsymbol I(\boldsymbol\theta)$ である。意味・規約は共通正本を参照する。
