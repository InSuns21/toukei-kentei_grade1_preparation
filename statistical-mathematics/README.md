...
- 求めること自体が設問であるモーメント母関数・確率母関数・尤度・Fisher情報量などを既知公式として置く。
...
`audit:granularity` は失敗させる検証ではなく、共通規約や上記の代表例と比べて、行間を追加確認すべきファイルを優先順位付きで抽出する監査です。検出された語句や短さだけで機械的に不合格とはせず、採点対象の導出が実際に飛んでいるかを本文で確認します。
