...
ここで使う **Neyman–フィッシャー因子分解定理**は、同時確率密度関数または同時確率質量関数を
...
連続分布からの独立標本なので、3個の標本値がすべて一致する確率は0である。したがって $Y_{(3)}-Y_{(1)}>0$ となる確率は1であり、上の条件付き分散は正である。

したがって全分散公式の第2項を最小にするには
...
と最小値・最大値だけを通じて母数に依存するので **Neyman–フィッシャー因子分解定理**から $(Y_{(1)},Y_{(3)})$ は十分。
