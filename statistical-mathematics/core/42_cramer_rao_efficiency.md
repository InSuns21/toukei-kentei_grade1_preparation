...
2. $\mu$ の不偏推定量に対するクラーメル・ラオ下限を求め、$\bar X$ が効率的であることを示せ。
3. $\mu^2$ を推定する不偏推定量
...
4. $\mu^2$ を推定する場合のクラーメル・ラオ下限と比較せよ。
...
### 2. クラーメル・ラオ下限と $\bar X$ の効率性

使うのは **クラーメル・ラオ不等式**である。$W$ が $g(\mu)$ の不偏推定量、すなわち
...
を確認しているので、この正規モデルではクラーメル・ラオ不等式を適用できる。
...
なのでクラーメル・ラオ下限は0になる。しかし $\operatorname{Var}(T)=2\sigma^4/n^2>0$ であり、この点では下限が0というだけで有用な比較にならない。
...
よって $\mu$ の不偏推定量のクラーメル・ラオ下限は $\sigma^2/n$。また
...
$g(\mu)=\mu^2$ では $g'(\mu)=2\mu$ なのでクラーメル・ラオ下限は
...
- クラーメル・ラオ不等式を適用し、$\bar X$の不偏性・分散・効率性を示す: 5点
- $T$の不偏性と分散を、正規分布の4次モーメントを含めて導く: 6点
- $\mu^2$のクラーメル・ラオ下限との比較・$\mu=0$での退化の説明: 4点
