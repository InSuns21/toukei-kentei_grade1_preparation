...
4. $E[Y\mid T]$を求め、$\tau(\lambda)$の一様最小分散不偏推定量推定量を与えよ。
...
で確認でき、値は0から1の間なので可積分である。したがって上の推定量が一意な一様最小分散不偏推定量推定量。
...
これは完備十分統計量の可積分な不偏関数なので **Lehmann–Scheffé 定理**から一様最小分散不偏推定量。
