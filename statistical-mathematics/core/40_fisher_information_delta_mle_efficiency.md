...
ここで使うのは **正則MLEの漸近正規性定理**である。必要な条件のうち本問で重要なのは、
...
注意として、$\widehat\lambda=1/\bar X$ は有限標本では一般に不偏ではない。したがってここで比較しているのは「不偏推定量に対する有限標本Cramér–Rao不等式をそのままMLEへ適用した」という意味ではなく、**正則MLEの漸近情報下限**との比較である。
...
支持 $x>0$ は母数に依存せず、真値 $\lambda>0$ は内部点、対数尤度は滑らかで $I_1(\lambda)=\lambda^{-2}\in(0,\infty)$。よって **正則MLEの漸近正規性定理**から
