...
$\Lambda$ の事前分布を、形状母数 $r>0$、率母数 $\beta>0$ のGamma分布
...
3. $E[X]$ を全期待値の公式から求めよ。
4. $\operatorname{Var}(X)$ を全分散の公式から求め、Poisson分布より過分散になることを確認せよ。
...
固定した平均母数を持つPoisson分布なら「分散=平均」だが、ここではPoisson強度 $\Lambda$ 自体が個体間で変動する。その追加変動 $\operatorname{Var}(\Lambda)$ が周辺分散に加わるため、過分散が生じる。
...
従って $\operatorname{Var}(X)>E[X]$ であり、Poisson強度の混合により過分散となる。

## 採点基準

- 周辺確率質量関数（積分設定・Gamma積分）: 6点
- 負の二項分布との母数対応: 4点
- 全期待値による平均: 4点
- 全分散による分散・過分散の解釈: 6点
