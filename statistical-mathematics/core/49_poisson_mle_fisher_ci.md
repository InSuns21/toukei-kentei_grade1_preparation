# Core 22 ポアソン最尤推定量・フィッシャー情報量・信頼区間
...
とする。フィッシャー情報量・漸近分布・ワルド信頼区間については真値 $\lambda>0$ を仮定する。

1. 上の確率質量関数から尤度を作り、$\lambda$ の最尤推定量を求めよ。
2. $\lambda$ に関する1標本および $n$ 標本のフィッシャー情報量を求めよ。
3. 最尤推定量の漸近分布を求めよ。
4. $n=100$, $\sum_{i=1}^nX_i=400$ のとき、$z_{0.975}=1.96$ を用いた95% ワルド信頼区間を求めよ。
...
これは漸近的なワルド信頼区間であり、有限標本での正確区間ではない。標本が小さい、または $\lambda$ が小さいと、対称なワルド信頼区間が0未満へはみ出すことがある。
