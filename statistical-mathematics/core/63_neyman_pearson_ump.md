...
> **先に理論を整理したい場合**  
> Neyman–Pearson補題の正確なステートメントと証明、および正規分布の母平均検定を使った**最強力検定・一様最強力検定・一様最強力不偏検定**の違いは、[補足：Neyman–Pearson補題から最強力・一様最強力・一様最強力不偏検定へ](#/statistical-mathematics/core/63a_neyman_pearson_mp_ump_umpu)を参照。  
...
以下では、この直感をNeyman–Pearson補題で厳密に示す。

---

### 1. 単純対立に対するNeyman–Pearson検定

#### 1.1 なぜ最初に $\lambda_1$ を1点に固定するのか

Neyman–Pearson補題が直接扱うのは
...
のどれを選んでも、尤度比は $T$ の増加関数であり、Neyman–Pearson補題から棄却域は
...
ただし本問では、各 $\lambda_1<\lambda_0$ にNeyman–Pearson補題を適用することで、その中身を直接確認している。
...
$\lambda_1<\lambda_0$ なのでこれは $T$ の増加関数。Neyman–Pearson補題より
