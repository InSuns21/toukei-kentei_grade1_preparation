...
**弱大数の法則**を使う。必要条件として本問で確認すべきなのは、$X_i$ が独立同分布で $E|X_i|<\infty$ であること。本問では $0<X_i<1$ なので可積分であり、
...
$0<X<1$ なので可積分な独立同分布標本。**大数の法則**で $\bar X\to_p m$、$g$ は $m<1$ で連続なので **連続写像定理**から一致。

さらに有限分散なので **中心極限定理**、$g$ は $m$ で微分可能なので **Delta法**を使え、
