...
4. このモデルに通常のクラーメル・ラオ議論を機械的に適用できない理由を説明せよ。
...
### 4. クラーメル・ラオ不等式を通常形で使えない理由

通常の **クラーメル・ラオ不等式**の導出では、少なくとも母数近傍で共通支持を持ち、密度を母数で微分して積分と微分を交換できることが重要である。これにより
...
したがって通常の正則クラーメル・ラオ理論の条件が破れており、その公式を機械的に適用してはならない。
...
通常の **クラーメル・ラオ不等式**は共通支持等の正則条件を使うが、本問は支持 $(0,\theta)$ が母数依存で、通常のスコア平均0の議論も壊れる。
