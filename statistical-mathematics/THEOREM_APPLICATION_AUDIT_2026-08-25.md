...
また「定理名を増やす」こと自体は目的としない。定義やpmf/CDFから短く直接導出できる場合は、ブラックボックス定理を減らして直接導出する。
...
- `core/44_uniform_nonregular_mle.md`
  - 最大値の `Beta(n,1)` を公式引用せず、CDF→密度→積分からモーメントを導出。
  - Cramér–Raoの共通支持条件が壊れ、スコア平均0の正則議論が成立しないことを明示。
- `core/46_moment_estimation_delta.md`
  - LLN、連続写像定理、CLT、Delta法、正則MLE漸近正規性を名前付きで分離。
  - 可積分性、有限分散、真値での連続・微分可能性、内部点・共通支持・有限正情報量を確認。
- `core/49_poisson_mle_fisher_ci.md`
  - MLEの母数空間を $\lambda\ge0$ とし、全観測0の境界MLEと、漸近論で真値 $\lambda>0$ を置く内部点条件を区別。
  - 漸近分布はCLTで直接正当化し、LLNとSlutskyによるplug-in SEを明示。
- `core/57_inverse_variance_blue_mle.md`
  - BLUEは分散最小化で直接証明。
  - CRLB一致を述べる前に、共通支持、滑らかさ、内部点、有限正情報量、不偏性を確認。
- `standard/47_binomial_wald_coverage.md`
  - Wald区間をCLT→LLN→Slutskyから導出。
