...
- `Exp(λ)` の rate / scale、ワイブル・パレート など母数化に流儀がある分布は必ず式を明示する。標準正規・一様分布のような既知分布でも、その密度が直後の計算の出発点になる場合は式を併記する。
...
| 16 | 52 | A | A | [パレート 最尤推定量・有限標本バイアス・効率](52_pareto_mle_bias_efficiency.md) |
