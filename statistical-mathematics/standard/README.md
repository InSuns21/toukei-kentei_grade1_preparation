...
100大問の層構成を変えず、公式範囲・過去問接続上の補完価値が高い題材を置く。

- [補充1 ゼロ切断ポアソン分布・観測尤度・母数推定](supplement_zero_truncated_poisson_mle.md)
  - 統計応用理工学 2016 問4「Poisson最尤推定・ゼロ切断分布」のテーマ索引に接続。
  - 切断後の条件付き分布、モーメント法、最尤推定方程式、解の一意性、切断と打切りの違いを扱う。
  - 数値方程式は記号式までで完答とし、関数電卓を要求しない。
