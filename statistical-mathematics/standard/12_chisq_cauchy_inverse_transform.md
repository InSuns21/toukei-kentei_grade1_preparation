...
3. $U\sim U(0,1)$ から標準コーシー乱数を生成する式を求めよ。
4. $Y=1/X$ とする。$Y$ の分布を求めよ。
...
f_Y(y)
&=f_X(1/y)\frac1{y^2}\\
&=\frac1{\pi\left(1+1/y^2\right)}\frac1{y^2}\\
&=\frac1{\pi(1+y^2)}.
\end{aligned}
$$
...
$$
\boxed{Y=\frac1X\sim\operatorname{Cauchy}(0,1)}.
$$

すなわち、標準コーシー分布は逆数変換 $x\mapsto1/x$ に対して分布が変わらない。この性質は、標準コーシー分布が正規変数の比 $Z_1/Z_2$ として表せることから、分子と分母を入れ替えても同じ分布になることとも整合する。
...
また $Y=1/X$ では $x=1/y$、$|dx/dy|=1/y^2$ だから
...
従って $1/X$ も標準コーシー分布に従う。

## 採点基準

- 同時正規密度と変数変換からコーシー密度を導出: 6点
- $\arctan$ 変換: 5点
- 逆関数法: 5点
- 逆数変換とコーシー分布の反転不変性: 4点
