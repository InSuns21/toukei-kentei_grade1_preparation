...
$$
\int h(x)\,dx
=\int \frac{h(x)}{g(x)}g(x)\,dx
$$

と書くには、少なくとも **$h(x)\neq0$ となる領域で $g(x)>0$** である必要がある。そうでなければ必要な領域を提案分布が全く標本化せず、比 $h(x)/g(x)$ も定義できない。

本問では $h(x)=x^4$ で、$0<x<1$ では $g(x)=2x>0$ だから条件を満たす。従って
...
$$
\boxed{
\widehat I_{\mathrm{imp}}
=\frac1n\sum_{i=1}^n\frac{X_i^3}{2}
}.
$$

$x=0$ では $g(0)=0$ だが、同時に被積分関数 $x^4$ も0である。さらに連続分布では一点 $X=0$ の確率は0であり、$x^4/g(x)=x^3/2$ を0まで連続延長すれば問題はない。
...
重点サンプリングでは、被積分関数が非零の領域で $g>0$ であることを確認した上で
