...
まず $T$ の $\theta_0$ 下の分布が連続とする。累積分布関数を $F_{\theta_0}$ とすると
...
つまり最悪ケース保証を失い、P値を過小評価してサイズが $\alpha$ を超える可能性がある。plug-inが妥当になるには、ピボット性、条件付き推論、プロファイル尤度の漸近理論など別の正当化が必要であり、「帰無側の最尤推定量を代入した」というだけでは足りない。
