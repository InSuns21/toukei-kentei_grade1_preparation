...
1. 一般に両側対立では一様最強力検定検定が存在しにくい理由を述べよ。
...
$\mu>\mu_0$ に対する最強力検定は大きい $\bar X$ を棄却側へ置き、$\mu<\mu_0$ では小さい $\bar X$ を棄却側へ置く。左右で望ましい棄却方向が逆なので、全対立に同時に最強となる単一の一様最強力検定検定は一般には作れない。
...
ここで使うのは **1母数指数型分布族の両側一様最強力不偏検定検定定理（Neyman構造）**である。概略として、
