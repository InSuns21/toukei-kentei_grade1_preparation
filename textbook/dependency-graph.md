...
RNG3 は RNG2 を直接 prerequisite とする。整域上の整除・単元・同伴元・既約元・素元・最大公約元・Bézout 恒等式を導入し、素元と素な主イデアルを対応させる。Euclid 関数の strict decrease により互除法の有限停止と最大公約元・Bézout 表示を証明し、最小 Euclid 関数値の元から Euclid 整域が単項イデアル整域となることを示す。さらに PID では $(a)+(b)$ が最大公約元を生成し、既約元が素元になること、有限既約分解が存在することを証明して PID が一意分解整域となることまで閉じる。
