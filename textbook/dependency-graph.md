...
MOD2 は MOD1 と RNG4 を直接 prerequisite とする。PID 上自由加群間の準同型を行列で表し、Bézout 変形と RNG3 のイデアル昇鎖停止から Smith 標準形の存在を証明する。小行列式イデアルの行列同値不変性から不変因子の一意性を導き、PID 上自由加群の部分加群が自由であることを用いて有限生成加群を有限行列の余核へ落とす。そこから PID 上有限生成加群の構造定理、有限生成 Abel 群の構造定理、基本因子表示を導く。最後に MOD1 の $F[x]$-加群を構造定理へ入れ、有限次元線形自己写像の巡回分解と LA4 の Jordan 構造を接続する。
