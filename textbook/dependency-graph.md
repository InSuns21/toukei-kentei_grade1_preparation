...
FLD3 は FLD2 と GRP2 を直接 prerequisite とする。有限体の標数と素体を定め、Frobenius 写像を構成して有限体上で自己同型になることを証明する。有限 Abel 群の最大位数元と多項式の根の個数評価から有限体の乗法群の巡回性を導き、$x^{p^n}-x$ の根全体が体になることと FLD2 の分解体の存在・一意性から $\mathbb F_{p^n}$ の存在・一意性を閉じる。塔の公式から部分体判定 $\mathbb F_{p^m}\subset\mathbb F_{p^n}\iff m\mid n$ を証明し、相対 Frobenius の位数・固定体、有限体拡大の分離性・正規性まで扱う。一般の Galois 群・固定体・中間体対応は FLD4 へ送る。
