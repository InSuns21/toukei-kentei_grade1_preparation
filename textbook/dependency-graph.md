...
GEO1--GEO3 は実装済みなので、幾何学主線の入口を次のとおり公開する。
...
GEO2 は曲線の一次速度と Leibniz 則を満たす微分作用素の同値性を閉じ、写像の微分・余接空間・接束を canonical に担当する。GEO3 は RA6A の逆関数定理を使って定数階数定理を証明し、はめ込み・沈め込み・埋め込み・正則値・埋め込み部分多様体を canonical に担当する。GEO4 以降は未実装のため reader-facing DAG へ先行登録しない。LA3E は将来の GEO7「テンソル場・微分形式」へ接続する代数準備として保持する。
