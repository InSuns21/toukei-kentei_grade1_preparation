...
FLD4 は FLD3 と GRP3 を直接 prerequisite とする。基礎体固定自己同型・自己同型群・固定体・有限 Galois 拡大を導入し、有限分離拡大の埋め込み数と正規性から Galois 群の位数が拡大次数に等しいことを証明する。Artin の独立性と固定体定理から有限 Galois 理論の基本定理を導き、中間体と部分群の包含反転対応、次数と指数の対応、正規部分群と正規中間拡大の対応、制限準同型による商群同型まで閉じる。$x^3-2$ の分解体、二重二次拡大、有限体で対応を具体化し、作図可能性・根号による可解性は FLD5 へ送る。
