...
RNG1 は GRP2 を直接 prerequisite とする。環・可換環・単位元を持つ環、部分環、零因子・整域・体を導入し、環準同型の核がイデアル、像が部分環になることを証明する。加法剰余類上の積が代表元によらず定まることとイデアル条件の同値を示して商環を構成し、環の第一同型定理と $\mathbb Z$ のイデアル分類までを核心証明付きで閉じる。
