...
GEO17 以降は未実装なので reader-facing DAG へ先行登録しない。
...
後続の FDM1–FDM4、FEM1–FEM7、MC1–MC4、QMC1–QMC8 は DREAM_THEATER_ENCORE_V_REWRITE_PLAN.md の計画対象であり、各講の実装完了時に実際の直接依存だけを追加する。

読者向け入口は F0_00R5_EncoreV_Numerical_FEM_MonteCarlo/index.md。現在の実装済み数値解析本編は NA12 までで、次の実装対象は FDM1「熱方程式と差分法の導入」である。
