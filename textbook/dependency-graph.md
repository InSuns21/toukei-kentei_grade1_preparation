...
とする。A1Bで実数の上限性質・Archimedes性、B0で点列・部分列・「十分大きい添字」、Bで距離空間、B1で位相空間・近傍・部分空間位相・位相的収束・位相的連続性を導入する。A2・A3・A3Aは選択公理とZornの補題を追う集合論側の深掘りであり、この局所列の必須前提にはしない。
