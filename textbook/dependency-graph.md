...
FLD4 は FLD3 と GRP3 を直接 prerequisite とする。基礎体固定自己同型・自己同型群・固定体・有限 Galois 拡大を導入し、有限分離拡大の埋め込み数と正規性から Galois 群の位数が拡大次数に等しいことを証明する。Artin の独立性と固定体定理から有限 Galois 理論の基本定理を導き、中間体と部分群の包含反転対応、次数と指数の対応、正規部分群と正規中間拡大の対応、制限準同型による商群同型まで閉じる。$x^3-2$ の分解体、二重二次拡大、有限体で対応を具体化する。

FLD5 は FLD4 と GRP4 を直接 prerequisite とする。定規とコンパスの交点計算を実二次拡大列へ翻訳し、作図可能性との同値と次数の2冪障害を証明する。交換子部分群・導来列・可解群を構成し、根号拡大の各段階を必要な1の根を添加した上で可換 Galois 拡大へ直す。逆向きは素数次数巡回拡大の Kummer 表示を Lagrange の分解式から証明し、標数0で「根号による可解性」と「Galois 群の可解性」の同値を閉じる。最後に $A_5$ の単純性、$S_5$ の非可解性、$x^5-4x+2$ の Galois 群が $S_5$ であることを示し、一般五次の根号公式が存在しない理由まで到達する。GRP1--FLD5 の学部標準抽象代数主線はここで完結する。
...
F0-00F1 半正定値行列 ───────────┐
VC1 勾配・Hessian ──────────────┴→ OPT1 凸集合・凸関数・凸最適化
                                      ↓
F0-00C1 Heine--Borel ────────────┐
F0-00D1 ノルム ──────────────────┤
F0-00E1 内積・直交射影 ──────────┴→ OPT2 射影・支持超平面・分離・Farkas
                                      ↓
F0-00A1 上限・下限 ─────────────────→ OPT3 閉真凸関数・劣微分・法錐
...
OPT1 凸集合・凸関数・凸最適化 ─────────┐
NA12 無制約最適化・共役勾配法 ─────────┼→ OPT7 滑らかな凸最適化
RA4 微積分学の基本定理 ────────────────┘

OPT2 射影・支持超平面・分離・Farkas ─────┐
OPT3 閉真凸関数・劣微分・法錐 ──────────┼→ OPT8 非滑らか・近接最適化
OPT7 滑らかな凸最適化 ──────────────────┘

OPT5 Lagrange 双対・Slater 条件・KKT ─────┐
OPT6 KKT の幾何学的導出・制約想定 ───────┤
OPT7 滑らかな凸最適化 ───────────────────┼→ OPT9 制約付き数値最適化
OPT8 非滑らか・近接最適化 ───────────────┘

OPT2 射影・支持超平面・分離・Farkas ─────┐
OPT5 Lagrange 双対・Slater 条件・KKT ────┴→ OPT10 線形計画 I：多面体・極点・双対

OPT9 制約付き数値最適化 ───────────────────┐
OPT10 線形計画 I：多面体・極点・双対 ─────┴→ OPT11 線形計画 II：単体法・内点法・感度解析

OPT5 Lagrange 双対・Slater 条件・KKT ───────┐
OPT6A 錐制約・一般化 KKT ──────────────────┼→ OPT12 二次計画・錐計画入門
OPT11 線形計画 II：単体法・内点法・感度解析 ─┘

OPT10 線形計画 I：多面体・極点・双対 ─────────┐
OPT11 線形計画 II：単体法・内点法・感度解析 ─┴→ DOPT1 整数計画・LP 緩和
DOPT1 整数計画・LP 緩和 ─────────────────────→ DOPT2 ネットワーク最適化
DOPT2 ネットワーク最適化 ─────────────────────→ DOPT3 マッチング・割当問題
DOPT3 マッチング・割当問題 ─────────────────────┐
LA3C 標準線形代数 III-C：行列式の計算・可逆性・乗法性 ─┴→ DOPT4 全単模性・整数多面体

F0-02C2 関数解析II：線形汎関数・双対空間・Riesz表現 ─→ RKHS1 再生核 Hilbert 空間・Moore--Aronszajn ─→ RKHS2 正則化問題の表現定理 ─→ RKHS3 カーネルリッジ回帰
OPT2 射影・支持超平面・分離・Farkas ───────────────┐
OPT12 二次計画・錐計画入門 ─────────────────────┴→ RKHS4 最大マージンとハードマージン SVM
...
NUMLAB1 は PYNUM1、NUMLAB0、NA1–NA12 を直接 prerequisite とする。基本は各理論章1本のブラウザ実験だが、NA4・NA5・NA8 は学習対象そのものを高水準 API が代行するため、手動構築を先に行い、その後に SciPy / NumPy を使う実務編を置く。NA4 は Lagrange 補間の自前実装と `BarycentricInterpolator`、NA5 は3点 Gauss--Legendre 求積のモーメント構成と `leggauss`、NA8 は Cholesky 成分漸化式の自前実装と `np.linalg.cholesky` を対にする。Newton 法内の `np.linalg.solve` や固有値・厳密解の検算など、learning objective の下位処理にはライブラリ利用を許す。NUMLAB1 は計15本の exercise lab を持ち、各実験を `textbook/numerical-lab-links.yaml` の stable anchor と experiment ID で追跡する。
