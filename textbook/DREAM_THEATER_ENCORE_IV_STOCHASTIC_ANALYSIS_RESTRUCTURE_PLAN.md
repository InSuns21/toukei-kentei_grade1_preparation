...
- Phase 5：TSA6「状態空間・Kalman フィルタ・イノベーション」を実装。線形正規状態空間モデル / 予測分布・フィルタ分布 / 正規ベクトルの条件付け / Kalman の予測・更新再帰 / Joseph 形 / 共分散 Riccati 再帰 / イノベーション列の直交性と正規独立性 / 観測列との情報同値性 / イノベーション尤度 / ARMA(1,1) の二次元状態空間表現を、主要証明・直接例・A4/B3/C1 演習・全問詳細解答まで閉じた。
- TSA6 の direct prerequisite は TSA1・P3-03・LA3B・F0-00P7 とした。TSA1 の最良線形予測・イノベーション、P3-03 の多変量正規分布・条件付き分布を Kalman 再帰の canonical dependency として再利用し、多変量イノベーション尤度の `\det S_t` に LA3B、尤度・対数尤度の定義に F0-00P7 を使う。TSA2--TSA4 は読順上の先行章だが、TSA6 の核心証明に不要なため direct prerequisite へは追加しない。
- 有限時点の Kalman 再帰は同時正規ベクトルの条件付けから完全導出した。一般行列 Riccati の漸近収束を名前だけで完成扱いせず、局所レベルモデルでは Riccati 写像の正の不動点と縮小性から定常予測分散への収束を章内で完全証明し、一般行列の収束理論は可検出性・可安定化性を要する後続の線形システム論として境界を明示した。
- 観測イノベーションは過去観測に条件付き平均 0 であり、異時点イノベーションと直交することを塔則から証明した。さらに線形正規性から独立性へ上げ、観測列とイノベーション列が同じ σ-加法族を生成することを帰納法で示したうえで、条件付き正規密度の積からイノベーション尤度を導出した。
- 現在地：Phase 5 は TSA1 → TSA2 → TSA3 → TSA4 → TSA5 → TSA6 完了。Encore IV 再編主線 STO1--STO14 / TSA1--TSA6 は完了。
