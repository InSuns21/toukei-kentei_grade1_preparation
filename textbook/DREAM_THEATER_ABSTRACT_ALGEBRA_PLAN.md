...
GRP1「群・部分群・巡回群・置換群」から FLD1「体拡大・代数的元・最小多項式」までの 11 講は 2026-09-23 に実装完了した。

FLD1 では RNG4 の多項式環・既約性と LA1 の基底・次元・一次独立を受け、体拡大を基礎体上のベクトル空間として読み、拡大次数と塔の公式を証明した。有限拡大の元が代数的であることを一次従属から導き、評価準同型の核から最小多項式の存在・一意性・既約性を確立した。さらに

$$
F(\alpha)=F[\alpha]\cong F[x]/(m_\alpha),
\qquad
[F(\alpha):F]=\deg m_\alpha
$$

を証明し、既約多項式からその根を含む体を商環として構成した。
