...
GRP1「群・部分群・巡回群・置換群」から FLD2「分解体・分離性・正規性」までの 12 講は 2026-09-23 に実装完了した。

FLD2 では FLD1 の最小多項式・単純代数拡大を受け、既約因子の根を一つずつ添加して分解体の存在を証明した。最小多項式の根を指定する一段の埋め込み延長から有限代数拡大上への延長を構成し、分解体の同型を除く一意性を導いた。さらに形式微分と $\gcd(f,f')$ による重根判定から分離性を、共役根を全て含む条件から正規性を構成し、有限拡大について正規性と分解体であることの同値まで閉じた。
