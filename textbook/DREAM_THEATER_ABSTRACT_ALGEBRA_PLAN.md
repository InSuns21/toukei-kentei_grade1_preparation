...
- 相対 Frobenius の位数と固定体
- 有限体拡大が分離的かつ正規であること（Galois 拡大という一般概念は FLD4 で導入）
...
9. MOD1 加群・部分加群・商加群・自由加群 ✅ 2026-09-23
10. MOD2 Smith 標準形・PID 上有限生成加群 ✅ 2026-09-23
...
15. FLD5 Galois 理論の応用：作図可能性・根号による可解性 ✅ 2026-09-24
...
GRP1「群・部分群・巡回群・置換群」から FLD5「Galois 理論の応用：作図可能性・根号による可解性」までの **全15講を実装完了**した。FLD1--FLD2 は 2026-09-23、FLD3--FLD5 は 2026-09-24 に完了した。

FLD5 では、定規とコンパスによる作図を座標体上の一次・二次方程式へ翻訳し、

$$
\alpha\text{ が作図可能}
\iff
\alpha\text{ が実二次拡大列の最上段に入る}
$$

ことを証明した。塔の公式から作図可能な代数的数の次数が2の冪になる必要性を導き、立方体倍積・60度角三等分・正七角形の不可能性と、正五角形の作図可能性を具体化した。

後半では交換子部分群・導来列・可解群を構成し、可解列による特徴付けと閉性を証明した。根号拡大について、必要な1の根を添加した一段根号拡大の Galois 群が可換になること、可換 Galois 段階列の正規閉包が可解になることを示した。逆向きは素数次数巡回拡大の Kummer 表示を Lagrange の分解式から導き、標数0で

$$
f\text{ が根号によって可解}
\iff
\operatorname{Gal}(f/F)\text{ が可解}
$$

を双方向とも閉じた。

最後に $A_5$ の共役類を直接数えて単純性を証明し、$S_5$ の非可解性を導いた。具体例

$$
x^5-4x+2
$$

について、Eisenstein の既約判定、実根が3個であること、複素共役が互換を与えること、推移性と Cauchy の定理から5-巡回置換を得ることを組み合わせ、

$$
\operatorname{Gal}(x^5-4x+2/\mathbb Q)\cong S_5
$$

を示した。従って根号で解けない五次方程式が実在し、全ての五次方程式を覆う普遍的根号公式が存在しないことまで到達した。

FLD5 の direct prerequisite は **FLD4 + GRP4** とする。これで、集合・写像を入口にした GRP1--GRP4、RNG1--RNG4、MOD1--MOD2、FLD1--FLD5 の学部標準抽象代数主線は完結した。
