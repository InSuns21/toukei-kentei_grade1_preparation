...
GRP1「群・部分群・巡回群・置換群」、GRP2「準同型・剰余類・正規部分群・商群」、GRP3「群作用・軌道・安定化群・共役」、GRP4「Cauchy の定理・Sylow の定理・有限群への応用」、RNG1「環・環準同型・イデアル・商環」、RNG2「素イデアル・極大イデアル・中国剰余定理」、RNG3「整除・Euclid 整域・PID・UFD」は 2026-09-23 に実装完了。Phase B の環論は整除理論と一意分解まで進んだ。
...
RNG3 で整域上の整除・単元・同伴・既約元・素元を構成し、Euclid の互除法から最大公約元と Bézout 等式を導いた。さらに Euclid 整域 ⇒ 単項イデアル整域 ⇒ 一意分解整域を核心証明付きで閉じ、$\mathbb Z[\sqrt{-5}]$ により既約元が素元でないと一意分解が壊れる機構も確認した。RNG4 では多項式環へ進み、体上の除法と $F[x]$ の Euclid 整域性、Gauss の補題、UFD 上の多項式環、既約多項式の標準判定を扱う。
