...
GRP1「群・部分群・巡回群・置換群」、GRP2「準同型・剰余類・正規部分群・商群」、GRP3「群作用・軌道・安定化群・共役」、GRP4「Cauchy の定理・Sylow の定理・有限群への応用」、RNG1「環・環準同型・イデアル・商環」、RNG2「素イデアル・極大イデアル・中国剰余定理」、RNG3「整除・Euclid 整域・PID・UFD」は 2026-09-23 に実装完了。Phase B の環論は一意分解整域まで進んだ。
...
RNG3 で整域上の整除・単元・同伴元・既約元・素元・最大公約元・Bézout 恒等式を構成し、Euclid の互除法から `Euclid 整域 ⇒ PID ⇒ UFD` を核心証明付きで閉じた。RNG4 では多項式環固有の除法、Gauss の補題、原始多項式、既約多項式と標準的な既約判定へ進む。
