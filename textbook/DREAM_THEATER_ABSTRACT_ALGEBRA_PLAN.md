...
5. RNG1 環・環準同型・イデアル・商環
6. RNG2 素イデアル・極大イデアル・中国剰余定理
...
GRP1「群・部分群・巡回群・置換群」、GRP2「準同型・剰余類・正規部分群・商群」、GRP3「群作用・軌道・安定化群・共役」、GRP4「Cauchy の定理・Sylow の定理・有限群への応用」は 2026-09-23 に実装完了。これで Phase A の群論主線は完了した。
...
GRP3 完了により LIE2 / LIE4 に必要な群作用・共役の prerequisite は既に満たされている。GRP4 は有限群論を Sylow 理論まで閉じ、後続 FLD5 の可解群・Galois 理論応用へ再利用できる canonical owner となる。抽象代数系列は RNG1--RNG4--MOD1--MOD2--FLD1--FLD5 へ進み、群・環・体の学部標準コアを独立に完結させる。
