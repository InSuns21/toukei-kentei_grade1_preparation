...
講義が過積載になっていないかは [DREAM THEATER 講義粒度・分割監査](f0-dream-theater-granularity-audit.md) を正本とする。内容を補強する前に粒度を確認し、必要なら先に分割する。
...
02Bは射影→分離→Farkasを証明責務として保持し、SVM凸包幾何を02B1へ分離。C1AはHilbert射影定理の存在・一意性・直交分解を証明責務として独立させ、C2のRiesz証明が明示的にC1Aを前提とする。
