...
- [x] RKHS2「正則化問題の表現定理」を実装。標本部分空間への直交射影が訓練値を保ちながら RKHS ノルムを減らすことから表現定理を証明し、Gram 行列による有限次元化、特異 Gram 行列での係数非一意性、存在・一意性の十分条件、連続線形観測への一般化まで canonical 化した。
- 次の実装対象：**RKHS3「kernel ridge regression」**
