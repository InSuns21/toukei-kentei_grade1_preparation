...
F0-00からこのFacadeが案内する順に本文を読み直し、次の2点を監査しています。

1. その時点でまだ定義されておらず、読者が「なにこれ？」となる概念がないか。
2. 後続章が直接依存する主要命題に、十分な証明が付いているか。

監査結果と証明追加計画は [DREAM THEATER 通読監査：未定義概念と証明補完計画](textbook/f0-dream-theater-proof-audit.md) に集約しています。

> **TODO（証明補完）**  
> 監査でP1に分類した主要命題には、今後この教材内で完全証明を追加します。現時点で特に優先度が高いのは、Lebesgue積分の収束定理群、Carathéodory構成、Radon--Nikodym定理、条件付き期待値、Lévy連続性定理、Plancherel定理、一般化KKT、Itô公式、Wold分解、Herglotz定理です。

また、D2では **Borel σ代数・積測度が定義前に使われる**という実際の読者ブロッカーが見つかっています。これは証明以前のP0として先に修正します。

証明を意図的に本講座の外へ置く発展定理は、単に省略するのではなく **「この教材では定理として使う」** と明示します。したがって今後は、本文を読めば「証明済み」「証明TODO」「意図的な黒箱」を区別できる状態にします。
