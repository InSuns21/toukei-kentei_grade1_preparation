...
集合・写像の基礎から、群・環・加群・体へ進む学部標準の抽象代数系列です。群論の主線 GRP1--GRP4 を閉じ、環論の RNG1 まで実装済みです。
...
GRP1--GRP4 で群・商構造・群作用・Sylow 理論までの有限群論主線を閉じます。RNG1 では環・部分環・零因子・整域・体から入り、環準同型の核をイデアルとして捉え、商環の積が代表元によらず定まる条件と環の第一同型定理までを核心証明付きで構成します。次の抽象代数主線は RNG2 の素イデアル・極大イデアル・中国剰余定理です。
