...
集合・写像の基礎から、群・環・加群・体へ進む学部標準の抽象代数系列です。群論の主線 GRP1--GRP4 と、環論の主線 RNG1--RNG4 を実装済みです。
...
GRP1--GRP4 で群・商構造・群作用・Sylow 理論までの有限群論主線を閉じます。RNG1--RNG3 では環・イデアル・中国剰余定理から Euclid 整域・単項イデアル整域・一意分解整域まで進み、RNG4 で体上の多項式除法、Gauss の補題、UFD 上の多項式環、代表的な既約判定まで閉じます。次の実装主線は MOD1 の加群論で、体論側は FLD1 へ分岐できます。
