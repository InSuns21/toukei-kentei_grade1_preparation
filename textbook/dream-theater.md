...
集合・写像の基礎から、群・環・加群・体へ進む学部標準の抽象代数系列です。群論の主線 GRP1--GRP4 は実装済みです。
...
GRP1 で群・部分群・巡回群・置換群を構成し、GRP2 で準同型・剰余類・商群・同型定理を閉じます。GRP3 では群作用、軌道・安定化群、共役類、中心・中心化群、類等式、有限 $p$-群の中心非自明性まで進みます。GRP4 では Cauchy の定理と Sylow の三定理を核心証明付きで閉じ、正規 Sylow 部分群、位数 $pq$ の群、内部半直積の入口まで有限群の構造判定へ進みます。次の抽象代数主線は RNG1 の環・イデアル・商環です。
