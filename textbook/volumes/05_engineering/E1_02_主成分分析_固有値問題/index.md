...
- 目安時間: 25分
- 主題: 主成分分析
- 使用技術: PCA-EIGEN-1, PCA-VAR-1, PCA-RATIO-1
...
$\operatorname{Var}(Y_2)=10^4\sigma_2^2$, $\operatorname{Cov}(X_1,Y_2)=100\sigma_{12}$。相関係数は不変。したがって分散共分散PCAは一般に変わるが、相関行列PCAは変わらない。
...
PCAは全分散最大方向なので第1変数を選びやすい。そのため第1PCだけ残すと第2変数方向の群差を失い得る。PCAは分散要約、LDAは群分離が目的。
...
固有値6,1。第1方向 $(2,1)^{\mathsf T}/\sqrt5$、寄与率 $6/7$。射影は $P_1=\frac15\begin{pmatrix}4&2\\2&1\end{pmatrix}$。再構成誤差1。単位差が大きい場合は相関行列PCAを検討する。
...
- 分散共分散行列PCAと相関行列PCAを尺度から選べる。
- PCA・白色化・線形判別分析の目的の違いを説明できる。
