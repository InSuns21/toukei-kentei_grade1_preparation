...
群 $g$ の事前確率を $\pi_g$、正規密度を $f_g(\boldsymbol x)$ とします。ベイズ則より
...
なお、2群のフィッシャー型線形判別でも最適な判別方向は、群内共分散を $\Sigma$ とみなせば
...
となります。正規モデルからのベイズ LDAとフィッシャーの判別方向が同じ形になるのは、この共通の「平均差を共分散で補正する」という構造によります。
...
共通共分散・等事前確率なら、まず各群へのMahalanobis距離を比較します。白色化すれば「どちらの中心に近いか」です。ベイズ判別関数が必要なときは、その後で正規密度を展開し、共通の二次項が消えることを確認します。
...
ベイズ則の分母は群共通なので $\pi_gf_g$ を比較する。対数密度の二次形式を展開し群共通項を除くと
