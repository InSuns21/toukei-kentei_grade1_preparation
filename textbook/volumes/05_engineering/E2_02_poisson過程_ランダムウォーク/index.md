...
状態 $0,1,\ldots,N$ を動き、0と$N$を吸収状態とします。位置$i$から始めて$N$へ0より先に到達する確率を $h_i$ とすると
$$h_0=0,\qquad h_N=1.$$
内部状態では最初の1歩で条件付けて
$$
...
$p=q=1/2$ なら
$$2h_i=h_{i+1}+h_{i-1}.$$
これは第2差分が0という式なので $h_i=A+Bi$。境界条件から
$$
\boxed{h_i=\frac iN}.
$$
...
0または$N$へ吸収されるまでの平均ステップ数を $m_i$ とします。境界では
$$m_0=m_N=0.$$
内部状態では最初の1歩を加えて
$$
...
$$
二次式 $m_i=i(N-i)$ が境界条件とこの差分方程式を満たすので
$$
\boxed{m_i=i(N-i)}.
$$
...
第2差分0なので $h_i=A+Bi$。境界から $A=0,B=1/8$。よって
$$h_3=3/8.$$
###### 本番答案
$2h_i=h_{i+1}+h_{i-1}$、境界 $h_0=0,h_8=1$ より $h_i=i/8$、したがって $3/8$。
###### 採点基準
境界4点、再帰式6点、一次解5点、代入5点。合計20点。
<!-- solution-end -->
...
##### 解答
###### 詳細解答
$m_i$を平均時間とすると $m_0=m_8=0$、内部で
$$m_i=1+\frac12m_{i+1}+\frac12m_{i-1}.$$
対称ランダムウォークでは解は $m_i=i(N-i)$。$N=8,i=3$ より
$$m_3=3(8-3)=15.$$
確認すると第2差分は$-2$で境界も満たす。
###### 本番答案
$m_i=1+(m_{i-1}+m_{i+1})/2$ の解 $m_i=i(8-i)$ より15。
###### 採点基準
境界4点、再帰式6点、解形確認5点、値5点。合計20点。
<!-- solution-end -->
...
$p\ne q$なので一般解は
$$h_i=A+B(q/p)^i=A+B(2/3)^i.$$
$h_0=0$から$A=-B$、$h_5=1$から
$$B\{(2/3)^5-1\}=1.$$
したがって
...
$$
###### 本番答案
再帰式を解いて $h_i=\{1-(2/3)^i\}/\{1-(2/3)^5\}$、ゆえに $h_2=135/211$。
###### 採点基準
境界・再帰6点、一般解5点、境界適用5点、数値4点。合計20点。
<!-- solution-end -->
