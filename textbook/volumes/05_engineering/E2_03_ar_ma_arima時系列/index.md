...
時系列では観測の順序そのものが情報を持ちます。昨日と今日の値が似る、ショックの影響が数期残る、トレンドがある、といった時間依存を無視して独立標本として扱うことはできません。本章ではまず定常性・自己共分散・自己相関を定義し、観測系列から標本ACFを作る方法を確認したうえで、AR、MA、ARMA、ARIMAを「どの過去情報が現在へ残るか」という観点から組み立てます。
...
実データではまず時系列プロットを見て、平均水準が変化するトレンド、周期的な季節性、ばらつきの変化、外れたショックがないかを確認する。ARMAの理論ACFを当てはめる前に、そもそも定常系列として扱えるかを見る必要がある。
...
$$
分母を $n-h$ とする流儀もあるので、問題文で定義が与えられたらそれに従う。モデルの $\rho(h)$ は母集団側の量、$\hat\rho(h)$ は有限個の観測から計算する推定値である。
...
トレンドがある系列に差分を取り、差分後の平均やACFが時間に依存しない形へ近づくかを確認する。ただし必要以上に差分すると、かえって不要なMA構造を作ることがある。
...
理工学の時系列解析、自己回帰過程、移動平均過程、ARIMA過程を担当する。AR/MAの公式を個別暗記するのではなく、観測系列から標本ACFを作り、白色雑音との共分散を取って理論自己共分散を導けることを重視する。
