...
$$
の零点がすべて単位円 $|z|=1$ の外側にあるとき、現在値を過去のinnovationの収束する和として表せる因果的な弱定常解を持ちます。

これはAR(1)の $|\phi|<1$ の一般化です。AR(1)なら零点は
$$
z=\frac1\phi
$$
なので、$|z|>1$ はちょうど $|\phi|<1$ と同値です。
...
弱定常とみなせる3系列A,B,Cについて、十分大きい標本から次のコレログラムが得られた。`|` より右の棒は正、左向きの棒は負の標本相関を表し、`.` は0付近を表す。小さな棒は標本変動の範囲内と考えてよい。
