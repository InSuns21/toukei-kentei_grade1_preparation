...
プロビット分析では潜在連続変数の「閾値を超えたか」だけを二値で観測しました。トービット分析では、ある閾値までは検閲される一方、閾値を超えた観測では連続値そのものが見えます。

<a id="def-e1-04-tobit"></a>
...
<!-- definition-example-start: def-e1-04-tobit -->
**定義の確認**  
$c=0$ の支出データで「支出なし」は潜在支出が0以下だったことだけを示し、正の支出では金額そのものを観測すると考えます。二値しか見ないプロビットとは観測情報量が異なります。
...
=
\Phi\!\left(
\frac{c-x_i^\mathsf T\beta}{\sigma}
\right),
$$
$y_i>c$ なら
$$
\frac1\sigma
\phi\!\left(
\frac{y_i-x_i^\mathsf T\beta}{\sigma}
\right)
$$
です。したがって左検閲集合を $C$、非検閲集合を $U$ とすれば
$$
L(\beta,\sigma)
=
\prod_{i\in C}
...
検閲された $Y$ に通常の最小二乗法をそのまま適用すると、0などの検閲点を本来の潜在値そのものとして扱ってしまいます。検閲機構がモデルどおりなら、トービット尤度を使うのが自然です。

### 3A.1 例：プロビット・トービット・通常回帰の違い

- 結果が「購入した/しない」だけならプロビットやロジット。
- 購入しない人は0、購入した人は正の金額まで観測され、0が潜在量の検閲と解釈できるならトービット。
- 連続値が検閲なしで観測されるなら通常の線形回帰。

モデル選択は応答変数が実際にどう観測されたかで決まります。
...
<!-- definition-example-start: def-e1-04-hinge-loss -->
**定義の確認**  
