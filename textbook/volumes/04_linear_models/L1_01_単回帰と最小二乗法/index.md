...
## 3A. 公式シラバス補完：平均への回帰（回帰効果）

<a id="def-l1-01-regression-to-mean"></a>

<!-- formal-statement-start -->
> **定義（平均への回帰・回帰効果）**  
> 同じ性質を繰り返し測るとき、1回目に極端な値を示した集団では、2回目の条件付き平均が母平均に近づく現象を平均への回帰（回帰効果）という。
<!-- formal-statement-end -->

<!-- definition-example-start: def-l1-01-regression-to-mean -->
**定義の確認**  
標準化した2回の測定 $(Z_1,Z_2)$ が相関 $\rho$ の2変量正規分布に従えば
$$
E[Z_2\mid Z_1=z]=\rho z.
$$
$|\rho|<1$ なら、$z$ が極端でも次回の条件付き平均の絶対値は $|z|$ より小さく、0すなわち母平均側へ寄ります。
<!-- definition-example-end -->

平均への回帰は「極端な値を選んだこと」と測定誤差・個体内変動から自然に起こり、介入の効果とは限りません。前後比較で極端な対象だけを選ぶ研究では特に注意が必要です。

### 3A.1 例：成績最下位群だけ補習する

最下位群を選んで補習後に点数が上がっても、その一部は平均への回帰だけで説明できる可能性があります。因果効果を評価するには比較群や無作為割付などの研究設計が必要です。
...
<a id="ex-l1-01-regression-to-mean"></a>
#### L101-B05 平均への回帰（回帰効果）

- Level: B
- 目安時間: 10分
- 主題: 平均への回帰

標準化した2回の測定の相関が $\rho=0.6$ で、1回目が平均より2標準偏差高かった。

1. 2回目の条件付き平均は平均より何標準偏差高いか。
2. この低下だけから介入の悪影響を結論できない理由を述べよ。
...
$E[Z_2\mid Z_1=2]=0.6\times2=1.2$。極端な1回目を条件に選んだだけで、次回の期待値は母平均側へ寄る平均への回帰が生じるため、変化だけでは因果効果を識別できない。

##### 本番答案

条件付き平均は1.2標準偏差上。極端値選択による平均への回帰があるので、変化だけを介入効果とみなせない。
