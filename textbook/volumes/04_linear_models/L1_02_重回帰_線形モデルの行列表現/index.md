...
<a id="def-l1-02-multiple-correlation"></a>

<!-- formal-statement-start -->
> **定義（重相関係数）**  
> 切片を含む重回帰モデルで、観測ベクトル $Y$ と当てはめ値 $\widehat Y$ のPearson相関係数を重相関係数という。非負の値を採用し、
>
> $R=\operatorname{Corr}(Y,\widehat Y)=\sqrt{R^2}$
>
> と書く。ただし $\widehat Y$ が定数でない場合を考える。
<!-- formal-statement-end -->

<!-- definition-example-start: def-l1-02-multiple-correlation -->
**定義の確認**  
決定係数が $R^2=0.64$ なら重相関係数は $R=0.8$ です。説明変数が1個でも複数でも、切片あり最小二乗では当てはめ値と観測値の相関の二乗が決定係数になります。
<!-- definition-example-end -->

### 3A.1 例：符号を付けない

単回帰の相関係数は正負を持ちますが、重回帰では複数説明変数をまとめた予測との結びつきを表すため、重相関係数は $0\le R\le1$ の非負量として扱います。
...
<a id="ex-l1-02-multiple-correlation"></a>
#### L102-A05 重相関係数
...
切片を含む重回帰で決定係数が0.81であった。重相関係数を求め、その値の範囲を答えよ。
...
$R=\sqrt{0.81}=0.9$。重相関係数は $0\le R\le1$。
