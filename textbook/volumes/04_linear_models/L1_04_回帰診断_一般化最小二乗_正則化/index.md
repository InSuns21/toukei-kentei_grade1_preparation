...
- 残差プロットと DW 比から非線形性、不均一分散、系列相関の兆候を読み取る。
- 一般の誤差分散共分散行列の下で一般化最小二乗法推定量を導出し、白色化との関係を説明する。
...
| 範囲 | 本章の内容 |
|---|---|
| 残差分析 | 残差分散、Student化残差、残差プロット、Q-Qプロット |
| 回帰診断法 | レバレッジ、Cook距離、影響点、系列相関、DW比、診断から対処への流れ |
| 一般化最小二乗推定 | 一般化最小二乗法、WLS、白色化、通常最小二乗法との違い |
...
1. L1-02: $\hat\beta=(X^{\mathsf T}X)^{-1}X^{\mathsf T}y$、ハット行列、ガウス・マルコフの定理、一般線形仮説。
...
| $\operatorname{Var}(\varepsilon\mid X)=\sigma^2I$ | 通常最小二乗法がBLUE、通常の標準誤差 | 不均一分散・相関誤差で効率と標準誤差が崩れる |
| 正規性 | 小標本での厳密な$t,F$推測 | 外れ値への感度、厳密分布の崩れ |
| $X$ が列フルランク | 係数の一意な識別 | 完全共線性では$(X^{\mathsf T}X)^{-1}$が存在しない |

「残差が正規でないから通常最小二乗法は必ず偏る」のように、仮定の役割を混同しないことが大切です。
...
![高レバレッジ点・大残差点・影響点の違い](./figures/influence-diagnostics.svg)

図の横方向は「説明変数空間でどれだけ離れているか」、縦方向は「回帰面からどれだけ離れているか」を表します。Cook距離は、この2種類の外れ方が当てはめ全体をどれだけ動かすかへまとめた指標と考えると整理しやすくなります。
...
![残差診断で現れる代表的なパターン](./figures/residual-diagnostics.svg)

上の図は「形 → 疑う仮定」を対応させたものです。同じ残差でも、横軸を当てはめ値にするか観測順序にするかで見ている問題が変わります。
...
### 4.3 Durbin--Watson比（DW比）

観測が時間順など意味のある順序で並んでいるとき、隣り合う残差が似た符号・大きさを持つかを数値化する代表的な診断量が **Durbin--Watson比（DW比）**です。

$$
\boxed{
DW
=\frac{\displaystyle\sum_{t=2}^n(e_t-e_{t-1})^2}
{\displaystyle\sum_{t=1}^n e_t^2}
}
$$

隣接残差の1次自己相関を $\hat\rho_1$ とすると、端の影響を無視した近似として

$$
\boxed{DW\approx2(1-\hat\rho_1)}
$$

と理解できます。したがって、

- $DW\approx2$: 強い1次系列相関は見えにくい。
- $DW<2$: 正の系列相関を疑う。
- $DW>2$: 負の系列相関を疑う。

という方向をまず読みます。ただし、**「2から何だけ離れたら有意か」を固定値だけで判定する検定ではありません**。厳密な判定では標本サイズや説明変数数に依存する境界値を使います。また、DW比は主に1次の系列相関を見る診断なので、残差 vs 観測順序の図と組み合わせて読むのが基本です。

![DW比と残差列の対応](./figures/durbin-watson-patterns.svg)

図では、正の系列相関では隣接残差が似るため差 $(e_t-e_{t-1})$ が小さくなり、DW比が2より小さくなりやすいことが見えます。逆に正負が交互に現れる負の系列相関では隣接差が大きくなり、DW比は2より大きくなりやすくなります。

### 4.4 正規Q-Qプロット

誤差の正規性は、主に小標本での厳密な $t,F$ 推測や外れ値感度に関係します。Q-Qプロットで系統的な湾曲や端の大きなずれがあれば、重い裾や歪みを疑います。

![正規Q-Qプロットの読み方](./figures/normal-qq-reading.svg)

基準直線への「近さ」だけでなく、**どの部分がどちら向きに外れるか**を見ます。両端が外側へ離れるなら重い裾、全体が一方向へ系統的に湾曲するなら歪みを疑う、という読み分けが基本です。
...
![1点が回帰直線を強く動かす数値例](./figures/influential-point-numeric.svg)

第5点 $(2,9)$ を除いた4点だけなら回帰直線は $\hat y=2.6+1.2x$ です。第5点を加えると $\hat y=3.4+2x$ へ変わります。以下では、この変化をレバレッジ、Student化残差、Cook距離で数値化します。
...
したがって変換後のモデルへ通常最小二乗法を適用すればよく、その推定量は一般化最小二乗法と一致します。
...
楕円は「方向によって誤差の分散が違い、成分どうしも相関している」ことを表します。$L^{-1}$ によって円へ変換できれば、変換後は通常最小二乗法が想定する等分散・無相関の形になります。

### 7.1 WLSは一般化最小二乗法の対角版
...
実務では $\Omega$ 自体が未知で推定することも多く、その場合を実行可能一般化最小二乗法と呼びます。一方、平均構造の係数推定は通常最小二乗法のままにして、標準誤差だけを不均一分散に頑健化する方法もあります。目的が「効率よく推定すること」なのか「標準誤差を正しく評価すること」なのかを分けて考えます。
...
![多重共線性とVIF](./figures/multicollinearity-vif.svg)

説明変数どうしがほぼ一直線なら、データが存在しない方向の情報が乏しくなります。これが $X^{\mathsf T}X$ の小さい固有値として現れ、逆行列で係数の不確実性が大きく増幅されます。
...
です。したがって強い共線性があっても、それ自体は通常最小二乗法の不偏性を壊しません。
...
説明変数を追加すると、通常最小二乗法の RSS は増えません。したがって RSS だけで候補モデルを選ぶと、常に大きいモデルが有利になります。
...
![LASSOのL1制約と軟閾値](./figures/lasso-l1-geometry.svg)

罰則形は、適切な $t$ に対して「残差平方和を最小にしつつ $\sum_j|\beta_j|\le t$」という制約形で見ることもできます。L1制約領域は角を持つため、二乗誤差の等高線が座標軸上の角で接しやすく、係数が厳密に0になる幾何的な理由が見えます。
...
| 不均一分散 | 残差が扇形 | WLS/一般化最小二乗法、分散モデル、頑健標準誤差 |
| 相関誤差 | 残差が時間順に連続、DW比が2から系統的に離れる | 一般化最小二乗法、時系列・状態空間モデル |
| 多重共線性 | VIF大、係数標準誤差大、符号不安定 | 変数再設計、追加データ、正則化 |
...
- 目安時間: 25分
- 主題: 回帰診断
- 使用技術: 通常最小二乗法、ハット値、Student化残差、Cook距離
...
2. 扇形は不均一分散の兆候である。分散構造が分かるなら WLS/一般化最小二乗法、未知なら分散モデルや頑健標準誤差などを検討する。
...
2. 扇形 → 不均一分散。WLS/一般化最小二乗法または頑健な分散評価を検討。
...
- Level: D
- 目安時間: 35分
- 主題: 回帰診断・一般化最小二乗法・多重共線性・正則化
- 使用技術: 残差分散、VIF、一般化最小二乗法、LASSO
...
3. 一般化最小二乗法の白色化変換を $L$ を用いて書き、変換後誤差の分散を示せ。
4. LASSO の2係数を求めよ。
...
各診断の対象は異なる。レバレッジとCook距離は観測単位の影響、多重共線性は設計行列における係数識別の不安定性、一般化最小二乗法は誤差の分散共分散構造、LASSOはバイアスを許して係数を縮める推定・選択の問題を扱う。したがって、たとえばVIFが高いから一般化最小二乗法を行う、Cook距離が高いからLASSOを使う、という直接対応にはならない。
...
4. 一般化最小二乗法推定量を式で書き、通常最小二乗法と異なる重みが入る場所を示せ。
...
通常最小二乗法の $X^{\mathsf T}X$ と $X^{\mathsf T}y$ に対し、一般化最小二乗法では $\Omega^{-1}$ が観測方向の重みとして両方に入る。
...
- [Standard 39 一般化最小二乗法](../../../../applied-rikou-80/standard/39_gls.md): GLS推定量、分散、白色化、通常最小二乗法との比較。
...
- 残差 vs 観測順序と DW 比から、1次系列相関の方向を説明できる。
- Q-Qプロットの系統的なずれから、正規性の問題を診断できる。
- 一般化最小二乗法推定量を重み付き二次形式から導き、白色化で説明できる。
