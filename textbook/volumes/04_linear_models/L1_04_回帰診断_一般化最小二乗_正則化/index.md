...
- 一般の誤差分散共分散行列の下で 一般化最小二乗法 推定量を導出し、白色化との関係を説明する。
...
「残差が正規でないから 通常最小二乗法 は必ず偏る」のように、仮定の役割を混同しないことが大切です。
...
したがって変換後のモデルへ 通常最小二乗法 を適用すればよく、その推定量は 一般化最小二乗法 と一致します。
...
楕円は「方向によって誤差の分散が違い、成分どうしも相関している」ことを表します。$L^{-1}$ によって円へ変換できれば、変換後は通常の 通常最小二乗法 が想定する等分散・無相関の形になります。
...
実務では $\Omega$ 自体が未知で推定することも多く、その場合を実行可能一般化最小二乗法と呼びます。一方、平均構造の係数推定は 通常最小二乗法 のままにして、標準誤差だけを不均一分散に頑健化する方法もあります。目的が「効率よく推定すること」なのか「標準誤差を正しく評価すること」なのかを分けて考えます。
...
です。したがって強い共線性があっても、それ自体は 通常最小二乗法 の不偏性を壊しません。
...
説明変数を追加すると、通常最小二乗法 の RSS は増えません。したがって RSS だけで候補モデルを選ぶと、常に大きいモデルが有利になります。
...
3. 一般化最小二乗法 の白色化変換を $L$ を用いて書き、変換後誤差の分散を示せ。
4. LASSO の2係数を求めよ。
...
4. 一般化最小二乗法 推定量を式で書き、通常最小二乗法と異なる重みが入る場所を示せ。
...
通常最小二乗法 の $X^{\mathsf T}X$ と $X^{\mathsf T}y$ に対し、一般化最小二乗法 では $\Omega^{-1}$ が観測方向の重みとして両方に入る。
