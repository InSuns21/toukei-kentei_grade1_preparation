...
- [I2-01 の最尤推定量の漸近正規性](../I2_01_漸近推測_Delta法/index.md#thm-i2-01-mle-asymptotic-normality)

## この章で解けるようになる問題

- 95%信頼区間の95%が何を意味するかを被覆確率で説明できる。
...
## 0. まず95%信頼区間を2本、数値で作る

一般論へ入る前に、最も基本的な正規母平均の区間を実際に計算します。

### 0.1 母標準偏差が既知ならZ区間

母標準偏差が $\sigma=5$ g と分かっている製品重量を25個測り、
...
95%信頼区間では $z_{0.975}\approx1.96$ を使うので、半幅は
...
という形です。後で、この形がなぜ正しいのかをピボット量から導きます。

### 0.2 母標準偏差が未知ならt区間

今度は母標準偏差が未知で、正規母集団から
...
ただし $\sigma$ を同じ標本から推定したので、有限標本では標準正規分布ではなく自由度15のt分布を使います。
...
です。同じ「推定値 ± 分位点 × 標準誤差」でも、未知分散ではt分布を使うため、同じ標準誤差ならZ区間より少し広くなります。

### 0.3 「95%」は観測後の母数の確率ではない

たとえば最初の標本から $[100.04,103.96]$ が得られた後、頻度論では母平均 $\mu$ は固定値です。したがって

> 「この固定区間に $\mu$ が95%の確率で入る」

とは読みません。

95%というのは、**同じ標本抽出と同じ区間構成法を何度も繰り返したとき、作られる区間の95%が固定された真の母数を含む**という性質です。以下では、この意味を被覆確率として正式に定義します。
...
正しい頻度論的解釈は、**同じ標本抽出と区間構成を何度も繰り返せば、作られた区間の少なくとも95%が真の母数を含む**、です。

観測後の固定区間 $[a,b]$ について、母数を固定値として扱う頻度論では
...
$X_i\overset{\mathrm{iid}}{\sim}N(\mu,\sigma^2)$、$\sigma$ 既知とします。標準正規分布の $1-\alpha/2$ 分位点を $z_{1-\alpha/2}$ とすれば
...
上の区間では下端・上端が $\bar X$ に依存するため、標本抽出前にはランダムです。また各 $\mu$ について
...
ピボット量の確率式と母数について解いた不等式は標本ごとに同値なので、事象
...
は同じです。前者の確率が $1-\alpha$ なら後者の確率も $1-\alpha$ です。
...
が $N(0,1)$ に従い、その分布は未知の $\mu$ に依存しません。従ってピボット量です。一方、$\bar X-\mu$ の分布は $N(0,\sigma^2/n)$ なので、$\sigma$ まで未知ならそのままでは未知母数を含み、完全なピボット量ではありません。
...
カイ二乗分布を使う母分散の区間では、4の反転で端点が逆になるため要注意です。
...
が必要です。標本サイズ設計そのものは S1-03 の主題ですが、信頼区間の幅からこの式が出ることは押さえます。
...
正規母集団のt区間は、標本サイズが小さくても $T\sim t_{n-1}$ が厳密に成り立つことから構成されています。従って「$n$ が大きいからだいたい95%」ではなく、正規性の仮定の下で有限標本でも被覆確率が正確に $1-\alpha$ です。
...
$$
\frac cb\le\sigma^2\le\frac ca.
$$

従って**大きいカイ二乗分位点が下端、小さい分位点が上端**に来ます。
...
$(-\infty,\bar X+z_{1-\alpha}\sigma/\sqrt n]$ は上端だけが有限の片側区間です。真の $\mu$ がこの区間に含まれる確率は
...
元の区間が厳密で $g$ が単調なら、区間変換後も被覆確率は元と同じです。これは単なる一次近似であるデルタ法区間より強い性質です。

### 9.2 例: 指数分布の平均寿命

率母数 $\lambda$ の区間が $[L_\lambda,U_\lambda]$ なら、平均寿命 $m=1/\lambda$ は単調減少変換なので
...
$X_1,\ldots,X_n\overset{\mathrm{iid}}{\sim}\mathrm{Exp}(\lambda)$ を率母数表示で考えます。和
...
$U(0,\theta)$ は支持が母数に依存するため通常の正則な最尤推定量の理論から外れます。しかし、**正則性がないことと信頼区間が作れないことは別**です。適切な有限標本ピボット量があれば厳密区間を作れます。
...
頻度論では母数 $\theta$ は固定値であり、標本から作る区間 $[L(X),U(X)]$ がランダムです。95%とは、同じ標本抽出と区間構成を繰り返したとき
...
標準誤差は $3/6=0.5$。上側95%信頼限界は

$$
20+1.645\times0.5=20.8225,
$$

従って $\boxed{\mu\le20.8225}$。両側95%区間は

$$
20\pm1.96\times0.5
$$

より $\boxed{[19.02,20.98]}$。片側では片側の裾に5%、両側では左右に2.5%ずつ置くため分位点が異なります。
...
成功数が小さく境界に近いため、正規近似に基づく有限標本被覆は不安定である。
...
指数分布の率母数 $\lambda$ の95%信頼区間が $[0.2,0.5]$ であった。平均寿命 $m=1/\lambda$ の95%信頼区間を求めよ。また、なぜ端点が逆になるか説明せよ。
...
$g(\lambda)=1/\lambda$ は $\lambda>0$ で単調減少なので
...
正規母集団から $n=25$、$\bar X=12$、$S^2=9$ を得た。次を用いてよい。
...
$t$ ピボットとカイ二乗ピボットが有限標本で厳密に成り立つには母集団の正規性が必要です。
...
前者は有限標本で $2\lambda S\sim\chi^2_{20}$ を用い、後者は最尤推定量の漸近正規近似を用いています。$n=10$ で両者が一致する理由はありません。
...
$g'(\theta)=1/(2\sqrt\theta)$ なので、$\hat\theta=6.25$ では
...
この分布は $\theta$ に依存しないため $R$ はピボット量です。また

$$
P(R\ge\alpha^{1/n})=1-\alpha.
$$

事象 $M/\theta\ge\alpha^{1/n}$ は

$$
\theta\le\frac M{\alpha^{1/n}}
$$

と同値で、常に $\theta\ge M$ なので
...
を考える。制約 $b^n-a^n=1-\alpha$ を使って $b$ を $a$ の関数として表せ。
4. $W$ を $a$ で微分し、許される範囲で $W$ が単調減少することを示せ。
5. この区間族で相対幅を最小にする $a,b$ と、その信頼区間を求めよ。
...
従って相対幅は $a$ が最大のとき最小です。許される最大値は

$$
a=\alpha^{1/n}
$$

で、このとき $b=1$。従って最短の区間は
...
なので $a$ を最大にすればよい。$b\le1$ より最大 $a=\alpha^{1/n}$、そのとき $b=1$。従って

$$
\boxed{\theta\in[M,M\alpha^{-1/n}]}.
$$
...
95%の意味は、同じ標本抽出と区間構成を繰り返したとき、作られた区間の95%が固定された真の $\mu$ を含むということです。

また $t_\nu$ が自由度の増加とともに標準正規分布へ近づくことは分位点の近似に関する話です。母集団非正規でもt統計量が有限標本で厳密にt分布に従うことを意味しません。大標本で中心極限定理等による別の漸近近似が使える場合はありますが、その根拠は別です。
