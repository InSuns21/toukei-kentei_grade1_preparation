...
標準正規分布の分布関数を $\Phi$ とし、$z_{0.95}=1.645$ とする。

1. $\bar X$ による棄却域を求めよ。
2. 真の母平均を $\mu$ としたときの検出力関数 $\pi_n(\mu)$ を求めよ。
3. $n=9,16$ のそれぞれについて、$\mu=0,0.4,0.8$ における検出力を求めよ。
4. 3. の値と検出力関数の形を用いて、$n=9$ と $n=16$ の検出力曲線を同一座標上に手で概形として描け。
...
|---:|---:|---|---:|
| 9 | 0 | $P(Z>1.645)$ | $0.05$ |
| 9 | 0.4 | $P(Z>1.645-3\times0.4)=P(Z>0.445)$ | 約 $0.33$ |
| 9 | 0.8 | $P(Z>1.645-3\times0.8)=P(Z>-0.755)$ | 約 $0.77$ |
| 16 | 0 | $P(Z>1.645)$ | $0.05$ |
| 16 | 0.4 | $P(Z>1.645-4\times0.4)=P(Z>0.045)$ | 約 $0.48$ |
| 16 | 0.8 | $P(Z>1.645-4\times0.8)=P(Z>-1.555)$ | 約 $0.94$ |
...
を通ることを押さえる。さらに $\mu>0$ では、同じ $\mu$ に対して $\sqrt n\,\mu$ が大きい $n=16$ の方が検出力が高い。したがって $n=16$ の曲線は $n=9$ の曲線より早く立ち上がる。どちらも $\mu$ の増加とともに単調に増加し、最終的に1へ近づく。

![模範解答：n=9とn=16の右片側検定の検出力曲線](./power_curve_hand_sketch_answer.svg)

図を厳密な縮尺で再現する必要はない。$(0,0.05)$ と上表の4点を打ち、**単調増加・$n=16$ が上側・1へ漸近**という形を保って滑らかに結べばよい。
<!-- solution-end -->
