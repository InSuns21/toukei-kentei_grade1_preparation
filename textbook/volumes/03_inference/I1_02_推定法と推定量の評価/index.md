...
$n\ge2$ で $T_2$ の方が小さい。支持が $\theta$ に依存するため通常のクラーメル・ラオの不等式 の正則条件が破れる。
...
なのでクラーメル・ラオの不等式 は $\lambda/n$。$T_1$ の分散はこれに一致します。
...
よって $T_1$ は平均二乗誤差で一致的。$T_2$ は分布が $n$ に依存せず一致しない。
...
$g(\lambda)=e^{-\lambda}=q$ なので $g'=-q$。従ってクラーメル・ラオの不等式 は
