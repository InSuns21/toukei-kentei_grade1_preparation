...
| 最小二乗法 | 残差平方和の最小化、位置母数。回帰モデルへの一般化は L1-01 / L1-02 |
| BLUE | 線形不偏推定量、逆分散重み、「best」の比較クラス |
...
同じ母数に対して複数の推定量を構成できる場合があります。このとき、推定量は不偏性だけでなく、分散・平均二乗誤差・一致性・相対効率などを用いて比較します。
...
<!-- formal-statement-start -->
> **定義（BLUE）**  
> ある母数を推定する線形不偏推定量のクラスの中で、分散が最小となる推定量を **最良線形不偏推定量（BLUE: Best Linear Unbiased Estimator）** という。
<!-- formal-statement-end -->

<!-- definition-example-start: def-i1-02-blue -->
**定義の確認**  
独立な不偏測定 $Y_1,Y_2$ の分散がそれぞれ $1,4$ なら、$wY_1+(1-w)Y_2$ の分散を最小にする重みは $w=4/5$ です。線形かつ不偏というクラスの中で分散を最小にする、という BLUE の「best」を具体化しています。
<!-- definition-example-end -->

### 6.1 「best」は比較するクラスを固定してから言う

BLUE の **B** は「考えられるすべての推定量の中で最良」という意味ではありません。比較対象を

1. 観測値の**線形結合**であり、
2. 推定対象に対して**不偏**である

ものに限定し、その中で分散が最小という意味です。したがって、非線形推定量やバイアスを許した推定量まで含めれば別の比較問題になります。

### 6.2 回帰モデルの行列版は L1-02 へ

ここでは BLUE を「独立な不偏測定値をどう重み付けするか」という最小構成で理解します。$Y=X\beta+\varepsilon$ のような回帰モデルで係数ベクトル全体を比較するには、行列で不偏性と分散共分散行列を扱う準備が必要です。その一般化は後続の [L1-02](../../04_linear_models/L1_02_重回帰_線形モデルの行列表現/index.md) で扱います。
...
### I1-02-B06 パレート下端最尤推定量のバイアス・不偏補正・一致性
...
### I1-02-C03 3つの不偏測定値から BLUE を作る

- Level: C
- 目安時間: 25分
- 主題: BLUE
- 使用技術: 線形不偏性、分散、Cauchy--Schwarz不等式

独立な $Y_1,Y_2,Y_3$ が共通の平均 $\mu$ をもち、

$$
\operatorname{Var}(Y_1)=1,\qquad
\operatorname{Var}(Y_2)=4,\qquad
\operatorname{Var}(Y_3)=9
$$

を満たすとする。線形推定量

$$
T=w_1Y_1+w_2Y_2+w_3Y_3
$$

を考える。

1. $T$ が $\mu$ の不偏推定量となる条件を求めよ。
2. 一般に $v_i=\operatorname{Var}(Y_i)$ とするとき、Cauchy--Schwarz不等式から
   $$
   \operatorname{Var}(T)\ge\frac{1}{\sum_i1/v_i}
   $$
   を示せ。
3. 等号条件から分散を最小にする重みを求めよ。
4. 今回の $v_1=1,v_2=4,v_3=9$ について、重みと最小分散を具体的に求めよ。
...
$$
\operatorname{Var}(T)=\sum_i v_iw_i^2.
$$

不偏条件と Cauchy--Schwarz不等式から

$$
1
=\left(\sum_iw_i\right)^2
=\left(\sum_i(\sqrt{v_i}w_i)\frac1{\sqrt{v_i}}\right)^2
\le
\left(\sum_iv_iw_i^2\right)
\left(\sum_i\frac1{v_i}\right).
$$
...
すなわち $w_i=c/v_i$ のときに成立します。不偏条件から
...
$$
\boxed{\operatorname{Var}(T)=\frac{36}{49}}.
$$

分散が小さい測定値ほど大きな重みを持つ、という逆分散重みが3変数でもそのまま現れます。

##### 本番答案

不偏条件は $\sum_iw_i=1$。独立性より $\operatorname{Var}(T)=\sum_iv_iw_i^2$ で、

$$
1=(\sum_iw_i)^2
\le
(\sum_iv_iw_i^2)(\sum_i1/v_i)
$$

だから

$$
\operatorname{Var}(T)\ge1/(\sum_i1/v_i).
$$

等号条件より $w_i\propto1/v_i$。したがって

$$
\boxed{(w_1,w_2,w_3)=(36,9,4)/49},
\qquad
\boxed{\min\operatorname{Var}(T)=36/49}.
$$

##### 採点基準

- 不偏条件: 4点
- 分散式とCauchy--Schwarz: 7点
- 等号条件と一般重み: 5点
- 数値重みと最小分散: 4点
...
- `MATH-2024-Q1`: 回帰係数の推定・検定・検出力。本章では推定量評価の一般原理とスカラー BLUE までを担当し、回帰係数の行列表現と線形モデルでの最小分散性は L1-02、検定・検出力は I3 系で扱う。
...
- 逆分散重みが線形不偏推定量の中で最小分散になることを説明できる。
