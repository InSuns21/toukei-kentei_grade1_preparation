...
- Gauss--Markovの定理がいう「最良」の意味を行列で説明する。
- Bernoulli、Poisson、正規、指数分布の Fisher 情報量を計算する。
- Cramér--Rao不等式を導出し、推定量が下限を達成するか判定する。
- 一様分布のような非正則モデルで、Cramér--Rao下限を無条件に使ってはいけない理由を説明する。
...
| BLUE | 逆分散重み、Gauss--Markovの定理 |
| 不偏性 | バイアス、標本分散、順序統計量の補正 |
| 一致性 | 確率収束、MSEからの十分条件、大数則 |
| 有効性 | 分散比較、Cramér--Rao下限の達成、相対効率 |
| Fisher情報量 | スコア二乗、二階微分表示、独立標本での加法性 |
| Cramér--Rao不等式 | 一般の $g(\theta)$ に対する分散下限と等号条件 |

## 前提知識チェック

1. I1-01: 尤度、対数尤度、スコア関数を計算できる。
2. P4-02: 確率収束と大数則の意味を説明できる。
3. L1-02: $Y=X\beta+\varepsilon$ の行列表現と OLS 推定量を知っている。
...
この式は「不偏なら必ず最良」ではないことを示します。少しバイアスを許して分散を大きく下げれば、MSEが小さくなる場合があります。
...
<!-- formal-statement-start -->
> **定義（BLUE）**  
> 線形不偏推定量のクラスの中で分散、ベクトル母数では共分散行列、が最小となる推定量を最良線形不偏推定量という。
<!-- formal-statement-end -->
...
任意の線形不偏推定量 $AY$ は $AX=I$ を満たします。OLSの係数行列を $A_0=(X^\mathsf TX)^{-1}X^\mathsf T$ とし、$A=A_0+D$ と書くと $DX=0$ です。すると共分散の差に交差項が残らず、$DD^\mathsf T$ だけが残ります。
...
は半正定値です。したがって OLS は線形不偏推定量の中で共分散行列が最小です。
<!-- proof-end -->

重要なのは、Gauss--Markovが「すべての推定量の中で最良」とは言っていないことです。比較対象は**線形かつ不偏**な推定量です。また正規性は BLUE の結論自体には不要です。
...
<!-- formal-statement-start -->
> **定義（有効推定量）**  
> 正則モデルで不偏推定量が Cramér--Rao 下限を達成するとき、その推定量を本章では有効推定量という。
<!-- formal-statement-end -->

### 8.2 Bernoulli標本平均は下限を達成する
...
従って $\bar X$ は Cramér--Rao 下限を達成します。
...
### 9.1 一様分布では Cramér--Rao を機械適用しない
...
$X_1,\ldots,X_n$ は平均 $\mu$、分散 $\sigma^2<\infty$ の i.i.d. 標本とする。
...
よって MSE $\to0$ なので $\boxed{T_n\to_p\mu}$。
...
2. それぞれの分散を求めて比較せよ。
3. この比較に通常の Cramér--Rao 下限をそのまま使わない理由を述べよ。
...
一様分布は支持 $[0,\theta]$ が母数に依存し、通常の正則条件を満たさないため、標準形の Cramér--Rao 不等式を機械適用できません。
...
$n\ge2$ で $T_2$ の方が小さい。支持が $\theta$ に依存するため通常の CRLB の正則条件が破れる。
...
### I1-02-C01 Poisson推定量の不偏性・一致性・有効性

- Level: C
- 目安時間: 25分
- 主題: 推定量評価の連結問題
- 使用技術: 不偏性、一致性、Fisher情報量、CRLB
...
4. Fisher情報量を求め、$T_1$ が Cramér--Rao 下限を達成することを示せ。
...
$T_1$ は不偏で分散が0へ収束するので一致的です。一方 $T_2=X_1$ の分布は $n$ を増やしても変わりません。たとえば $0<\varepsilon<1/2$ を固定すると、Poisson分布は離散なので一般に
...
なので CRLB は $\lambda/n$。$T_1$ の分散はこれに一致します。
...
よって $T_1$ は MSE $\to0$ で一致的。$T_2$ は分布が $n$ に依存せず一致しない。

$$
I_n(\lambda)=n/\lambda
$$

より CRLB $=\lambda/n$ で、$T_1$ は達成する。
...
$X_1,\ldots,X_n$ は形状 $a>0$、尺度 $b>0$ の Gamma 分布に従う。次を用いてよい。
...
### I1-02-C03 Gauss--Markovを行列で使う

- Level: C
- 目安時間: 30分
- 主題: BLUE
- 使用技術: 線形不偏性、共分散行列、半正定値
...
4. 共分散行列の差が $\sigma^2DD^\mathsf T$ になることを示し、BLUEを結論せよ。
...
### I1-02-C04 一般の関数 $g(\theta)$ に対するCRLB
...
正則な1母数モデルでスコアを $U(\theta)$、Fisher情報量を $I(\theta)$ とする。$T$ は $g(\theta)$ の不偏推定量とする。

1. $E[U]=0$ を用いて $\operatorname{Cov}(T,U)=E[TU]$ と書けることを示せ。
2. $E[T]=g(\theta)$ を微分し、$E[TU]=g'(\theta)$ を導け。
3. Cauchy--Schwarz不等式から Cramér--Rao 不等式を導け。
...
### I1-02-D01 Poissonで同じ量を推定する二つの不偏推定量

- Level: D
- 目安時間: 40分
- 主題: 十分性・分散改善・CRLB
- 使用技術: Poisson母関数、条件付き分布、分散比較
...
2. $q$ の不偏推定量に対する Cramér--Rao 下限を求めよ。
3. $T_1$ の効率を「CRLB / 分散」で定義して求めよ。
4. 
   $$
   T_2=\left(1-\frac1n\right)^S
   $$
   も $q$ の不偏推定量であることを示せ。
5. $\operatorname{Var}(T_2)$ を求め、$n\to\infty$ で Cramér--Rao 下限との比が1へ近づくことを示せ。
...
$\mathbf1(X_i=0)$ は成功確率 $q$ の Bernoulli 変数なので
...
Poisson標本の Fisher 情報量は

$$
I_n(\lambda)=\frac n\lambda.
$$

$g(\lambda)=e^{-\lambda}=q$ なので $g'=-q$。従って CRLB は
...
一方 $S\sim\mathrm{Poisson}(n\lambda)$ です。Poisson変数の確率母関数
...
$S\sim\mathrm{Poisson}(n\lambda)$ のPGFから
...
### I1-02-DRILL01 Poissonの零確率を推定する

- Level: C
- 目安時間: 30分
- 主題: 不偏性・十分統計量・Fisher情報量・効率
- 使用技術: Poisson、PGF、CRLB、分散比較
...
- `MATH-2023-Q1`: Poisson分布の不偏推定量・一致推定量。本章では C01 と DRILL01 で、不偏性と一致性を分離して判定する技能へ接続する。
...
- バイアス・分散・MSEを混同せず計算できる。
- MSE分解を導出できる。
- 不偏性と一致性を別の性質として判定できる。
- バイアス $\to0$、分散 $\to0$ から一致性を示せる。
- BLUE の「best」が比較する推定量のクラスを説明できる。
- Gauss--Markovの定理を $D=A-A_0$ の分解で証明できる。
- スコアから Fisher 情報量を計算できる。
- 独立標本で Fisher 情報量が加法的になる理由を説明できる。
- Cramér--Rao不等式を共分散と Cauchy--Schwarz から導出できる。
