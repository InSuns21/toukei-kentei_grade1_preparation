...
観測データが得られたとき、「どの母数なら、このデータがもっとも自然に出てきたと考えられるか」を測るのが尤度です。確率モデルを、今度はデータを固定して母数の関数として読み替えることで、点推定・尤度比検定・Bayes推定・期待値最大化法へ続く共通言語が得られます。
...
- [P3-01 主要な離散分布](../../02_distributions/P3_01_主要な離散分布/index.md): Bernoulli・二項・Poisson分布。
- [P3-02 主要な連続分布](../../02_distributions/P3_02_主要な連続分布/index.md): 正規・指数・一様分布。
- I1-02 推定法と推定量の評価: Fisher情報量、Cramér--Rao不等式、不偏性・一致性・有効性へ進む。
- I3-02 尤度比・Wald・Score検定: 本章の尤度とスコアを検定統計量へ使う。
...
- Bernoulli、Poisson、指数、正規分布の標準的な最尤推定量を導出する。
...
2. P3-01/P3-02: Bernoulli、Poisson、指数、正規、一様分布の確率関数・密度を書ける。
...
$X_1,\ldots,X_n$ を独立な Bernoulli$(p)$ とし、$S=\sum_i X_i$ とします。
...
と書きます。Fisher情報量そのものの性質は I1-02 で扱います。
...
一方、Bernoulli標本の具体的な0/1列を観測したなら
...
より $\hat\lambda_{\rm ML}=1/\bar X$。MLEの不変性から
...
だが、その値は $\lambda=0$ でしか達成されない。従って $\Theta=(0,\infty)$ ではMLEは存在しない。
...
$L(\lambda)=e^{-n\lambda}$ は $\lambda>0$ で単調減少。上限は $\lambda\downarrow0$ で1だが0は母数空間外なのでMLEは存在しない。$\lambda\ge0$ なら $\boxed{\hat\lambda=0}$。
...
独立な Bernoulli$(p)$ 試行を20回行い、14回成功した。

1. $p$ の尤度を定数倍を除いて書き、MLEを求めよ。
2. $p=0.5$ と $p=0.7$ の尤度比 $L(0.7)/L(0.5)$ を求めよ。
3. 対数尤度差 $\ell(0.7)-\ell(0.5)$ を書け。
4. 成功数が0の場合、MLEをスコア方程式だけで探すのが不適切な理由を述べよ。
...
2. 区間の長さ $\eta=2\theta$ のMLEを求めよ。
3. 密度を $\theta$ について微分する前に支持を確認すべき理由を説明せよ。
4. $M<\theta$ のとき $P_\theta(M\le m)$ を求め、MLEが標本平均ではなく最大値に依存する理由をデータ情報の観点から説明せよ。
...
4. 区間中心 $m=(a+b)/2$ と幅 $w=b-a$ のMLEを求めよ。
...
なお全観測値が同一なら $X_{(1)}=X_{(n)}$ で、$a<b$ を保ったまま幅を0へ近づけると尤度が無限大へ発散するため、有限のMLEは存在しない。この点も母数空間と最大値の存在確認が必要であることを示す。
...
支持が母数依存で最大点が制約境界にあるため、通常のスコア方程式中心の解法は適さない。全観測同値なら幅を0へ近づけて尤度が発散し、MLEは存在しない。
...
1. 尤度と対数尤度を $T$ で表せ。
2. スコア $U(\lambda)$ を求め、$\lambda$ のMLEを導出せよ。
3. その点が最大であることを確認せよ。
4. 平均寿命 $\mu=1/\lambda$ のMLEを求めよ。
...
- 再パラメータ化された量のMLEを不変性で求める。
- Score検定へつながるスコアおよび有効スコアを計算する。

後続の I1-02 で推定量の良さ、I2-01 でMLEの漸近分布、I3-02 で尤度比・Wald・Score検定へ接続します。
...
- Bernoulli、Poisson、指数、正規、一様分布のMLEを導出できる。
- MLEの不変性から変換後の母数のMLEを求められる。
