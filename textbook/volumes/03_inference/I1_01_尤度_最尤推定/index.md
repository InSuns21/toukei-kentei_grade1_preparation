...
### I1-01-C05 パレート分布の下端母数と支持依存最尤推定
...
- ラプラス分布では絶対偏差和から中央値、パレート分布では支持条件から最小値が最尤推定量になることを導出できる。
