...
まず1母数モデルで3検定の構成と相互関係を理解し、その後に多母数の一般制約や局所漸近正規性への拡張を発展として扱います。

| 区分 | 内容 |
|---|---|
| **基本** | 尤度比の構成、Wilksの定理の使い方、1母数ワルド型検定、1母数スコア型検定、1母数での3検定の漸近同値性、制約付き最尤推定、迷惑母数の再推定、再母数化不変性 |
...
### 1.1 先に同じデータで3つを並べてみる

公式を導く前に、3つがどの程度似た数字になるかだけ見ます。
...
値は完全には一致しません。しかし全部ほぼ4です。5%水準の $\chi^2_1$ 臨界値は約3.84なので、3検定とも帰無仮説を棄却します。

ここでまず押さえることは、

- スコア型検定は **帰無値 $p_0=0.5$** でばらつきを評価する。
- ワルド型検定は **推定値 $\hat p=0.6$** でばらつきを評価する。
- 尤度比検定は **尤度の最大値そのものの差**を見る。

という違いです。有限標本では評価する場所が違うので数字も少し違います。しかし正則な大標本では、どれも同じ局所的な尤度の二次近似を見ているため漸近的に一致します。以下では、この3つを順に定義・導出します。
...
> を尤度比統計量として用いる。$\Lambda$ が小さい、同値に $G^2$ が大きい側を棄却側に取る検定を **尤度比検定** という。
...
> **発展項目**  
> 以下は「なぜ自由度 $r$ のカイ二乗になるのか」を一般の多母数モデルで厳密化する方向の話です。局所漸近正規性や情報行列を用いるため、基本では結論だけ使えれば十分です。
...
> をワルド型統計量という。$W$ が大きい側を棄却側に取る検定を **ワルド型検定** という。正則条件の下で $H_0$ のもと $W\xrightarrow{d}\chi_1^2$ である。
<!-- formal-statement-end -->
...
> **発展項目**  
> ここでは情報行列・多変量デルタ法を使います。ここまでの1母数の理解には必須ではありません。
...
> をスコア型統計量という。$S$ が大きい側を棄却側に取る検定を **スコア型検定** という。正則条件の下で $H_0$ のもと $S\xrightarrow{d}\chi_1^2$ である。
<!-- formal-statement-end -->
...
> 情報行列と一般制約のワルド型検定を使う発展演習です。基本では省略して構いません。
