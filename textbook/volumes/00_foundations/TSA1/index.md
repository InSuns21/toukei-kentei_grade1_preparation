...
**定義の確認**  
まず $t_1,\dots,t_n$ が相異なるなら、ベクトル $(Z_{t_1},\dots,Z_{t_n})$ の同時分布は共通の一時点分布の積で決まります。時刻に重複がある場合も、どの成分同士が同じ確率変数を表すかという一致関係は、全時刻へ同じ $h$ を加えても変わりません。従っていずれの場合も、時間移動後のベクトルは同じ同時分布を持ち、
...
第1項は $\mathcal H_{t-1}$ に属します。一方、弱ホワイトノイズ性から $Z_t$ は各 $Z_s$ $(s<t)$ と直交します。従って過去の $Z_s$ の任意の有限線形結合とも直交し、内積の連続性によりその閉線形包全体とも直交します。各 $X_s$ $(s\le t-1)$ は過去の $Z$ の有限線形結合なので、その閉線形包である $\mathcal H_{t-1}$ に対して
