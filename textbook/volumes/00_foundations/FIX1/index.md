...
一次元の閉区間なら、連続性と符号変化を使う標準的な議論でかなり直接に示せます。しかし二次元以上では、各座標を別々に調べても、同じ点で全座標が同時に一致するとは限りません。
...
\text{頂点へ有限個のラベルを付ける}
\Longrightarrow
\text{全種類のラベルがそろう小領域を見つける}
\Longrightarrow
\text{極限で }f(x)=x\text{ を得る}
}
$$

という有限組合せ論から連続写像の存在定理へ進みます。

中心になる有限組合せ論の主張は、境界上で許されるラベルを制限すると、全種類のラベルがそろう小領域が必ず現れるという偶奇原理です。この有限の主張を、細分の大きさを 0 へ近づけながら繰り返すと、[Heine--Borel の定理](../F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel/index.md#thm-f0-00c1-02)と連続性が働き、もとの方程式を満たす点が現れます。

後続では、この「一点を返す写像」の存在論を、最適反応や需要のような「点の集合を返す対応」へ拡張していきます。そのため、この章では有限次元での存在証明の土台を、組合せ論から極限操作まで含めて閉じます。
...
となる分割列が必要です。そのため、反復するたびにメッシュを確実に小さくできる標準的な細分操作を使います。
...
従って [有限次元の閉凸集合への射影定理](../OPT2/index.md#thm-opt2-projection)により、任意の
...
従って [有限次元の閉凸集合への射影定理](../OPT2/index.md#thm-opt2-projection)から、各 $x\in\mathbb R^m$ に一意な最近点
