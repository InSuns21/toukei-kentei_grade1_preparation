...
中心になるのは **Sperner の補題**です。Sperner の補題自体は有限個の単体と有限個のラベルだけを扱います。それにもかかわらず、細分のメッシュを 0 へ近づけると、[Heine--Borel の定理](../F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel/index.md#thm-f0-00c1-heine-borel)と連続性が働き、Brouwer の不動点定理が現れます。
...
である $(n-1)$ 次元面を「扉」とみなし、小単体を部屋とみなして有限グラフを作ります。

- 完全ラベル単体には扉が1枚
- $0,\dots,n-1$ だけをすべて使う単体には扉が2枚
- その他には扉が0枚
...
隣接頂点のラベルが異なる辺を通過するたび、現在のラベルは 0 と 1 の間で切り替わります。
...
である $(n-1)$ 次元小面を **扉**と呼びます。

まず、$\Delta^n$ の境界にある扉を調べます。
...
なら、扉にはラベル $j$ が必要なので、その面上に扉は存在できません。
...
帰納法の仮定により、この境界面上の完全ラベル $(n-1)$ 単体、すなわち境界上の扉の個数は奇数です。
...
- 内部の扉を共有する二つの $n$ 次元小単体の頂点を辺で結ぶ。
- 境界上の扉を持つ小単体の頂点を $\ast$ と結ぶ。

三角形分割の定義から、内部の $(n-1)$ 次元面はちょうど二つの $n$ 次元小単体に接し、境界面は一つだけに接するので、このグラフは正しく定まります。

外部頂点 $\ast$ の次数は、境界上の扉の個数に等しいので奇数です。
...
扉を作るにはラベル $n$ の頂点を一つ除けばよく、その方法は一通りだけです。
...
重複したラベルを持つ二頂点のうち、どちらか一方を除くと扉になります。
...
ラベル $0,\dots,n-1$ のどれかが欠けているので、どの $(n-1)$ 次元面も扉になりません。
...
各分割で、前節の座標差から Sperner ラベルを付けます。Sperner の補題により、毎回完全ラベル小単体が一つ取れます。
...
前節で見たように $S$ は標準単体とアフィン同相なので、標準単体版の Brouwer 不動点定理を $F$ へ適用できます。
...
1. 境界上の扉は、元の辺 $\operatorname{conv}\{e_0,e_1\}$ 上にしか存在しないことを示せ。
2. その元の辺上の扉の個数が奇数であることを一次元の場合から示せ。
3. 各小三角形について、扉の枚数が
   - 完全ラベルなら1
   - ラベル0,1のみを両方含むなら2
   - その他なら0
   となることを示せ。
4. 外部頂点を加えたグラフの次数の偶奇から、完全ラベル小三角形の個数が奇数であることを示せ。
...
細分頂点を順番にたどると、ラベルが0から1、または1から0へ切り替わる小辺が扉です。
...
0または1のどちらかが欠ける場合や、ラベル2を含むが三種類すべては揃わない場合には、$\{0,1\}$ を持つ辺はありません。

従って扉は0枚です。

最後に各小三角形をグラフ頂点とし、共通の内部扉を持つ二つを辺で結びます。

境界扉を持つ三角形は外部頂点 $\ast$ と結びます。

$\ast$ の次数は境界扉の個数なので奇数です。
...
従って各段階で Sperner の補題から取った完全ラベル単体の代表点 $x_k$ について
