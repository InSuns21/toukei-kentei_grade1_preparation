...
最後に、$\gamma([a,b])$ はコンパクトで $f$ は連続なので $|f|$ は最大値 $M$ を持ちます。各滑らかな区間の Riemann 和に

$$
\left|\sum_k a_k\right|\le\sum_k|a_k|
$$

を適用して細分の幅を0へ送れば、積分についても絶対値を積分の内側へ移した上からの評価が得られます。従って
...
$\gamma$ が $C^1$ である一つの区間を考えます。[複素微分可能](../CA1/index.md#def-ca1-complex-differentiable) の定義から
