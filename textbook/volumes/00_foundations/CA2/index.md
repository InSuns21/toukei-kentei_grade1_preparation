...
ただし $z\to z_0$ で $\varepsilon(z)\to0$ と書けます。
...
と置けば、$T_n$ の直径が0へ縮み $z_0\in T_n$ なので $\eta_n\to0$ です。ML評価から
...
各 $t\in[0,1]$ について、[局所原始関数](#cor-ca2-local-primitive) を持つ円板 $D_t$ を $\gamma(t)$ のまわりに取り、その閉包が $\Omega$ に入るよう小さくします。$\gamma([0,1])$ はコンパクトなので有限個で覆えます。また $\gamma$ はコンパクト区間上連続なので一様連続です。
...
を十分細かく取れば、各小区間の像 $\gamma([t_{j-1},t_j])$ が一つの円板 $D_j$ に含まれるようにできます。端点 $\gamma(t_{j-1}),\gamma(t_j)$ を直線で結び、これらの弦をつないだ多角形曲線を $P$ とします。円板は凸なので弦も $D_j$ に含まれます。
...
各頂点 $v$ で $\widetilde H(v)=H(v)$ と置き、各小三角形の内部では三頂点の値を重心座標でアフィンに補間します。一つの頂点 $v_0$ を固定すると、その三角形内の $\widetilde H$ の値は三頂点の像の凸結合なので

$$
|\widetilde H(x)-H(v_0)|<\frac\delta3.
$$
