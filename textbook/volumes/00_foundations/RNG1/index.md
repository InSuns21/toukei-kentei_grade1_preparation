...
> **この章の停止線**
>
> 環・部分環・零因子・整域・体、環準同型、イデアル、主イデアル、商環、さらに準同型の核で割った商環と像の同型までを扱います。素イデアル・極大イデアル・中国剰余定理は RNG2、Euclid 整域・単項イデアル整域・一意分解整域は RNG3、多項式環の本格的な構造論は RNG4 へ送ります。
...
$(R,+)$ は可換群なので、[部分群判定](../GRP1/index.md#thm-grp1-subgroup-test)から $(S,+)$ は $(R,+)$ の部分群です。従って $0\in S$ であり、加法・加法逆元でも閉じています。
...
商群では、積を代表元によらず定めるために正規部分群が必要でした。環では加法群が可換なので、全ての加法部分群は [正規部分群](../GRP2/index.md#def-grp2-normal-subgroup) です。
...
従って [部分環判定](#prop-rng1-subring-test) から $\operatorname{Im}f$ は $S$ の部分環です。$\square$
<!-- proof-end -->
...
[上の定義](#def-rng1-principal-ideal)から
...
[環の第一同型定理](#thm-rng1-first-isomorphism-ring)から
...
4. [環の第一同型定理](#thm-rng1-first-isomorphism-ring)から商環を同定せよ。
...
$I$ に属する正整数全体は空でありません。[自然数の通常順序が整列であること](../F0_00A1D_順序_全順序_最小最大_整列/index.md#def-f0-00a1d-well-order)から、この集合には最小の元が存在するので、それを $n$ と取ります。
...
従って [部分環判定](#prop-rng1-subring-test) から $H$ は $\mathbb Z[x]$ の部分環です。
...
3. [環の第一同型定理](#thm-rng1-first-isomorphism-ring)から
   $$
...
4. 写像
   ...
   $$
   に[環の第一同型定理](#thm-rng1-first-isomorphism-ring)を適用し、
   $$
   R/I
   $$
   を同定せよ。
