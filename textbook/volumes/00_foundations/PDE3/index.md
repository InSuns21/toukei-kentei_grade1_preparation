...
この四つを、極値の制御・エネルギー法・Fourier 級数・Fourier 変換という別々の道具から同じ現象として読みます。
...
> **証明境界**  
> 本章は一次元熱方程式の古典解を対象にします。温度の極値制御、一意性、エネルギー散逸、有限区間の Fourier 正弦級数解、実数全体の熱核表示までは本文で閉じます。弱解、Sobolev 空間、一般の放物型作用素、半群論は Encore III 以降へ送り、本章の証明へ逆輸入しません。
...
温度の極値を評価するときは「どの境界を見ればよいか」を明示しておく必要があります。
...
熱方程式の最も重要な構造の一つを、次の定理として定式化します。
...
最大原理は「温度は最大値を増幅しない」という物理的説明だけでなく、解が一意に決まることを証明します。
...
最大原理は最大温度を支配しました。次は温度分布全体の大きさを積分で測ります。
...
有限和なので、和と微分を交換して問題ありません。各項について
...
この段階では無限級数と微分の交換はまだ一度も使っていません。無限和へ移るときだけ、収束の正当化が必要になります。
...
なので、微分後の級数も一様収束します。従って $t>0$ では級数と微分を交換でき、
...
[Gaussian の Fourier 変換](../FOU3/index.md#lem-fou3-gaussian-transform)を照合すると、周波数乗数 $e^{-\kappa t\xi^2}$ に対応する空間側の核は Gaussian です。
...
3. 任意の整数 $m\ge0$ について $\partial_x^mG_t$ は「多項式 × Gaussian」の形で $L^1(\mathbb R)$ に属します。従って固定した $t>0$ で畳み込み積分を微分でき、
...
上側評価は最大原理を $u$ に適用します。放物型境界上で $u\le7$ なので
...
- 最大原理の $\varepsilon t$ 摂動の役割を説明し、証明を再構成できる。
- 最大原理から一意性・比較原理・非負性を導ける。
...
- 正時刻で指数減衰が級数と微分の交換を正当化する仕組みを説明できる。
