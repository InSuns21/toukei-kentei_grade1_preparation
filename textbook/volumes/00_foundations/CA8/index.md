...
> **複素解析 II の幾何的入口**。CA5 では平面領域上で解析接続と monodromy を扱い、CA7 では Riemann 写像定理まで進んだ。本章では「局所的には複素平面に見えるが、大域的には平面領域ではない空間」を Riemann 面として定式化する。対数や平方根の多価性は、値を無理に一つ選ぶ問題ではなく、適切な被覆 Riemann 面へ持ち上げれば通常の一価正則関数として扱える。
...
一般の被覆空間分類、普遍被覆の一般存在定理、deck transformation の体系、Riemann--Roch、divisor、genus、一様化定理は本章の主線には入れない。
...
$X,Y$ を Riemann 面、$F:X\to Y$ を連続写像とする。$p\in X$ に対し、$p\in U$ の chart $(U,\varphi)$ と $F(p)\in V$ の chart $(V,\psi)$ を取り、
...
上の定義で、ある一組の chart を使って $\psi\circ F\circ\varphi^{-1}$ が正則なら、同じ点を含む任意の別の chart を使っても座標表示は正則である。
<!-- formal-statement-end -->
...
別の chart $(\widetilde U,\widetilde\varphi)$、$(\widetilde V,\widetilde\psi)$ を取る。共通部分で
...
局所座標を変えても「何次の零点か」「何次の極か」が変わらないことを確認しておく。これがないと、Riemann 面上の divisor や留数へ進む以前に局所データ自体が不安定になる。

<a id="def-ca8-meromorphic-function"></a>
<!-- formal-statement-start -->
### 定義（Riemann 面上の有理型関数）

Riemann 面 $X$ 上の関数 $f:X\to\widehat{\mathbb C}$ が **有理型**であるとは、各点 $p\in X$ の chart $(U,\varphi)$ において
...
球面はコンパクトなので $|g|$ は最大値を取る。最大点の chart に移せば通常の[最大値原理](../CA3/index.md#thm-ca3-maximum-modulus)が使え、$g$ は定数である。従って $f$ は多項式と有限個の主部分数の和、したがって有理関数である。$\square$
<!-- proof-end -->
...
Riemann 面 $X$ の各 chart $(U_\alpha,z_\alpha)$ に正則関数 $f_\alpha$ が与えられ、重なりで
...
**定義の確認**。後で作る複素トーラス $\mathbb C/\Lambda$ の chart 同士は平行移動でつながるので $dz_\alpha/dz_\beta=1$ である。従って平面上の $dz$ は商へ降り、トーラス上の正則微分を与える。
<!-- definition-example-end -->
...
で定める。二つの chart が重なると、代表元は格子元 $\lambda$ だけずれるので遷移写像は
...
$\mathbb C/\Lambda$ の局所 chart は平面の小円板を格子で同一視して作る。chart の重なりで起きることは
...
<a id="ex-ca8-a01"></a>
#### CA8-A01 Riemann 球面の二 chart を直接確認する
- Level: A
...
どちらも0を除いた領域で正則だから二 chart は正則に両立する。
...
ここでは winding number を直接計算せず、被覆上の終点がホモトピー類を検出している。
<!-- solution-end -->
...
1. chart 遷移が $z\mapsto z+\lambda$ なので $dz$ が大域的な正則微分へ降りることを示せ。
...
1. chart 遷移
   ...
   dw=dz.
   $$
   従って各 chart で係数1の局所微分 $dz$ を置けば、重なりで一致する。[正則微分の定義](#def-ca8-holomorphic-one-form)を満たす。
...
- 正則写像の定義が chart の選択に依存しないことを、座標遷移の合成として証明した。
...
- 一般被覆分類、普遍被覆の一般存在、deck transformation の体系、分岐被覆の一般論、divisor、Riemann--Roch、一様化定理は使用していない。
