...
[LA3B の定義](../LA3B/index.md#def-la3b-matrix-determinant)で導入した $\det$ を、$R$ 上でも同じ有限和・有限積の式で用います。可換環 $R$ 上の $k\times k$ 行列 $C=(c_{ij})$ に対して
...
という素元冪巡回分解が得られます。ここで $p$ は同伴を除いた素元の代表、$m_{p,e}$ は非負整数です。
...
この素元冪巡回分解で $q$ が $p$ と同伴でない素元なら、$p^N$ と $q^e$ は互いに素です。Bézout 等式から
...
左辺を復元する右辺は $M$ 自身から定まるので、全ての素元冪 $p^e$ の重複度が一意に決まります。
...
一意性も [PID 上有限生成加群の構造定理](#thm-mod2-pid-structure) の一意性から従います。

さらに各 $n_i$ を素数冪へ分解し、[中国剰余定理（有限個のイデアル）](../RNG2/index.md#thm-rng2-crt-finite)を使えば、有限部分は素数冪位数の巡回群の直和へ分かれます。
...
[PID 上有限生成加群の構造定理](#thm-mod2-pid-structure)から得られる基本因子は、一般には
