...
> **この章の停止線**
>
> 本章では可換 PID 上の有限生成加群を扱います。体拡大・代数的元・最小多項式の体論は FLD1 へ送ります。LA4 の Jordan 標準形そのものは再証明せず、最後に加群構造論との対応だけを整理します。

---

## 1. 行列の左右から基底を変える

$R$ を PID とします。自由加群間の準同型
...
左から掛けた行列の行列式は $1$ なので $\mathbb Z$ 上可逆です。したがってこの変形は基底を変えただけで、準同型そのものの同型類は変えていません。
...
>
> 一般の PID では不変因子は単元倍を除いて考える。$\mathbb Z$ では正、$F[x]$ では monic を選べば代表を一意に正規化できる。
<!-- formal-statement-end -->
...
ここでは行列式を未定義の道具として持ち込みません。可換環 $R$ 上の $k\times k$ 行列 $C=(c_{ij})$ に対して
...
と Leibniz 公式で定めます。この式から、行の交換で符号が変わること、一つの行に関して線形であること、同じ二行を持つ行列式が $0$ になることが直接従います。以下で必要なのはこの三性質だけです。
...
その核は前節の定理により有限階数自由加群です。核の包含写像を行列で表し、その行列を Smith 標準形にします。対角行列の余核なら一成分ずつ読めます。
...
不変因子の同伴類の一意性は [Smith 不変因子の一意性](#thm-mod2-invariant-factor-uniqueness)から従います。

最後に自由階数 $r$ の一意性を、RNG4 までで既知の分数体だけを使って確認します。

$K=\operatorname{Frac}(R)$ とします。$R$-加群 $M$ の元
...
$
a_1x_1+\cdots+a_\ell x_\ell=0,
\qquad
a_i\in R
$

から全ての $a_i=0$ が従うことでした。
...
で $T$ をねじれ部分とします。自由部分の標準基底から $r$ 個の $R$-線形独立な元が得られます。
...
を取ります。有限個のねじれ元 $t_j$ それぞれを消す非零元を掛け合わせれば、整域性により非零な $a\in R$ で
...
を考えます。$v_1,\dots,v_{r+1}$ を $K^r$ のベクトルとみなすと、$r+1$ 本なので $K$ 上一次従属です。係数の分母を払えば、全ては $0$ でない
...
従って $r$ は「$M$ に含まれる $R$-線形独立な元の最大個数」として $M$ 自身から決まり、一意です。$\square$
<!-- proof-end -->
...
不変因子表示は divisibility chain を見せるのに向き、基本因子表示は素数ごとの局所構造を見せるのに向きます。同じ加群の二つの座標系です。
...
$F[x]$ の単元は非零定数なので、各 $f_i$ は monic に正規化でき、divisibility chain も保てます。$\square$
<!-- proof-end -->

### 10.1 一つの巡回因子は companion block を表す
...
したがって $x$ の作用、すなわち $T$ は companion matrix 型になります。これが有理標準形の加群論的な中身です。
...
における $x$ の作用は、固有値 $\lambda$、大きさ $e$ の Jordan block に対応します。
...
1. 基本因子を求めよ。
2. $V$ の $F$-次元を求めよ。
3. $T$ の Jordan block の大きさを、LA4 の結果を用いて読み取れ。
4. なぜ本章の構造定理だけでは「全ての体で Jordan block まで分解できる」とは言えないのか説明せよ。
...
は固有値 $\alpha$、大きさ $e$ の Jordan block 一個に対応します。
...
から固有値 $\lambda$ の大きさ $1$ の block、

$$
(x-\lambda)^3
$$

から固有値 $\lambda$ の大きさ $3$ の block、

$$
(x-\mu)^2
$$

から固有値 $\mu$ の大きさ $2$ の block

が得られます。

つまり Jordan block の大きさは
