...
自由階数 $r$ も一意です。実際、$K=\operatorname{Frac}(R)$ を分数体とし、上の同型へ $K$ を係数拡大すると、$R/(d_i)$ の各ねじれ成分は $K$ 上 $0$ になります。したがって
...
ただしこの最後の一意性確認に用いたテンソル積を本章の主道具にはしません。テンソル積を未習として読む場合は、同じ $r$ が Smith 標準形の零行数から決まり、その数が同値変形で変わらないことを用いればよいです。$\square$
<!-- proof-end -->

### 7.1 テンソル積を使わない自由階数の一意性

上の存在証明で $M$ はある行列 $A:R^q\to R^n$ の余核でした。

Smith 標準形の非零対角成分数を $t$ とすると、自由部分の階数は
...
$t$ は $\Delta_t(A)\ne0$ となる最大の $t$ であり、小行列式イデアルにより行列同値から不変です。

別の有限表示を取った場合にも、自由部分を商で潰して得る最大ねじれなし商の階数として同じ数が得られます。本章では以後、構造定理の標準形に現れる $r$ を自由階数と呼びます。
