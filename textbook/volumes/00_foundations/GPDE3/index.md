# GPDE3：弱微分を \(L^p\) と組み合わせ、Sobolev 空間を作る
...
Sobolev 空間でも事情は同じです。弱微分は積分恒等式で定義されるため、測度 0 の集合上の値を変えても変化しません。
...
> \]
>
> をすべての多重指数 \(\alpha\) satisfying \(|\alpha|\le k\) に対して満たすとき、
>
> \[
> u\in W^{k,p}(\Omega)
> \]
...
微分情報を含む一方で Hilbert 空間なので、直交性・Riesz 表現・Lax--Milgram などの Hilbert 空間の道具を使えます。
...
mollifier による近似を Sobolev norm へ持ち上げるには、GPDE2 の \(L^1\) 平行移動連続性を \(L^p\) へ拡張します。
...
しかも \(H^1\) は Hilbert 空間であり、Cauchy 列の極限を取っても \(H^1\) の中に残ります。
...
- \(H^k=W^{k,2}\) の内積を書き、Hilbert 空間であることを説明できる。
