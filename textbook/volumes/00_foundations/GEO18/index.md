...
[GEO15](../GEO15/index.md) では [Hopf--Rinow の定理](../GEO15/index.md#thm-geo15-hopf-rinow) により、完備性と最短測地線の存在を大域的に結びました。[GEO17](../GEO17/index.md) では Jacobi 場と指数形式を使い、曲率が共役点と測地線の最短性へ現れる仕組みを構成しました。
...
> **定義（定曲率模型空間・比較関数）**  
> 実数 $\kappa$ に対し、断面曲率が恒等的に $\kappa$ である本章で比較の基準に用いる標準的な完備 Riemann 多様体を **定曲率模型空間**と呼ぶ。
>
...
もう一度微分し、[Jacobi 方程式](../GEO17/index.md#thm-geo17-geodesic-variation-jacobi)を使うと
...
[Jacobi 方程式](../GEO17/index.md#thm-geo17-geodesic-variation-jacobi)から
...
[Rauch の比較定理](#thm-geo18-rauch)から、その区間では非零 Jacobi 場は再び0になれません。
...
断面曲率は $K\le0$ なので、[Rauch の比較定理](#thm-geo18-rauch)を $\kappa=0$ に適用すると
...
さらに [Riemann 幾何の Gauss の補題](../GEO14/index.md#thm-geo14-gauss-lemma)から、指数写像の微分は放射方向と角方向を直交させます。
...
完備性と [Hopf--Rinow の定理](../GEO15/index.md#thm-geo15-hopf-rinow)から、$p$ と $q$ を結ぶ単位速最短測地線
...
ここで失われた仮定は単連結性です。後の Cartan--Hadamard の証明は「指数写像が被覆を与える」ところまではそのまま進みますが、**被覆が一枚だけである**という最後の結論だけが壊れます。
...
完備性と [Hopf--Rinow の定理](../GEO15/index.md#thm-geo15-hopf-rinow)から、任意の初期速度 $v\in T_pM$ に対する測地線は時刻1まで定義されます。
...
[非正曲率での指数写像の微分の拡大性](#prop-geo18-exp-expansion)から
...
[逆関数定理](../RA6A/index.md#thm-ra6a-inverse-function)により $\exp_p$ は局所微分同相です。

$T_pM$ は Euclid 内積を入れれば完備です。

従って [拡大局所微分同相の被覆補題](#lem-geo18-covering)から
...
この被覆について、同じ始点を持つ二つの持ち上げは局所的な一意性から一致します。
...
[Rauch の比較定理](#thm-geo18-rauch)から、それより前では非零 Jacobi 場は0へ戻れません。
...
2. $K\le0$ なら Rauch 比較と Gauss の補題から
   ...
   [逆関数定理](../RA6A/index.md#thm-ra6a-inverse-function)から局所微分同相です。
   ...
   接空間 $T_pM$ は完備だから、この局所逆写像は短い正規球上の放射曲線に沿って途中で逃げずに延長でき、$\exp_p$ は $M$ の被覆を与えます。

3. $M$ が単連結なら全ての閉曲線は定値曲線へ縮められます。

   同じ点 $q$ に二つの前像 $v_0,v_1$ があるとすると、接空間内で $v_0$ と $v_1$ を結ぶ曲線の像は $q$ を基点とする閉曲線になります。

   その閉曲線を定値曲線へ縮め、上で説明した局所逆写像の貼り合わせの一意性を使うと、持ち上げの終点は始点と一致しなければなりません。
