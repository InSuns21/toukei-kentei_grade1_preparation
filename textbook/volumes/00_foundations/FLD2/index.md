...
>
> $m_{\alpha,K}\in K[x]$ を $\alpha$ の最小多項式とし、$\beta\in\Omega$ が
>
> $$
...
> **意図的な黒箱**
>
> 任意の体 $F$ が代数閉包を持つこと自体は、本章では証明しません。その一般証明は Zorn の補題を用いる大きな存在論です。本章で有限個の多項式の根を作るだけなら、前節の分解体存在定理で十分です。ここでは埋め込みを一つの共通の体へ入れて比較するためだけに代数閉包を固定します。
...
一つずつ $\alpha_j$ を加えます。現在の埋め込みで最小多項式の係数を送ると、代数閉体 $\overline F$ ではその多項式に根があります。そこで一段の埋め込み延長補題を使います。
...
つまり形式微分が多項式の次数を「見失う」ことがあります。これは単なる計算上の珍事ではなく、共役根が一つに重なる現象を生みます。具体例は正規性まで導入した後で詳しく計算します。

---

## 9. 正規性：一つの共役根が入ったら全て入る

分離性が「根が重ならない」性質だったのに対し、正規性は「共役な根を取りこぼさない」性質です。
...
分解体 $\Rightarrow$ 正規では、$K$ にある根 $\alpha$ の共役根 $\beta$ を代数閉包で一つ取ります。一段の埋め込み延長で $\alpha\mapsto\beta$ とし、それを $K$ 全体へ延長します。分解体の埋め込み安定性から像は再び $K$ なので $\beta\in K$ です。
...
#### FLD2-A02 $\mathbb Q(\sqrt2)$ の二つの埋め込み
- Level: A
...
3. $x^4-2$ は素数 $2$ に関する Eisenstein の既約判定により $\mathbb Q[x]$ で既約です。従って
...
1. $q$ が $F[x]$ で既約であることを、$\mathbb F_p[t]$ 上の Eisenstein の既約判定から示せ。
...
従って Eisenstein の既約判定から $q$ は $\mathbb F_p[t][x]$ で既約です。
...
2. それらが三つある理由を一段の埋め込み延長補題から説明せよ。
...
$x^3-2$ は $2$ に関する Eisenstein の既約判定から $\mathbb Q[x]$ で既約です。従って
...
4. まず $\alpha$ の像を三根のどれか一つに選びます。一段の埋め込み延長により
...
次に $\omega$ の最小多項式は $x^2+x+1$ で、係数は全て $\mathbb Q$ にあるので、どの中間埋め込みの下でも係数は変わりません。その二根 $\omega,\omega^2$ のどちらを選んでも、再び一段の埋め込み延長により
