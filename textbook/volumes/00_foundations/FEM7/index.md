...
最後に、流線風上 Petrov--Galerkin 法（streamline-upwind/Petrov--Galerkin method; SUPG）を導入し、
...
## 1. 要素 Péclet 数：メッシュは境界層を解像しているか
...
FDM4 で示した通り、これは離散最大値原理の係数構造を壊し、交互振動を許します。
...
中心型離散最大値原理で必要だった「両隣係数が非正」という構造が壊れています。
...
- FDM4：中心差分、風上差分、格子 Péclet 数、数値拡散、離散最大値原理
