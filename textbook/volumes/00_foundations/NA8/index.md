...
<!-- formal-statement-start -->
### 定理（ピボット選択なし LU 分解の存在判定と一意性）

$A\in\mathbb R^{n\times n}$ とする。

行交換を行わない Gauss 消去法が零ピボットなしで最後まで進むための必要十分条件は、
...
と $L$ を単位下三角、$U$ を上三角に取る LU 分解は一意である。
<!-- formal-statement-end -->
...
第 $k-1$ 段階まで消去できたとします。消去は左から単位下三角行列を掛ける操作なので行列式を変えません。左上 $k\times k$ ブロックの上三角化が進んだ状態では
...
各段階の行交換を置換行列で表し、消去行列とまとめれば、最終的に

$$
PA=LU
$$

という形を得ます。すでに記録した乗数を含む $L$ の行も、後から行交換が起きたときには同じように交換しておけばよいです。
...
列フルランクの $X\in\mathbb R^{m\times n}$ に対して最小二乗問題
...
本文の定理より、行交換なしの LU 分解が存在するための必要十分条件は
...
なので $A(2)$ 自体が特異です。したがって一意解を与える正方連立方程式ソルバとしては解けません。
<!-- solution-end -->
