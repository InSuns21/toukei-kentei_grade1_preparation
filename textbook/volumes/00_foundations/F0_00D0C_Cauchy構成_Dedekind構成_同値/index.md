...
ここで $a+b\in\Phi(x)+\Phi(y)$ かつ $r<a+b$ です。Dedekind切断の下方閉性の定義をこの不等式に適用すると
...
体公理が実際に移ることを、分配法則で確認します。$A=\Phi(a)$、$B=\Phi(b)$、$C=\Phi(c)$ とすると、加法保存と積の定義から
...
a+b>x+y-c/2>r.
$$
$a+b\in\Phi(x)+\Phi(y)$ であり、$r<a+b$ なので、和切断の下方閉性の定義から $r\in\Phi(x)+\Phi(y)$。以上で両包含が示された。
