...
$x=\iota(r)$、$r\in\mathbb Q$ を定義へ代入すると
...
2行目では定数列埋め込み $\iota$ が順序を保つことを使いました。つまり Cauchy 側の有理数は、Dedekind 側の標準切断へそのまま送られます。
...
> $\mathbb R_C$ を前章で構成した Cauchy 実数の順序体、$\mathbb R_D$ を Dedekind切断全体とし、
$$
\Phi:\mathbb R_C\to\mathbb R_D,
\qquad
\Phi(x)=\{q\in\mathbb Q:\iota(q)<x\}
$$
> とする。$\Phi$ は全単射で順序を保ち、全ての $x,y\in\mathbb R_C$ について、Dedekind側で既に定義した切断の加法に対し
$$
\Phi(x+y)=\Phi(x)+\Phi(y)
$$
> が成り立つ。また $x,y>0$ なら、前章で定義した正の切断の積に対し
$$
\Phi(xy)=\Phi(x)\Phi(y)
$$
> が成り立つ。そこで任意の $A,B\in\mathbb R_D$ に対し
$$
AB:=\Phi\!\left(\Phi^{-1}(A)\Phi^{-1}(B)\right),
\qquad
-A:=\Phi\!\left(-\Phi^{-1}(A)\right)
$$
> と定め、$A\ne0^*$ なら
$$
A^{-1}:=\Phi\!\left(\Phi^{-1}(A)^{-1}\right)
$$
> と定める。この積は正の切断について前章の積と一致し、これらの演算と包含順序により $\mathbb R_D$ は完備順序体になる。さらに全ての $q\in\mathbb Q$ について
$$
\Phi(\iota(q))=q^*,
$$
> かつ全ての $x,y\in\mathbb R_C$ について
$$
\Phi(xy)=\Phi(x)\Phi(y)
$$
> となる。従って $\Phi$ は有理数を固定する順序体同型である。
<!-- formal-statement-end -->

### 証明の見取り図

加法では $r<x+y$ の正の余裕を三つに分け、$x,y$ を有理数で**下から**近似します。正の元の乗法では有理上界 $M$ を固定し、

$$
xy-ab=x(y-b)+b(x-a)
$$

を使って、下からの有理近似 $a,b$ の積を $xy$ へ近づけます。ここまでを切断の定義から直接確認した後、負号・負の元を含む積・逆元は、全単射 $\Phi$ を通じて Cauchy 側の順序体構造を移送します。これにより、まだ確立していない Dedekind 側の逆元や符号規則を証明途中で先取りしません。
...
ここで $a+b\in\Phi(x)+\Phi(y)$ かつ $r<a+b$ です。Dedekind切断の条件3をこの二つの事実に適用すると
...
Cauchy 実数の順序体性から、正数を掛けると不等号の向きは保たれます。したがって $0<a<x$ と $0<b<y$ から

$$
ab<xb<xy.
$$

よっていずれの場合も $r<xy$、すなわち $r\in\Phi(xy)$。従って
...
$\Phi$ は全単射なので、任意の $A,B\in\mathbb R_D$ には一意な
...
と定めます。正の $A,B$ では順序保存性から $a,b>0$ であり、直前に示した正の積保存により、この定義は前章の正の切断積と一致します。したがって既存の正の積を変更するのではなく、負の元を含む場合へ一意に拡張しています。

この定義により、全ての $x,y\in\mathbb R_C$ で
...
体公理が実際に移ることを、分配法則で確認します。$A=\Phi(a)$、$B=\Phi(b)$、$C=\Phi(c)$ とします。すでに示した

$$
\Phi(u+v)=\Phi(u)+\Phi(v)
$$

と、今回の取り決め $\Phi(uv)=\Phi(u)\Phi(v)$ を順に使うと
...
結合則・交換則も同じく Cauchy 側の恒等式へ $\Phi$ を適用すれば得られます。また $A\ne0^*$ なら
...
順序との両立も移ります。例えば $A\le B$、$0^*\le C$ とすると、順序保存性から $a\le b$、$0\le c$ です。Cauchy 側が順序体なので $ac\le bc$。再び順序保存性を使えば

$$
AC\le BC.
$$

加法についても $A\le B$ なら $A+C\le B+C$ が同様に従います。よって $\mathbb R_D$ は順序体です。

さらに前章で、$\mathbb R_D$ の任意の空でない上に有界な部分集合が上限を持つことを、切断の和集合として直接証明しました。したがって $\mathbb R_D$ は完備順序体です。

以上より $\Phi$ は有理数・加法・乗法・順序を保つ全単射、すなわち有理数を固定する順序体同型です。$\square$
<!-- proof-end -->
...
a+b>x+y-c/2>r.
$$
$a+b\in\Phi(x)+\Phi(y)$ かつ $r<a+b$ です。Dedekind切断の条件3をこの二つに適用すると、$r$ も $\Phi(x)+\Phi(y)$ に属します。以上で両包含が示された。

##### 本番答案
一方向は $a<x,b<y\Rightarrow a+b<x+y$。逆向きは $r<x+y$ の正の余裕 $c$ を取り、$x,y$ を下から $c/4$ 未満で有理近似して $r<a+b$ とし、Dedekind切断の条件3を使う。
##### 採点基準
- 第1包含：6点
- 正の余裕の取得：4点
- 下からの有理近似：6点
...
- 残りの積・負号・逆元を全単射 $\Phi$ で移送するとき、正の積候補と矛盾しないことを説明できる。
