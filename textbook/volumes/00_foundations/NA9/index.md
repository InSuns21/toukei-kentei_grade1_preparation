...
定常反復が任意の初期値から収束するかどうかは、行列のスペクトル半径だけで完全に判定できます。

<a id="thm-na9-stationary-convergence"></a>
<!-- formal-statement-start -->
### 定理（定常反復法の行列のスペクトル半径による収束判定）
...
> 行列 $A$ が可逆であることと、反復行列 $G$ の行列のスペクトル半径が1未満であることは別問題
...
[行列のスペクトル半径による収束判定](#thm-na9-stationary-convergence)から Jacobi 法は任意の初期値に対して収束します。
<!-- proof-end -->
...
[定常反復法の行列のスペクトル半径による収束判定](#thm-na9-stationary-convergence)から、任意の初期値に対して固定点へ収束します。
