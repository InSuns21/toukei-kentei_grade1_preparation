...
は $x<0$ で負になります。もし $f$ が $\mathbb R$ 上で凸なら、[$C^2$ 凸関数の Hessian 判定](../F0_00G_凸集合_凸関数_凸最適化/index.md#thm-f0-00g-hessian-convexity)により全点で $f''(x)\ge0$ が必要です。これは成り立たないため、$f$ は凸ではありません。
<!-- solution-end -->
