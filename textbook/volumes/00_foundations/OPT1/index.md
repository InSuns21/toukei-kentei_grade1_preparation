...
という一本の流れを組み直し、その上に後続で必要になる **凸錐**、**準凸性・準凹性**、**経済学で現れる凸選好との違い**を追加します。
...
> **定義（凸錐）**  
> 集合 $K\subset\mathbb R^n$ が凸錐であるとは、任意の $x,y\in K$ と任意の $\alpha,\beta\ge0$ に対して
> $$
> \alpha x+\beta y\in K
> $$
> が成り立つことをいう。
...
一般の滑らかな関数では接平面は局所近似にすぎません。凸関数では接平面が **全空間で下から支える** ため、
...
$C_2$ は単位円板です。Euclid ノルムの三角不等式から
...
は $x<0$ で負になります。もし $f$ が $\mathbb R$ 上で凸なら、$C^2$ 関数の Hessian 判定により全点で $f''(x)\ge0$ が必要です。これは成り立たないため、$f$ は凸ではありません。
<!-- solution-end -->
...
あるいは元の二変数問題で、目的関数が凸、実行可能集合が凸なので、局所最小点は大域最小点です。

さらに狭義凸性から、その大域最小点は一意です。
<!-- solution-end -->
