...
<!-- solution-start -->
#### 詳細解答
$\{U_i\}_{i\in I}$ を $f(K)$ の開被覆とする。連続性から $\{f^{-1}(U_i)\}_{i\in I}$ は $K$ の開集合族であり、$U_i$ が $f(K)$ を覆うのでその逆像は $K$ を覆う。$K$ の開被覆が有限部分被覆を持つという仮定から、有限個 $i_1,\ldots,i_m$ で
$$
...
$$
は $X$ の開被覆である。仮定により有限部分被覆を取り、最後に $F$ と交差すれば $X\setminus F$ は消え、有限個の $U_i$ が $F$ を覆う。
