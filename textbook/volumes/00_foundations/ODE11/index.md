...
パラメータを動かすと平衡点の個数や安定性、周期軌道の有無が変わることがあります。本章では一般中心多様体論へ進む前に、標準正規形を直接解き、周期軌道を Poincaré 写像の固定点として調べます。
...
> **定義（局所分岐）**  
> パラメータ付き自律系 $x'=F(x,\mu)$ で、ある $\mu=\mu_*$ を通過すると平衡点・周期軌道の局所的な個数または安定性が質的に変化するとき、$\mu_*$ で局所分岐が起こるという。
...
では $\mu<0$ に平衡点がなく、$\mu>0$ では $x=\pm\sqrt\mu$ の二点が現れます。
...
> $x'=\mu-x^2$ では、$\mu<0$ に平衡点はなく、$\mu=0$ に半安定平衡点 $x=0$、$\mu>0$ に不安定平衡点 $-\sqrt\mu$ と安定平衡点 $\sqrt\mu$ が存在する。
<!-- formal-statement-end -->

<!-- proof-start -->
### 証明

平衡点方程式は $x^2=\mu$ です。$f(x)=\mu-x^2$ とすると
...
1. $f=x(\mu-x)$ なので平衡点は $0,\mu$。$f_x=\mu-2x$ より
   $$
   f_x(0)=\mu,
   \qquad
   f_x(\mu)=-\mu.
...
2. $f=x(\mu-x^2)$。$\mu>0$ では $0,\pm\sqrt\mu$ が平衡点で
   $$
   f_x=\mu-3x^2.
   $$
...
$x'=\mu-x^2$ の平衡点と安定性を $\mu<0,=0,>0$ で分類せよ。

<!-- solution-start -->
##### 詳細解答
$\mu<0$ は平衡点なし。$\mu=0$ は半安定な0。$\mu>0$ は $\pm\sqrt\mu$ で、$f'=-2x$ より正根が安定、負根が不安定です。
<!-- solution-end -->
...
<!-- solution-start -->
##### 詳細解答
平衡点は $0,\mu$。$f_x(0)=\mu$, $f_x(\mu)=-\mu$ なので $\mu=0$ で安定性を交換します。
<!-- solution-end -->

#### ODE11-A03 pitchfork
- Level: A

$x'=\mu x-x^3$ の平衡点を分類せよ。

<!-- solution-start -->
##### 詳細解答
$0$ は常に平衡点。$\mu>0$ で $\pm\sqrt\mu$ が追加。$f_x=\mu-3x^2$ より $\mu<0$ では原点安定、$\mu>0$ では原点不安定、非零二枝は安定です。
<!-- solution-end -->
...
$z'=(\mu+i)z-|z|^2z$ を極座標に直し、$\mu$ が0を通るときの平衡点と周期軌道の安定性を説明せよ。
