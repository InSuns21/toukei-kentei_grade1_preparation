...
> **この章で重要な見方**  
> 線形計画では「最適解を計算する」前に、なぜ候補を実行可能領域の角へ絞れるのか、なぜ双対変数が下界の証明書になるのか、なぜ主問題と双対問題の最適値が一致するのかを理解します。OPT11 の単体法（simplex method）は、この幾何を実際のピボット操作へ変換する章です。
...
は多面体ですが、後で定義する意味での「角」に当たる点を一つも持ちません。
...
多面体の「角」を理解する前に、「局所的に動ける方向」を定義します。
...
です。$d_j\ge0$ の成分では $x_j+td_j\ge x_j\ge0$ なので非負性を保ちます。
...
は係数がすべて 0 ではない一次関係です。よって $\{a_j:j\in I_+(x)\}$ は一次従属です。
...
$\operatorname{rank}A=m$ なので、$A$ の列空間の次元は $m$ です。現在の一次独立な列族の本数が $m$ 未満なら、その張る部分空間は列空間全体ではないため、その外にある $A$ の列を1本追加できます。この操作を本数が $m$ になるまで繰り返し、その添字集合を $B$ とします。
...
<a id="def-opt10-degenerate-bfs"></a>
<!-- formal-statement-start -->
> **定義（退化した基本実行可能解）**  
> $A\in\mathbb R^{m\times n}$、$\operatorname{rank}A=m$ とし、$x$ を基底 $B$ に対応する基本実行可能解とする。
>
...
したがって、この基本実行可能解は退化しています。
<!-- definition-example-end -->

退化は単体法のピボットで重要になりますが、アルゴリズム上の扱いは OPT11 に回します。
...
- **下界証明書**：$A^{\mathsf T}y\le c$ を満たす $y$ は、すべての主実行可能解に対する目的値の下界を与える。
- **相補性**：最適点では、正の主変数に対応する制約が等号になり、二つの目的値の差が成分ごとに消える。
...
- 双対可行 $y$ は
  ...
  $$
  という主目的値の下界証明書を与える。
- [Farkas の補題](../OPT2/index.md#thm-opt2-farkas)により、有限最適値を持つ LP では双対証明書が最適値まで到達し、
  $$
  p^*=d^*
  $$
  となる。
