...
だけからできています。式の形は単純ですが、ここには後続の simplex 法、内点法、整数計画、minimax、協力ゲーム、assignment market の骨格がほぼ全部現れます。
...
> **この章で重要な見方**  
> 線形計画では「最適解を計算する」前に、なぜ候補を極点へ絞れるのか、なぜ双対変数が下界の証明書になるのか、なぜ主問題と双対問題の最適値が一致するのかを理解します。OPT11 の simplex 法は、この幾何を実際の pivot 操作へ変換する章です。
...
このような点を **退化した基本実行可能解**と呼びます。退化は simplex 法の pivot で重要になりますが、アルゴリズム上の扱いは OPT11 に回します。
...
特に $v>0$ と $s_2>0$ ではありません。変数 $v$ と余剰変数 $s_2$ が正なので、それぞれ対応する双対制約は等号になっています。
...
この簡潔さが、simplex 法と主双対内点法の両方を支えます。
...
この種の infeasible / unbounded の分類は OPT11 で simplex 法とともに扱います。
...
これが simplex 法で cycling を考える理由の一つです。
...
7. 「極点」「基本実行可能解」「双対価格」「相補性」が同じ最適解を別方向から説明していることを文章でまとめよ。
...
- **双対価格**：双対可行 $y$ は、すべての主実行可能解に対する目的値の下界を与える証明書になる。
...
次の OPT11 では、この有限個の基底候補を実際にどう移動するかを扱います。simplex 法の pivot、退化、実行不能・非有界の判定、対数障壁と中心路、内点法、双対価格と感度解析へ進みます。
