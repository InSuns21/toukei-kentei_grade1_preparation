...
$L^2([0,1])$ では元はほとんど至る所等しい関数の同値類です。ある代表元 $f$ の一点 $x_0$ だけを変更して得た $g$ は $L^2$ では同じ元ですが、
...
この式が「再生」の意味です。点での値が内積によって再現されています。
...
よって第1引数の表示を変えても値は変わりません。対称性により第2引数についても同じです。従って双線形形式は表示によらず定義されます。
...
<!-- proof-start -->
### Moore--Aronszajn の定理の証明のまとめ

11--15節により、正半定値核 $K$ から $K$ を再生核とする RKHS $\mathcal H_K$ を構成できました。
...
Moore--Aronszajn の定理は、任意の集合 $\mathcal X$ 上の正半定値核から RKHS を構成します。位相、測度、コンパクト性は仮定していません。

一方、Mercer の定理は、コンパクト空間上の連続核と測度などの追加構造の下で、対応する積分作用素の固有関数展開を与える定理です。
...
Riesz 表現定理では、連続線形汎関数の作用素ノルムは表現ベクトルのノルムに等しいので
...
- Moore--Aronszajn の構成で、内積の well-definedness、点評価の有界性、完備化後の関数同定、一意性を順に説明できる。
