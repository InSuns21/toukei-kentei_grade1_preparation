...
この考え方が、後半で点集合を決める整数ベクトルの成分を一つずつ選ぶ構成法へつながります。
...
FOU1 では $2\pi$ 周期の一次元関数に対して複素指数係数を定義しました。本章では周期を1へ規格化し、多次元へ拡張します。一次元で変数を $\theta=2\pi x$ と置けば、FOU1 の規約と同じ内容です。

ここでは各座標を1周期ごとに同一視した周期領域を **単位トーラス** と呼び、代表元を $[0,1)^s$ に取ります。本章で必要なのはこの周期同一視だけで、多様体論の構造は用いません。
...
この「構成を代数で設計する」考え方は、QMC4 以降のデジタルネット（digital net）でも別の代数構造として現れます。

ただしデジタルネットでは、格子則とは異なる桁構造に沿った周波数解析が主役になります。
