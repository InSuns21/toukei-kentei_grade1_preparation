# F0-00F1 固有空間・実対称行列・直交対角化・PSD

F0-00Fで線形写像のkernelとimageを準備しました。この講義では自己写像の固有方向から、実対称行列の正規直交固有基底とPSDまでを組み立てます。

$$\boxed{\text{固有空間}\to\text{対称性}\to\text{Rayleigh商}\to\text{直交対角化}\to\text{PSD}}$$
...
前節までで、単位球面上のRayleigh商最大点から単位固有ベクトル $q_1$ を1本得られ、$q_1^\perp$ が $A$ に対して不変になることを示しました。そこで次元を1つ落とした $q_1^\perp$ に同じ議論を繰り返します。
...
統計で半正定値行列の平方根を使う場面でも、この構成がそのまま働きます。
...
**次：[F0-00F2 長方形行列の直交分解](../F0_00F2_SVD_特異値_作用素ノルム/index.md)**
