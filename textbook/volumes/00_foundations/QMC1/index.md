...
<!-- definition-example-start: def-qmc1-uniform-sequence -->
**定義の確認**：細かくなる中点格子を順につなぐ
...
をこの順に連結して一つの無限点列 $(x_n)_{n\ge1}$ を作ります。

この点列が一様分布することを定義から確かめます。
...
を固定します。第 $m$ ブロックの $m$ 個の中点は幅 $1/m$ の各小区間に1点ずつあるので、
...
実際、$B$ の両端を含む高々2個の小区間だけが、長さによる期待点数と完全には一致しません。

第 $1$ ブロックから第 $M$ ブロックまでに現れる点数を
...
なら、$S_{M-1}$ から $N$ までに追加される点は高々 $M$ 個です。したがって、区間 $B$ の点数誤差はブロック端 $S_{M-1}$ での誤差に高々 $M$ を加えた量で抑えられ、

$$
\left|
\#\{1\le n\le N:x_n\in B\}
-
N(b-a)
\right|
\le
...
任意の $B=[a,b)$ で成り立つため、この点列は定義どおり一様分布します。

この例の要点は、各ブロックの点数誤差が定数量なのに対し、$M$ ブロックまでの総点数は二次的に増えることです。
<!-- definition-example-end -->
...
この公式を各座標へ順に使うと、多変数関数を「1側の境界値と混合偏導関数の積分」に分解できます。この後の求積誤差恒等式で必要なのは、その反復だけです。
