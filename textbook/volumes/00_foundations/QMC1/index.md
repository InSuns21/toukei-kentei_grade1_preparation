...
「一様に見える」を、すべての $[0,t)$ に対する最悪のずれとして数値化するのがスター・ディスクレパンシーです。

---

## 1. 局所ディスクレパンシーとスター・ディスクレパンシー
...
| 点は通常ランダム | 点は通常決定論的 |
| 誤差は確率変数 | 誤差は決定論的 |
| 分散・標準誤差・CLT を使う | discrepancy・variation を使う |
...
Koksma--Hlawka の不等式では、最高次元の混合変動だけでは足りません。
...
## 6. Hlawka--Zaremba 恒等式：積分誤差を discrepancy で書き直す
...
3. 積分順序を交換すると、「$\boldsymbol x_n$ が $[0,\boldsymbol t)$ に入るか」という指示関数が現れる。
...
この恒等式は「なぜ discrepancy が積分誤差へ現れるのか」を直接説明しています。

---

## 7. Koksma--Hlawka の不等式
...
逆に、MC1 の中心極限定理に基づく区間は確率的であり、有限標本で必ず誤差を囲う決定論的上界ではありません。

両者は似た形の「誤差幅」でも、論理の種類が異なります。

---

## 10. 低 discrepancy が得られたら何が起こるか
...
1. Koksma--Hlawka の不等式から誤差上界を求めよ。
...
### QMC1-B01 一般直方体の discrepancy を star discrepancy で抑える
...
### QMC1-B02 一様分布から star discrepancy へ：有限格子による一様化
...
### QMC1-C01 仮定した低 discrepancy 点集合で Monte Carlo と比較する
...
固定次元で仮定した discrepancy 評価が使え、かつ $V_{\mathrm{HK}}(f)$ が有限で適度なら、QMC の決定論的上界は $N^{-1}$ に近く、MC の $N^{-1/2}$ より速い形です。
