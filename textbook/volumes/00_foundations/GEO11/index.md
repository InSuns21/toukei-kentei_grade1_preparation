...
超曲面を局所パラメータ表示すると、位置ベクトルの二階微分は「接方向」と「法線方向」に分かれます。接方向の係数は第一基本形式だけから決まり、法線方向の係数は第二基本形式です。さらに、同じベクトルを二つの順序で微分した結果が一致しなければならないため、第一・第二基本形式には二種類の整合条件が現れます。
...
第二基本形式の係数を微分するときに繰り返し現れる組合せを、本章では
...
ここではこの式を座標計算の記号としてだけ使います。一般の接続と共変微分は GEO13 で改めて構成します。
...
> とその一階微分から計算される。
>
> 従って、局所微分同相が第一基本形式を保つなら、その対応する点で Gauss 曲率も一致する。
<!-- formal-statement-end -->
...
局所微分同相が第一基本形式を保つ場合、この式から対応する点の Gauss 曲率も保たれます。$\square$
<!-- proof-end -->
...
第一基本形式 $G$ だけでは埋め込み方は決まりません。第二基本形式 $B$ まで与え、Gauss--Codazzi を満たすよう要求すると、局所形状が剛体運動を除いて決まります。
...
> さらに
>
> - $\lambda=0$ なら、$M$ は局所的にある余次元1のアフィン部分空間の開部分である。
...
2. 外向き法線に対する $B$ を求めよ。
3. [Gauss 方程式](#thm-geo11-gauss-codazzi)から $R_{1212}=0$ を確認せよ。
...
[Gauss 方程式](#thm-geo11-gauss-codazzi)から
...
[Gauss の驚異の定理](#thm-geo11-egregium)から
...
3. [超曲面の基本定理](#thm-geo11-fundamental-hypersurface)から、他の局所実現について何が言えるか。
...
3. [超曲面の基本定理](#thm-geo11-fundamental-hypersurface)により、同じ $G,B$ を持つ任意の局所実現は、十分小さい連結近傍上でこの円柱に剛体運動を施したものです。
<!-- solution-end -->
