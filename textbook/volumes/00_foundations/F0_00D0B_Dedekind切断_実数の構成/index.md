...
ここで分配法則や逆元まで切断の集合計算だけで再証明することもできますが、長く技術的で、本章の主役である「順序の穴を集合で埋める」機構から外れます。次章では Cauchy 実数との全単射 $\Phi$ を構成し、
...
を証明します。これにより Cauchy 側の体公理がこの切断演算へ移り、加法逆元・乗法逆元・分配法則を含む順序体構造が重複なく確定します。
...
一方、切断演算が加法逆元・乗法逆元・分配法則まで含む完全な体公理を満たすことは、次章の Cauchy 構成との同値で閉じます。そこで $\Phi$ が全単射かつ加法・乗法を保つことを証明すれば、既に順序体である $\mathbb R_C$ の全ての体法則が $\mathbb R_D$ へ移ります。これは証明の省略ではなく、**同じ結果を二重に証明しないための canonical dependency** です。
