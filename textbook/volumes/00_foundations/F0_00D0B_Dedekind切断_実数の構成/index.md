...
とします。$0\in1^*$、$1\notin1^*$ なので非空な真部分集合です。$q<1$ かつ $p<q$ なら $p<1$ なので下方閉です。また $q<1$ なら

$$
q<\frac{q+1}{2}<1
$$

なので最大元を持ちません。従って $1^*$ は Dedekind切断であり、定義どおり $1^*\in\mathbb R_D$ です。

$\sqrt2$ に対応する $A_{\sqrt2}$ も前節で4条件を全て確認したので $A_{\sqrt2}\in\mathbb R_D$ です。境界点を直接持たなくても、その境界より左の有理数全体を一つの Dedekind実数として扱えます。
<!-- definition-example-end -->
...
$r<s$ なら、任意の $q\in r^*$ は $q<r<s$ を満たすので $q\in s^*$ です。したがって

$$
r^*=\{q:q<r\}\subseteq\{q:q<s\}=s^*.
$$
...
です。加法逆元の具体式は後続の同値章で Cauchy 側の加法逆元を移送して確定します。ここでは、加法の候補が切断として閉じ、有理数上の加法と一致するところまでを自力で閉じます。

---

## 5. 正の元の乗法候補

正の切断 $A,B>0^*$ について、正の有理数部分を掛けた左側集合を
...
「$q\le0$」を最初から含めるのは、正の実数の積の左側には全ての非正有理数が入るからです。この集合が下方閉で最大元を持たないことは定義から直接確認でき、正の標準切断では

$$
r^*s^*=(rs)^*
$$

となります。逆包含では $q<rs$ の正の余裕を使い、$r,s$ をそれぞれ少し下から有理数 $a,b$ で近似して $q<ab<rs$ とします。

ここで分配法則や逆元まで切断の集合計算だけで再証明することもできますが、長く技術的で、本章の主役である「順序の穴を集合で埋める」機構から外れます。次章では Cauchy 実数との全単射 $\Phi$ を構成し、切断の加法について

$$
\Phi(x+y)=\Phi(x)+\Phi(y)
$$

を全ての $x,y$ で証明し、正の $x,y$ については上の積候補が

$$
\Phi(xy)=\Phi(x)\Phi(y)
$$

を満たすことを直接証明します。そのうえで負号・負の元を含む積・逆元を $\Phi$ によって移送します。これにより Cauchy 側の体公理が $\mathbb R_D$ へ移り、加法逆元・乗法逆元・分配法則を含む順序体構造が重複なく確定します。
...
### 6.1 この章で閉じたこと／次章へ送ること

ここまでで、Dedekind切断だけから
...
を構成しました。特に**順序完備性そのものはこの章で証明済み**です。

一方、切断演算が加法逆元・乗法逆元・分配法則まで含む完全な体公理を満たすことは、次章の Cauchy 構成との同値で閉じます。そこで $\Phi$ が全単射かつ順序と加法を保ち、正の元では上の積候補を保つことを証明します。その後、残りの演算を $\Phi$ で移送すれば、既に順序体である $\mathbb R_C$ の体法則が $\mathbb R_D$ へ移ります。これは証明の省略ではなく、**同じ結果を二重に証明しないための canonical dependency** です。
...
- 有理数の標準切断と順序保存を示せる。
- 切断の加法が切断になることを証明できる。
- 正の積がどのような左側集合として定義されるか説明できる。
- 上に有界な切断族の上限が和集合になることを証明できる。
- 完全な体公理を次章の Cauchy-Dedekind 同型から移送する理由を説明できる。
