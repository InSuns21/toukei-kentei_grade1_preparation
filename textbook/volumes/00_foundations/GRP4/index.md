...
> **この章の停止線**
>
> Sylow 理論を使った有限群の標準的な構造判定と、内部半直積の入口までを扱います。一般の群拡大、群コホモロジー、有限単純群の分類、一般表現論へは進みません。
...
そのような部分群が本当に存在することが Sylow の第一定理です。

---

## 3. Sylow の第一定理：最大の $p$-部分群は必ず存在する
...
Cauchy の定理を中心 $Z(G)$ に適用すると、位数 $p$ の元
...
## 5. Sylow の第二定理：全ての $p$-部分群は Sylow 部分群の中へ共役で入る
...
有限群の構造判定では、まず Sylow の第三定理で $n_p$ を絞り、$n_p=1$ を見つけるのが標準的な第一手です。
...
の両方が Sylow の第三定理と両立します。

つまり Sylow の個数条件は非常に強力ですが、常に群を一意に決めるわけではありません。

---

## 9. 内部半直積：正規部分群と補部分群から群を組み立てる
...
従って Sylow の第三定理だけからはこの候補を消せません。追加の構造情報が必要です。
<!-- solution-end -->
...
により $G$ から $S_3$ への準同型が定まります。

$a,b$ は $G=NH$ を生成し、$r,s$ は $S_3$ を生成するので、この準同型は全射です。

両群の位数はともに6なので全射は単射でもあり
