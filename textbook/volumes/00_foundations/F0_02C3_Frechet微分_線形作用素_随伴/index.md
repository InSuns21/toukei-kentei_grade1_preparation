...
第 $j$ 項は第 $j$ 座標だけを動かす一変数関数なので、$h_j\ne0$ なら一変数平均値定理により、$a^{(j-1)}$ と $a^{(j)}$ の間の点 $\xi_j$ が存在して
...
$\phi(t)=f(a+th)$ にRA3の一変数Taylor定理を適用すると

$$
\phi(1)
=\phi(0)+\phi'(0)+\frac12\phi''(0)+R(h).
$$

上の導関数表示を代入すれば主要3項は所望の形になります。二階微分の連続性から、$h\to0$ のとき線分上で $D^2f(a+th)-D^2f(a)$ の作用素ノルムが一様に0へ近づくので、剰余は $o(\|h\|^2)$ です。従って結論を得ます。$\square$
<!-- proof-end -->
...
$f:U\subset\mathbb R^n\to\mathbb R$ の全偏微分が $a$ の近傍で存在し、各偏微分が $a$ で連続するとする。Section 7の証明を参照せず、座標ごとの差分分解と一変数平均値定理だけから $f$ が $a$ でFréchet微分可能であることを証明せよ。
...
各項へ一変数平均値定理を使い、線分上の点 $\xi_j$ を取れば
...
座標増分へ望遠和分解し、各項に一変数平均値定理を適用する。偏微分の連続性で係数差を $o(1)$、$\ell^1$–Euclidノルム比較で残差を $o(\|h\|)$ と評価する。
