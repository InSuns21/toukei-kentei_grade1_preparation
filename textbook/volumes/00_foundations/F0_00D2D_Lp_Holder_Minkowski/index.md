...
とします。一般の $L^p$ ノルムは第2節で正式に定義しますが、この二点空間では数え上げ測度なので

$$
\|f\|_p
:=
\left(|f(1)|^p+|f(2)|^p\right)^{1/p}
\qquad(1\le p<\infty),
$$
...
Hölderでは積 $|fg|$ を扱いたいのですが、$f$ と $g$ は別々の $L^p,L^q$ 情報しか持っていません。[そこでYoungの不等式](#lem-f0-00d2d-01)を使い、**積を $p$ 乗と $q$ 乗の和へ分解**します。
...
[Youngの不等式](#lem-f0-00d2d-01)から各点で
...
で抑え、二つの積へ[Hölder](#thm-f0-00d2d-01)を使います。共役指数 $q=p/(p-1)$ を選ぶと
...
積分し、それぞれに[Hölder](#thm-f0-00d2d-01)を使います。
...
$r=q/p>1$ とし、その共役指数を $s$ とします。[Hölder](#thm-f0-00d2d-01)より
...
$[0,1]$ 上で $f(x)=x$、$g(x)=x^2$ とする。$p=2,q=2$ の[Hölder](#thm-f0-00d2d-01)により $\int_0^1x^3dx$ を上から評価せよ。
...
が[Minkowski](#thm-f0-00d2d-02)から従うことを示せ。
