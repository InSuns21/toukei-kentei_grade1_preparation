...
この定理から、例えば $f'\ge0$ なら単調増加です。実際 $x<y$ に平均値定理を適用すると
$$
f(y)-f(x)=f'(c)(y-x)\ge0.
$$
また $|f'|\le M$ なら
...
-(g(b)-g(a))(f(b)-f(a))=0.
$$
したがってRolleの定理から、ある $c\in(a,b)$ で $H'(c)=0$ です。微分すると
$$
H'(c)
=(f(b)-f(a))g'(c)
-(g(b)-g(a))f'(c),
...
ここでRolleの定理を繰り返します。まず $\Phi(a)=\Phi(x)=0$ から、$a$ と $x$ の間に $\xi_1$ が存在して $\Phi'(\xi_1)=0$。さらに $\Phi'(a)=\Phi'(\xi_1)=0$ なので、再びRolleの定理からその間に $\xi_2$ が存在して $\Phi''(\xi_2)=0$。これを $n+1$ 回繰り返すと、$a$ と $x$ の間にある $\xi$ が存在して
$$
\Phi^{(n+1)}(\xi)=0
$$
となります。
...
<!-- solution-start -->
**解答**：$x=y$ なら自明です。$x\ne y$ とすると、$x,y$ の間の閉区間に平均値定理を適用して、ある $c$ が存在し
$$
...
<!-- solution-start -->
**解答**：任意の $x<y$ を $I$ から取ります。$[x,y]$ 上で平均値定理を適用すると、ある $c\in(x,y)$ が存在して
$$
f(y)-f(x)=f'(c)(y-x).
$$
