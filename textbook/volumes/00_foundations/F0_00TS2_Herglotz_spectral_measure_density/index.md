...
$[-\pi,\pi]$ の有理点と両端点を可算列 $q_1,q_2,\dots$ と並べます。$(G_n(q_1))_n$ は有界なので収束部分列を持ちます。その部分列から $q_2$ で収束する部分列を抜き、これを繰り返して対角部分列を取ると、全ての $q_r$ で $G_{n_r}(q_r)$ ではなく
