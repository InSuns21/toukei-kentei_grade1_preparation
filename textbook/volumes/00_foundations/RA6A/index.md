...
各成分に [微積分学の基本定理II](../RA4/index.md#thm-ra4-ftc2) を適用すると
...
> を $f$ の **レベル集合** と呼ぶ。特に $n=3$ のとき、この集合を **レベル曲面** とも呼ぶ。
>
> さらに $f\in C^1(U)$ とし、点 $p\in L_c$ で

$$
Df(p)\neq0
$$

> なら $p$ を正則点と呼ぶ。ここで $Df(p):\mathbb R^n\to\mathbb R$ は Fréchet 微分であり、$Df(p)\neq0$ は零写像ではないという意味である。このとき陰関数定理により、$p$ の近くの $L_c$ は実際に二次元の $C^1$ 級曲面になり、これを **正則レベル曲面**（一般の $n$ では正則レベル集合）と呼ぶ。
<!-- formal-statement-end -->
...
は半径 $\sqrt c$ の球面です。点 $p=(x,y,z)$ における Fréchet 微分は
...
球面上では $p\neq0$ なので、例えば $h=p$ と取れば

$$
Df(p)[p]=2\|p\|^2=2c>0.
$$

従って $Df(p)$ は零写像ではなく、この球面は正則レベル曲面です。
...
[正則レベル集合の接空間公式](#cor-ra6a-regular-level-set)から
