...
Euclid 空間では、点の近くを一つの座標系で表せます。しかし球面やトーラスのような空間全体を、特異点なしに一枚の Euclid 座標で覆うことはできません。
...
を取ります。前節で両者の座標変換と逆変換がともに滑らかであることを確認しました。従って $\mathcal A\cup\mathcal B$ は滑らかなアトラスであり、二つのアトラスは両立します。
<!-- definition-example-end -->
...
$p$ は $U\cap W$ の任意の点でした。Euclid 空間上の滑らかさは局所的な性質なので、$\chi\circ\varphi^{-1}$ は定義域全体で滑らかです。逆変換も同様です。
...
二枚は $S^n$ を覆い、重なりの座標変換と逆変換は

$$
y\longmapsto\frac{y}{\|y\|^2}
$$

で、$y\ne0$ 上で滑らかです。従って二枚は滑らかなアトラスです。前節の極大化定理により標準滑らかな構造が定まります。$\square$
<!-- proof-end -->
...
と書きます。同値類 $q(x)$ は $[x]$ と書きます。

### 7.1 Hausdorff 性と第二可算性

まず $q$ は開写像です。$O\subseteq S^n$ が開なら
...
その像は $[x]$ と $[y]$ を分離する互いに素な開近傍になります。従って $\mathbb{RP}^n$ は Hausdorff です。
...
非零スカラー倍をしても比は変わらないので well-defined です。逆写像は、$i$ 番目の成分を $1$ にして作ったベクトル $v(u)$ に対して
...
です。従って $\varphi_i$ は同相写像です。

二つの座標 $U_i,U_j$ が重なるところでは、$j$ 番目に対応する座標成分が非零であり、座標変換は各成分をその非零成分で割る有理式になります。分母が0にならない開集合上なので滑らかで、逆変換も同じ形です。
...
$U_i$ は $q(\{x\in S^n:x_i\ne0\})$ であり、$q$ が開写像なので開です。$\varphi_i$ は $\mathbb R^n$ への同相写像であり、$U_i$ は全体を覆います。従って $\mathbb{RP}^n$ は $n$ 次元位相多様体です。

重なりの座標変換は、非零な一つの座標で各成分を割る有理式です。分母が0でない領域では $C^\infty$ 級で、逆変換も同じ形です。従って全ての座標近傍は滑らかに両立し、滑らかなアトラスをなします。極大化により標準滑らかな構造が定まります。$\square$
<!-- proof-end -->
...
です。各成分は滑らかで逆変換も滑らかなので、積座標どうしは滑らかに両立します。$\square$
<!-- proof-end -->
...
$p$ と $f(p)$ の十分小さい近傍へ制限して、必要な合成が全て定義されるようにします。
...
右辺の中央は仮定により滑らかです。左右の座標変換は、それぞれ $M,N$ の滑らかな構造に属する座標どうしの変換なので滑らかです。

従って三つの滑らかな写像の合成は滑らかであり、
...
## 11. 位相的な同定に使う compact → Hausdorff の道具

多様体の具体例では、ある商空間や貼り合わせ空間が既知の空間と同相であることを示したい場面が頻繁に出ます。

この位相的部分は GEO1 で再証明しません。TOP2 の

- [コンパクト空間から Hausdorff 空間への連続全単射](../TOP2/index.md#thm-top2-compact-hausdorff-bijection)
...
1. $O\subseteq S^n$ を開集合とします。反転 $a(x)=-x$ は同相写像なので $-O$ も開です。
   ...
   は開です。商位相の定義により $q(O)$ は開です。従って $q$ は開写像です。
...
3. $[x]\ne[y]$ とします。このとき $\{x,-x\}$ と $\{y,-y\}$ は互いに素です。有限個の正数
   ...
   両者は反転で不変です。1より $q(O_x),q(O_y)$ は開で、$[x],[y]$ をそれぞれ含みます。像も互いに素なので Hausdorff です。
...
4. $\{x\in S^n:x_i\ne0\}$ は開で、1よりその像 $U_i$ は開です。
   ...
   は非零スカラー倍で値が変わらないので well-defined です。
   ...
   一方 $\varphi_i\circ q$ は $x_i\ne0$ の領域で座標比からなる連続写像で、商写像の普遍性により $\varphi_i$ も連続です。従って $\varphi_i$ は同相写像です。
...
5. $U_i\cap U_j$ では $x_j/x_i\ne0$ です。$\varphi_i$ 座標で $i$ 番目を1に正規化した同次座標を考えると、$\varphi_j$ 座標へ移る操作は「全成分を $j$ 番目の非零成分で割り、$j$ 番目を除く」ことです。
   ...
   の形です。$u_j\ne0$ 上では $C^\infty$ 級です。逆変換も $i,j$ を交換した同じ形なので滑らかです。
