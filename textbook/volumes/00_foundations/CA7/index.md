# CA7 標準複素解析 VII：正則関数列・関数族のコンパクト性・単連結領域の円板表示

> **標準複素解析コア VII**。CA1--CA6 では、一つの正則関数を微分・積分・展開し、その零点・極・解析接続・調和性を調べてきた。本章では視点を「関数の列・関数の族」へ移す。Cauchy 積分公式が与える強い局所制御から収束部分列を取り出し、零点と単射性を極限へ移し、最後に単連結真部分領域を単位円板へ双正則に写すところまで進む。
...
- [CA2 の単連結領域における閉曲線積分の消滅](../CA2/index.md#thm-ca2-cauchy-simply-connected)
...
- [TOP5 の距離空間コンパクト性の逐次的特徴付け](../TOP5/index.md#thm-top5-metric-sequential)

一方、一般の Arzelà--Ascoli の定理、一様化定理、Riemann 面、Runge の近似定理は使わない。関数族から収束部分列を取り出す核心部分も、一般関数解析のコンパクト性定理を黒箱にせず、複素解析で必要な有限近似点集合と対角化を本文で証明する。
...
**定義の確認**：有限個の正則関数からなる族は正規族である。実際、任意の列では少なくとも一つの関数が無限回現れるので、その定数部分列を取れば局所一様収束する。後で、有限族でなくても局所有界性だけから同じ部分列抽出ができることを証明する。
<!-- definition-example-end -->
...
この系は後の円板表示の存在証明で重要である。関数族から取った極限が「正則である」だけでは足りず、極値問題の候補であり続けるために**単射性まで保存する必要がある**からである。

---

## 6. 単連結領域では零点を持たない正則関数の平方根を取れる

後で行う単連結真部分領域の円板表示の存在証明では平方根を二回使う。一回目は候補族が空でないことを示すため、二回目は極値写像が円板全体を覆うことを示すためである。
...
$h'/h$ は $\Omega$ 上正則である。単連結性を使うのはここ一箇所であり、[CA2 の単連結領域における閉曲線積分の消滅](../CA2/index.md#thm-ca2-cauchy-simply-connected)により $h'/h$ の原始関数を取る。その指数関数の半分を使えば平方根ができる。
...
前節の円板表示とは別に、複素解析では「境界を越えて正則性を貼る」問題も重要である。まず閉三角形積分の消滅から正則性を判定する補題を証明し、それを用いて実軸を越える共役対称な延長を正当化する。
