...
一方、一般の Arzelà--Ascoli の定理、一様化定理、Riemann 面、Runge の近似定理は使わない。特に Montel の定理は一般関数解析のコンパクト性定理を黒箱にせず、複素解析で必要な有限ネットと対角化を本文で証明する。
...
ここでは「微分した級数も正規収束する」とまでは主張していない。必要なのは、元の級数の正規収束から部分和が局所一様収束し、正則関数列の極限定理によって**導関数の部分和**も局所一様収束する、という順序である。
...
一般の Arzelà--Ascoli の定理は使わない。コンパクト性から各精度 $1/m$ の有限ネットを取り、その合併を可算稠密集合にする。各稠密点で Bolzano--Weierstrass を順に適用して対角部分列を作り、最後に同程度連続性で「稠密点での収束」を「全点での一様 Cauchy 性」へ持ち上げる。
...
さて任意の列 $(f_n)\subset\mathcal F$ を取る。局所有界性とコンパクト性から各 $K_m$ 上で $(f_n)$ は一様有界であり、[Cauchy 評価による同程度連続性](#lem-ca7-cauchy-equicontinuity)も成り立つ。
...
ここで「各点 $z$ で値列 $(f_n(z))$ が有界だから、点ごとに部分列を取る」だけでは足りない。非可算個の全点で同じ部分列を使う必要がある。有限ネットと対角化は、その量化順序を正しくそろえるための装置である。
...
とする。最後の $f'(a)>0$ は「複素数 $f'(a)$ が正の実数」という意味であり、回転の自由度を固定している。
...
を考える。[候補族の非空性](#lem-ca7-riemann-class-nonempty)から $M>0$、Cauchy 評価から $M<\infty$ である。
...
単射正則写像の導関数は消えない。実際 $f'(z_0)=0$ なら Taylor 展開の最初の非零項が次数 $m\ge2$ となり、十分小さい近傍で $m$ 枚の局所分枝が生じて単射性に反する。従って $f'(z)\ne0$。

複素逆関数定理により $f^{-1}$ は各点の近傍で正則であり、これらは全域の逆写像として一致する。従って $f$ は双正則である。$\square$
<!-- proof-end -->
...
では Riemann 写像定理を使うまでもなく、CA6 で計算した
...
基点 $a$ だけを固定して導関数の偏角を固定しなければ、$e^{i\theta}f$ という回転の自由度が残る。正規化条件は存在のためではなく**一意性のため**に働く。
...
[正規収束する正則関数級数の項別微分](#cor-ca7-normal-series-termwise-differentiation)を適用すると
...
項別微分を許しているのは単なる各点収束ではなく、正規収束から得られる局所一様収束である。
<!-- solution-end -->
...
$f\not\equiv0$ と仮定する。もし $f(a)=0$ なら、Hurwitz の定理により $a$ の十分小さい近傍で、十分大きい $n$ の $f_n$ が零点を持つ。これは各 $f_n$ が零点を持たないという仮定に反する。従って $f$ は零点を持たない。
...
と近づいてきた零点の極限である。Hurwitz の定理はまさにこの「零点が局所的に追跡できる」ことを一般に保証する。
<!-- solution-end -->
...
は単射正則で $|H|\le1/2$。$H(a)$ を0へ送る円板自己同型を合成し、最後に回転を掛けて導関数を正の実数へ向ければ $\mathcal S_a$ の元を得る。
...
回転を掛けて $\psi'(a)$ を正の実数へ向ければ $\widetilde f\in\mathcal S_a$ で
...
この証明で各道具の役割は分離されている。単連結性は正則平方根を作り、Cauchy 評価は極値の有限性を与え、Montel は極値列から極限を取り、Hurwitz は単射性を極限へ移し、平方根と円板自己同型が極値を改善して全射性を強制する。
<!-- solution-end -->
...
- Montel の定理では一般 Arzelà--Ascoli の定理を黒箱にせず、Cauchy 評価、有限ネット、稠密点上の対角抽出、コンパクト exhaustion を順に証明した。
...
- 正規化条件 $f(a)=0$, $f'(a)>0$ が回転自由度を除き、一意性を与えることを Schwarz の補題から示した。
