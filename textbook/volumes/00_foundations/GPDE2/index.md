...
GPDE3 では、この「粗い対象の微分が何階まで $L^p$ 関数として残るか」を関数空間の定義へ昇格させ、Sobolev 空間へ進みます。
...
のような高階偏微分を繰り返し扱います。そこで、微分回数を一つの組で表す次の記法を導入します。
...
粗い関数を直接扱うだけでなく、一度滑らかにしてから計算し、最後に元へ戻る方法が欲しくなります。そのため、全質量を 1 に保ちながら小さな球へ集中する滑らかな平均核を用意します。
