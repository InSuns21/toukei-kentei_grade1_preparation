...
Web ページの重要度を「受け取ったリンクの本数」だけで決めると、どのページからのリンクも同じ重みになります。しかし自然には、**重要なページからリンクされること自体を高く評価したい**はずです。すると重要度は自己参照的に決まります。

ページ $j$ の重要度を、そのページから出るリンクへ等分して渡すとします。ページ重要度を並べた確率ベクトルを $r$、リンク構造から作った列確率行列を $P$ とすれば、リンクを一回たどった後の重要度は
...
- 確率 $1-\alpha$ ではリンクを無視し、あらかじめ決めた確率分布 $v$ に従って別ページへ移動する。
