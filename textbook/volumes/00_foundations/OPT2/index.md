...
> **この章の停止線**
>
> 劣微分・法錐の一般論、Fenchel 双対、KKT はまだ使いません。有限次元の内積と凸集合だけから分離論を構成します。
...
つまり、**最近点を一つ見つければ、集合と外の点を分ける超平面が自然に出てくる**わけです。
...
は作れません。$C$ の点は境界 $x_1=0$ にいくらでも近づけるからです。
...
外の点と集合を分けるだけでなく、凸集合の境界そのものを支える超平面も重要です。経済学では支持価格、最適化では劣勾配・法線へつながります。
...
$x_0\in\partial C$ なので、$C$ の外の点列 $z_k\notin C$ を
...
OPT1 で導入した凸錐は、Farkas の補題で「非負係数による表現可能性」を表します。
...
Farkas の補題では、行列 $A$ の列 $a_1,\dots,a_m$ が生成する錐
...
では正にならないので、符号を反転しても $A^{\mathsf T}y\le0$ を保てません。この候補は使えません。

そこで両列に非正、$b$ に正となる条件を解きます。$y=(y_1,y_2,y_3)$ とすると
...
$\mathbb R_+^2$ への最近点は、負の第1成分を0へ切り上げ、第2成分はそのまま残して
