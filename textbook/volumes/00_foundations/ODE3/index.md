...
各成分をスカラー級数として評価します。$A^k$ の成分数は有限なので、$k$ 乗で増える部分はある定数の $k$ 乗で抑えられ、$k!$ がそれを上回ります。導関数の級数も同じ方法で一様収束するため、[微分と極限の交換定理](../RA5/index.md#thm-ra5-derivative)を成分ごとに使えます。半群性は絶対収束した二つの級数の積を次数ごとにまとめ、二項定理で閉じます。
...
次に半群性を示します。各成分で絶対収束しているため級数の積を次数ごとにまとめられます。
...
$t=t_0$ では積分が0、指数が $I$ なので $X(t_0)=x_0$ です。一意性は、二つの解の差が斉次系を満たし、[斉次線形系の初期値問題](#thm-ode3-homogeneous-ivp)の一意性から0になることに従います。$\square$
<!-- proof-end -->
...
> 定係数斉次線形系 $x'=Ax$ の平衡点 $x=0$ を考え、初期時刻を0とする。$\|\cdot\|_2$ を Euclid ノルムとする。
>
...
まず、固定行列 $P=(p_{ij})$ による乗算はノルムを高々定数倍します。実際、Cauchy--Schwarz 不等式から各行について
...
[斉次線形系の初期値問題](#thm-ode3-homogeneous-ivp)より
...
固有値は $-2,3$ です。正の実部を持つ固有値 $3$ があるので、[スペクトル安定性判定](#thm-ode3-spectral-stability)により原点は Lyapunov不安定です。実際、第二固有方向では小さい初期値も $e^{3t}$ 倍に増大します。
<!-- solution-end -->
...
なので $x(t)\to0$ です。唯一の固有値は $-1$ で実部が負です。非自明 Jordan block は多項式因子を作りますが、負の指数減衰が有限次数の多項式成長に勝つため、[スペクトル安定性判定](#thm-ode3-spectral-stability)より原点は指数安定、従って漸近安定です。
<!-- solution-end -->
...
が非有界です。境界固有値の block が1次でないため、[スペクトル安定性判定](#thm-ode3-spectral-stability)より原点は不安定です。
<!-- solution-end -->
...
- 半群性 $e^{sA}e^{tA}=e^{(s+t)A}$ と逆行列 $e^{-tA}$ を導ける。
