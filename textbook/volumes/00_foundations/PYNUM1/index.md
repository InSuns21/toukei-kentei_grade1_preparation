...
> **停止線**
>
> 本章では Python の高度なオブジェクト指向、デコレータ、`yield` を使う反復子構文、非同期処理、packaging、Web 開発、pandas 中心のデータ分析は扱いません。
>
...
同じ乱数生成方式と同じ初期化条件を用いる限り、同じ初期化値から同じ呼出し順序で同じ疑似乱数列を再現できる。
<!-- formal-statement-end -->
...
のように実験入口で 初期化値を受け取り、必要な関数へ `rng` を渡す形を基本にします。
...
大規模な離散化計算では行列の大半が0になることが多いため、後続 NUMLAB で重要になります。

### 12.3 `scipy.sparse.linalg`

疎行列向けの直接法、反復法、大規模行列計算の入口です。
...
1. `X.shape == (4,2)` を確認せよ。
2. 各行の Euclid 長さを求めよ。
3. 各行を自分の長さで割って単位ベクトルへ正規化せよ。
4. 正規化後の各行の長さが1であることを `np.allclose` で確認せよ。
5. 長さの配列へ `[:, None]` が必要な理由を shape から説明せよ。
...
1. 初期化値2026から乱数生成器 `rng1`、`rng2` を別々に作り、それぞれ標準正規乱数を5個生成する。
2. `rng1` からさらに5個生成する。
3. 初期化値2027から `rng3` を作り、5個生成する。
...
- Level: C
- 主題: 数値実験の統合
- 使用技術: ベクトル化、dtype、誤差評価、log-log 図、誤差の減衰指数
- 計算量: 大
...
2. 各 $N$ の誤差の大きさを `dtype=float` の NumPy 配列へ保存せよ。
