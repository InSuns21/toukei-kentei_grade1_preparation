...
Poisson の零 Dirichlet 問題では、例えば bounded \(C^{1,1}\) 領域では標準的な大域楕円型正則性定理によりこの評価が成り立ちます。二次元の凸多角形領域でも同様の \(H^2\) 正則性が成り立つのが標準的な結果です。

一方、内角が \(\pi\) を超える reentrant corner を持つ多角形では、一般には \(H^2\) 正則性が失われます。
...
## 10. reentrant corner：正則性仮定は飾りではない
...
しかし reentrant corner では \(\beta<1\) です。
...
<a id="ex-fem4-b03"></a>
### FEM4-B03 reentrant corner で標準証明のどこが壊れるか
- Level: B
...
reentrant corner の例が示したように、解や双対解の大域正則性が落ちれば、標準次数を導く証明機構も失われます。
