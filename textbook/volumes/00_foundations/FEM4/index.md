...
[L2 誤差に対する楕円型双対問題](#def-fem4-l2-dual-problem)から、任意の $v\in V$ に対して
...
しかし良い近似 $I_hu$ を一様定数で保証できなくなるため、$h$ だけで一様な誤差次数を記述できません。
...
<a id="ex-fem4-a03"></a>
### FEM4-A03 補助問題の右辺が $H^{-1}$ に入ることを示す
- Level: A
...
ここから $|z|_{H^2}$ を $\|e\|_{L^2}$ で戻すために、補助問題の大域 $H^2$ 正則性が必要になります。
<!-- solution-end -->

<a id="ex-fem4-b02"></a>
### FEM4-B02 一次元 Poisson 補助問題を最後まで解く
- Level: B
...
1. 
   ...
   h.
   $$
2. $e=u-u_h$ と置き、補助問題の右辺に $g=e$ を入れて
   $$
   \|e\|_{L^2}^2
   =
   \int_\Omega
...
$H^1$ 評価は Céa と $u$ 自身の補間誤差だけから出ており、補助問題を使っていません。
