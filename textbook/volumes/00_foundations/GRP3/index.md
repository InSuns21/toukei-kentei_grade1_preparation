...
> **この章の停止線**
>
> 素数ごとの部分群の存在・共役・個数を体系的に調べる理論は GRP4 に送ります。本章では、その準備となる集合上の変換の一般論と、素数冪位数の有限群への最初の応用までを閉じます。
...
- 中心の元の軌道は1点。
- 非中心元の軌道は共役類。
- その大きさは[軌道・安定化群公式](#thm-grp3-orbit-stabilizer)により中心化群の指数。
...
[群の類等式](#thm-grp3-class-equation)を法 $p$ で見ると
$$
|G|
\equiv
|Z(G)|
...
1. 点 $1$ の軌道を求めよ。
2. 点 $1$ の安定化群を求めよ。
3. [軌道・安定化群公式](#thm-grp3-orbit-stabilizer)を確認せよ。
...
2. $\{1,2\}$ の軌道を求めよ。
3. $\{1,2\}$ の安定化群を具体的に求めよ。
4. [軌道・安定化群公式](#thm-grp3-orbit-stabilizer)を確認せよ。
...
p.
$$
素数位数の群は GRP2 の [群論の Lagrange の定理](../GRP2/index.md#thm-grp2-lagrange)から巡回群なので、
$$
G/Z(G)
$$
は巡回群です。
