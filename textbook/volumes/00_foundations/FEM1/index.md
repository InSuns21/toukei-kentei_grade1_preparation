...
> **この章の停止線**  
> 本章では有限次元部分空間 $V_h\subset V$ を抽象的に与えます。三角形分割、基準要素、節点自由度、局所基底、assembly は FEM2、補間誤差と mesh の形状正則性は FEM3、具体的な $h$ 依存誤差評価は FEM4 で扱います。
...
ここで添字 $h$ は後で mesh 幅を表しますが、本章ではまだ mesh を導入しません。
...
が mesh 幅 $h$ に対してどれくらい小さいかは分かりません。
