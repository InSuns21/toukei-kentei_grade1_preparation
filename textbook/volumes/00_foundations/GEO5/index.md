...
一つの滑らかなベクトル場 $X$ は、各点で「今この方向へ進め」という速度を指定します。そこで曲線 $\gamma(t)$ に

$$
\gamma'(t)=X_{\gamma(t)}
$$

を課し、この方程式の解がどの範囲で存在して一意になるか、初期点を動かしたとき解がどう変化するか、そして初期点ごとの時間発展を一つの滑らかな写像としてまとめられるかを順に調べます。

後半では二つの滑らかなベクトル場 $X,Y$ を関数へ順に作用させ、

$$
X(Yf)-Y(Xf)
$$

という差が何を表すかを調べます。この非可換性は、座標表示・時間発展・局所座標の三つの観点を結びます。

ODE の局所存在・一意性そのものは再証明しません。[ODE1 の Picard--Lindelöf の局所存在・一意性](../ODE1/index.md#thm-ode1-picard-lindelof)と、[ODE4 の非線形自律系](../ODE4/index.md#def-ode4-nonlinear-autonomous-system)を使います。一方、多様体上で局所座標を変えても同じ幾何学的曲線を表すこと、局所解の貼り合わせ、時間発展の合成則、二つの方向微分の可換子が満たす代数法則は本章で閉じます。
...
北極と南極では $X=0$、それ以外では緯線方向を向きます。後でこの滑らかなベクトル場が生む時間発展は $z$ 軸回りに点を回す運動になることが分かります。
<!-- definition-example-end -->

---

## 2. 滑らかなベクトル場は滑らかな関数を方向微分する
...
ここでは「滑らかなベクトル場から導分を作る」向きだけを使います。任意の大域導分 $D:C^\infty(M)\to C^\infty(M)$ から滑らかなベクトル場を復元する逆向きも成り立ちますが、その証明には本章の prerequisite 外の大域的な局所化論が必要です。そのため、この同値性は本章の証明依存にはしません。
...
次に $\gamma_1,\gamma_2$ がともに $\gamma_i(0)=p$ を満たす積分曲線とします。0の近くで両曲線は同じ座標近傍 $U$ に入り、その座標表示は同じ方程式と初期条件
...
初期点ごとの時間発展を単なる点ごとの解の集合ではなく滑らかな写像として扱うには、初期値依存性が必要です。
...
が連続、まして滑らかとは限りません。時間と初期点を同時に動かす写像を作るには両方向の滑らかさが必要なので、この補題が橋になります。
...
を考えます。ODE1 で用いた縮小写像による反復構成と同じ評価により、$T_a$ は「$a$ から距離 $r/2$ 以内に留まる連続曲線」の集合を自分自身へ写し、縮小率 $Lh<1$ の縮小写像になります。従って一意な不動点 $u_a$ が存在します。
...
よって $a\mapsto u_a$ は、上の時間区間全体での最大差に関して連続です。
...
この補題では「滑らかな ODE なら解も初期値に滑らかに依存する」を黒箱にせず、同じ縮小写像の機構から初期値微分が出る場所を明示しました。
...
$a<0$ なら $t\downarrow1/a$ で絶対値が無限大になり、
...
この滑らかなベクトル場は $\mathbb R$ 全体で定義されていても、積分曲線が全時間存在するとは限りません。
<!-- definition-example-end -->
...
と定めます。別の $\beta$ でも $t\in I_\beta$ なら、[積分曲線の局所存在・一意性](#thm-geo5-local-integral-curve)から
...
まず $p\in M$ を固定します。座標近傍で[自律系の解の初期値への滑らかな依存](#lem-geo5-smooth-dependence)を適用すると、ある $\varepsilon>0$ と $p$ の開近傍 $V$ が存在して
...
この命題で重要なのは「滑らかなベクトル場の座標成分が有界」という座標依存の言い方ではなく、**コンパクト性によって有限個の局所存在時間から一様な正の時間を取り出せる**ことです。

---

## 9. 微分同相写像で接方向を別の多様体へ運ぶ
...
一般の滑らかな写像 $F:M\to N$ では、同じ $q\in N$ に複数の $p$ が写る可能性があるため、この式だけから $N$ 上の一意な滑らかなベクトル場を作れるとは限りません。微分同相写像なら $F^{-1}(q)$ が一意なので問題が消えます。
...
<!-- formal-statement-start -->
> **命題（押し出しと流れの共役）**  
> $F:M\to N$ を微分同相写像、$X$ を $M$ 上の滑らかなベクトル場、$Y=F_*X$ とする。$X,Y$ の最大流をそれぞれ $\Phi,\Psi$ とする。
>
> 両辺が定義される範囲で
>
> $$
...
> と定める。
>
> この作用で定まる滑らかなベクトル場 $[X,Y]$ を $X,Y$ の **Lie 括弧**という。
<!-- formal-statement-end -->

まず、右辺が本当に滑らかなベクトル場になることを確認します。
...
定義は $X(Yf)-Y(Xf)$ という座標を使わない式だったので、この座標公式は**座標変換しても同じ滑らかなベクトル場を表します**。座標公式そのものから不変性を確認するより、座標を使わない定め方から不変性が自動的に従う点が重要です。
...
<!-- formal-statement-start -->
> **定理（Lie 括弧の基本恒等式）**  
> 滑らかなベクトル場 $X,Y,Z$ と $f\in C^\infty(M)$ に対して次が成り立つ。
>
> 1. 双線形性
>
> $$
...
4では、滑らかなベクトル場を $C^\infty(M)$ 上の線形作用素として合成します。
...
Lie 括弧は単なる座標計算ではありません。滑らかなベクトル場 $X$ の流れで $Y$ を運んだときの一次変化率です。
...
です。$t=0$ で微分し、[流れによる Lie 括弧の解釈](#thm-geo5-bracket-flow)を使えば
...
## 14. 非零な滑らかなベクトル場は局所座標で一本の座標方向に直せる

GEO6 の Frobenius の定理では、複数の滑らかなベクトル場を同時に座標方向へ直せる条件を調べます。その前に、**一本だけなら非零である限り必ず直せる**ことを証明します。
...
$t$ 方向の微分は $X_p$、$z$ 方向の微分は横断面の接方向です。合わせて $T_pM$ の基底になるので、[逆関数定理](../RA6A/index.md#thm-ra6a-inverse-function)から $F$ は局所座標になります。この座標では時間 $t$ がそのまま第一座標です。
...
[RA6A の逆関数定理](../RA6A/index.md#thm-ra6a-inverse-function)から、$F$ は $(0,0)$ の近くから $p$ の近くへの微分同相写像です。その逆写像が局所座標
...
- Level: A
- 狙い: 滑らかなベクトル場から自律 ODE を立て、流れと群則を具体計算する
...
3. $X$ が完備でないことを説明せよ。
4. 滑らかなベクトル場が $\mathbb R$ 全体で定義されることと、完備性が別問題である理由を述べよ。
...
滑らかなベクトル場 $X,Y$ と滑らかな関数 $f,g$ に対して次を証明せよ。
...
これは原点からの距離を保存しながら角度を $t$ だけ増やす運動なので、初期点が $M$ にあれば全時刻で $M$ に留まります。従って $X$ は完備です。
...
流れの可換性も直接分かります。$\Phi_t$ は角度移動、$\Psi_s$ は正のスカラー倍なので
...
という可換性が、二つの流れの可換性と、二つの滑らかなベクトル場を同時に座標方向へ直せることの両方に現れています。

GEO6 では、この現象を $k$ 次元の滑らかなベクトル場族へ一般化します。ただし一般には「個々の滑らかなベクトル場が非零」だけでは足りず、生成する線形分布が Lie 括弧で閉じることが必要になります。
<!-- solution-end -->
...
本章では、GEO2 の点ごとの接ベクトルを滑らかに束ねて滑らかなベクトル場
...
という通常の自律 ODE です。[Picard--Lindelöf の局所存在・一意性](../ODE1/index.md#thm-ode1-picard-lindelof)から、多様体上でも積分曲線が局所的に一意に存在します。
...
次の GEO6 では、一本の滑らかなベクトル場ではなく $k$ 次元の接方向の族を考えます。そこで問題になるのは、
