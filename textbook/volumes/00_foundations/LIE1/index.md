...
この二つが滑らかに両立すると、単位元 $e$ の一つの接空間だけから群全体の不変ベクトル場を復元できます。さらに [GEO5 の Lie 括弧](../GEO5/index.md#def-geo5-lie-bracket)を単位元へ戻すと、$T_eG$ 自身に非可換性を記録する括弧が入ります。
...
> **この章の停止線**
>
> 本章では群構造と接空間の対応までを閉じます。1 パラメータ部分群・指数写像・Adjoint 表現は LIE2、Lie 部分群と古典群の体系的計算は LIE3、滑らかな群作用・等質空間・Maurer--Cartan 形式は LIE4 へ送ります。一般表現論、半単純 Lie 環、root system、最高ウェイト理論はさらに後続の独立系列へ送ります。
...
この例は多様体としては一次元ですが、加法群 $\mathbb R$ とは群演算が異なります。後で左不変ベクトル場を計算すると、この違いが座標係数に現れます。
...
この定理により、有限次元の接空間 $T_eG$ と、見かけ上は無限個の点に値を持つ大域ベクトル場の空間 $\mathfrak X_L(G)$ が同じ情報を持つと分かります。

群の平行移動が、局所データを群全体へ運んでいるからです。
...
従って $u\in T_0\mathbb R^n\cong\mathbb R^n$ に対応する左不変ベクトル場は定ベクトル場

$$
X^u_x=u.
$$

二つの定ベクトル場の係数は全て定数なので、[GEO5 の座標公式](../GEO5/index.md#thm-geo5-bracket-coordinate)から
...
LIE2 では1パラメータ部分群と指数写像を通じて、この無限小構造から群の局所構造を読みます。
...
<!-- formal-statement-start -->
> **命題（一般線形群 $GL(n,\mathbb R)$ は Lie 群）**
>
> 可逆な $n$ 次実正方行列全体
>
$$
...
<!-- formal-statement-start -->
> **定理（$\mathfrak{gl}(n,\mathbb R)$ の Lie 括弧は行列交換子）**
>
...
$GL(n,\mathbb R)$ は行列空間の開集合なので、これらを通常の $\mathbb R^{n^2}$ 値ベクトル場として微分できます。
...
群 $GL(2,\mathbb R)$ の非可換性が、単位行列近傍では行列交換子として現れています。
...
- 不変ベクトル場は単位元の接ベクトルだけで決まる。
- 群の非可換性が双線形な括弧へ圧縮される。
- LIE2 では1パラメータ部分群と指数写像がこの接空間を群へ戻す。
...
1. 任意の $A,B$ について
   ...
   $$
   を示せ。
2. $i\ne j$ に対する行列単位 $E_{ij}$ がある交換子として書けることを示せ。
3. 対角行列
   $$
   E_{ii}-E_{jj}
   $$
   も交換子として書けることを示せ。
4. 跡が0の任意の行列が有限個の交換子の線形結合として書けることを示せ。
5. 従って、全ての交換子で張られる部分空間が
   $$
...
よって $A=D+N$ も交換子の線形結合です。

5. 全ての交換子が張る部分空間を $C$ とします。
...
という1パラメータ部分群と、それを生む指数写像です。さらに共役作用を微分して Ad / ad を構成し、群の内部対称性と Lie 環の括弧を結びます。
