...
従って $\sum |S_{n+1}-S_n|$ は収束し、$S_n$ はある実数 $C$ に収束する。よって
...
Euler の極限公式の証明は $z$ のコンパクト集合上で一様だった。従って $0\le a\le1$ に対して一様に
...
実際、Euler の極限公式を $z=1+a$ に用いて関数等式で有限積を書き直せばこの比が得られる。
...
次章 [CA12](../CA12/index.md) では、Gamma 関数を Mellin 変換側の因子として使い、Riemann zeta 関数の解析接続と関数等式へ進む。
