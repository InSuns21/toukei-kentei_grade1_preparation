...
<a id="thm-mc1-strong-consistency"></a>
...
<a id="thm-mc1-standard-error-consistency"></a>
...
$$
\boxed{
\text{これは有限標本で厳密な区間ではなく、CLT に基づく漸近近似}
}
$$
...
という二乗平均平方根誤差も、有限分散 CLT も使えません。

「標本平均が収束する」と「通常の $N^{-1/2}$ 正規誤差評価が使える」は別の主張です。

### 11.3 標本が iid でない

疑似乱数列や Markov chain からの標本では、厳密な iid 条件をそのまま仮定できない場合があります。
...
Monte Carlo が数値 PDE や SDE と組み合わさると、期待値を取りたい対象そのものを厳密には計算できないことがあります。
...
- MC4：bias・variance・1標本コストを複数解像度でどう釣り合わせるか。
...
これは CLT に基づく近似区間であり、有限標本で厳密に95%被覆するという主張ではありません。
