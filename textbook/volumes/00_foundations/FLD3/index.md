...
分離的であることは、任意の元 $\alpha$ の代数的元の最小多項式が $x^{q^m}-x$ を割ることと、その形式微分が $-1$ で重根を持たないことから出ます。
...
次に $\alpha\in\mathbb F_{q^m}$ を任意に取ります。$\alpha$ の $\mathbb F_q$ 上の代数的元の最小多項式を $m_\alpha(x)$ とすると、
...
任意の $\alpha\in K$ の $E$ 上の代数的元の最小多項式は $x^{64}-x$ を割るため重根を持たず、$\alpha$ は $E$ 上分離的です。
