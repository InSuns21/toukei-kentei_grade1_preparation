...
$K^\times=\{a_1,\ldots,a_N\}$ とします。多項式除法を使うと、$a_1$ が根なので $x-a_1$ を因子として取り出せます。残りの $a_i$ は $a_i-a_1\ne0$ だから商の根のまま残ります。この操作を繰り返すと
...
一意性は逆向きです。位数 $p^n$ の任意の有限体 $K$ では、非零元 $a$ に対し [群の Lagrange の定理](../GRP2/index.md#thm-grp2-lagrange) から
...
$K=\{a_1,\ldots,a_{p^n}\}$ と並べます。各 $a_i$ は $f_n$ の根なので、上と同じ多項式除法の反復により
...
両辺はともにモニックで次数 $p^n$ だから

$$
f_n(x)=\prod_{a\in K}(x-a).
$$

従って $K$ の中ですでに完全分解しており、全ての根が $K$ を生成するので、$K$ は $f_n$ の $\mathbb F_p$ 上の分解体です。
...
しかし $\mathbb F_{q^m}$ の全要素が $x^{q^j}-x$ の根なら、多項式除法を根ごとに反復して

$$
\prod_{a\in\mathbb F_{q^m}}(x-a)
\mid
x^{q^j}-x
$$

を得ます。左辺の次数は $q^m$、右辺の次数は $q^j$ ですが $j<m$ なので

$$
q^m>q^j,
$$

となり矛盾です。従って $\Phi_q$ の位数は $m$ です。
...
を満たす元です。$\mathbb F_q$ の全要素が固定点です。一方、もし固定点が $q+1$ 個以上あれば、そのうち異なる $q+1$ 個を $a_1,\ldots,a_{q+1}$ として同じ多項式除法を行うことで
...
となり、次数 $q+1$ の多項式が次数 $q$ の多項式を割ることになって矛盾します。従って固定点はちょうど $\mathbb F_q$ です。
<!-- proof-end -->
...
次に $\alpha\in\mathbb F_{q^m}$ を任意に取ります。$\alpha$ に対する **代数的元の最小多項式** を、基礎体を $\mathbb F_q$ として $m_\alpha(x)$ と書くと、

$$
f(\alpha)=0
$$

なので [代数的元の最小多項式の整除性](../FLD1/index.md#thm-fld1-minimal-polynomial)から
...
$3\mid6$ と $2\mid6$ なので [有限体の部分体判定](#thm-fld3-subfield-criterion)から
...
が全ての $x\in K$ で成り立ちます。$\Phi$ が恒等写像でないことは、上で求めた固定体

$$
E=\mathbb F_8
$$

が $K=\mathbb F_{64}$ より真に小さいことからも分かります。
...
任意の $\alpha\in K$ に対する **代数的元の最小多項式** は、基礎体を $E$ とすれば $x^{64}-x$ を割るため重根を持たず、$\alpha$ は $E$ 上分離的です。
