...
です。これが一般 Stokes の定理です。微積分学の基本定理、Green の定理、Gauss--Ostrogradsky の発散定理、Kelvin--Stokes の定理は全てこの一式へ入ります。
...
一般 Stokes の定理では「領域の境界」を多様体自身の構造として扱います。符号計算を見通しよくするため、本章では標準半空間を
...
したがって $(\bar x)$ は $\partial M$ 上の滑らかなアトラスを定めます。$\square$
<!-- proof-end -->
...
$M$ を向き付けられた $n$ 次元多様体とし、$\omega$ を向き付けられた座標近傍 $(U,x)$ の中にコンパクトな台を持つ $n$ 形式とします。
...
> $$
>
> と定める。台が $U$ の中でコンパクトなので、被積分関数を $x(U)$ の外で0とした通常の多重積分として扱える。
<!-- formal-statement-end -->
...
よって [RA7 の多変数変数変換定理](../RA7/index.md) を適用すると
