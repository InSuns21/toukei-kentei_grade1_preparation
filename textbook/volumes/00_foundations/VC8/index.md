...
核 $|x-y|^{-1}$ は $y=x$ で発散しますが、三次元では $y=x$ を含む小球上でも積分は有限です。実際、$x$ のまわりの半径 $\varepsilon$ の球では
...
$f$ はコンパクトな台を持つので、固定した $x$ では十分大きな $r$ に対して $M_x(r)=0$ です。さらに $x$ を小さなコンパクト近傍 $K$ の中で動かしても、ある共通の $R$ を取れば $r\ge R$ で $f(x+r\omega)=0$ です。
...
と書けます。$f\in C_c^2$ なので、一階・二階の各偏微分は有界であり、有限区間 $[0,R]$ 上では積分内で $x$ 微分を行えます。よって $u\in C^2(K)$ です。$K$ は任意だったので
...
と置きます。$b$ は $r=1$ で 3 階微分まで 0 につながるので、$F$ はコンパクトな台を持つ $C^3$ 級ベクトル場です。

まず $R(y)\cdot y=0$、$\operatorname{div}R=0$ であり、$\nabla b$ は $y$ と平行です。したがって
...
\rho(y)
=
\operatorname{div}F(y)
=
(1-r^2)^3(3-11r^2).
$$

また $by$ は勾配場なので回転は 0 です。従って
...
$A_1(0),A_2(0)$ は被積分関数の奇性から 0 です。$y=r\theta$、$\theta\in S^2$ と書くと、単位球面上の方向平均は

$$
\frac1{4\pi}
\int_{S^2}
r^2(\theta_1^2+\theta_2^2)\,d\theta
=
\frac23r^2
$$
...
同じコンパクト台のベクトル場から得た源 $\rho$ と渦 $\omega$ を、同じ Newton 核で広げて二つのポテンシャルを作っていることが確認できます。
<!-- definition-example-end -->
...
です。したがって $y=x$ の近くを含めてもこの積分は有限です。
<!-- solution-end -->
...
1. [Helmholtz 分解](#thm-vc8-helmholtz)からスカラーポテンシャルが 0 になることを示せ。
2.
   $$
