...
第1条件は「$Y$ は現在の情報 $\mathcal G$ だけで決まる」、第3条件は「$\mathcal G$ で見えるどの領域でも平均量を保存する」という意味です。条件付き期待値は点ごとの値ではなく、後で示すように **a.s. の意味で一意な同値類**として決まります。
...
左辺は $|Z_nM|\le\|Z\|_\infty|M|$、右辺は $|Z_nX|\le\|Z\|_\infty|X|$ で支配され、$M,X\in L^1$ です。[優収束定理](../F0_00D2B_単調収束_Fatou_優収束/index.md#thm-dominated-convergence)から
...
<!-- solution-start -->
#### 詳細解答
1. 自明な $\sigma$ 代数に可測な実数値関数はa.s.定数である。定数 $c$ が条件付き期待値なら $A=\Omega$ の積分一致から $c=E[X]$ である。実際、定数 $E[X]$ は定義3条件を満たす。
