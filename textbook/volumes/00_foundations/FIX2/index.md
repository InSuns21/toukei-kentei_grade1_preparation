...
- **上半連続性**：近くへ動いても、遠い新しい値が突然現れない。
- **下半連続性**：いま存在する値が、近くへ動いた瞬間に突然消えない。
...
> を対応とする。
>
> $x\in X$ において $\Gamma$ が **下半連続**であるとは、$\Gamma(x)$ と交わる任意の開集合 $V\subset Y$ に対し、ある $\delta>0$ が存在して
>
$$
d_X(x',x)<\delta
\quad\Longrightarrow\quad
...
> となることをいう。
>
> すべての $x\in X$ で下半連続なら、$\Gamma$ は下半連続であるという。
<!-- formal-statement-end -->
...
したがって、下半連続性が防ぐのは **既存の値の突然消失**です。

<!-- definition-example-start: def-fix2-lower-hemicontinuity -->
**定義の確認**：先ほどの $B$ は同点で下半連続ではない
...
## 8. 下半連続性は「値を追跡できる」と言い換えられる

下半連続性には、距離空間で使いやすい点列判定があります。
...
下半連続なら、$y$ の半径 $1/m$ の球は近くの値集合と必ず交わります。
...
逆に下半連続でなければ、ある開集合 $V$ に対し $x_n\to x$ なのに

$$
\Gamma(x_n)\cap V=\varnothing
$$

となる列を作れます。ところが $y\in\Gamma(x)\cap V$ を追跡できるなら $y_n\to y$ なので、最終的に $y_n\in V$ となって矛盾します。

<!-- proof-start -->
### 証明

まず $\Gamma$ が $x$ で下半連続とします。
...
逆に 2 を仮定し、$\Gamma$ が $x$ で下半連続でないとします。
...
従って $\Gamma$ は $x$ で下半連続です。
<!-- proof-end -->

この命題は「下半連続」を非常に具体的にします。

> 基準点のどの値 $y$ も、近くの入力に対して選び直した値 $y_n$ で追跡できる。

---

## 9. 上半連続と下半連続は互いに代用できない
...
同点 $q=1/2$ では値集合が広がります。近くへ動いても値はその大きな集合 $[0,1]$ から外へ出ないので上半連続ですが、内部の値 $1/2$ は少し動くと消えるので下半連続ではありません。
...
一方、すでに確認した通り $q=1/2$ では下半連続ではありません。
...
は $p\ne1$ へ少し動かすと消えます。従って $p=1$ では下半連続ではありません。
...
$x=0$ において、それぞれが上半連続か、下半連続かを判定せよ。
...
どれほど $x'>0$ を0へ近づけてもこの状況は変わりません。従って $\Gamma_1$ は0で下半連続ではありません。
...
$$
\boxed{
\Gamma_1:\ \text{上半連続だが下半連続でない},
}
$$

$$
\boxed{
\Gamma_2:\ \text{下半連続だが上半連続でない}.
}
$$
<!-- solution-end -->
...
2. $\Gamma_f$ が $x$ で下半連続なら $f$ は $x$ で連続である。
...
5. 任意の $x_n\to x$ と任意の $y\in\Gamma(x)$ に対して
   ...
   $$
   を満たすことを示せ。
6. 下半連続性の点列判定を使って、$\Gamma$ が下半連続であることを示せ。
...
閉区間なので閉かつ有界であり、$\mathbb R$ の Heine--Borel によりコンパクトです。
...
Heine--Borel により、これらは有界です。従ってある $M>0$ が存在して、すべての $x\in[0,1]$ について
...
従って [下半連続性の点列判定](#prop-fix2-lhc-sequential)の条件を満たし、
...
したがって、連続な二つの端点に挟まれた区間対応は、上半連続かつ下半連続です。
<!-- solution-end -->
