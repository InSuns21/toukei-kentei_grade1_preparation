...
とします。もちろん $x_n\to0$ ですが、Cauchy列の見方では極限0を使わず、十分大きい $m,n$ に対して
...
を考えます。この列の後半同士は好きなだけ近くなるので、$\mathbb Q$ の中でもCauchy列です。
...
完備性は、**Cauchy列が指し示す極限に対して空間に穴がない**ことだと捉えられます。
...
### 1.1 直感：収束は「点との距離」、Cauchyは「列内部の距離」
...
[前節の「収束列はCauchy列」](#thm-convergent-implies-cauchy)より $(q_n)$ はCauchy列です。しかし $\sqrt2\notin\mathbb Q$ なので、$\mathbb Q$ の中にはこの列の極限がありません。
...
各 $x_n\in F$ であり、$F$ は閉集合なので、F0-00Bの[閉集合の点列特徴付け](../F0_00B_距離空間_開集合_閉集合_収束/index.md#thm-f0-00b-01)から
...
したがって $F$ は $F$ 内の収束列の極限を全て含み、[閉集合の点列特徴付け](../F0_00B_距離空間_開集合_閉集合_収束/index.md#thm-f0-00b-01)より閉集合です。$\square$
<!-- proof-end -->
...
しかし $\sqrt2\notin\mathbb Q$ です。もし $q_n$ が $\mathbb Q$ 内の $q$ に収束すれば、$\mathbb R$ でも $q_n\to q$ となり、[極限の一意性](../F0_00B_距離空間_開集合_閉集合_収束/index.md#prop-f0-00b-01)から $q=\sqrt2$ となって矛盾します。
...
一方 $\sqrt2\notin\mathbb Q$ で、[極限の一意性](../F0_00B_距離空間_開集合_閉集合_収束/index.md#prop-f0-00b-01)より $\mathbb Q$ 内の別の極限も存在しない。よって $\mathbb Q$ は非完備。
...
$[0,1]$ は閉かつ有界なので[Heine–Borel](../F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel/index.md#thm-f0-00c1-02)よりコンパクトです。コンパクト距離空間は完備なので完備でもあります。
...
$\mathbb R$ は完備ですが有界でないので[Heine–Borel](../F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel/index.md#thm-f0-00c1-02)よりコンパクトではありません。
...
[完備空間の閉部分集合は完備](#thm-f0-00d-01)なので
...
は閉集合。よって[完備空間の閉部分集合の定理](#thm-f0-00d-01)から
