...
<a id="def-f0-02c2-continuous-linear-functional"></a>

<!-- formal-statement-start -->
> **定義（連続線形汎関数）**  
> ノルム空間 $X$ 上の線形汎関数 $\ell:X\to\mathbb R$ が、ノルムから定まる位相について連続であるとき、$\ell$ を **連続線形汎関数** といいます。すなわち、$x_n\to x$ なら
...
<!-- definition-example-start: def-f0-02c2-continuous-linear-functional -->
**定義の確認**：有限次元の係数ベクトルは連続線形汎関数を作る
...
<a id="thm-f0-02c2-continuity-boundedness"></a>

<!-- formal-statement-start -->
> **定理（線形汎関数の連続性と有界性の同値）**  
> ノルム空間 $X$ 上の線形汎関数 $\ell:X\to\mathbb R$ について、次は同値です。
>
> 1. $\ell$ は連続である。
> 2. $\ell$ は $0$ で連続である。
> 3. ある $M<\infty$ が存在して、任意の $x\in X$ に対して
...
条件3を満たすことを、この章では線形汎関数が **有界** であるといいます。入力のノルムが小さいのに出力だけが無制限に大きくなることを禁止する条件です。

1から2は連続性の定義から直ちに従います。残る二方向を実際に示します。
...
したがって $x\to y$ なら $\ell(x)\to\ell(y)$ です。実際、$\ell$ はLipschitz連続です。

---

## 5. 0で連続なら有界

逆に、線形汎関数 $\ell$ が0で連続だとします。
...
$x=0$ でも $\ell(0)=0$ なので、$M=2/\delta$ とすればすべての $x$ に対して
...
<a id="prop-f0-02c2-dual-vector-space"></a>

<!-- formal-statement-start -->
> **命題（双対空間のベクトル空間構造）**  
> $\ell,m\in X^*$、$a,b\in\mathbb R$ に対して

$$
(a\ell+bm)(x):=a\ell(x)+bm(x)
$$

> と点ごとに演算を定めると、$a\ell+bm\in X^*$ です。従って $X^*$ はこの演算について実ベクトル空間になります。
<!-- formal-statement-end -->
...
さらに $\ell,m$ は連続線形汎関数です。[線形汎関数の連続性と有界性の同値](#thm-f0-02c2-continuity-boundedness)から、ある有限な $M,N$ が存在して
...
従って再び同じ定理から $a\ell+bm$ は連続です。

零汎関数 $0(x)=0$ も $X^*$ に入り、$\ell$ の加法逆元は $-\ell$ です。残りのベクトル空間の公理は実数の演算から点ごとに従います。
<!-- proof-end -->
...
> と定めます。これを $\ell$ の **双対ノルム** といいます。
<!-- formal-statement-end -->

$\ell$ は連続なので、[連続性と有界性の同値](#thm-f0-02c2-continuity-boundedness)から、ある $M<\infty$ が存在して $|\ell(x)|\le M\|x\|$ です。特に $\|x\|\le1$ なら $|\ell(x)|\le M$ なので、上のsupremumは有限です。

<!-- definition-example-start: def-f0-02c2-dual-norm -->
**定義の確認**：座標を読む汎関数の双対ノルム

$X=\mathbb R^2$ にEuclidノルムを入れ、$\ell(x_1,x_2)=x_1$ とします。$\|x\|_2\le1$ なら

$$
|\ell(x)|=|x_1|\le\|x\|_2\le1
$$

なので $\|\ell\|_{X^*}\le1$ です。一方 $x=(1,0)$ では $\|x\|_2=1$ かつ $|\ell(x)|=1$ なので
...
<a id="prop-f0-02c2-dual-norm-is-norm"></a>

<!-- formal-statement-start -->
> **命題（双対ノルムはノルムである）**  
> 上で定めた $\|\cdot\|_{X^*}$ は、ベクトル空間 $X^*$ 上のノルムです。
<!-- formal-statement-end -->

<!-- proof-start -->
- $\|\ell\|_{X^*}\ge0$ は絶対値から従います。
...
<a id="prop-f0-02c2-dual-norm-basic-estimate"></a>

<!-- formal-statement-start -->
> **命題（双対ノルムの基本評価）**  
> 任意の $\ell\in X^*$ と $x\in X$ に対して
...
> が成り立ちます。さらに $\|\ell\|_{X^*}$ は、すべての $x\in X$ に対して $|\ell(x)|\le M\|x\|$ を満たす定数 $M$ のうち最小のものです。
<!-- formal-statement-end -->
...
をすべての $x$ で満たすなら、$\|x\|\le1$ 上で $|\ell(x)|\le M$ なので、supremumの定義から $\|\ell\|_{X^*}\le M$ です。従って $\|\ell\|_{X^*}$ が最小の定数です。
<!-- proof-end -->
...
$a\ne0$ なら $x=a/\|a\|_2$ を選べば等号を達成します。$a=0$ の場合も両辺0なので
...
なので、[連続性と有界性の同値](#thm-f0-02c2-continuity-boundedness)から連続です。
...
この評価から、$\|f\|_\infty\le1$ なら $|\ell(f)|\le1$ なので $\|\ell\|_{C([0,1])^*}\le1$ です。一方、定数関数 $f\equiv1$ は $\|f\|_\infty=1$ かつ $\ell(f)=1$ なので、この上界を実際に達成します。従って
...
ここまでの定義を同じ関数空間でまとめて確認します。$X=C([0,1])$ にsupノルムを入れ、
...
も $X^*$ の元です。これは[双対空間のベクトル空間構造](#prop-f0-02c2-dual-vector-space)の具体例にもなっています。
<!-- definition-example-end -->
...
## 11. Riesz表現定理：Hilbert空間の双対をベクトルで表す

Hilbert空間 $H$ では、驚くほど有限次元に近い状況が戻ってきます。

<!-- formal-statement-start -->
> **定理（Riesz表現定理）**  
> $H$ を実Hilbert空間とします。任意の $\ell\in H^*$ に対して、一意な $g\in H$ が存在し、任意の $x\in H$ に対して

$$
\boxed{
\ell(x)=\langle g,x\rangle_H
}
$$
...
ところがHilbert空間なら[Riesz表現定理](#ref-riesz-representation)により
...
従って $T$ は有界で、[線形汎関数の連続性と有界性の同値](#thm-f0-02c2-continuity-boundedness)から連続なので $T\in X^*$ です。$\|x\|\le1$ として上式のsupremumを取れば、求めるノルム不等式も得られます。
...
と置きます。これは $1/2$ を頂点とする幅 $2/n$ の三角形で、その台は $[0,1]$ の中に収まり、
...
#### 本番答案
$L^2$ はa.e.同値類なので一点の値は代表元で変わり、点評価はwell-definedでない。さらに $n\ge2$ として
...
2. $H=L^2([0,1])$ としたとき、[Riesz表現定理](#ref-riesz-representation)における代表元 $g$ を求め、$\|\ell\|_{H^*}$ を求めよ。
...
です。[Riesz表現定理](#ref-riesz-representation)より
...
- 連続線形汎関数を定義し、線形汎関数の連続性と有界性が同値であることを証明できる。
- $X^*$ が点ごとの和・スカラー倍でベクトル空間になる理由を示せる。
- 双対ノルムを定義し、それがノルム公理を満たすことを説明できる。
- $x/\|x\|$ への正規化から双対ノルムの基本評価を導き、最小定数として解釈できる。
