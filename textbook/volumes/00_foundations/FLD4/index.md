...
> **この章の停止線**
>
> 本章では有限 Galois 理論の基本定理と、正規部分群・商群までを閉じます。定規とコンパスによる作図、可解群、根号による可解性、一般五次方程式への応用は FLD5 へ送ります。無限 Galois 理論は扱いません。
...
FLD2 では、[分離拡大](../FLD2/index.md#def-fld2-distinct-conjugates)と[正規拡大](../FLD2/index.md#def-fld2-conjugate-root-closure)を別々に定義しました。
...
- 分離性は、共役根の候補が重ならず、体の埋め込みを次数の個数だけ作れることを保証する。
- 体拡大の正規性は、それらの体の埋め込みの像が拡大体 $L$ の外へ逃げないことを保証する。
...
$E_{i-1}$ までの体の埋め込みを一つ固定すると、$\alpha_i$ の像はその代数的元の最小多項式の根から選びます。
...
個すべて異なるので、一段進むたびに体の埋め込みの個数がこの次数倍されます。
...
$\alpha_i$ の $E_{i-1}$ 上の代数的元の最小多項式を
...
従って $E_{i-1}$ の各体の埋め込みは、ちょうど $d_i$ 個の $E_i$ の埋め込みへ延長されます。

$E_0=F$ には恒等な体の埋め込みが一つだけなので、反復すると $L=E_r$ の $F$-埋め込みの個数は
...
体拡大の正規性があると、FLD2 の [有限正規拡大と分解体](../FLD2/index.md#thm-fld2-finite-splitting-equivalence) により $L$ はある $F$ 係数多項式の分解体です。
...
任意の $F$-埋め込み $\sigma:L\to\Omega$ に対し、分解体が基礎体固定の体の埋め込みで保たれることから
...
逆に $F$ を固定する自己同型は当然 $F$ を固定する体の埋め込みです。
...
$\alpha$ の $K$ 上の代数的元の最小多項式を $m_{\alpha,K}$ とすると、代数的元の最小多項式の整除性から
...
任意の $\alpha\in L$ の $F$ 上の代数的元の最小多項式
...
これに体の埋め込み延長を組み合わせると、$E$ の全ての $F$-共役根が $E$ の中へ戻るため、$E/F$ は正規になります。
...
$p$ は $\alpha$ の代数的元の最小多項式なので、[一段の埋め込み延長](../FLD2/index.md#lem-fld2-one-step-field-map) により
...
さらに $L/E$ も有限代数拡大なので、もう一度体の埋め込み延長を使って
...
$x\in E$ の $F$ 上の代数的元の最小多項式を $m_{x,F}$ とすると、
...
では、$\alpha$ の像の候補は代数的元の最小多項式の異なる根です。
...
4. 「同じ位数 $6$ の Galois 群でも、群が $S_3$ か $C_6$ かで中間拡大が正規かどうかが違う」ことを、正規部分群の観点から説明せよ。
...
4. 有限分離拡大 $L/F$ では、代数閉包への $F$ を固定する体の埋め込みがちょうど $[L:F]$ 個存在する。
5. 体拡大の正規性を加えると全ての体の埋め込みが $L$ 自身へ戻るので、有限 Galois 拡大では
   $$
