...
> **この章の停止線**
>
> 本章では有限 Galois 理論の基本定理と、正規部分群・商群までを閉じます。定規とコンパスによる作図、可解群、根号による可解性、一般五次方程式への応用は FLD5 へ送ります。無限 Galois 理論と Krull 位相は扱いません。
...
まず Artin の独立性から、ある $\alpha_1,\dots,\alpha_n$ を選んで
...
逆向きは、$H$ の各自己同型が $K$-線形写像であり、Artin の独立性から $L$ 上一次独立であることを使います。
...
ここでは式番号を参照記号として使わず、直後の議論だけで用います。
...
この定理は Galois 対応の「正規性まで含めた完全版」です。
...
1. $16=2^4$ なので、FLD3 の相対 Frobenius の定理から
...
$x^3-2$ は Eisenstein の既約判定から $\mathbb Q$ 上既約なので
...
5. 正規性を加えると全ての埋め込みが $L$ 自身へ戻るので、有限 Galois 拡大では
   ...
   $$
   となる。
6. Artin の独立性から、有限自己同型群 $H$ に対して
   $$
   [L:L^H]=|H|
   $$
   が従い、さらに $\operatorname{Gal}(L/L^H)=H$ となる。
7. 有限 Galois 理論の基本定理により、中間体 $E$ と部分群 $\operatorname{Gal}(L/E)$ は包含反転で一対一対応する。
8. 対応する $E$ と $H$ について
   $$
   [L:E]=|H|,
   \qquad
