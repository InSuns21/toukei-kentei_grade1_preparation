...
**用語の確認**：異なる二点を、それぞれを含む互いに交わらない開集合で分離できる位相空間を **Hausdorff空間** という。

$X=\{0,1\}$ に
$$
\tau=\{\varnothing,\{1\},X\}
$$
を入れる。定数列 $x_n=1$ が1だけでなく0にも収束することを示せ。さらに、0と1を互いに交わらない開集合で分離できないことを確認し、この例で点列の極限が一意でなくなることを説明せよ。
...
さらに0と1を互いに素な開近傍で分離できない。0を含む唯一の非空開集合 $X$ は1も含むからである。従ってこの空間はHausdorffではない。

Hausdorff空間では点列の極限は一意だが、この例ではその仮定が満たされていない。そして実際に、同じ定数列 $x_n=1$ が0と1の二つへ収束しており、極限の一意性が失われることを直接確認できる。
