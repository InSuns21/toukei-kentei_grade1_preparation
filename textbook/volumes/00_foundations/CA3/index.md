...
ここから積分の相殺を有限問題へ落とします。$\Omega'=U\setminus\{z\}$ と置きます。まず [CA2 の曲線の多角形化](../CA2/index.md#lem-ca2-polygonal-replacement) を $G$ と二つの円周 $C_R,C_\rho$ にそれぞれ適用します。すると閉多角形曲線 $P_R,P_\rho\subset\Omega'$ が取れて

$$
\int_{C_R}G\,d\zeta=\int_{P_R}G\,d\zeta,
\qquad
\int_{C_\rho}G\,d\zeta=\int_{P_\rho}G\,d\zeta
$$

となり、各円周と対応する多角形曲線は基点を固定したホモトピーで結ばれます。

$P_R$ から $C_R$ へのホモトピー、上で作った $H$、$C_\rho$ から $P_\rho$ へのホモトピーを $s$ 方向につなぎます。こうして連続写像

$$
K:[0,1]\times[0,2\pi]\to\Omega'
$$
...
を満たすものが得られます。最後の等式は、途中の各曲線が閉曲線であることを表します。さらに $t=0$ と $t=2\pi$ の二辺でたどる共通の道は区分的線形にできます。実際、多角形化の前後では基点が固定され、中央の $H$ では
...
$K$ の像は $\Omega'$ 内のコンパクト集合です。従って像と閉集合 $\mathbb C\setminus\Omega'$ の距離は正です。また $K$ はコンパクトな長方形上で一様連続です。そこで [CA2 のホモトピーの相対区分線形化](../CA2/index.md#lem-ca2-relative-pl-homotopy) の証明と同じように、境界上の $P_R,P_\rho$ と二本の共通側辺の折れ点をすべて含む十分細かい格子を取り、各小長方形を二つの三角形へ分けます。格子頂点では $K$ の値を取り、各小三角形内では三頂点の値をアフィンに補間して $\widetilde K$ を作ります。格子を十分細かくしたので $\widetilde K$ は $\Omega'$ から出ません。境界の各小辺では元の境界曲線がすでに線形なので、$\widetilde K$ は境界上で $K$ と一致します。

各パラメータ三角形の像は $\Omega'$ 内の通常の三角形または退化三角形です。非退化なら [三角形版 Cauchy–Goursat](../CA2/index.md#thm-ca2-goursat-triangle) により $G$ の境界積分は0で、退化した場合も辺の往復が相殺して0です。全三角形の境界積分を足すと内部辺は逆向きに二回現れて消えます。$t=0$ と $t=2\pi$ の二辺は同じ区分的線形曲線を互いに逆向きにたどるので、ここも相殺します。残るのは $P_R$ と $P_\rho$ だけで、長方形境界の向きから

$$
\int_{P_R}G\,d\zeta-
\int_{P_\rho}G\,d\zeta=0.
$$

最初の多角形化で積分値が変わらないことを戻せば
...
これは「小円を除いた領域の正向き境界では、外周が正向き、内周が負向きになる」ことを、円周を多角形化してから有限三角形の相殺へ落として証明したものです。$\square$
<!-- proof-end -->
...
この節で証明するのは、一変数の非定数正則関数について、開集合の像が再び開集合になるという性質です。証明には本章で得た零点の孤立性と最大値原理だけを使います。

<a id="thm-ca3-open-mapping"></a>
<!-- formal-statement-start -->
### 定理（正則関数は開集合を開集合へ写す）

$\Omega$ を領域、$f:\Omega\to\mathbb C$ を非定数正則関数とする。このとき任意の開集合 $V\subset\Omega$ に対して $f(V)$ は $\mathbb C$ で開である。
<!-- formal-statement-end -->
...
なので零点がありません。さらに境界はコンパクトでこの下界は一様なので、境界の外側を含む十分小さい近傍まで分母の非消滅が保たれます。従って閉円板全体を含む開近傍で $h$ は正則です。
