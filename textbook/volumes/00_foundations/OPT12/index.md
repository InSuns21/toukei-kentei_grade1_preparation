...
を一つの凸錐として扱います。これが二次錐であり、線形計画を自然に拡張する二次錐計画の入口になります。
...
よって $Q$ は全空間では正定値でなくても、$\ker A$ 上では正定値です。本文の「零空間上の正定値性と KKT 行列の正則性」から KKT 行列は正則です。
...
4. 二つの制約を
   ...
   1-w-b\le0
   $$
   と書き、乗数 $\alpha_1,\alpha_2\ge0$ を付けて KKT 条件を書け。
5. KKT 条件から $\alpha_1^*,\alpha_2^*$ を求めよ。
