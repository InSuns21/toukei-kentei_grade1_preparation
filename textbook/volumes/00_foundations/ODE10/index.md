...
$q$ から出る前向き軌道は不変性により $\Omega$ 内に留まります。$\Omega$ はコンパクトなので、その軌道の正の極限集合も非空です。そこから一点 $p$ を取ります。

$p\in\Omega$ かつ $F(p)\ne0$ なので、[局所横断線と正の極限集合の交点](#lem-ode10-transversal-omega)を $p$ に適用し、短い横断線 $\Sigma$ を取ると

$$
\Omega\cap\Sigma=\{p\}.
$$

一方 $p$ は $q$ の軌道の正の極限点です。従って $q$ の軌道は $p$ の近くへ無限回戻り、上の横断長方形を通るたびに $\Sigma$ と交わります。その交点は全て $q$ の軌道上にあり、不変性から $\Omega$ に属します。ところが $\Omega\cap\Sigma=\{p\}$ なので、各交点は全て同じ点 $p$ です。
...
つまり $p$ を通る軌道は周期軌道です。$q$ 自身も同じ軌道上にあるので、$q$ は周期軌道上にあります。
