# NA7 数値解析 VII：ODE 数値解法 II—Runge–Kutta 法・絶対安定性
...
- **収束論**：$h\to0$ で正しい解へ近づくか
- **絶対安定性**：ある $h$ で減衰モードを減衰として再現するか

は別の問いです。

両方必要です。高次でも安定領域の外で使えば発散し得ますし、安定でも低次なら精度は上がりません。
...
## 3. 古典4段4次 Runge–Kutta 法

最も有名な陽的 Runge–Kutta 法は
...
であり、一段関数が必要な一様 Lipschitz 条件を満たす有限時間区間では大域誤差は
...
これは「陽的方法の係数をもっと賢く選べば stiff 問題の刻み幅制約を完全に消せる」という期待には限界があることを示します。
...
したがって台形法は「発散しない」という A 安定性は持つものの、この極端に速いモードを一歩で消す能力は弱いことが分かります。

これが L 安定性を別に考える理由です。

---

## 10. theta 法で三つの方法を統一する
