...
VC1 では勾配・発散・回転を Cartesian 座標の成分で定義しました。本章では、その成分計算を **添字記法**で組み直し、基底を回しても意味が変わらない量と、基底に応じて成分だけが変わる量を区別します。

主役は三次元 Euclidean 空間の **デカルトテンソル（Cartesian tensor）** です。一般の多様体上のテンソル、共変・反変成分、Christoffel 記号、共変微分は扱いません。直交 Cartesian 基底の間の変換に限定することで、連続体力学・流体・電磁気で使う計算言語を、線形代数とベクトル解析だけから構成します。
...
<!-- formal-statement-start -->
> **定義（二階デカルトテンソル）**  
> 三次元 Euclidean 空間上の線形写像 $T:\mathbb R^3\to\mathbb R^3$ を考える。正規直交 Cartesian 基底での成分を $T_{ij}$ とし、
>
> $$
> T(v)_i=T_{ij}v_j
> $$
...
VC6 で見たように、曲線座標で物理成分へこの式をそのまま機械適用してはいけません。本章の添字計算は **一定の Cartesian 基底**を前提にしています。
...
**VC7 は「Euclidean 空間で一定の直交 Cartesian 基底を回す」範囲だけを完成させます。**
