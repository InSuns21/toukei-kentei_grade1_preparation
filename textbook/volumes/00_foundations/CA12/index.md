...
eta 関数は解析接続の「入口」を見せるが、本章の主証明には使わない。$1-2^{1-s}$ 自身が零点を持つため、これだけで全平面への解析接続を一気に作るより、後で示す対称関係を使う方が関数等式まで同時に得られる。
...
## 8. 負の偶数に現れる零点と臨界帯（critical strip）
...
という臨界帯に入る。しかし $\Re s=1$ の零点不存在の証明は素数定理へ接続する深い段階なので、本章では黒箱として使わない。
...
「$1+2+3+\cdots=-1/12$」という表現を通常の級数の和として読んではいけない。$\zeta(-1)=-1/12$ は、$\Re s>1$ で定義した関数を解析接続した値であり、通常には収束しない級数へ通常の意味で和を与えたものではない。
...
$s$ をコンパクト集合 $K$ に制限すると二つの冪は $t^A,t^B$ で一様に抑えられる。従って 被積分関数 は

$$
C_K e^{-\pi t}(t^A+t^B)
$$

で抑えられ、これは $[1,\infty)$ 上可積分である。各 $t$ で 被積分関数 は $s$ の整関数なので、局所一様収束する積分として $H$ は整関数になる。
