...
- 縮小不動点反復：連続する反復値の差
- 一般の方程式：残差 $|f(x_n)|$
- Newton 法：Newton 更新量 $|x_{n+1}-x_n|$

を使います。ただし残差や更新量が小さいだけで根誤差が小さいとは限らないため、後で条件を明示します。

---

## 1. まず根を「囲う」：符号変化から存在を保証する
...
一方、零点が偶数回重なっている場合は符号が変わらないことがあります。例えば
...
<!-- definition-example-start: def-na2-bisection -->
**定義の確認**。$f(x)=x^3+x-1$ を $[0,1]$ から二分します。最初の中点は $m_0=1/2$ で、
...
一方 $f(1)=1>0$ です。したがって最初の更新では符号変化を保つ
...
1. 各段階で符号変化を保つので、[中間値定理](../RA2/index.md#thm-ra2-ivt) により各区間は少なくとも一つ零点を含む。
...
<!-- definition-example-start: def-na2-shrink-map -->
**定義の確認**。$I=[1,2]$ 上で
...
<a id="prop-na2-derivative-shrink"></a>
<!-- formal-statement-start -->
### 命題（導関数による縮小性判定）
...
この差を足し上げると幾何級数が現れ、反復列が Cauchy だと分かります。閉区間は完備なので極限が区間内に存在し、縮小評価から従う連続性により極限が不動点になります。
...
縮小評価から、$x_n\to\alpha$ なら $|g(x_n)-g(\alpha)|\le q|x_n-\alpha|\to0$ です。したがって
...
Newton 法は単根の近くで二次収束します。ここが速度差の核心です。
...
Newton 法の二次収束は **単根**の近くでの性質です。重根では一般に収束が遅くなります。
...
従って Newton 法は単根の十分近くでは少なくとも二次の速さで収束する。
<!-- formal-statement-end -->
...
3. Newton 写像を
   ...
   $$
   を示せ。
4. $g([1,2])\subset[1,2]$ を示し、[閉区間上の縮小不動点反復](#thm-na2-shrink-fixed-point)から $x_0\in[1,2]$ なら Newton 反復が $\alpha$ へ収束することを示せ。
...
したがって、初期区間を19回二分した後の区間 $[a_{19},b_{19}]$ の中点 $m_{19}$ を採用すれば
...
[平均値定理](../RA3/index.md#thm-ra3-mvt) により任意の $x,y\in[1,2]$ に対して
...
これは局所二次収束定理と矛盾しません。定理は単根 $\alpha$ の **十分近く**に初期値を取ることを要求しています。
...
を示したので、[平均値定理](../RA3/index.md#thm-ra3-mvt) から $g$ は縮小率 $q=2/3$ の縮小写像です。

従って [閉区間上の縮小不動点反復](#thm-na2-shrink-fixed-point) により、任意の
...
従って $\alpha$ は単根で、$f$ は $C^2$ 級です。本文の局所二次収束定理を適用できます。

より直接には、[Taylor の定理](../RA3/index.md#thm-ra3-taylor) から Newton 誤差は、$x_n$ と $\alpha$ の間のある $\xi_n$ を用いて
...
一方 Newton 法は、大域的には周期反復や巨大な更新量 の可能性がありますが、単根の近くへ入れば誤差が二乗される二次収束を持ちます。
...
- 連続関数の符号変化と [中間値定理](../RA2/index.md#thm-ra2-ivt) から根の存在を保証した。
...
- 単根の近くで
  ...
  C|e_n|^2
  $$
  となる局所二次収束を [Taylor の定理](../RA3/index.md#thm-ra3-taylor) から証明した。
