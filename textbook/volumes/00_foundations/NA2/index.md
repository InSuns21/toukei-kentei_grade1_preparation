...
Newton 法の二次収束は **単純根**の近くでの性質です。多重根では一般に収束が遅くなります。
...
です。多重根では $f'(\alpha)=0$ なので、局所二次収束定理の証明で必要だった
...
多重根では $f'(\alpha)=0$ なので、同じ形の一様な下界を作れません。
...
- 多重根では $f'(\alpha)=0$ のため二次収束が失われ得ることを確認した。
