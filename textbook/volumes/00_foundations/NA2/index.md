...
Newton 法の二次収束は **単純根**の近くでの性質です。重複根では一般に収束が遅くなります。
...
です。重複根では $f'(\alpha)=0$ なので、局所二次収束定理の証明で必要だった
...
重複根では $f'(\alpha)=0$ なので、同じ形の一様な下界を作れません。
...
一方 Newton 法は、大域的には周期反復や巨大な更新量 の可能性がありますが、単純根の近くへ入れば誤差が二乗される二次収束を持ちます。
...
- 重複根では $f'(\alpha)=0$ のため二次収束が失われ得ることを確認した。
