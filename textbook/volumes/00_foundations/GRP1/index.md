...
本章では、まず公理から基本法則を導き、次に「大きな構造の中の小さな構造」「一つの元から生成される構造」「全単射の合成による具体例」「左移動による置換実現」という順に進みます。

> **この章の停止線**
>
> 積を保つ写像の一般論、核・像、剰余類、Lagrange の定理、正規部分群、商構造、同型定理は GRP2 で扱います。本章では左移動が積を保存する単射になるところまでを直接確かめ、一般理論へは進みません。
