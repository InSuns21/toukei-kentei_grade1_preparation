...
> **この章の停止線**
>
> 積を保つ写像の一般論、核・像、剰余類、有限群の位数に関する一般公式、商構造、同型定理は GRP2 で扱います。本章では左移動が積を保存する単射になるところまでを直接確かめ、一般理論へは進みません。
...
この定理は有限群一般に対する部分群の位数公式を使っていません。GRP2 では、その一般原理を剰余類の分割から証明し、「有限群の部分群の大きさは群の大きさを割る」へ拡張します。
...
次の GRP2 では、積を保つ写像の一般理論へ進みます。その後、核・像、剰余類による群の分割、有限群の部分群位数公式、共役で保たれる部分群、剰余類に演算を入れる商構造、同型定理へ進みます。
