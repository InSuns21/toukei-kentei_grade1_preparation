# GRP1 抽象代数 I：群・部分群・巡回群・置換群

<!-- definition-example-audit: strict -->

[F0-00A](../F0_00A_集合_写像_上限下限/index.md) では、集合、直積集合、写像、全単射、合成写像を扱いました。本章では、集合の上に一つの二項演算を置き、その演算が「元を掛け合わせ、元に戻り、逆向きにも戻れる」ための最小公理を抽出します。
...
> **この章の停止線**
>
> 群準同型、核・像、剰余類、Lagrange の定理、正規部分群、商群、同型定理は GRP2 で扱います。本章で Cayley の定理を証明するときには「積を保存する単射」という式は使いますが、それを一般化した群準同型の理論へは進みません。
...
左消去について $ab=ac$ とします。丢辺の左から $a^{-1}$ を掛けると
...
を満たす組 $(i,j)$ を inversion と呼び、その個数を $N(\sigma)$ とします。
...
隣り合う二点を交換する互換を使って、一列表記を昇順へ並べ替えます。隣接交換は inversion 数をちょうど1だけ変えます。したがって必要な交換回数の偶奇は最初の inversion 数で決まります。
...
があれば、その二項を交換します。この操作は inversion 数をちょうど1減らします。

実際、交換した二項どうしの inversion が一つ消えます。また、それ以外の値 $c$ と $a,b$ の大小関係は、$a,b$ の位置が隣接しているため、二項をまとめた inversion の総数を変えません。

inversion 数は非負整数なので、この操作は有限回で停止します。停止した一列表記には inversion がなく、
...
隣接互換を一つ掛けるたび inversion 数の偶奇が反転するので、$\sigma$ を隣接互換 $k$ 個で作れば
...
で奇数です。従って任意の互換は sign が $-1$ です。

$\sigma=\tau_1\cdots\tau_k$ が任意の互換分解なら、各 $\tau_i$ を上の隣接互換分解に置き換えると、全体の隣接互換数の偶奇は $k$ と同じです。一方その偶奇は $N(\sigma)$ の偶奇に等しいので
...
$\rho\rho^{-1}=\operatorname{id}$ なので sign の積公式から
...
となります。しかし $r^{i-j}$ は回転なので頂点間の巡回順序を保ち、$s$ は $k\mapsto-k$ という反転です。具体的には $s(0)=0$ ですが、非自明な回転 $r^m$ は $0$ を固定しません。一方 $r^0=e$ は $1$ を固定しますが $s(1)=-1\not\equiv1\pmod n$ です。従って矛盾です。
...
を満たす写像を一般に **群準同型**と呼びます。そして Cayley の定理の $\lambda$ は単射な群準同型だった、と言い直せるようになります。
...
| $D_n$ | 正 $n$ 角形の対称性の合成 | 恒等変換 | 逆の対称性 | 回転と反転 |
...
よって異なる群要素は異なる左移動へ送られます。これが Cayley 表現の忠実性です。
<!-- solution-end -->
...
次の GRP2 では、積を保つ写像を群準同型として一般化します。その後、核・像、剰余類、Lagrange の定理、正規部分群、商群、群の同型定理へ進みます。
