...
\int_u^v f=G(v)-G(u)
$$
に同様に二つの極限との差を三角不等式で評価すればよいです。逆向きでは $r_n=b-1/n$ を十分大きい $n$ について取り、$(G(r_n))$ がCauchy列であることから極限 $L$ を得ます。任意の $r$ が $b$ に十分近いとき、さらに $r_n>r$ となる $n$ を取り
$$
|G(r)-L|
...
両者は非負項の単調増加部分和・部分積分です。一方が上に有界なら以上の上下評価から他方も上に有界になります。したがって [非負広義積分の有界性判定](#thm-ra4a-nonnegative) と [有界単調数列の収束定理](../RA1/index.md#thm-ra1-monotone) により、級数と広義積分は同時に収束します。上に有界でなければ同時に $+\infty$ へ発散します。$\square$
<!-- proof-end -->
...
2. $B=cA$ とすると
   ...
   $$
3. $c$ によって極限値が変わるため、左右を独立に無限遠へ送った極限は存在しません。実際、右半直線の積分値は $\frac12\log(1+B^2)\to+\infty$、左半直線の積分値は $-\frac12\log(1+A^2)\to-\infty$ です。主値0は対称切断という特定の規則による正則化です。
<!-- solution-end -->
...
なお、ここで使った「対数は任意の正のべきより遅く増える」は追加の微分定理なしでも確認できます。$t=\log x$ とし、整数 $N>|q|$ を取ると、指数関数の級数表示から
$$
e^{\eta t}\ge\frac{(\eta t)^N}{N!}.
$$
したがって
$$
0\le\frac{t^{|q|}}{e^{\eta t}}\le\frac{N!}{\eta^N}t^{|q|-N}\to0.
$$
$x=e^t$ に戻せば $(\log x)^{|q|}=o(x^\eta)$ が従います。
<!-- solution-end -->
