...
STO14 では、このランダム測度を使って 跳躍 stochastic 積分、Lévy 過程、Lévy--Itô 分解、跳躍 を含む Itô 公式へ進みます。本章ではそこまで逆輸入しません。
...
この $\lambda t$ は「平均を引いただけ」に見えますが、確率解析ではより構造的に **補償過程** と呼びます。
...
有限状態では 率 が一様有界だったため、自動的に 有限時間内に無限回跳躍しない でした。
...
ただし、複合ポアソン過程 の特性関数、Lévy exponent、Lévy 過程 としての位置付けは **STO14 の 正本範囲** とします。本章では ポアソンランダム測度 とその 補償 を構成するところで止めます。
...
2. 跳躍連鎖 の 遷移行列 を求めよ。
3. $p(t)=P_1(X_t=1)$ が満たす常微分方程式 を書け。
4. $p(t)$ を解け。
