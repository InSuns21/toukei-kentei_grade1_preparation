# CA9 標準複素解析 IX：二重周期有理型関数・Weierstrass $\wp$ 関数

> **複素解析 II の二周期関数編**。CA8 で構成した複素トーラス $\mathbb C/\Lambda$ 上では、正則関数は定数しかない。しかし有理型関数まで許せば、格子の幾何を反映した豊かな関数が現れる。本章ではまず格子の全ての元を周期に持つ有理型関数を定義し、続いて Weierstrass の $\wp$ 関数を級数から構成する。収束、周期性、Laurent 展開、微分方程式、半周期、加法公式までを一続きに証明し、最後に複素トーラスと非特異三次曲線の対応の入口へ進む。
...
Jacobi の二重周期関数体系、モジュラー形式、モジュラー群、複素乗法、楕円曲線の算術は本章の主線には入れない。
...
は $\mathbb C\setminus\Lambda$ 上で導関数0である。格子は閉離散集合なので、任意の二点を結ぶ折れ線を有限個の格子点を避けるよう微小に動かせる。従って $\mathbb C\setminus\Lambda$ は連結であり、$H_\lambda$ は一つの定数 $c_\lambda$ に等しい。偶性を使うと
...
$|z|\le r<d$ に制限し、$q=r/d<1$ と置く。非零格子点では $|\omega|\ge d$ だから
...
\frac1{|\omega|^2}
\sum_{n=1}^{\infty}
(n+1)\left(\frac r{|\omega|}\right)^n \\
&\le
\frac{2r}{(1-q)^2}\frac1{|\omega|^3}.
\end{aligned}
$$
...
を使った。[格子冪和の収束](#cor-ca9-lattice-summability)により右辺を $\omega$ について足した級数は収束する。従って二重級数は $|z|\le r$ で絶対一様収束し、和の順序を交換できる。

$n$ が奇数なら $\omega$ と $-\omega$ の寄与が相殺する。$n=2k$ の項だけが残り、
...
一方、$\wp'$ は各格子点に三重極を持つ。零点・極が境界に来ないよう基本平行四辺形を平行移動して数えれば、トーラス上では格子類 $[0]$ に三重極が一つあるだけなので、極の総重複度は3である。[零点数と極数の一致](#thm-ca9-zero-pole-balance)から零点の総重複度も3である。
...
と書くと、無限遠点は $[X:Y:Z]=[0:1:0]$ の一つだけである。この点では左辺から右辺を引いた斉次多項式の $Z$ 偏微分が $Y^2=1$ となるため、無限遠点も非特異である。
...
- [Laurent 展開](../CA4/index.md#thm-ca4-laurent)を本章の $\wp$ に適用すると $g_2,g_3$ が現れ、
  $$
