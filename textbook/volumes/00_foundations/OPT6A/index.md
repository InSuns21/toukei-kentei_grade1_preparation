...
> 上の誤差境界そのものは、本章では定理として使用します。完全証明には集合値写像の開写像性・計量正則性（metric regularity） を体系的に構成する必要があり、本章の射程を超えます。ここでは証明で実際に使う「線形化の全射性から有界な一次補正を得る → $C^1$ 剰余を反復吸収する → 距離評価を得る」という核心機構まで示しました。通常制約 $K=\mathbb R_+^m$ では、後の Robinson 制約想定と MFCQ の同値により OPT6 の陰関数定理を用いた完全証明へ戻ります。
