...
<!-- formal-statement-start -->
> **補題（\(H^1\) difference quotient estimate）**  
...
GPDE5 で使った Hilbert 空間の弱コンパクト性から弱収束部分列を取り、その弱極限が distributional derivative \(\partial_k u\) であることを discrete integration by parts から確認します。
...
difference quotient の部分積分により、左辺の \(\nabla u\) へ \(D_h^k\) を移すと
...
<!-- formal-statement-start -->
> **定理（Poisson 方程式の interior \(H^2\) regularity）**  
...
8. difference quotient criterion で \(\partial_k\partial_j u\in L^2\) を得る。
...
difference quotient の部分積分を左辺へ使うと
...
difference quotient criterion を \(\partial_j u\) へ適用すると
...
Young の不等式により、任意の \(\varepsilon>0\) に対して
...
<!-- formal-statement-start -->
> **定理（局所 Lipschitz 係数の interior \(H^2\) regularity）**  
...
が \(h\) に依らず成り立つため、その項は lower-order error として Young の不等式で吸収できます。
...
Poisson の場合と同じく difference quotient criterion を各 \(\partial_ju\) に適用して
...
| 境界 regularity | global \(H^2\) | interior theorem だけでは境界特異点を除けない |
...
<a id="ex-gpde9-a03"></a>
#### GPDE9-A03 harmonic function の Caccioppoli estimate
- Level: A
...
difference quotient criterion により
