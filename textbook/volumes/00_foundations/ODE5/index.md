...
となり、**微分方程式と初期データが一つの代数式へ同時に入ります**。
...
- 部分積分の無限遠境界項が本当に消えるか。
- 遅延入力をどう時間領域へ戻すか。
- 時間領域の積分による重ね合わせを、なぜ $s$ 領域の積として扱えるか。
...
> 指数位数を仮定すれば上の収束が保証されます。逆向きまで主張しているわけではありません。
...
Laplace 法が初期値付き方程式に強い理由はここです。部分積分の $t=0$ の境界項が初期値になります。
...
まず $n=1$ を示します。$s$ は $f,f' $ の指数位数より十分大きく取ります。$R>0$ に対して
...
したがって、$u_3$ の役割は値を1にすることよりも、**時刻3より前を0に切り落とすスイッチ**として働くことにあります。
<!-- definition-example-end -->
...
1. 変換表・部分分数分解・移動則で時間領域の**候補**を作る。
2. 必要なら候補をもう一度順変換して $F(s)$ に戻ることを確認する。
3. 初期値付き方程式なら候補を元の ODE と初期データへ代入する。
4. ODE2 の [連続係数線形高階初期値付き方程式の存在・一意性](../ODE2/index.md#thm-ode2-linear-ivp) により、その候補が唯一の解であることを確定する。
...
が使えます。これは $\mathcal L\{t\}=1/s^2$ に[指数移動則](#thm-ode5-frequency-shift)を適用したものです。

---

## 6. 時間領域の重ね合わせが $s$ 領域の積になる
...
まず合成積自体の絶対収束を確認します。指数位数の評価を有限区間まで定数を大きくして延長すれば、ある $M,N>0$ に対し
...
各 $y^{(k)}$ を微分則で変換すると、$s^kY$ と初期値多項式に分かれます。$Y$ の係数を集めると、ちょうど上で定めた多項式 $P(s)$ になります。
...
この定理は「変換した式が解を自動生成する」とまでは言っていません。逆変換候補を得たら元の ODE と初期データへ代入します。そこで条件を満たせば、ODE2 の [初期値付き方程式の一意性](../ODE2/index.md#thm-ode2-linear-ivp) により唯一の解です。
...
を考えます。[時間移動則](#thm-ode5-time-shift)から
...
[時間移動則](#thm-ode5-time-shift)を逆向きに読むと
...
> を満たすとき、本章では $h$ をその方程式の応答核という。合成積 $h*f$ が元の ODE と零初期データを満たすことを確認できれば、それが零初期値応答である。
<!-- formal-statement-end -->
...
[Laplace変換の積公式](#thm-ode5-convolution)から
...
工学では、この $h$ にインパルス応答という呼び名が使われます。理想的な瞬間入力を Dirac delta で表す説明もありますが、本章では delta を通常関数として扱いません。
...
つまり Laplace 変換は「微分方程式なら何でも解く魔法」ではなく、初期値付き定係数線形系を、時間領域の微分から $s$ 領域の代数へ変える道具です。
...
<a id="ex-ode5-a03"></a>
#### ODE5-A03 [時間移動則](#thm-ode5-time-shift)
- Level: A
...
2. $f(r)=r^2$ と置けば $g(t)=u_3(t)f(t-3)$。[時間移動則](#thm-ode5-time-shift)より
...
を Laplace 変換で解き、最後に元の ODE と初期データを確認せよ。
...
$\mathcal L\{t\}=1/s^2$ に[指数移動則](#thm-ode5-frequency-shift)を使えば
...
[高階線形初期値付き方程式の存在・一意性](../ODE2/index.md#thm-ode2-linear-ivp)より、これが唯一の解です。
...
なので[時間移動則](#thm-ode5-time-shift)から
...
よって[時間移動則](#thm-ode5-time-shift)を逆向きに使い
...
1. 応答核 $h(t)=\sin(\omega t)/\omega$ と入力 $f(t)=1$ の合成積を直接計算せよ。
2. Laplace 変換でも同じ結果を得よ。
3. 元の ODE と初期データを確認せよ。
...
同じ [高階線形初期値付き方程式の存在・一意性](../ODE2/index.md#thm-ode2-linear-ivp) により、この解が唯一です。
...
- $\sin,\cos$ の変換を微分則から再構成し、[指数移動則](#thm-ode5-frequency-shift)・[時間移動則](#thm-ode5-time-shift)を定義から証明した。
- 逆変換は一般反転定理を無断使用せず、変換表・部分分数・移動則で候補を作って順変換または ODE へ代入して検証する方針にした。
- 片側合成積を定義し、有限三角領域の積分交換を直接示した上で、絶対収束の尾部評価から[Laplace変換の積公式](#thm-ode5-convolution)を証明した。
- 定係数線形 IVP が $P(s)Y(s)=F(s)+\text{初期値多項式}$ へ変わる一般式を導いた。
- 階段入力を[時間移動則](#thm-ode5-time-shift)で処理し、遅延前後の解を実際に確認した。
- ODE2 の Green 核と Laplace 領域の伝達因子を合成積で結び付けた。
