...
が成り立つわけではないので、この経路では必要以上に粗くなります。代わりに二項展開を使います。
...
2-ノルムではこの上界は本質的に鋭いです。最大特異値方向に元の解を置き、最小特異値方向へ右辺を摂動すると、相対誤差の増幅率を $\kappa_2(A)$ まで達成できます。
...
1. 厳密解 $x$ を求めよ。
2. $\widetilde x=(1,0)^{\mathsf T}$ の相対残差を 2-ノルムで求めよ。
3. 相対前方誤差を求めよ。
...
次の [NA2](../NA2/index.md) では、この「条件数と安定性を分ける」という見方を、非線形方程式・不動点反復・Newton 法へ持ち込みます。
