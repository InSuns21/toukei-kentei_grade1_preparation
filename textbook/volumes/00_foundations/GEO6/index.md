...
- Level: A
- 狙い: kernel 分布・Lie 括弧・レベル集合を直接計算で接続する
...
最後に 4 \Rightarrow 1 を示します。4 から各点を通る積分多様体が存在するので D は可積分です。[可積分なら対合的](#prop-geo6-integrable-implies-involutive) により D は対合的です。
...
難しい向きでは、一本のベクトル場を flow-box theorem で直線化し、対合性からその流れが分布を保存することを示しました。そのうえで横断面上の階数 k-1 の分布へ帰着し、帰納法で適応座標を構成しました。
