...
答えは Lie 括弧です。指定した方向に沿う二つの滑らかなベクトル場を順に動かしたとき、その交換差が指定方向の外へ飛び出さないこと、すなわち分布が Lie 括弧で閉じていることが、局所的に部分多様体へ積分できることと同値になります。これが Frobenius の定理です。
...
- **滑らかさ**：局所枠と局所流を作り、ベクトル場の直線化定理を適用するために必要です。
...
従って Frobenius の定理は、対合的な階数 k 分布に対して局所的に n-k 個の独立な関数が存在し、その共通の逆像 $F^{-1}(c)$ が積分多様体になることも述べています。
...
難しい向きでは、一本の非零な滑らかなベクトル場をベクトル場の直線化定理で直線化し、対合性からその流れが分布を保存することを示しました。そのうえで横断面上の階数 k-1 の分布へ帰着し、帰納法で適応座標を構成しました。
