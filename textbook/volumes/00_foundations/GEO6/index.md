# GEO6 線形分布と Frobenius の定理

<!-- definition-example-audit: strict -->

[GEO5](../GEO5/index.md) では、一本の非零ベクトル場なら局所座標を選んで座標方向へ直せることを示しました。本章では「各点で k 次元の接方向を指定したとき、その方向全体が実際の k 次元部分多様体の接空間になるのはいつか」を扱います。

答えは Lie 括弧です。指定した方向に沿う二つのベクトル場を順に動かしたとき、その交換差が指定方向の外へ飛び出さないこと、すなわち分布が Lie 括弧で閉じていることが、局所的に部分多様体へ積分できることと同値になります。これが Frobenius の定理です。
...
各点で許される方向は xy 平面に平行で、z 方向は許されません。後で積分多様体は
...
と置けば、[GEO3 の正則レベル集合の接空間](../GEO3/index.md#thm-geo3-level-tangent-kernel)から各ファイバー F^{-1}(c) は局所的な積分多様体です。つまり「独立な n-k 個の保存量がある」なら、その共通レベル集合が葉になります。
...
よって D は対合的ではありません。前節の命題の対偶から D は可積分ではありません。

ここで失われている機構は明確です。X 方向へ少し進むと Y の z 成分 x が変化します。したがって X と Y の微小な交換運動は z 方向のずれを生み、その方向が D の外に出ます。もし D に接する曲面が存在すれば、その曲面上の接ベクトル場の Lie 括弧も接していなければならないので矛盾します。
...
です。その鍵は、D に沿うベクトル場の流れが D 自身を保つことです。

<a id="lem-geo6-flow-preserves-distribution"></a>
<!-- formal-statement-start -->
> **補題（対合的分布はその切断の局所流で保存される）**  
...
と書けます。[GEO5 の流れによる Lie 括弧の公式](../GEO5/index.md#thm-geo5-bracket-flow)を使うと、引き戻された k 本のベクトルは有限次元の線形常微分方程式を満たします。従って初期時刻で張っていた k 次元部分空間から外へ出ません。
...
<a id="def-geo6-adapted-coordinates"></a>
<!-- formal-statement-start -->
> **定義（分布に適応した局所座標）**  
...
適応座標があれば可積分性は一目で分かります。u^{k+1},\dots,u^n を定数に固定した座標平面が積分多様体だからです。また座標ベクトル場は可換なので対合性も直ちに従います。
...
また t 成分が0の二つのベクトル場の Lie 括弧も t 成分が0なので
...
直線化座標では $\widetilde A,\widetilde B$ は $\partial_t$ 成分を持たないため、$t=0$ 上で計算した 周囲多様体上の Lie 括弧は、$S$ 上のベクトル場 $A,B$ の Lie 括弧と一致します。したがって
...
- **滑らかさ**：局所枠と局所流を作り、ベクトル場の直線化定理 を適用するために必要です。
...
従って Frobenius の定理は、対合的な階数 k 分布に対して局所的に n-k 個の独立な関数が存在し、その共通レベル集合が積分多様体になることも述べています。
...
これは本章の意味での一定階数の滑らかな線形分布ではありません。したがって Frobenius の定理をそのまま適用できません。
...
2. [X,Y] を計算せよ。
3. [X,Y]\notin D を示せ。
4. Frobenius の定理から D が可積分でないことを結論せよ。
...
よって D は対合的ではありません。Frobenius の定理より D は可積分ではありません。
<!-- solution-end -->
...
です。[GEO3 の正則レベル集合の接空間公式](../GEO3/index.md#thm-geo3-level-tangent-kernel)から、その接空間は $\ker df=D$ です。
<!-- solution-end -->
...
- Level: B
- 狙い: 「共通レベル集合へ接する方向は Lie 括弧で閉じる」を関数への作用から証明する
...
この証明ではレベル集合そのものを先に作らなくても、保存量 F^a を X,Y が変化させないことから対合性が直接出ています。
<!-- solution-end -->
...
各含意で、Frobenius の定理、沈め込みの局所標準形、正則レベル集合の接空間公式のどれを使うか明示せよ。

- Level: C
- 狙い: Frobenius の定理を「Lie 括弧」「座標」「保存量」「葉」の四つの姿で統合する

<!-- solution-start -->
**詳細解答**

まず 1 \Rightarrow 2 は Frobenius の定理そのものです。D が対合的なら、p の近傍で
...
よって各レベル集合は積分多様体です。また各 q は c=F(q) のレベル集合上にあるので、近傍はこれらの局所積分多様体で埋め尽くされます。
...
- 積分多様体：その保存量の共通レベル集合が k 次元の葉になる。
<!-- solution-end -->
...
Frobenius の定理は、この必要条件が局所的には十分でもあることを述べます。
...
難しい向きでは、一本のベクトル場を ベクトル場の直線化定理 で直線化し、対合性からその流れが分布を保存することを示しました。そのうえで横断面上の階数 k-1 の分布へ帰着し、帰納法で適応座標を構成しました。
...
となります。したがって積分多様体は F の局所レベル集合です。
