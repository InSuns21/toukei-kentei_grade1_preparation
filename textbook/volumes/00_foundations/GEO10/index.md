...
曲線については「どれだけ曲がるか」と「どれだけ空間内でねじれるか」を曲率と捩率で測ります。超曲面については、接空間上の内積を第一基本形式、法線の変化を形作用素と第二基本形式で記録します。形作用素が自己共役になることから主曲率が現れ、二次元曲面では Gauss 曲率と平均曲率が得られます。

ここで重要なのは、曲率を公式集として覚えることではありません。

- 曲線では、弧長パラメータによって速度の大きさを取り除く。
- 超曲面では、単位法線場の微分を接空間上の線形作用素として読む。
- その作用素を第一基本形式で対角化すると、主曲率が現れる。
...
と固定します。この規約では、半径 $R$ の球面に外向き単位法線を取ると主曲率は $-1/R$ です。文献によっては $S=dN$ を採用するため全主曲率と平均曲率の符号が反転しますが、本章以降は上の規約で統一します。
...
ここから曲線ではなく、$\mathbb R^{n+1}$ 内の $n$ 次元超曲面を扱います。
...
単位法線場は一般には大域的に取れるとは限りません。本章の形作用素は、単位法線場が選べる局所領域上で定義します。法線を $N$ から $-N$ へ変えると形作用素と第二基本形式の符号も反転します。
...
はそれ自体が滑らかな写像です。これを単位球面への写像として見るのが Gauss 写像です。
...
グラフが接平面から二次の項でどちら向きへ離れるかが、その点の主曲率へ直接現れます。
