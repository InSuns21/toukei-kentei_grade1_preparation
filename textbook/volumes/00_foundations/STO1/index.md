...
<!-- formal-statement-start -->
> **定義（ランダム時刻の最小・最大記法）**  
> $\sigma,\tau:\Omega\to[0,\infty]$ を二つの random time とする。各 $\omega\in\Omega$ に対して
>
$$
...
$$
>
> はともに stopping time である。  
> また固定した $t\ge0$ に対し、$\tau\wedge t$ は $[0,t]$ 値の $\mathcal F_t$ 可測確率変数である。  
> 一方、$t\vee\tau$ は stopping time ではあるが、一般には $\mathcal F_t$ 可測ではない。
<!-- formal-statement-end -->
...
$\sigma,\tau$ は stopping time なので右辺の二事象はともに $\mathcal F_u$ に属します。従って
...
よって $\sigma\wedge\tau$ は stopping time です。
...
なので $\sigma\vee\tau$ も stopping time です。
...
また $a<0$ なら空集合です。区間 $(-\infty,a]$ は $\mathbb R$ の Borel $\sigma$ 代数を生成するので、これで $\rho$ は $\mathcal F_t/\mathcal B([0,t])$ 可測です。
