...
まず、ここで何を比較するのかを具体化します。

消費者が二つの財だけを選ぶなら、一つの選択肢を

$$
x=(x_1,x_2)
$$

と書けます。$x_1$ は第1財の量、$x_2$ は第2財の量です。例えば

$$
x=(2,1)
$$

は「第1財を2単位、第2財を1単位」という一つの候補を表します。
...
この $X$ が、この章でいう **選択集合**です。まだ価格や所得は入っていません。つまり $X$ は「そもそも比較対象にする候補の宇宙」であり、MICRO2 で価格と所得を入れて絞り込む「実際に買える候補」とは役割が違います。

この段階で知りたいのは、「どの候補が買えるか」ではなく、

> 二つの候補 $x,y$ を見せられたとき、消費者はどちらを少なくとも同じくらい望むのか

です。

この比較規則を数学の対象にしたものが選好です。
...
選好関係は本来「二点を比べる規則」ですが、後で最適化と結びつけるには、点ごとの比較を集合の形へ直した方が便利です。

例えば基準となる候補 $y$ を一つ固定して、

> $y$ と同じくらい良いか、それより良い候補を全部集める

と考えます。その集まりに名前を付けて扱います。

この見方へ移ると、「好み」が幾何学になります。その集合が丸くつながっているか、凸か、閉じているか、といった性質を使って消費者問題を調べられるようになります。
...
凸性を経済学で使う理由は、「極端な二案がどちらも十分よいなら、その中間案も突然ひどくはならない」という性質を捉えたいからです。
...
は、二つの配分を割合 $\lambda:1-\lambda$ で混ぜた候補です。

ここでいう「混ぜる」は確率的な混合を意味するとは限りません。財の量そのものを平均した新しい消費候補を作る、という幾何学的操作です。
