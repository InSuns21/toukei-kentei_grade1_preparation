...
<!-- formal-statement-start -->
> **定義（ベクトル線積分）**  
> $F:\Omega\to\mathbb R^n$ を連続ベクトル場、$\gamma:[a,b]\to\Omega$ を区分的 $C^1$ 曲線とする。
>
$$
\int_\gamma F\cdot dr
:=
...
$\phi(\gamma(t))$ を 1 変数関数として微分すれば、連鎖律で被積分関数がそのまま出ます。
...
<!-- formal-statement-start -->
> **定義（保存場・ポテンシャル）**  
> 開集合 $\Omega\subset\mathbb R^n$ 上の連続ベクトル場 $F$ が、ある $\phi\in C^1(\Omega)$ により
>
$$
F=\nabla\phi
$$
>
> と書けるとき、$F$ を保存場、$\phi$ を $F$ のスカラーポテンシャルと呼ぶ。
<!-- formal-statement-end -->
...
<!-- formal-statement-start -->
> **定義（経路独立）**  
> 経路連結な $\Omega$ 上のベクトル場 $F$ について、同じ始点 $p$ と終点 $q$ を持つ任意の区分的 $C^1$ 曲線 $\gamma_1,\gamma_2$ に対し
>
$$
...
したがって $F$ は実際にポテンシャル $\phi$ を持つ保存場であり、積分値は経路に依存しません。
<!-- definition-example-end -->
...
> $\Omega$ を経路連結な開集合、$F$ を連続ベクトル場とする。次は同値である。
>
> 1. $F$ は保存場である。
> 2. $F$ の線積分は経路独立である。
...
1 から 2 は線積分の基本定理です。2 から 3 は閉曲線を定数曲線と比較します。3 から 1 では基準点から $x$ までの線積分をポテンシャルの定義にして、短い線分を足したときの増分を計算します。
...
<!-- formal-statement-start -->
> **定義（星型領域）**  
> 開集合 $\Omega\subset\mathbb R^n$ が点 $a\in\Omega$ に関して星型であるとは、任意の $x\in\Omega$ と $0\le t\le1$ に対して
>
$$
a+t(x-a)\in\Omega
$$
...
は原点に関して星型です。実際 $x\in B(0,1)$ と $0\le t\le1$ に対し
...
<!-- formal-statement-start -->
> **定理（星型領域上の初等 Poincaré の補題）**  
> $\Omega\subset\mathbb R^3$ を $a$ に関して星型とし、$F\in C^1(\Omega;\mathbb R^3)$ が
>
$$
\nabla\times F=0
$$
...
とポテンシャルを明示的に作ります。回転が 0 という条件は Jacobian の対称性
...
ここで星型仮定は、$a$ から $x$ への線分全体が $\Omega$ 内に残り、上のポテンシャルが定義できるために使われています。

---

## 7. 穴があると回転が 0 のでも保存場とは限らない
...
一般の単連結領域で回転が 0 であることから保存場を導く議論は、VC5 の Stokes 定理を得てから閉じます。VC2 で後続定理を逆輸入しません。

---

## 8. 単連結の意味

本系列で単連結は直感的に「領域内の任意の閉曲線を、領域の外へ出ずに一点へ連続的に縮められる」ことを表します。

- 円板・球は単連結。
- 穴あき平面 $\mathbb R^2\setminus\{0\}$ は単連結ではない。
- 星型 ⇒ 単連結。

ただし一般位相空間の基本群はここでは導入しません。VC5 では古典的な Stokes の定理の適用範囲の中で、この条件がポテンシャルの存在をどう支えるかを扱います。
...
向きを反転するとベクトル線積分は符号を変えるので、時計回りでは
...
についてポテンシャルを求め、$(0,0)$ から $(1,2)$ までの線積分を求めよ。
...
#### VC2-B02 星型領域上の Poincaré の補題の構成
- Level: B
- 目安時間: 30分
...
について、原点以外でスカラー回転が 0 であることを直接確認し、半径 $R>0$ の円周上の循環を求めよ。
...
回転が 0 のでも閉曲線積分が消えないため保存場ではありません。
<!-- solution-end -->
...
1. スカラー回転が 0 であることを確認せよ。
2. $\mathbb R^2$ が星型であることを使って保存場であると結論せよ。
3. $(0,0)$ から $(1,1)$ へ、直線 $\gamma_1(t)=(t,t)$ と折れ線
   $$
   (0,0)\to(1,0)\to(1,1)
   $$
   の二経路で線積分を直接計算し、一致を確認せよ。
4. ポテンシャルを求め、その端点差とも一致することを確認せよ。
...
2. $\mathbb R^2$ は原点に関して星型です。従って [星型領域上の初等 Poincaré の補題](#thm-vc2-poincare-star) の二次元版により $F$ は保存場です。
...
VC2 では曲線と循環を扱いました。VC3 では二次元の面を $\mathbb R^3$ に埋め込み、法線、面積要素、流束を定義します。

その準備が整うと、VC4 の Gauss--Ostrogradsky の発散定理、VC5 の Stokes の定理が「境界の積分」と「内部の微分」を結びます。
