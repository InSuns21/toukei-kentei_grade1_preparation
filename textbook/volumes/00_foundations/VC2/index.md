...
VC1 では ベクトル場 の局所微分を調べました。本章では 場 を **曲線に沿って積分**し、仕事・循環・ポテンシャルを結びます。
...
$\phi(\gamma(t))$ を 1 変数関数として微分すれば、連鎖律で 被積分関数 がそのまま出ます。
...
$$
>
> と書けるとき、$F$ を保存場、$\phi$ を $F$ の スカラーポテンシャルと呼ぶ。
<!-- formal-statement-end -->
...
<!-- formal-statement-start -->
> **定義（経路独立）**  
> 経路連結な $\Omega$ 上の ベクトル場 $F$ について、同じ始点 $p$ と終点 $q$ を持つ任意の区分的 $C^1$ 曲線 $\gamma_1,\gamma_2$ に対し
>
$$
...
> $\Omega$ を 経路連結な開集合、$F$ を連続ベクトル場とする。次は同値である。
>
> 1. $F$ は保存場である。
> 2. $F$ の線積分は経路独立である。
...
<!-- formal-statement-start -->
> **定義（星型領域）**  
> 開集合 $\Omega\subset\mathbb R^n$ が点 $a\in\Omega$ に関して 星型であるとは、任意の $x\in\Omega$ と $0\le t\le1$ に対して
>
$$
a+t(x-a)\in\Omega
$$
...
は原点に関して 星型です。実際 $x\in B(0,1)$ と $0\le t\le1$ に対し
...
Euclidean 空間の凸領域なら任意の点を中心として 星型です。
...
と ポテンシャルを明示的に作ります。回転が 0 という条件は Jacobian の対称性
...
ここで 星型仮定は、$a$ から $x$ への線分全体が $\Omega$ 内に残り、上の ポテンシャル が定義できるために使われています。
...
一般の単連結領域 で 回転が 0 であることから保存場を導く議論は、VC5 の Stokes 定理を得てから閉じます。VC2 で後続定理を逆輸入しません。
...
ただし一般位相空間の 基本群 はここでは導入しません。VC5 では古典的な Stokes の定理の適用範囲の中で、この条件が ポテンシャルの存在 をどう支えるかを扱います。
...
について ポテンシャルを求め、$(0,0)$ から $(1,2)$ までの線積分を求めよ。
...
2. $\mathbb R^2$ が 星型であることを使って保存場であると結論せよ。
...
2. $\mathbb R^2$ は原点に関して 星型です。従って [星型領域上の初等 Poincaré の補題](#thm-vc2-poincare-star) の二次元版により $F$ は保存場です。
