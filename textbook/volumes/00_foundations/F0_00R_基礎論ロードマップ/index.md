...
です。A1Bで実数の上限性質・Archimedes性、B0で点列・部分列・「十分大きい添字」、Bで距離空間、B1で位相空間・近傍・部分空間位相・位相的収束・位相的連続性を導入します。A2・A3・A3Aは選択公理とZornの補題を追う集合論側の深掘りであり、この最短列の必須前提ではありません。

D0系列では、Dで導入した完備性を「実数は穴のない空間」という説明で止めず、実際の構成として閉じます。D0で有理Cauchy列の商から $\mathbb R_C$ を作り、D0Aで任意の距離空間へ一般化し、D0BでDedekind切断から $\mathbb R_D$ を作り、D0Cで

$$
\mathbb R_C\cong\mathbb R_D
$$

を示します。これにより、後続で使う「Cauchy完備性」と「上限性質」が同じ実数体系の完全性の二つの表現として接続されます。
...
そこで標準ルートでは、D→D0→D0A→D0B→D0Cで完備性と実数構成を閉じたあと、いったん線形代数へ進みます。
...
- **集合・位相・コンパクト性を未定義語なしで読む**：A → A1 → A1B → B0 → B → B1 → C → C1 → C2 → D。
- **実数の構成と完備化を追う**：D → D0 → D0A → D0B → D0C。Cauchy完備性と上限性質が同じ実数体系でつながるところまで読む。
...
標準ルートでは、**実数の完備化を Cauchy 構成と Dedekind 構成の両方から閉じ、Banach空間へ入る前に有限次元線形代数を理論として一度閉じ、その後は関数解析 → 凸解析 → KKT → RKHSを一方向に読む**構成にします。
