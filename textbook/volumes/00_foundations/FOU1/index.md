...
Fourier 級数の入口で最初にやることは、無限級数の収束判定ではありません。周期的な波形から、**どの周波数がどれだけ含まれているかを一周期積分で取り出すこと**です。

本章では $2\pi$ 周期の実数値関数を中心に、次の順で進みます。

1. $1,\cos nx,\sin nx$ を掛け合わせて一周期積分し、異なる周波数が消えることを直接計算する。
2. その消去則から実 Fourier 係数を導く。
3. 係数から作る有限和が、固定次数の三角多項式の中で平均二乗誤差を最小にすることを証明する。
4. 偶奇性と半区間展開を使って係数計算を簡略化する。
5. $x$、矩形波、$|x|$、$x^2$、三角形波の係数を手計算する。
6. 無限級数の理論を使わず、有限個の Fourier 係数に対するエネルギー上界を証明する。

前提は [RA4 Riemann/Darboux積分・FTC](../RA4/index.md) です。区分的連続関数だけを扱うので、本章の積分は Riemann 積分で足ります。Lebesgue 積分や Hilbert 空間の一般論を入口の前提にはしません。

> **この章の停止線**  
> 係数を計算できることと、その係数から作った無限級数が元の関数へ近づくことは別問題です。本章では有限次数の係数計算と近似までを閉じます。無限級数の収束の意味、跳びを含む波形での極限、平均化、無限個の係数を使う等式は FOU2 で扱います。
...
一般の周期関数から $A_n,B_n$ に相当する量を取り出すには、「狙った周波数だけを残す」積分公式が必要です。
...
$m\ne n$ なら積和公式に現れる $m-n,m+n$ はどちらも非零整数です。そのため一周期積分で消えます。$m=n$ のときだけ $\cos0x=1$ が残ります。
...
$m\ne n$ の $\sin mx\sin nx$ も積和公式によって非零周波数の余弦二つへ分かれるので積分は0です。また $\sin mx\cos nx$ は奇関数なので対称区間上の積分は0です。
...
この定理は、Fourier 解析で周波数ごとの係数を分離できる理由そのものです。
...
<!-- formal-statement-start -->
> **定義（実 Fourier 係数）**  
> $f$ に対し、次の数を $f$ の **実 Fourier 係数** という。
...
とします。[三角関数系の一周期積分消去則](#thm-fou1-frequency-cancellation)から

$$
a_0=6,\qquad a_2=2,\qquad b_3=-4
$$

で、それ以外は0です。したがって $S_3f=f$ です。積分公式が、もともと入っている周波数の振幅をそのまま回収しています。
<!-- definition-example-end -->
...
次数を上げると、定義された係数のうち使う周波数が増えていきます。ここでは有限和しか扱っていないため、無限級数の極限は不要です。
<!-- definition-example-end -->
...
$\cos mx$ を掛けると $A_m\cos^2mx$ 以外が消え、$\sin mx$ を掛けると $B_m\sin^2mx$ 以外が消えます。

<!-- proof-start -->
### 証明

$T_N$ をそのまま積分すると、[三角関数系の一周期積分消去則](#thm-fou1-frequency-cancellation)により定数項以外は0です。したがって
...
次に $T_N\cos mx$ を積分すると $A_m\cos^2mx$ だけが残るので
...
## 4. なぜこの係数を一般の関数にも使うのか

一般の $f$ を有限個の周波数で近似するとき、誤差を

$$
E(T):=\int_{-\pi}^{\pi}|f(x)-T(x)|^2\,dx
$$

で測ります。ここで必要なのは平方を展開する計算だけです。

<a id="thm-fou1-least-squares"></a>
<!-- formal-statement-start -->
> **定理（Fourier有限和の固定次数最小二乗性）**  
> $S_Nf$ を $f$ の N次 Fourier 有限和とする。任意の $N$ 次以下の実三角多項式 $T_N$ に対して
...
$S_Nf$ は $f$ と同じ $0,1,\ldots,N$ 次の係数を持ちます。そのため $f-S_Nf$ に $1,\cos mx,\sin mx$ を掛けて積分すると0です。別の三角多項式へ動かした分は交差項を作らず、二乗誤差へ非負量を加えるだけです。

<!-- proof-start -->
### 証明

$R_N:=T_N-S_Nf$ と置きます。実 Fourier 係数の定義と[三角関数系の一周期積分消去則](#thm-fou1-frequency-cancellation)から
...
等号なら $\int_{-\pi}^{\pi}|R_N|^2\,dx=0$ です。$R_N$ は連続なので、もしある点で $R_N\ne0$ なら、その点の十分近くでも $|R_N|^2$ は正になります。その区間の積分は正になって矛盾するため $R_N\equiv0$、すなわち $T_N=S_Nf$ です。
<!-- proof-end -->

この定理は固定した有限次数についての主張です。無限級数の収束は一切使っていません。
...
<!-- formal-statement-start -->
> **定理（偶奇性による Fourier 係数の消失則）**  
> $f$ の実 Fourier 係数を $a_0,a_n,b_n$ とする。$f$ が偶関数なら、すべての $n\ge1$ で $b_n=0$ であり、次が成り立つ。
...
> $f$ が奇関数なら、$a_0=0$ かつすべての $n\ge1$ で $a_n=0$ であり、次が成り立つ。
...
偶関数と奇関数の積は奇関数、偶関数どうし・奇関数どうしの積は偶関数です。対称区間上では奇関数の積分が0、偶関数の積分は半区間積分の2倍になります。

<!-- proof-start -->
### 証明

$f$ が偶関数なら $f(x)\sin nx$ は奇関数です。したがって
...
この係数列から無限級数の候補を作ることはできますが、それがどこで元の関数に一致するかは本章では主張しません。
...
とし、$2\pi$ 周期に延長します。$q$ は奇関数なので $a_0=a_n=0$ です。
...
したがって $1,3,5,\ldots$ 次の周波数だけが残ります。跳びの位置で無限級数がどう振る舞うかは FOU2 の問題です。
...
です。矩形波では $1/n$、$|x|$ では $1/n^2$ が現れました。波形の滑らかさと係数の減り方の一般関係は、条件を明示して後続で扱います。
...
を使うと、正弦側と余弦側の係数を整数添字一つにまとめられます。本章ではこの恒等式だけを使い、複素解析の定理は使いません。
...
実数値 $f$ なら $c_{-n}=\overline{c_n}$ です。したがって有限和は
...
複素指数表示は新しい周波数を追加しているのではなく、実表示の二系列を一つに整理しているだけです。

---

## 7. 半区間正弦・余弦係数

関数が $[0,L]$ 上でしか与えられていないとき、$[-L,L]$ へ対称に延ばす方法が二つあります。左右を同符号で延ばせば余弦側、反対符号で延ばせば正弦側だけが残ります。

<a id="def-fou1-half-range"></a>
<!-- formal-statement-start -->
> **定義（半区間正弦・余弦係数）**  
> $L>0$ とし、$g$ を $[0,L]$ 上の区分的連続な実数値関数とする。左右を同符号で延ばす場合の係数を
...
反対符号で延ばした関数は $x$ です。したがって

$$
B_n=\frac{2(-1)^{n+1}}n.
$$

元の $[0,\pi]$ 上では同じ $g(x)=x$ でも、区間外へ作った周期波形が違うので係数列も異なります。
<!-- definition-example-end -->

半区間で使う二系列には、端点で直接確認できる違いがあります。
...
は $x=0,L$ で0です。この違いは後で微分方程式を扱うときにも役立ちますが、本章では対称延長と係数計算だけを確定します。

---

## 8. 有限個の係数が持てるエネルギーの上限

固定次数の最小二乗性から、無限級数の理論を使わずに重要な評価が得られます。

<a id="thm-fou1-finite-energy-bound"></a>
<!-- formal-statement-start -->
> **定理（Fourier係数の有限エネルギー不等式）**  
> $f$ の実 Fourier 係数を $a_0,a_n,b_n$ とする。任意の非負整数 $N$ に対し
...
[Fourier有限和の固定次数最小二乗性](#thm-fou1-least-squares)の証明で得た交差項0の式を使います。元の二乗積分は、有限 Fourier 近似の二乗積分と残差の二乗積分に分かれます。
...
さらに[三角関数系の一周期積分消去則](#thm-fou1-frequency-cancellation)により、$|S_Nf|^2$ を展開したときの異なる周波数どうしの交差項は消えます。よって
...
+\sum_{n=1}^{N}
\left(a_n^2\frac{\pi}{\pi}+b_n^2\frac{\pi}{\pi}\right)\\
...
この評価は有限 $N$ に対する「$\le$」です。無限個の係数を使って等号へ進むには、有限次数では捉えていない成分が極限で残らないことを別途示す必要があります。その部分は FOU2 で扱います。
...
2. **偶奇性を見る。** 対称性だけで0になる係数を先に消す。
3. **区分点で積分を分ける。** 矩形波のような波形は定義区間ごとに積分する。
4. **部分積分の回数を見積もる。** $x$ なら1回、$x^2$ なら2回が基本になる。
5. **有限和へ戻して確認する。** どの周波数が残ったかを見る。
6. **無限級数との等号は保留する。** 係数が求まったことと極限の議論は分ける。

この「係数を作る段階」と「極限を証明する段階」の分離が、Fourier 解析で最初に身につけるべき整理です。
...
3. $\sin mx$ は奇関数、$\cos nx$ は偶関数なので積は奇関数です。対称区間上の積分は0です。
<!-- solution-end -->
...
偶奇性だけで0と分かるのは正弦側であり、余弦側はさらに積分して判定します。
<!-- solution-end -->
...
最初の境界項は0です。残る積分へ $u=x$, $dv=\sin nx\,dx$ を使うと
...
#### B2. 同じ $g(x)=x$ から二種類の半区間係数を作る

$g(x)=x$ を $0\le x\le\pi$ で考えます。

1. 半区間余弦係数 $A_0,A_n$ を求めてください。
2. 半区間正弦係数 $B_n$ を求めてください。
3. 二つの係数列が異なる理由を、$[-\pi,\pi]$ へ延ばした関数の形から説明してください。
...
同符号で延ばすと $[-\pi,\pi]$ 上では $|x|$、反対符号で延ばすと $x$ になります。元の半区間上では同じ値でも、延長後の周期波形が違うため係数列も異なります。
<!-- solution-end -->
...
3. $n=1,3,5$ を用いる $S_5q$ を書き、$x=\pi/2$ での値を求めてください。
...
これは有限次数の値であり、$N\to\infty$ の極限は使っていません。
<!-- solution-end -->
...
5. 矩形波では係数に $1/n$、三角形波では $1/n^2$ が現れる違いを、波形の形と結び付けて説明してください。ただし一般定理の証明は不要です。
...
矩形波には跳びがあり、今回の計算では係数に $1/n$ が現れました。三角形波は折れ曲がりはあるものの関数自体は連続で、係数には $1/n^2$ が現れました。この二例から、より滑らかな波形では高い周波数の係数が速く小さくなる傾向が見えます。ただし一般化には追加条件が必要なので、ここでは観察に留めます。
<!-- solution-end -->
...
本章で閉じたのは Fourier 級数の **係数計算と有限次数近似** です。

- 三角関数を一周期積分すると、異なる整数周波数どうしの積が消えることを直接証明した。
- その消去則から実 Fourier 係数を抽出した。
- 係数から作る有限和が固定次数で平均二乗誤差を最小にすることを証明した。
- 偶奇性により、計算前から0と分かる係数を判定した。
- 複素指数表示で正弦側・余弦側の情報を整数添字一つへまとめた。
- 半区間正弦・余弦係数を、二種類の対称延長から導いた。
- 有限個の係数の二乗和が元の二乗積分を超えないことを、有限次数の議論だけで証明した。
- $x$、矩形波、$|x|$、$x^2$、三角形波について手計算した。

次の FOU2 では、有限和の次数を増やしたときに元の関数へどの意味で近づくのかを扱います。FOU1 の証明はその後続理論や旧 Fourier 教材を仮定していません。
