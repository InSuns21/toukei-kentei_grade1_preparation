...
6. 完全性を使わず、有限個の Fourier 係数に対するエネルギー上界を証明する。
...
> **この章の停止線**  
> 係数を計算できることと、その係数から作った無限級数が元の関数へ収束することは別問題です。本章では有限次数の係数計算と近似までを閉じます。各点収束、跳躍点、Gibbs 現象、平均化、三角関数系の完全性、Parseval 等式、$L^2$ 収束は FOU2 で扱います。
...
ここで重要なのは、係数と有限和は無限級数の収束を知らなくても定義できることです。
...
等号なら $\int|R_N|^2=0$ です。$R_N$ は連続なので、もしある点で $R_N\ne0$ ならその近傍でも $|R_N|^2$ は正となり、積分も正になります。よって $R_N\equiv0$、すなわち $T_N=S_Nf$ です。
<!-- proof-end -->
...
> $f$ の実 Fourier 係数を $a_0,a_n,b_n$ とする。
>
> - $f$ が偶関数なら、すべての $n\ge1$ で $b_n=0$ であり
>
> $$
...
> $$
>
> - $f$ が奇関数なら、$a_0=0$ かつすべての $n\ge1$ で $a_n=0$ であり
>
> $$
...
です。ただし本章では、この無限級数が各点で $x$ に等しいとはまだ主張しません。
...
したがって $1,3,5,\ldots$ 次の周波数だけが残ります。跳躍点で無限級数が何へ近づくかは FOU2 の問題です。
...
です。矩形波では $1/n$、$|x|$ では $1/n^2$ が現れました。滑らかさと係数減衰の一般関係は後続で条件を明示して扱います。
...
ODE7 で現れた境界条件との関係を先に眺めると、端点で関数値を0にそろえたい場合には正弦系、端点で導関数を0にそろえたい場合には余弦系が自然に現れます。本章では係数計算だけを担当し、その一般的なスペクトル展開は後続へ送ります。

---

## 8. 有限個の係数が持てるエネルギーの上限

固定次数の最小二乗性から、完全性を使わずに重要な評価が得られます。
...
この評価は有限 $N$ に対する「$\le$」です。無限個の係数を使った等号へ進むには、三角関数系で見えない成分が残らないことを別途示す必要があります。FOU2 ではこの差を埋めます。
...
6. **無限級数との等号は保留する。** 係数が求まったことと収束定理は分ける。

この「係数を作る段階」と「収束を証明する段階」の分離が、Fourier 解析で最初に身につけるべき整理です。
...
- 有限個の係数の二乗和が元の二乗積分を超えないことを、完全性なしで証明した。
- $x$、矩形波、$|x|$、$x^2$、三角形波について手計算した。

次の FOU2 では「この有限和を次数とともに増やすと、元の関数へどの意味で近づくのか」を扱います。そこで各点での挙動、平均化、完全性、Parseval 等式、$L^2$ 収束を区別して閉じます。
