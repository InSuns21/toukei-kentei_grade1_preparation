...
2. 桁木を prefix ごとのランダム置換で組み替える **入れ子一様スクランブル**
...
> **停止線**
>
> 本章では一次のデジタルネットをランダム化し、不偏性・分散・反復標準誤差・ANOVA 接続までを扱います。滑らかさを複数の非零桁位置で測る高次 Walsh 重みと higher-order digital net は QMC8 へ送ります。
...
$$
\boxed{
\widehat{\operatorname{SE}}_{\mathrm{RQMC}}
=
\frac{S_R}{\sqrt R}
}
$$
...
全座標へ同じランダム置換族を共有して点集合全体を変換する操作を **入れ子一様スクランブル** と呼ぶ。
<!-- formal-statement-end -->
...
これは MC3 で、反対変量法や層化抽出が「独立性を絶対視せず、よい依存を作って分散を下げる」方法だったことともつながっています。
...
この \(f_u\) を \(u\) に対応する **関数的 ANOVA 成分** と呼ぶ。
<!-- formal-statement-end -->
...
がそのまま ANOVA 分解になっています。
<!-- definition-example-end -->

<a id="thm-qmc7-functional-anova-orthogonality"></a>
<!-- formal-statement-start -->
### 定理（関数的 ANOVA 分解の直交性と分散分解）

\(f\in L^2([0,1)^s)\) とし、上の ANOVA 成分 \(f_u\) を取る。
...
この例では第二座標の主効果が最も大きいことが、ANOVA 分解から直接見えます。

---

## 10. ANOVA と Walsh 周波数の支持を対応させる
...
ANOVA 成分 \(f_u\) は \(x_u\) だけに依存し、さらに各 active coordinate \(j\in u\) について平均0です。
...
<!-- formal-statement-start -->
### 系（デジタルシフト分散の ANOVA 支持分解）

QMC7 の [デジタルシフト QMC の厳密分散公式](#thm-qmc7-shift-exact-variance) の仮定を満たす \(f\) を取り、その ANOVA 成分を \(f_u\) とする。
...
なので、各 \(u\) について RQMC 分散へ寄与するのは、その ANOVA 成分の Walsh エネルギーのうち双対ネットに残った部分だけである。
<!-- formal-statement-end -->
...
ANOVA 成分と Walsh 支持の対応により、そのグループ全体のエネルギーが \(\sigma_u^2\) です。
...
次に ANOVA 直交分解から \(f_u\) の Walsh 係数は
...
3. \(\widehat{\operatorname{SE}}_{\mathrm{RQMC}}\) を求めよ。
...
5. \(\overline Z_4\)、\(S_4^2\)、\(\widehat{\operatorname{SE}}_{\mathrm{RQMC}}\) を求めよ。
6. 理論上の \(\operatorname{SE}(\overline Z_4)\) と標本からの推定値を比較せよ。
7. 周波数1の係数4が最も大きいにもかかわらず RQMC 分散へ寄与しない理由を説明せよ。
8. この4回の反復内にある合計8点を iid 8標本として標本分散を作ってはいけない理由を説明せよ。
...
合計8個の点値を iid 標本として扱うと、QMC が意図的に作った依存構造を無視することになります。
