...
I,T,T^2,\dots,T^{n^2}
$$
は一次従属で、全ては0でない係数 $a_0,\dots,a_{n^2}$ が存在して
$$
...
m_T\mid p.
$$
$p$ は相異なる一次因子を一度ずつしか持たないので、その首一約数 $m_T$ も相異なる一次因子の積です。
...
\deg r_{k+1}<\deg r_k.
$$
非零余りの次数は厳密に下がるので有限回で終わります。最後の非零余りは $f,g$ の最大公約多項式の定数倍です。互いに素なので定数倍して1とできます。

各式を
$$
r_{k+1}=r_{k-1}-q_kr_k
$$
と書き、最後から逆代入します。各余りは直前二つの余りの多項式係数線形結合なので、最後の1も最初の $f,g$ の多項式係数線形結合になります。従って
$$
1=af+bg.
$$
$\square$
...
またJordanブロックは上三角なのでLA3の三角行列の行列式から
$$
\chi_{J_k(\lambda)}(t)=(t-\lambda)^k.
$$
従って固有値 $\lambda$ に属する全ブロックのサイズ総和は、特性多項式中の $(t-\lambda)$ の指数、すなわち代数的重複度です。
