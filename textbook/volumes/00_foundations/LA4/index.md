...
\end{cases}
$$
で、補助事実が示されました。この議論は成分が多項式でも同じなので $M=tI-A$ に使えます。
...
af+bg=1
$$
と書けます。これはEuclidの互除法を逆向きにたどれば得られます。実際、互除法を
$$
r_{-1}=f,\qquad r_0=g,
$$
$$
...
$$
と続けると、最後の非零余りは $\gcd(f,g)$ の定数倍です。$f,g$ が互いに素なので、その最後の余りを定数倍して1とできます。そして各式を
$$
r_{k+1}=r_{k-1}-q_kr_k
$$
と逆代入していけば、最後の1が最初の $f,g$ の多項式係数線形結合
$$
1=af+bg
$$
として表されます。
...
f_1\cdots f_r\mid h
$$
です。まず2因子の場合を示します。$h=f_1c$ かつ $f_2\mid h$ とします。補助事実1から
$$
af_1+bf_2=1
$$
と書けます。両辺に $c$ を掛けると
...
h=f_1f_2d.
$$
よって $f_1f_2\mid h$ です。これを $f_1f_2$ と $f_3$、次に $f_1f_2f_3$ と $f_4$、というように繰り返せば一般の場合を得ます。各段階で、すでに掛け合わせた積と次の因子も互いに素です。
...
$p_j$ と $q_j$ は共通因子を持たないので、補助事実1により多項式 $a_j,b_j$ が存在して
$$
...
$$
つまり $h$ は全ての $p_k$ で割り切れます。$p_1,\dots,p_r$ は2つずつ互いに素なので補助事実2から
$$
