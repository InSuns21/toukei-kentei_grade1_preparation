...
という刻み幅制約が現れます。一方、後退 Euler 差分法は任意の $r>0$ で安定です。さらに陽的更新と陰的更新を一つの重み付き時間離散で統一し、重みをどう選べば刻み幅制約が消えるかまで求めます。

直接の前提は、[FDM1 の熱方程式差分法](../FDM1/index.md#def-fdm1-ftcs)と、[NA7 の安定関数・theta 法](../NA7/index.md#def-na7-theta-method)です。

> **この章の停止線**  
> 本章では摂動の増幅、周波数ごとの安定性判定、陽解法の刻み幅制約、陽的・陰的時間離散の安定性比較を扱います。局所打切り誤差を一般に定義し、「整合性＋安定性から収束」を大域誤差評価へつなぐ議論は FDM3 で扱います。
...
CFL という名称は方程式の型によって現れ方が異なります。本章では名称の一般論には踏み込まず、熱方程式の陽的差分法で現れる

$$
r\le\frac12
$$

を **FTCS の拡散 CFL 条件**と呼び、時間刻みと空間刻みを結ぶ安定性制約として扱います。
...
つまり FTCS の拡散 CFL 条件は Fourier 解析だけから偶然出てきたわけではありません。
...
### 例：$\theta=1/2$ では新旧時刻の中心二階空間差分を平均する
...
したがって時間区間の両端で中心二階空間差分を評価し、その平均で1ステップ進めます。
<!-- definition-example-end -->
...
$a$ が大きいモード、すなわち高周波側で特に $r$ も大きい設定では、1ステップで強く減衰します。
...
絶対値は1を超えないので安定ですが、$a$ が大きい高周波側のモードは符号を交互に変えながら残りやすいことが分かります。
