...
双曲型方程式での古典的 CFL 条件は数値的な依存領域と PDE の依存領域を比較する因果構造の条件です。熱方程式には有限伝播速度がありません。
...
この観察を境界条件込みの離散最大値原理へ整理するのは FDM3 です。
...
### 例：$\theta=1/2$ では新旧時刻の離散 Laplacian を平均する
...
したがって時間区間の両端で空間離散 Laplacian を評価し、その平均で1ステップ進めます。
<!-- definition-example-end -->
