...
6. hidden test は最終値だけでなく、点集合・双対条件・収束次数など理論上の性質を検査する。
...
**穴埋め課題：** 2座標の各2進桁を読み出し、出力整数の奇数・偶数位置へ交互に配置し、log-log 傾きから収束次数を求めてください。
...
| QMC8 | 桁交互配置で高次デジタル構成を作れる | 通常構成と交互配置後の実測収束次数 |
