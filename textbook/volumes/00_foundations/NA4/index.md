...
この定義は「表を作る漸化式」と同値です。その漸化式を次で導きます。
...
<a id="prop-na4-divided-difference-relation"></a>
<!-- formal-statement-start -->
### 命題（分割差分の漸化式）
...
隣り合う零点ごとに [Rolle の定理](../RA3/index.md#thm-ra3-rolle)を使うと、$g'$ は少なくとも $n+1$ 個の零点を持ちます。

さらにそれらの隣り合う零点へ [Rolle の定理](../RA3/index.md#thm-ra3-rolle)を使うと、$g''$ は少なくとも $n$ 個の零点を持ちます。
...
- **$\omega_{n+1}$ の最高次係数が1**：$(n+1)$ 階微分がちょうど $(n+1)!$ になる。
...
$t_0,\dots,t_m$ は $[-1,1]$ に並ぶ $m+1$ 個の異なる点なので、[中間値定理](../RA2/index.md#thm-ra2-ivt)から隣り合う点の各区間に少なくとも一つずつ根があります。
...
4.
   ...
   $$
   を求めよ。
5. この値が 最高次係数が1の4次多項式の最小可能な区間での絶対値の最大値である理由を本文の最小最大定理から説明せよ。
...
[最高次係数1の多項式に対する Chebyshev の最小最大性](#thm-na4-chebyshev-minimax)により、これは最高次係数が1の $n+1$ 次節点多項式の区間での絶対値の最大値として最小可能です。
...
Chebyshev 節点は $[-1,1]$ 上で節点多項式の区間での絶対値の最大値を最小にし、等間隔高次補間で現れる Runge 現象を理解するための基準になります。
