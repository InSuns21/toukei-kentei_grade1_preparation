...
$q$ と $q_*$ はどちらも monic な $m$ 次多項式なので、最高次項が打ち消し合い、
...
次数 $n$ の補間では節点多項式は monic な $n+1$ 次多項式です。
...
4.
   ...
   $$
   を求めよ。
5. この値が monic な4次多項式の最小可能な一様ノルムである理由を本文の最小最大定理から説明せよ。
...
本文の Chebyshev 最小最大定理で $m=4$ とすると、monic な4次多項式 $q$ は必ず
...
Chebyshev 最小最大定理により、これは monic な $n+1$ 次節点多項式の一様ノルムとして最小可能です。
...
次の [NA5 数値積分・直交多項式・Gauss 型積分](../NA5/index.md) では、補間多項式を積分するという発想から Newton--Cotes 型公式へ進み、さらに節点そのものを積分精度のために設計する Gauss 型積分へ進みます。
