...
はコンパクトです。ある $\rho>0$ を取り、その閉 $\rho$ 近傍

$
K
=
\{y:\operatorname{dist}(y,\Gamma_T)\le\rho\}
$

を考えます。$F\in C^1$ かつ $K$ はコンパクトなので
...
まず $x_h$ が $K$ から出ないことを確認します。$x_h$ の $K$ からの最初の退出時刻を $\tau\le T$ と仮定します。$0\le t\le\tau$ では $x(t),x_h(t)\in K$ なので平均値の積分形を使えて
...
$\|A_h\|\le L$ なので [Grönwall の不等式](#lem-ode8-gronwall)から、少なくとも $0\le t\le\tau$ では
...
となるように $h$ を取っておけば、$t=\tau$ でも $x_h(\tau)$ は基準軌道 $x(\tau)$ から距離 $\rho$ 未満です。これは $K$ の境界へ初めて到達したという定義に反します。従って退出時刻は存在せず、$x_h(t)\in K$ が $0\le t\le T$ 全体で成り立ちます。
