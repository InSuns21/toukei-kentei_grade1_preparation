...
**greedy戦略が最大横断数を実現すること。**  
経路を一つ固定し、greedyに選ぶ買い時刻・売り時刻を

$$
s_1<t_1<s_2<t_2<\cdots
$$

とします。すなわち $s_1$ は $M_k\le a$ となる最初の時刻、$t_1$ は $s_1$ より後で $M_k\ge b$ となる最初の時刻で、その後も同じ規則を繰り返します。

一方、任意の admissible な $r$ 回の上向き横断を

$$
s'_1<t'_1<\cdots<s'_r<t'_r
$$

とします。最初の買い時刻の最小性から $s_1\le s'_1$、さらに $t_1$ は $s_1$ より後の最初の $b$ 到達時刻なので $t_1\le t'_1$ です。帰納的に $t_{j-1}\le t'_{j-1}$ と仮定すると、$t'_{j-1}<s'_j$ だから greedy の次の買い時刻は $s_j\le s'_j$、続く最初の売り時刻も $t_j\le t'_j$ となります。従って任意の $r$ 回の横断が存在すればgreedyも少なくとも $r$ 回を完成できます。

greedy自身が定義を満たす横断列なので、その完成回数は $U_N(a,b)$ 以下です。一方、上の議論から最大値 $U_N(a,b)$ 回の任意の横断列に対してgreedyも同数を完成できるため、**greedyの完成回数はちょうど $U_N(a,b)$** です。

完成した1回の売買では買値が $\le a$、売値が $\ge b$ なので利益は少なくとも $b-a$。
...
まずこのgreedy規則が $U_N(a,b)$ 回を本当に完成することを確認する。greedyの買い・売り時刻を $s_j,t_j$、任意の admissible な $r$ 回の横断を $s'_j,t'_j$ とする。最初の買い・売りをそれぞれ最も早く選ぶ定義から
...
さらに $t_{j-1}\le t'_{j-1}$ なら $t'_{j-1}<s'_j$ だから、次のgreedy買い時刻は $s_j\le s'_j$、その後の最初の売り時刻も $t_j\le t'_j$。帰納法により、任意の $r$ 回の横断があればgreedyも少なくとも $r$ 回を完成する。greedy自身もadmissibleなので、その完成回数は最大値 $U_N(a,b)$ と一致する。
...
2. B01ではgreedy戦略が最大横断数 $U_N(a,b)$ を実現することまで確認した。そのupcrossing評価から各有理 $a<b$ について $E[U_\infty(a,b)]<\infty$、従って $U_\infty(a,b)<\infty$ a.s.
