...
従って $f_n\to f$ は一様収束です。一様極限の連続性を直接確認すると、$x_0\in X$ と $\varepsilon>0$ に対し $n$ を
...
ここで重要なのは、空間 $X$ 自体の compact 性を仮定していないことです。完備性は「一様極限」と「各 $f_n$ が大きくなる場所は compact」という二つを組み合わせて出ています。
...
となる。ここで $T^+$ は正錐上では正部分 envelope で与えられ、
...
正部分 envelope は前節の補題により正錐上で加法的・非負斉次です。従って正錐拡張の補題により、同じ記号 $T^+$ で表す正線形汎関数へ一意に延長できます。
...
存在は任意の有界汎関数の有限符号付き Radon 表示で証明済みです。ノルム等式も直前の定理から従います。
...
は有限 Radon 測度二つの差です。従って前節の補題により $\sigma$ は有限符号付き Radon 測度です。
...
$f_n\in C_0(X)$ が $f$ へ一様収束し、$f$ が連続であるとする。任意の $\varepsilon>0$ について
...
正錐上の加法性から、$u=p-q$ ($p,q\ge0$) に対し

$$
T^+(u):=T^+(p)-T^+(q)
$$

と定めます。もし $p-q=p'-q'$ なら $p+q'=p'+q$ なので、正錐上の加法性により
