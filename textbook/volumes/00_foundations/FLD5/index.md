...
> **この章の停止線**
>
> 標数 $0$ の有限 Galois 理論の範囲で、作図問題と「四則演算と有限回の根号で方程式を解けるか」という問題を閉じます。正多角形については円分体との接続と正五角形・正七角形までを扱い、Gauss--Wantzel の完全分類は数論系列へ送ります。無限 Galois 理論、Krull 位相、一般 Kummer 理論、類体論、代数的整数論、有限群の分類理論は使いません。
...
$K$ 座標を持つ2点 $(x_1,y_1),(x_2,y_2)$ を通る直線は

$$
(y_1-y_2)x+(x_2-x_1)y+(x_1y_2-x_2y_1)=0
$$

と書けます。従って、その係数は全て $K$ に入ります。
...
です。したがって、作図可能な代数的数ではこの次数は必ず $2$ の冪です。作図可能性そのものの完全な判定は、前定理の「実二次拡大列へ入ること」です。次数だけを見ると、中間体がどのように積み重なるかという情報を捨ててしまいます。
...
従って Eisenstein の判定から $\Phi_p(x+1)$ は既約です。置換 $x\mapsto x+1$ は $\mathbb Q[x]$ の自己同型なので、$\Phi_p(x)$ も $\mathbb Q$ 上既約です。
...
したがって一段の根号添加を Galois 拡大として扱うには、基礎体に $n$ 乗根の全ての乗法因子、すなわち $1$ の $n$ 乗根が入っていることが重要です。
...
### 7.1 可換な段階を重ねると、全体の群は可解になる

<a id="def-fld5-normal-closure"></a>
<!-- formal-statement-start -->
> **定義（有限拡大の正規閉包）**
>
> 有限拡大 $E/F$ を固定した代数閉包の中で考える。
>
> $E$ を含む有限正規拡大 $N/F$ のうち、包含関係で最小のものを $E/F$ の **正規閉包**という。
<!-- formal-statement-end -->

<!-- definition-example-start: def-fld5-normal-closure -->
**定義の確認**

$E=F(\alpha_1,\ldots,\alpha_s)$ とし、各 $\alpha_i$ の $F$ 上の最小多項式を $m_i$ とします。
...
の分解体 $N$ は $E$ を含む有限正規拡大です。逆に、$E$ を含む任意の有限正規拡大は各 $m_i$ の全ての共役根を含むので $N$ を含みます。

従ってこの $N$ が正規閉包です。標数 $0$ では分離性もあるため $N/F$ は有限 Galois 拡大になります。
<!-- definition-example-end -->
...
なので [正規部分群と正規中間拡大の対応](../FLD4/index.md#thm-fld4-normal-subgroup-correspondence) から
...
[群の Lagrange の定理](../GRP2/index.md#thm-grp2-lagrange)から部分群の位数は $1$ または $p$ しかないため、部分群自体が
...
一方 [群の Lagrange の定理](../GRP2/index.md#thm-grp2-lagrange)から

$$
|N|\mid60.
$$

二つの $12$ を区別して選ぶことに注意すると、全ての相異なる候補は
...
ここで次数 $4$ と $5$ の間に、単なる計算量の増加ではない構造的な分岐点が現れます。
...
は素数なので、[群の Lagrange の定理](../GRP2/index.md#thm-grp2-lagrange)から非自明部分群は $A_3$ 全体しかありません。
...
一つの根の軌道の大きさは5なので、[軌道・安定化群公式](../GRP3/index.md#thm-grp3-orbit-stabilizer)から
