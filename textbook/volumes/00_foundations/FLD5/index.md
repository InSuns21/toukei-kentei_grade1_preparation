...
という有限 Fourier 変換を作ります。これが **Lagrange resolvent** です。
...
5-cycle $c$ の $S_5$ における中心化群は

$$
\langle c\rangle
$$

で位数 $5$ です。5-cycle は偶置換なので、この中心化群は全て $A_5$ に入ります。
...
2. 推移的な5点作用から群の位数は5で割れる。Cauchy の定理で 5-cycle が得られる。
3. $f$ が実根をちょうど3個持つことを微分で示す。複素共役は3個の実根を固定し、残る非実根2個を交換するので transposition になる。

5-cycle と transposition が同じ部分群に入れば、その部分群は $S_5$ 全体です。
...
$S_5$ の中で位数5の置換は 5-cycle です。

従って $G$ は一つの 5-cycle を含みます。
...
#### Step 3：複素共役が transposition を与える
...
に対応する5個の transposition を全て含みます。

$5$ は素数で $d\not\equiv0$ なので、$d$ を繰り返し加えると5頂点全てを巡ります。従ってこの辺グラフは連結です。

連結グラフの辺 transposition は全対称群を生成します。実際、道

$$
v_0-v_1-\cdots-v_m
$$

に沿って隣接 transposition を共役していけば端点 transposition $(v_0\,v_m)$ を作れ、固定した一頂点と全頂点を結ぶ transposition から任意の置換を生成できます。
...
2. $G$ は5個の根へ推移的に作用し、5-cycle を含む。
3.
   $$
   a=(4/5)^{1/4}
   $$
   と置き、$f(-a)>0>f(a)$ を示して、$f$ がちょうど3個の実根を持つことを示せ。
4. 複素共役が $G$ の中の transposition を与えることを示せ。
5. 5-cycle と transposition を含む $G$ が $S_5$ 全体であることを示せ。
...
5点の置換で位数5を持つものは 5-cycle なので、

$$
\boxed{G\text{ は 5-cycle を含む}}
$$
...
3個の実根は固定し、残る2個の非実根だけを交換するので、根の置換としては transposition です。

従って

$$
\boxed{G\text{ は transposition を含む}}.
$$

5. $G$ に含まれる 5-cycle を $r$、transposition を $\tau$ とします。
...
連結グラフの辺 transposition は全対称群を生成します。よって
...
9. 逆に、原始 $p$ 乗根を含む体上の巡回素数次数 Galois 拡大は Lagrange resolvent により一段の $p$ 乗根添加として書ける。
10. 以上から標数 $0$ では
    $$
    \boxed{
    \text{多項式が根号によって可解}
