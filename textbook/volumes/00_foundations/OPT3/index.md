...
> **この章の停止線**
>
> Fenchel 共役・Fenchel--Young の不等式・双対問題・KKT 乗数はまだ使いません。OPT4 以降では、本章の劣微分・法錐・接錐を既知として双対性へ進みます。
...
最小化列の極限で関数値が突然下へ抜けると、極限点へ最小値を渡せません。その落下を防ぐ条件が下半連続性です。
...
$m=\inf_x f(x)$ とし、$f(x_k)\downarrow m$ となる最小化列を取ります。十分大きい $k$ では $f(x_k)\le f(x_0)$ なので $x_k\in L$ です。コンパクト性から部分列 $x_{k_j}\to x^\ast\in L$ が存在します。下半連続性より
...
実効定義域が $\mathbb R^n$ より低い次元に載ることがあります。そのとき通常の内部は空でも、定義域の中では十分に「内側」の点を扱いたいので相対内部を使います。
...
について、$\operatorname{dom}f$、$\operatorname{epi}f$、$L_\alpha(f)$ を求めよ。さらに $f$ が閉真凸関数であることを確認せよ。
