...
PDE5 では長方形だけで、内部源の総量と境界法線微分の積分関係を直接計算しました。発散定理があれば同じ関係を領域一般へ拡張できます。
...
この明示式は「源点 $x$ に対応する鏡像点 $x^*$ を円板の外側へ置き、境界値を打ち消す」という像法の形になっています。
...
PDE5 までの楕円型理論を Green の視点で並べ直すと、次の対応が見えます。
