...
<!-- definition-example-start: def-geo2-germ -->
**定義の確認**
...
<!-- formal-statement-start -->
> **定義（滑らかな実ベクトル束）**  
> 滑らかな多様体 $M$ 上の階数 $k$ の滑らかな実ベクトル束とは、滑らかな多様体 $E$ と滑らかな全射
>
> $$
> \pi:E\to M
> $$
...
は各ファイバー上で線形同型であり、局所自明化を与えます。よって $TM$ は階数 $n$ のベクトル束です。$\square$
<!-- proof-end -->
...
<!-- formal-statement-start -->
> **命題（余接束の局所自明化）**  
> $M$ が $n$ 次元滑らかな多様体なら、$T^*M$ は自然に $2n$ 次元滑らかな多様体となり、$M$ 上の階数 $n$ のベクトル束になる。
>
...
従って接束の場合と同様に滑らかなアトラスが得られ、各ファイバーで線形な局所自明化を持つので階数 $n$ のベクトル束になります。$\square$
<!-- proof-end -->
...
$v\in T_x\mathbb R^{n+1}\cong\mathbb R^{n+1}$ に対し、Euclid 空間で微分すると
...
従って $dq_x(v)$ は球面の半径方向に直交する方向へ送られます。

特に $\|p\|=1$ なら、$w\perp p$ に対して
...
1. $t=0$ における速度を ambient な $\mathbb R^2$ のベクトルとして求めよ。
...
5. 2より球面上の曲線の速度は半径方向に直交する方向へ入ります。4より任意の $w\in p^\perp$ は実際に球面上の曲線の速度として実現できます。従って
   ...
   この同一視は球面の内在的な接空間を ambient な Euclid 空間の部分空間として具体化したものです。
<!-- solution-end -->
