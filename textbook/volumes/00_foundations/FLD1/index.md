...
> **この章の停止線**
>
> 本章では一つの根を添加して得られる体までを閉じます。多項式の全ての根を同時に入れる分解体、分離拡大、正規拡大は FLD2 で扱います。
...
「新しい元を一つ加えた」ことと「次数が1増える」ことは同じではありません。$\sqrt2$ 一個を加えた結果でも、次元は $2$ です。後で一般に、$\alpha$ を消すモニック既約多項式を選ぶと、その次数がこの次元になることを示します。
...
逆は一般には「一つの元が代数的なら任意の拡大 $K/F$ が有限」とは言えません。しかし一つの代数的元だけで生成した $F(\alpha)$ は有限になります。その次数を決める多項式を、次節で評価準同型から構成します。

---

## 5. 評価準同型の核から最小の関係式を選ぶ
...
>
> $\alpha$ を根に持つ $F[x]$ のモニック多項式のうち次数が最小のものを **代数的元の最小多項式**といい
>
$$
m_{\alpha,F}(x)
$$
...
<!-- formal-statement-start -->
> **定理（代数的元の最小多項式の存在・一意性と既約性）**
>
> 体拡大 $K/F$ と、$F$ 上代数的な元 $\alpha\in K$ を考える。
>
> このとき $m_\alpha\in F[x]$ は存在して一意であり、$F[x]$ で既約である。
>
...
生成元をモニックに正規化したものを $m_\alpha$ とします。既約性は、もし $m=gh$ と非自明に分解できたなら
...
最後に一意性を示します。同じ条件を満たす別のモニック多項式 $m'$ があれば
...
> **定理（単純代数拡大の商多項式環表示）**
>
> 体拡大 $K/F$ と、$F$ 上代数的な元 $\alpha\in K$ を考え、その **代数的元の最小多項式**を $m_\alpha$ とする。
>
> このとき
>
$$
...
従って $m_{\alpha,\mathbb Q}(x)=x^3-2$ で、
...
#### FLD1-A02 $i$ に対する $m_{i,\mathbb R}$
- Level: A

$\mathbb C/\mathbb R$ を考え、$i^2=-1$ とする。

1. $m_{i,\mathbb R}(x)$ を求めよ。
...
従って $\sqrt3\notin K$ です。$\sqrt3$ は $K$ 上で $x^2-3$ の根なので、$m_{\sqrt3,K}$ は次数 $2$ です。
...
#### FLD1-B01 $m_\alpha$ は全ての消去多項式を割る
- Level: B

$\alpha$ を $F$ 上代数的とし、その **代数的元の最小多項式**を $m_\alpha$ とする。
...
仮定から $f(\alpha)=0$、$m_\alpha(\alpha)=0$ なので
...
2. $m_{\alpha,\mathbb Q}(x)$ と $[K:\mathbb Q]$ を求めよ。
3. $K$ の任意の元の標準形を書け。
4. $\alpha-1$ の逆元を求めよ。
...
有限拡大では各元 $\alpha$ が必ず多項式関係を持ち、その関係の正本として $m_\alpha$
...
次の FLD2 では、一つの根だけでなく多項式の全ての根を入れる **分解体**へ進み、重根・分離拡大・正規拡大を整理します。
