...
連続Fourier解析
      │
      ├── 確率分布 ── 特性関数 ── 和の分布・中心極限定理
      │
      └── 標本列 ── 有限周波数表示 ── 周期的な畳み込み・高速計算・周波数の重なり
```

確率側では、すでに [特性関数](../F0_00P6_特性関数_中心極限定理/index.md#def-f0-00p6-characteristic-function) と [独立同分布中心極限定理](../F0_00P6A_iid_中心極限定理/index.md#thm-iid-clt) が正本として証明済みです。本章ではそれらを重複証明せず、特性関数を確率測度の Fourier 変換として読み直し、和の分布を周波数領域の積へ移す対応を閉じます。

信号処理側では、有限標本列の周波数表示を定義だけで済ませません。有限指数和の消去則から復元公式とエネルギー等式を導き、周期境界での畳み込みが周波数ごとの積へ変わることを証明します。そのうえで同じ周波数変換を高速に計算する仕組みと、等間隔標本化で異なる連続周波数が同じ標本列へ潰れる現象を式で確認します。

> **証明境界**  
> Shannon の標本化定理、Poisson 和公式、Schwartz 空間、緩増加超関数は本章の証明依存には入れません。この周波数の取り違え現象は「純粋な複素指数波を等間隔で標本化すると何が区別不能になるか」という有限計算で閉じます。
...
三つの和は1で、独立で 0 と 1 にだけ値をとる二つの確率変数の和の分布そのものです。
<!-- definition-example-end -->
...
4. $r=2$ で反転公式で復元し、1. と一致することを確認せよ。
