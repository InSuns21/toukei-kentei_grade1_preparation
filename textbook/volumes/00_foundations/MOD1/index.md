...
[RNG4](../RNG4/index.md) までで、環とそのイデアル、整除、一意分解、多項式環を学びました。[LA2](../LA2/index.md) では、体上のベクトル空間について部分空間・商空間・[第一同型定理](../LA2/index.md#thm-la2-first-isomorphism)を扱いました。
...
一方で、商・準同型・核と像を結ぶ同型という骨格はそのまま残ります。
...
という $F[x]$ の作用として読み替えます。これが MOD2 で Smith 標準形と PID 上の構造定理を線形自己写像の分類へ結び付ける入口です。

> **この章の停止線**
>
> 本章では環の作用、閉じた部分構造、剰余類、核と像を結ぶ同型、基底を持つ場合、ねじれ、線形自己写像から作る $F[x]$ の作用まで扱います。Smith 標準形、PID 上の構造定理、有限生成 Abel 群の分類は MOD2 へ送ります。
...
従って[加群の第一同型定理](#thm-mod1-first-isomorphism)から
...
3. [加群の第一同型定理](#thm-mod1-first-isomorphism)から商加群を同定せよ。
...
2. 
   ...
   $$
   を示せ。
3. [加群の第一同型定理](#thm-mod1-first-isomorphism)を使って
   $$
   \mathbb Z^2/
   \bigl(
   \mathbb Z(6,0)+\mathbb Z(-2,1)
...
$\varphi(1,0)=[1]$ なので、この元は[加群の第一同型定理](#thm-mod1-first-isomorphism)の同型の下で $[1]$ に対応します。
...
- [加群の第一同型定理](#thm-mod1-first-isomorphism)は
  $$
  M/\ker\varphi
  \cong
  \operatorname{Im}\varphi
