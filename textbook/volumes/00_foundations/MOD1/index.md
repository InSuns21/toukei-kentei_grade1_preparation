...
という $F[x]$ の作用として読み替えます。これが MOD2 で Smith 標準形と PID 上有限生成加群の構造定理を線形作用素の分類へ結び付ける入口です。

> **この章の停止線**
>
> 本章では加群の基本構造、商、第一同型定理、自由加群、ねじれ、線形作用素から作る $F[x]$-加群まで扱います。Smith 標準形、PID 上有限生成加群の構造定理、有限生成 Abel 群の分類は MOD2 へ送ります。
...
### 1.3 イデアルは環自身の部分加群になる

環 $R$ を左 $R$-加群として自分自身に作用させます。

左イデアル $I$ は、加法について部分群で、任意の $r\in R$、$x\in I$ に対して
...
従って左イデアルは、ちょうど正則左加群 $ {}_R R$ の部分加群です。

可換環なら、通常のイデアルと $R$ の部分加群が一致します。
...
## 11. 線形作用素は $F[x]$ を作用させる

[RNG4 の多項式環](../RNG4/index.md#def-rng4-polynomial-ring-degree)を、線形作用素へ作用させます。

$F$ を体、$V$ を $F$-ベクトル空間、$T:V\to V$ を線形作用素とします。
...
<!-- formal-statement-start -->
> **命題（線形作用素の $F[x]$-加群と不変部分空間）**
>
> $F$ を体、$V$ を $F$-ベクトル空間、$T:V\to V$ を線形作用素とする。
>
...
>
> さらに $F$-線形部分空間 $W\subset V$ に対して、次は同値である。
>
> 1. $W$ はこの $F[x]$-加群の部分加群である。
> 2. $W$ は $T$-不変、すなわち
>
$$
T(W)\subset W
...
逆に $T$-不変なら $T^k$ でも不変なので、任意の多項式 $p(T)$ でも閉じます。
...
に対して各 $a_kT^kw$ は $W$ に属し、その有限和も $W$ に属します。
...
MOD2 では、この $F[x]$-加群の構造を PID 上有限生成加群の構造定理で解析し、線形作用素の標準形との関係を整理します。
...
したがって $F[x]$-部分加群を調べることは、$T$-不変部分空間を調べることと同じです。

この翻訳が、加群構造論を線形作用素の分類へ運ぶ橋になります。
...
[線形作用素の $F[x]$-加群と不変部分空間](#prop-mod1-polynomial-module)により、部分空間が $\mathbb Q[x]$-部分加群であることと $T$-不変であることは同値です。
...
- 線形作用素 $T$ から
  ...
  p(x)\cdot v=p(T)v
  $$
  と定めると $V$ は $F[x]$-加群になり、$F[x]$-部分加群は $T$-不変部分空間と一致します。
...
次は **MOD2「Smith 標準形・PID 上有限生成加群」** です。そこで行列の基本変形を加群の生成関係へ読み替え、有限生成 Abel 群と線形作用素の構造を一つの定理から回収します。
