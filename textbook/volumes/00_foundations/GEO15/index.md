...
初期条件 $(p,v)$ の測地線について、任意の $T>0$ を取ります。

[GEO14 のアフィン再パラメータ化](../GEO14/index.md#prop-geo14-affine-reparam)より
...
$Tv\in T_pM=\mathcal D_p$ なので、正方向へ任意の有限時刻まで測地線を定義できます。

負方向は初期速度 $-v$ を使えば同様です。

従って最大測地線は $\mathbb R$ 全体で定義されます。$\square$
<!-- proof-end -->
...
<a id="prop-geo15-proper-implies-complete"></a>
<!-- formal-statement-start -->
...
[閉有界集合がコンパクトなら距離完備](#prop-geo15-proper-implies-complete)で証明済みです。
...
<a id="cor-geo15-properness"></a>
<!-- formal-statement-start -->
> **系（完備 Riemann 多様体は proper）**  
...
つまり cut point は「測地線が存在しなくなる点」ではなく、「その放射測地線がそれ以上は大域的最短でなくなる境界」です。
<!-- definition-example-end -->

### cut point と共役点は同じではない

GEO17 では Jacobi 場と共役点を扱います。

cut point が生じる機構には少なくとも
...
本章では cut locus を「最短性が壊れる場所」として定義するところまでに留め、指数写像の特異性や共役点との精密な関係は GEO17 へ送ります。
...
7. cut point が「測地線の存在終了点」ではなく「大域的最短性の終了点」であること。
...
その後 GEO17 で変分公式・Jacobi 場・共役点を導入すると、本章で入口だけ定義した cut locus と「なぜ測地線が最短でなくなるのか」を、変分の立場からさらに精密に解析できます。
