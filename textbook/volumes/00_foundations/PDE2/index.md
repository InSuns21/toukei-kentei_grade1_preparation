...
> **証明境界**  
> 本章は二変数の古典的な二階線形 PDE を対象とし、分類・線形座標変換・標準形・データの置き方の意味までを扱います。一般の多変数楕円型作用素、弱微分、Sobolev 空間、Lax--Milgram、弱解は Encore III 以降へ送ります。熱・波動・Laplace 方程式そのものの解法と一意性証明は PDE3--PDE5 で行います。
...
も、独立変数を $(x,t)$ と見れば二変数の二階線形 PDE です。時間微分 $u_t$ は一階なので、二階の主部には入りません。
...
「$x,y$ という名前の付け方で elliptic が hyperbolic に変わる」なら分類は役に立ちません。実際には、可逆な座標変換で型は保たれます。
...
<!-- formal-statement-start -->
> **定理（可逆線形座標変換による型の不変性）**  
> 二階線形 PDE の主係数行列を $M$ とする。可逆線形座標変換
>
> $$
> (\xi,\eta)^T=J(x,y)^T
> $$
...
> $$
>
> よって楕円型・放物型・双曲型の別は可逆線形座標変換で変わらない。
<!-- formal-statement-end -->

### 証明の見取り図

線形変換では $\xi_x,\xi_y,\eta_x,\eta_y$ が定数なので、二階微分を連鎖律で変換しても余分な一階項は生じません。主二次形式の係数は $J M J^T$ という合同変換を受けます。最後に行列式の積の公式を使います。
...
とします。$u(x,y)=U(\xi,\eta)$ と置くと

$$
u_x
=
\xi_x U_\xi+\eta_x U_\eta,
\qquad
u_y
=
\xi_y U_\xi+\eta_y U_\eta.
$$

$\xi_x,\xi_y,\eta_x,\eta_y$ は定数なので、もう一度微分すると二階微分の係数だけが積として現れます。
...
$U_{\xi\eta}$、$U_{\eta\eta}$ の係数も同様にまとめると、新しい主係数行列は
...
$(\det J)^2>0$ なので $\mathcal D$ の符号は変わりません。
<!-- proof-end -->

> **補足**  
> 非線形の滑らかな座標変換でも、主部は各点で Jacobian による同型の変換を受けます。座標関数の二階微分から生じる項は一階項へ落ちるため、主部の型は局所的に保存されます。本章では、この一般形を使う前に必要な線形変換だけを完全に確認しました。
...
型分類は、どのデータ配置が自然かを考える最初の手掛かりになります。ただし、**型だけを見れば well-posedness が自動的に決まるわけではありません**。領域、境界、低階項、係数の正則性、どのノルムで測るかも必要です。
...
## 11. Laplace方程式のCauchy問題は不安定になり得る
...
<!-- formal-statement-start -->
> **命題（Laplace方程式のCauchyデータに対する不安定性の例）**  
> $y_0>0$ を固定し、
>
> $$
> u_n(x,y)
...
> $$
>
> 従って Cauchy データは最大値ノルムで 0 へ近づくが、任意の固定した $y_0>0$ で
>
> $$
> \sup_x|u_n(x,y_0)|
> =
...
> $$
>
> したがって、このデータの測り方と内部の最大値ノルムに関して、Laplace 方程式の Cauchy 問題はデータへ連続依存しない。
<!-- formal-statement-end -->

### 証明の見取り図

$\sin(nx)$ は $x$ 微分で $-n^2$ を生み、$\sinh(ny)$ は $y$ 微分で $+n^2$ を生みます。境界では $\sinh(0)=0$ によって関数値が消え、法線方向微分も $1/n$ まで小さくできます。しかし内部では $\sinh(ny_0)$ が指数的に増えます。
...
これは「Laplace 方程式は解けない」という意味ではありません。PDE5 で扱う Dirichlet 問題のように、**楕円型に適した境界データ**を与える問題は良い性質を持ちます。
...
9. 実際の well-posedness は、型だけで決めず、領域・データ・ノルム・境界条件まで確認する。
...
2. 変数を $(x,t)$ とします。
   ...
   \mathcal D=0.
   $$
   放物型です。時間について一階なので、初期温度 $u(0,x)$ と必要な空間境界条件を与える形が自然です。
...
3. 変換後は
   ...
   -21<0.
   $$
   楕円型のままです。今回の変換は直交変換なので $|\det J|=1$ で、実際に判別式の値そのものも変わっていません。
<!-- solution-end -->
...
1. $u_n$ は Laplace 方程式を満たす。
2. $y=0$ での $u_n$ と $(u_n)_y$ は最大値ノルムで 0 へ近づく。
...
6. $|a|<1$, $|a|=1$, $|a|>1$ の三領域で、Laplace・heat・wave のどの主部に近いかを説明せよ。
...
- 主係数行列 $M$ と $\det M=-\mathcal D$ の関係を説明できる。
...
- Laplace 方程式の Cauchy 問題が、具体列によってデータへの連続依存を失い得ることを示せる。
