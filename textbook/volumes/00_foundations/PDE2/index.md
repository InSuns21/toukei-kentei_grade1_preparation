...
- 楕円型の代表式 $u_{xx}+u_{yy}=0$ は、領域の**境界**から内部を決める問題へ向かう。
...
<!-- formal-statement-start -->
> **命題（楕円型の代表式のCauchyデータに対する不安定性の例）**  
> $y_0>0$ を固定し、
>
> $$
> u_n(x,y)
...
> $$
>
> したがって、このデータの測り方と内部の最大誤差に関して、楕円型の代表式 $u_{xx}+u_{yy}=0$の Cauchy 問題はデータへ連続依存しない。
<!-- formal-statement-end -->
...
これは「楕円型の代表式 $u_{xx}+u_{yy}=0$は解けない」という意味ではありません。PDE5 で扱う Dirichlet 問題のように、**楕円型に適した境界データ**を与える問題は良い性質を持ちます。
...
1. $u_n$ は 楕円型の代表式 $u_{xx}+u_{yy}=0$を満たす。
...
- 楕円型の代表式 $u_{xx}+u_{yy}=0$の Cauchy 問題が、具体列によってデータへの連続依存を失い得ることを示せる。
