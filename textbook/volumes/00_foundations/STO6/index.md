...
は $t<1/4$ でのみ有限で、$t>1/4$ では有限値を取りません。従って十分長い時間区間では大域的 $L^2(B)$ 条件は失われます。
...
異なる $n$ の積分は[停止操作と確率積分の交換](#prop-sto6-stopping)により重なる区間で一致するので、標本路を貼り合わせられます。
...
$m\ge n$ なら $\tau_n\le\tau_m$ であり、[停止操作と確率積分の交換](#prop-sto6-stopping)から
...
[停止操作と確率積分の交換](#prop-sto6-stopping)から
