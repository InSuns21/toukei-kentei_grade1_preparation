...
準一様な一次有限要素メッシュ族では、有限要素関数に対する標準的な逆不等式から最大一般化固有値は
...
と評価され、最細スケールのモードでは同じ $h^{-2}$ 次数になります。

したがって前進 Euler 法の安定時間刻みは典型的に
...
となります。ここで準一様性を外すと、単一の代表メッシュ幅 $h$ だけでは最小要素の影響を表せないため、この簡潔な形の CFL 条件をそのまま使うことはできません。
