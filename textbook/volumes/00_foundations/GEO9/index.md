...
$k=1$ なので $K_H\omega$ は0形式、すなわち関数です。後で導く星型領域の公式をこの例に直接使うと
...
$f$ が少なくとも一点 $p$ で値 $c=f(p)$ を取るので $A_c$ は空でありません。[TOP3](../TOP3/index.md) の連結性より、非空で開かつ閉の部分集合は $M$ 全体しかないため
...
単射性の核心は「積分が0なら原始関数を作れる」です。周期写像
...
星型領域では Poincaré の補題により逆も成り立つので
...
1. $d\omega=0$ を確認せよ。
2. 星型領域でのホモトピー作用素の公式から $K_H\omega$ を求めよ。
3. $d(K_H\omega)=\omega$ を確認せよ。

- Level: A
- 狙い: Poincaré の補題の証明を抽象記号だけで終わらせず原始関数の構成として実行する
...
- Level: B
- 狙い: 円周の1次コホモロジーを一般定理へ丸投げせず周期関数の原始関数として証明する
...
- 星型領域の縮約ホモトピーから Poincaré の補題を証明できる。
...
- $H^1_{\mathrm{dR}}(S^1)\cong\mathbb R$ を周期関数の原始関数から直接証明できる。
