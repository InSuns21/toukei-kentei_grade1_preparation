...
一方、無限次元空間上の恒等作用素は一般にコンパクトではありません。第3節では、単位球の中に互いに一定距離以上離れた列を構成して、この事実を証明します。
...
もし $d=0$ なら $x$ は $M$ の閉包に入ります。[距離空間における閉集合の点列特徴付け](../F0_00B_距離空間_開集合_閉集合_収束/index.md#thm-f0-00b-01)より、$M$ が閉なので $x\in M$ となって矛盾します。
...
有限ランク作用素をどんどん細かくして近似で得られる例を扱うため、次の閉性が重要です。
...
証明中で Banach 性を使った場所は最後の「Cauchy 列が $Y$ 内で収束する」です。作用素ノルム近似だけで収束先が $Y$ の外へ逃げないことを保証しています。

<a id="cor-fa6-finite-rank-closure"></a>
<!-- formal-statement-start -->
### 系（有限ランク作用素の作用素ノルム収束先はコンパクト）
...
従って [有限ランク作用素の作用素ノルム収束先はコンパクト](#cor-fa6-finite-rank-closure) から $D_a$ はコンパクトです。
...
$0\le s,t\le1$ では $0\le st\le1$ なのでTaylor級数は一様収束し、たとえば
...
だから $\|T-T_N\|\to0$ です。よって [有限ランク作用素の作用素ノルム収束先はコンパクト](#cor-fa6-finite-rank-closure)から $T$ はコンパクトです。
<!-- solution-end -->
