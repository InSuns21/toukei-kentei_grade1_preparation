...
このとき $z\in B(x_{n_k},1/n_k)$ なら、[距離・距離空間の定義](../F0_00B_距離空間_開集合_閉集合_収束/index.md#def-f0-00b-01)の第三条件を $z,x_{n_k},x$ に適用して
...
3. [距離・距離空間の定義](../F0_00B_距離空間_開集合_閉集合_収束/index.md#def-f0-00b-01)の第三条件で、収束した中心の小球全体を一つの被覆要素へ押し込める。
...
となる $x_n$ を選べます。点列コンパクト性から $x_{n_k}\to x$ となる部分列を取ります。$x\in U\in\mathcal U$ を選び、開性から $B(x,r)\subseteq U$ とします。十分大きい $k$ では中心のずれも球の半径も $r/2$ 未満です。$z\in B(x_{n_k},1/n_k)$ に [距離・距離空間の定義](../F0_00B_距離空間_開集合_閉集合_収束/index.md#def-f0-00b-01)の第三条件を適用すると
...
## 10. 次に読む：連続分離と局所コンパクト性

正規空間の Urysohn の補題、局所コンパクト Hausdorff 空間の縮小、関数の台、コンパクト台連続関数、cutoff 関数は、補遺ではなく独立章 [TOP5A「Urysohn の補題・局所コンパクト性・cutoff」](../TOP5A/index.md) で扱います。

TOP6 の Baire・net/filter へ進むだけなら TOP5A は必須ではありません。一方、MT5「Radon 測度・Riesz–Markov」へ進む場合は TOP5A を先に読む構成です。
