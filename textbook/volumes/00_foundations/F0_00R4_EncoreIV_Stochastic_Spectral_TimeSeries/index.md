...
[TSA4「線形フィルタ・ARMA / ARIMA・周波数領域」](../TSA4/index.md#thm-tsa4-filter-spectrum) も実装済みです。絶対可算和可能な線形フィルタから伝達関数を導入し、TSA3 のスペクトル表現を用いて $F_Y=|H|^2F_X$ を証明しました。ARMA の単位円外根条件から因果性・可逆性を構成し、有理スペクトル密度、MA の ACF 打切り、AR の PACF 打切り、通常差分・季節差分の周波数零点、ARIMA で差分後系列を定常対象とする理由まで閉じています。一般 Kolmogorov--Szegő 因数分解は先取りせず、ARMA の有理スペクトル因子までを本章の射程としました。
