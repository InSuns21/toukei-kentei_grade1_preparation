...
[TSA5「エルゴード性・混合性・従属極限定理」](../TSA5/index.md#thm-tsa5-mean-ergodic) も実装済みです。標準経路空間の測度保存シフトからエルゴード性を定式化し、Hilbert 空間の平均エルゴード定理を固定点空間と差分空間の直交分解から証明しました。Birkhoff の一般 $L^1$ 版と一般 α-混合 CLT は技術的入力の境界を明示しつつ、定常エルゴード大数則、固定ラグ標本自己共分散の一致性、長期分散 $\sigma_{\mathrm{LR}}^2=2\pi f(0)$ を接続しています。さらに因果線形過程の CLT は有限 MA 打切りと iid CLT から完全証明し、因果 ARMA では $\sigma_Z^2\{\theta(1)/\phi(1)\}^2$ が標本平均の漸近分散になることまで閉じました。
