...
このページでは新しい数学用語を断りなく使わないため、**使う前に定義する**方針を取ります。既知として仮定するのは、ベクトル空間・行列・内積・ノルム・極限・微分といった大学初年級の線形代数・微積分だけです。
...
### 12.3 Robinson constraint qualification
...
Hahn--Banach自体の完全な証明には選択公理系の道具を使う標準的証明もあり、この補講の目的から外れるためここでは証明しません。重要なのは、**ここから先は新しい定理を導入している**と明示しておくことです。
...
一方、Hilbert空間では閉凸集合への直交射影定理が成立し、最近点を再び一意に取れます。

Banach空間一般ではさらに事情が微妙になり、空間の反射性・集合の弱コンパクト性など別の道具が必要になる場合があります。
