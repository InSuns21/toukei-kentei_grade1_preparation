...
STO4 では、この仕組みを Gaussian な有限次元分布へ適用して Brown 運動を構成します。したがって本章では Brown 運動の存在を仮定せず、「過程の存在」と「連続な標本路」を別々の問題として閉じます。
...
一つの区間で大きな増分が起こる確率を Markov 不等式で抑えると、全区間の union bound によって $2^n$ が一つ失われます。
