...
## 4. 半正定値共分散核からガウス過程を構成する

Kolmogorov 拡張定理の代表的な使い方として、平均関数と共分散核から確率過程を構成する問題があります。まず必要な過程の種類を定義し、その後で存在定理を示します。STO4 ではこの一般論にブラウン運動固有の共分散
$K(s,t)=\min(s,t)$
を代入します。

<a id="def-sto3-gaussian-process"></a>
...
<a id="thm-sto3-gaussian-process-existence"></a>

<!-- formal-statement-start -->
> **定理（平均関数と半正定値共分散核からのガウス過程構成）**  
> $T$ を任意の集合、$m:T\to\mathbb R$ を任意の関数とする。また
>
$$
K:T\times T\to\mathbb R
$$
>
> は対称で、任意の $n\ge1$、$t_1,\ldots,t_n\in T$、$a_1,\ldots,a_n\in\mathbb R$ に対して
>
$$
...
$$
I=\{t_1,\ldots,t_n\}\subset T
$$

を取ります。平均ベクトルと分散共分散行列を
...
仮定より $\Sigma_I$ は対称な半正定値行列です。[実対称行列の直交対角化定理](../F0_00F1_固有空間_スペクトル定理_PSD/index.md#thm-real-symmetric-spectral)により、ある直交行列 $Q$ と非負数 $\lambda_1,\ldots,\lambda_n$ が存在して
...
次に $I\subset J$ とします。$\mu_J$ に従うガウスベクトルから $J\setminus I$ の座標を捨てると、残ったベクトルもガウスで、その平均ベクトルと分散共分散行列はそれぞれ $m_I$ と $\Sigma_I$ です。多変量ガウス分布は平均と共分散で一意に決まるため、
...
したがって有限次元分布族 $(\mu_I)$ は整合的です。[Kolmogorov 拡張定理](#thm-sto3-kolmogorov-extension)を適用すると、これらを有限次元分布に持つ確率過程 $X$ が存在します。

構成した有限次元分布が全てガウスなので $X$ はガウス過程であり、各一時刻・二時刻の周辺分布から

$$
E[X_t]=m(t),
\qquad
\operatorname{Cov}(X_s,X_t)=K(s,t)
$$

が従います。これで存在が示されました。
<!-- proof-end -->

<!-- definition-example-start: def-sto3-gaussian-process -->
### 直接例：$K(s,t)=st$ から退化ガウス過程を作る

**定義の確認**

$m(t)=0$、$K(s,t)=st$ とします。任意の有限個の時刻 $t_1,\ldots,t_n$ と係数 $a_1,\ldots,a_n$ に対し、
...
従って $K$ は定理の仮定を満たし、共分散核 $st$ を持つガウス過程が存在します。
...
です。この例では全時刻の値が一つの $Z$ で決まるため、有限次元分布は一般に退化しています。退化ガウス分布を許す必要があることもここで分かります。
<!-- definition-example-end -->
...
を代入します。そこで必要になる追加作業は、$K(s,t)=\min(s,t)$ の半正定値性の確認と、連続な修正を得るための増分モーメント評価です。

---

## 5. 拡張定理 は標本路の正則性を何も保証しない
...
## 8. モーメントの次数を上げると、得られる Hölder 指数も上がる
...
3. $I\subset J$ で座標を減らしたとき、この分散分散共分散行列が対応する添字の行・列だけを残した部分行列へ移ることを確認せよ。
...
従って分散分散共分散行列は $\Sigma_J$ の対応する添字の行・列だけを残した部分行列となり、それは $\Sigma_I$ そのものです。
