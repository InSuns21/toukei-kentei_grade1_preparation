...
この「上へ越える」「下へ越える」を交互に繰り返します。各段階ではまだ使っていない次の正項または負項から順に取るので、各元の項は一度だけ使われ、最終的にすべての項が使われます。したがって得られる級数は再配列です。
...
各ブロックの端点の部分和は $L$ に収束し、ブロック内部でも動く幅は最後に使われる項の大きさで抑えられて0へ行きます。したがって再配列後の部分和全体が $L$ に収束します。$\square$
<!-- proof-end -->
...
<!-- solution-start -->
**解答**：$b_n=1/n$ は非負、単調減少、かつ0へ収束します。したがってLeibnizの交代級数判定から級数は収束します。
...
<!-- solution-start -->
**解答**：十分大きい $n$ では項は非負単調減少です。Cauchy凝縮判定を使うと
$$
...
一方 $b_n$ は仮定により非負単調減少で0へ収束します。したがって級数のDirichlet判定から
$$
\sum a_nb_n
=\sum(-1)^{n-1}b_n
$$
...
\frac1{n(\log n)^p(\log\log n)^q}
$$
は、$q$ が負でも対数の冪は任意の小さい正の $\varepsilon$ に対して $(\log n)^\varepsilon$ より遅くしか増えません。したがって十分大きい $n$ で
$$
(\log\log n)^{-q}\le (\log n)^{(p-1)/2}
$$
とでき、項は
$$
\frac1{n(\log n)^{(p+1)/2}}
$$
で上から抑えられます。$(p+1)/2>1$ なので収束します。

$p<1$ では同様に、対数対数因子では $(\log n)^{1-p}$ の不足を補えないため、十分大きい $n$ で
$$
\frac1{n(\log n)^p(\log\log n)^q}
\ge
\frac1{n(\log n)^{(p+1)/2}}.
$$
ここで $(p+1)/2<1$ なので右辺は発散し、元も発散します。
...
条件収束級数 $\sum a_n$ について、正部分 $p_n=\max(a_n,0)$ と負部分 $q_n=\max(-a_n,0)$ の和が両方とも $+\infty$ でなければならないことを、絶対収束との関係から証明せよ。その事実が再配列構成のどこで使われるか説明せよ。
