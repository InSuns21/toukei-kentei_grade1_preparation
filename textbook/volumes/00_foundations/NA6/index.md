# NA6 数値解析 VI：ODE 数値解法 I—時間離散化と収束
...
を考えます。[ODE1 の Picard--Lindelöf の定理](../ODE1/index.md#thm-ode1-picard-lindelof)は、適切な連続性と $y$ に関する Lipschitz 性の下で、短い時間区間に厳密解が一意に存在することを保証しました。
...
この章では、時間を離散化した更新を反復するときに、一歩ごとの近似誤差が固定有限時間の誤差へどう伝わるかを追います。
...
一歩で新たに生じる誤差が小さくても、過去の誤差を毎回強く増幅する方法なら、格子点全体の誤差は小さくなりません。逆に、一歩の誤差が $h^{p+1}$ 程度で、摂動増幅が固定有限時間で制御できれば、最終的な格子点誤差は $h^p$ 程度になります。
...
です。証明では ODE1 で導入した [$y$ に関する Lipschitz 性](../ODE1/index.md#def-ode1-lipschitz-y) と、RA3 の [Taylor の定理](../RA3/index.md#thm-ra3-taylor)を使います。

NA1 の「後方安定性」と、本章の「時間発展における摂動安定性」は同じ語を含みますが別の概念です。NA1 は計算結果を近い入力に対する厳密解と見なせるかを扱いました。本章では、各時刻の小さな誤差が時間更新でどの程度増幅されるかを扱います。
...
同じ文字 $y$ が連続関数と離散値に現れると混乱しやすいので、この章では厳密解の格子点値には大文字 $Y_n$ を使います。
...
<a id="def-na6-increment-function"></a>
<!-- formal-statement-start -->
### 定義（一段関数）
...
に現れ、現在の時刻 $t_n$、現在の近似値 $y_n$、刻み幅 $h$ から一歩の増分を決める関数
...
<!-- definition-example-start: def-na6-one-step, def-na6-increment-function, def-na6-euler-method -->
...
<!-- definition-example-start: def-na6-local-error -->
...
<!-- formal-statement-start -->
### 定理（一段関数の Lipschitz 性による有限時間摂動安定性）
...
ここで重要なのは、Lipschitz 定数 $L$ が $h\to0$ で無限大へ増大しないことです。
...
ここで ODE1 の Lipschitz 性が、存在一意性だけでなく数値誤差の増幅制御にも再登場しました。

同じ仮定が

- 連続問題では「二つの解が勝手に分岐しない」
- 離散問題では「二つの数値列の差が一段で増幅されすぎない」
...
この結果は、ODE1 で解の一意性を生んだ Lipschitz 性が、そのまま Euler 法の誤差増幅制御にも使えることを示しています。
<!-- solution-end -->
...
1. 数値列が Lipschitz 評価を使える領域 $[0,K]$ に留まること、
2. 厳密解も同じ領域に留まること、
