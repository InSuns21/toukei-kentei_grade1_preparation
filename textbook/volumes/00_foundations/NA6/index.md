...
を考えます。[ODE1 の Picard--Lindelöf の定理](../ODE1/index.md#thm-ode1-picard-lindelof)は、適切な連続性と $y$ に関する Lipschitz 条件の下で、短い時間区間に exact solution が一意に存在することを保証しました。

しかし、存在することと計算できることは別です。多くの非線形 ODE では、exact solution を閉じた式で書けません。そこで時間を有限個の点へ区切り、
...
NA1 の「後方安定性」と、本章の「時間発展における摂動安定性」は同じ語を含みますが別の概念です。NA1 は計算結果を近い入力に対する exact solution と見なせるかを扱いました。本章では、各時刻の小さな誤差が時間更新でどの程度増幅されるかを扱います。
...
同じ文字 $y$ が連続関数と離散値に現れると混乱しやすいので、この章では exact solution の格子点値には大文字 $Y_n$ を使います。
...
## 2. exact solution を一段だけ代入する：局所打切り誤差

数値法を評価するとき、いきなり最終時刻の誤差を見るのではなく、

> いまの値が exact だったとしても、次の一歩でどれだけ誤差を作るか
...
と、その初期値問題の exact solution $y(t)$ を考える。

exact solution の格子点値 $Y_n=y(t_n)$ に対して
...
**定義の確認**。$y(0)=1$ なら exact solution は
...
の exact solution $y$ が $[t_0,T]$ 上で $C^2$ 級であるとする。
...
exact solution に対して [Taylor の定理](../RA3/index.md#thm-ra3-taylor)を $t_n$ のまわりで一次まで適用します。$y\in C^2$ なので、ある
...
格子点 $t_n$ における exact solution の値を
...
一段法の一段関数 $\Phi$ が、考える領域で $y$ に関して一様に Lipschitz であり、ある $L\ge0$ に対して
...
が十分小さいすべての $h$ について成り立つとする。

このとき一段法は固定有限時間区間 $[t_0,T]$ 上で有限時間摂動安定であり、
...
いま exact solution の格子点値 $Y_n$ と数値解 $y_n$ を比較します。

exact solution は局所打切り誤差の定義から
...
2. 一段関数は考える領域で一様 Lipschitz で、ある $L\ge0$ に対して
...
したがってこの一段法は少なくとも $p$ 次で収束する。
<!-- formal-statement-end -->

<!-- proof-start -->
### 証明

exact solution の格子点列 $Y_n$ は
...
について、固定有限時間区間 $[t_0,T]$ 上で次を仮定する。

1. exact solution $y$ は $C^2$ 級である。
2. exact solution と Euler 法の数値解が通る領域で、$f$ は $y$ に関して Lipschitz で
...
exact solution を代入すると $Y_n=t_n$ なので
...
失った仮定はまさに摂動安定性です。整合性は「exact solution から一歩進むときの誤差」しか見ません。いったん生じた誤差を次のステップがどう扱うかは、別に制御しなければなりません。
...
> 固定した $T$ に対し、$h\to0$ としたとき $[t_0,T]$ 上で数値解が exact solution へ近づく
...
| 概念 | 何を測るか |
|---|---|
| 局所打切り誤差 | exact solution から一段だけ進んだときの誤差 |
| 整合性 | 刻み幅を小さくすると局所打切り誤差が0へ行くか |
| 摂動安定性 | すでに生じた誤差や外部摂動が増幅されすぎないか |
| 大域誤差 | 格子点で exact solution と数値解がどれだけ違うか |
...
1. exact solution を求め、局所打切り誤差が $\tau_{n+1}=-h$ であることを示せ。
2. $e_n=y_n-t_n$ が
...
3. exact solution も $0<y(t)<K$ に留まることを ODE1 の phase line の議論から説明せよ。
4. exact solution について
