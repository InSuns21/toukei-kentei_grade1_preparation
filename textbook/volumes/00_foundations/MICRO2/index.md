...
MICRO1 では、消費者が「どの候補をどれ以上に望むか」を選好として表しました。ただし、そこではまだ市場を入れていません。好きなだけ財を取れるなら、普通は「より多い方がよい」で話が終わってしまいます。

消費者問題が問題になるのは、**欲しいもの全部は買えない**からです。
...
この式はまだ記号だけ見ると無機質ですが、意味は単純です。$x$ は買う量、$p$ は各財の値段、$m$ は使える所得で、$p\cdot x$ は合計支出です。
...
まず「価格ベクトル」という言葉を普通の買い物として読みます。

財が $L$ 種類あり、第 $i$ 財の1単位あたり価格を $p_i$ とします。それらを一列に並べた

$$
p=(p_1,\dots,p_L)
$$

が価格ベクトルです。ベクトルという名前が付いていますが、ここでは単に **複数の財の値札をまとめて持つための入れ物** だと思えば十分です。
...
なら、第1財が1単位200円、第2財が1単位100円という意味です。消費量が
...
したがって内積 $p\cdot x$ は、ここでは抽象的な線形代数記号ではなく **買い物かごの合計金額** を表しています。
...
と書きます。$\mathbb R_{++}^L$ は「すべての成分が正」という意味なので、
...
を、消費に使える所得とします。したがって

$$
p\cdot x\le m
$$

は「合計支出が使える所得を超えない」という、ごく普通の予算制約です。
...
ここでコンパクト性を調べるのは、位相の復習のためではありません。

消費者が「一番よい候補」を本当に選べることを示すには、最大値へ近づく候補が
...
という購入量の上限を作ります。この上限が「最適候補が無限遠へ逃げる」ことを防ぎます。

つまりこの節のコンパクト性は、次節で需要の存在を保証するための装置です。
...
有限次元 Euclid 空間では [Heine--Borel の定理](../F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel/index.md#thm-f0-00c1-02) により閉かつ有界な集合はコンパクトなので、$B(p,m)$ はコンパクトです。
<!-- proof-end -->
...
経済学で「需要」というと、価格が上がれば何個買うか、所得が増えれば何個買うか、という反応を考えます。

ここではそれを原始的なルールとして与えません。価格 $p$ と所得 $m$ を入力し、
...
しかも最適候補が一つとは限りません。同じくらい好ましい候補が複数あれば、需要も集合になります。このため最初から「需要関数」と決めつけず、まず最大化点の集合として定義します。
...
従って [Weierstrass の最大最小定理](../F0_00C2_コンパクト性の応用_最大最小_最近点/index.md#thm-f0-00c2-01)により、ある
...
KKT 条件を使う前に、経済的な意味を先に見ておきます。

予算を使い切って二つの財を両方買っているとします。もし第1財へ1円回したときの効用増加が、第2財へ1円回したときより大きいなら、第2財への支出を少し減らして第1財へ移す余地があります。

最適点では、このような「少し支出を移せば改善できる」という余地が残っていてはいけません。
...
を同時に扱える形へ書き直す道具です。特に、ある財を全く買わない端点解でも同じ枠組みで扱えることが利点です。
...
Slater 条件を確認済みなので、[凸問題の KKT 条件](../OPT5/index.md#thm-opt5-kkt)から最適性との同値性が従います。
<!-- proof-end -->
...
二つの財をどちらも少し持っているとき、消費者の好みには

> 第1財を少し増やすなら、第2財をどれくらい減らしても同じ満足度でいられるか

という「交換のしかた」があります。

これは市場価格とは別のものです。価格比 $p_1/p_2$ は市場が提示する交換条件であり、これから定義する比は消費者の選好が生む内部的な交換条件です。

最適点では、この二つがどう関係するかを見るのが次節の中心です。
...
有限次元 Euclid 空間では [Heine--Borel の定理](../F0_00C1_コンパクト性_点列コンパクト性_Heine_Borel/index.md#thm-f0-00c1-02) により、閉かつ有界なのでコンパクトです。

2. $B(p,m)$ は非空コンパクトで、$u$ は連続です。

[Weierstrass の最大最小定理](../F0_00C2_コンパクト性の応用_最大最小_最近点/index.md#thm-f0-00c2-01)により $u$ は $B(p,m)$ 上で最大値を取ります。
...
[KKT 条件](../OPT5/index.md#thm-opt5-kkt)を用いて次を示せ。
...
1. 効用が狭義凹かつ狭義単調であることを確認せよ。
2. 最適解では予算を使い切ることを示せ。
3. 内点解を仮定して [KKT 条件](../OPT5/index.md#thm-opt5-kkt)から候補を求めよ。
...
5. $0<m\le\log2$ では第2財を0とする端点を調べます。
