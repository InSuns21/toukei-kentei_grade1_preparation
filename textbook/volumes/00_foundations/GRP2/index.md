...
剰余類どうしに代表元によらない積を入れるためには、この左右差が消える必要があります。
...
像が部分群であることは第2節ですでに確認しました。核の正規部分群条件は、$n\in\ker f$ を $gng^{-1}$ へ動かしても
$$
...
これで「核は剰余類に積を入れられる部分群」であることが分かりました。
...
gh(h^{-1}n_1h)n_2
$$
と変形します。正規部分群条件によって括弧内が再び $N$ に入るので同じ剰余類になります。
...
したがって、正規部分群条件を失うと「剰余類を元とする積」が代表元の選び方で変わってしまいます。壊れているのは群公理以前に、演算そのものの定義可能性です。
...
包含関係を保つことは、二つの対応写像の構成から直接確認できます。

最後に正規部分群条件を確認します。$H\trianglelefteq G$ なら
$$
(gN)(hN)(gN)^{-1}
=
ghg^{-1}N
...
したがって失った仮定は正規部分群条件です。正規部分群条件がないと代表元を変えたときに部分群の元を積の反対側へ移せず、積の剰余類が一定しません。つまり群公理の一部が後で失敗するのではなく、そもそも剰余類上の積が一意に定まりません。
<!-- solution-end -->
...
恒等置換では $\operatorname{sgn}=1$、互換 $(1\,2)$ では $\operatorname{sgn}=-1$ です。従って像には $1,-1$ の両方が入り
$$
...
2. 第三同型定理から
   ...
   $$
   を導け。
3. 左辺の商群の4つの剰余類を具体的に書き、それぞれが右辺のどの元に対応するか示せ。
...
次の GRP3 では、群が集合へ作用するときの点の分類・安定化群・共役を扱います。そこでは本章の剰余類と指数が
$$
G/G_x
$$
という形で再登場し、群作用の計数公式と類等式へ進みます。
