...
$P$ を底 $b$ の 次数 $\alpha$ のデジタル $(t_\alpha,m,s)$-net とし、
...
<!-- formal-statement-start -->
### 定理（桁交互配置による 次数 $\alpha$ デジタルネット構成）
...
点集合が 次数 4 でも、被積分関数の Walsh 係数が一次相当しか減衰しないなら、
...
3. $\rho_2(P)$ を求めよ。
4. この点集合が 次数 2 のデジタル $(0,1,1)$-net であることを確認せよ。
...
底2の 次数 2 のデジタル $(1,4,s)$-net $P$ を考える。
...
1. 次数 2 の条件から $\rho_2(P)$ の整数下限を求めよ。
...
ただしこれは関数側の2次 Walsh 減衰と点集合側の 次数 2 の条件を同時に仮定した結果です。
<!-- solution-end -->
...
従って4次元の通常ネットを2座標ずつまとめて、2次元の 次数 2 ネットへ変換します。
<!-- solution-end -->
...
6. もし交互配置をせず $n=m=3$ のまま 次数 2、$t_2=0$ を狙ったら不可能であることを精度障壁から示せ。
