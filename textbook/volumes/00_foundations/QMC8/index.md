...
最後に、既存の通常デジタルネットを $\alpha s$ 次元で作り、その座標桁を交互に並べ直して $s$ 次元の高次デジタルネットへ変換する構成を完全に追います。
...
> **停止線**
>
> 本章は高次デジタルネットの決定論的な周波数機構を扱います。QMC7 のランダム化と組み合わせた高次 RQMC の最適な二乗平均誤差率、重み付き Sobolev 空間での最適対数因子、多項式格子を用いた高次構成の高速 CBC 法は後続の計算機演習・発展文献へ送ります。
...
ただし、スクランブル後の最適な二乗平均誤差率を証明するには、QMC7 の [デジタルシフト QMC の厳密分散公式](../QMC7/index.md#thm-qmc7-shift-exact-variance) よりさらに細かな Walsh 係数対の解析が必要になるため、本章の主線には持ち込みません。
...
ここで不足しているのは十分な出力精度です。

構成が破綻する箇所は、下位 $n$ 桁が全て0の周波数
...
次に [桁交互配置による次数 α デジタルネット構成](#thm-qmc8-interlacing-order-alpha) を使います。
