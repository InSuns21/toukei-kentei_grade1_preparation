...
> **停止線**
>
> 本章は高次デジタルネットの決定論的な周波数機構を扱います。QMC7 のランダム化と組み合わせた高次 RQMC の最適な二乗平均誤差率、重み付き Sobolev 空間での最適対数因子、interlaced polynomial lattice の高速 CBC 構成は後続の計算機演習・発展文献へ送ります。
...
が成り立つとき、$P$ を **order $\alpha$ digital $(t_\alpha,m,s)$-net** と呼ぶ。

本章では本文中では **order $\alpha$ デジタルネット** と略記する。
<!-- formal-statement-end -->
...
\boxed{
P
\text{ は order }2\text{ digital }(0,1,1)\text{-net}
}
$$
...
この命題は、なぜ order $\alpha$ デジタルネットの標準的な構成で
...
<!-- formal-statement-start -->
### 定理（order $\alpha$ デジタルネットの高次 Walsh 誤差評価）

$P$ を底 $b$ の order $\alpha$ digital $(t_\alpha,m,s)$-net とし、
...
<!-- formal-statement-start -->
### 定理（桁交互配置による order $\alpha$ デジタルネット構成）

$P'$ を底 $b$ の digital $(t,m,\alpha s)$-net とする。
...
$P'$ は digital $(t,m,\alpha s)$-net なので、QMC5 の [$t$ 値と最小双対 NRT 重みの双対関係](../QMC5/index.md#thm-qmc5-t-value-duality) から
...
であり、$P$ は order $\alpha$ digital $(\tau_\alpha,m,s)$-net です。
<!-- proof-end -->
...
点集合が order 4 でも、被積分関数の Walsh 係数が一次相当しか減衰しないなら、
...
したがって「order $\alpha$ と名付けた」だけでは足りず、
...
3. $\rho_2(P)$ を求めよ。
4. この点集合が order 2 digital $(0,1,1)$-net であることを確認せよ。
...
- Level: B
- 主題: order $\alpha$ ネットの誤差評価
- 使用技術: 最小双対 Dick 重み・高次 Walsh 係数ノルム
- 計算量: 中

底2の order 2 digital $(1,4,s)$-net $P$ を考える。
...
1. order 2 条件から $\rho_2(P)$ の整数下限を求めよ。
...
ただしこれは関数側の2次 Walsh 減衰と点集合側の order 2 条件を同時に仮定した結果です。
<!-- solution-end -->
...
- Level: B
- 主題: 桁交互配置による品質パラメータ
- 使用技術: interlacing theorem・パラメータ評価
- 計算量: 中
...
従って4次元の通常ネットを2座標ずつまとめて、2次元の order 2 ネットへ変換します。
<!-- solution-end -->
...
- Level: C
- 主題: 高次 QMC の統合問題
- 使用技術: Dick 重み・精度障壁・order $\alpha$ 条件・高次誤差率・桁交互配置
- 計算量: 大

底2、$\alpha=2$ の高次準 Monte Carlo 法を考える。

通常の digital $(0,3,2)$-net を用意し、2座標を桁交互配置して一次元の点集合 $P$ を作る。
...
6. もし交互配置をせず $n=m=3$ のまま order 2、$t_2=0$ を狙ったら不可能であることを精度障壁から示せ。
...
周波数を逆にほどき、QMC5 の通常 $t$ 値の双対特徴付けと Dick 重みの比較を組み合わせることで、交互配置後の点集合が実際に order $\alpha$ デジタルネットになることまで証明しました。
