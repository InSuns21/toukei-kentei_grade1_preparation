...
`python-test` は読者画面では表示せず、ユーザーコード実行後に**同じ Python 名前空間**で評価します。
...
- **穴埋め**：`python-lab` の初期コードを編集する。
- **模範解答**：`python-solution` の完成コードをシンタックスハイライト付きで表示する。
- **実行結果**：標準出力、自動判定結果、Matplotlib の図を表示する。

模範解答には「穴埋め欄へ反映」ボタンも置きます。自力で詰まった場合に完成コードを読み、そのまま再実行できます。

### 4.3 問題文は通常 Markdown に置く

問題文を専用コードフェンスへ押し込めません。

ラボの直前に通常の Markdown として、
...
これにより問題文では KaTeX、リンク、表、箇条書きをそのまま使えます。問題文と模範解答を別形式で二重管理しません。
...
`python-solution` には `___` を残しません。

自動テストは単なる最終数値だけでなく、可能なら

- 正しい節点を構成したか
- 正しい離散化を使ったか
- 誤差量を正しい定義で計算したか
- 安定性・単調性など理論上の性質を満たすか

を検査します。

### 4.5 コード表示

`python-lab` と `python-solution` は、外部エディタを追加せず共通ランタイム側で Python の基本的なシンタックスハイライトを行います。
...
編集欄は textarea を入力面として維持するため、コピー・貼り付け・IME・アクセシビリティを専用エディタへ依存させません。Tab キーは Python の字下げ用に4空白を挿入します。
