...
まず事象とsigma代数の独立性を定義し、その後で、独立な確率変数の同時分布が二つの周辺分布から作る積測度へ分解することを示します。
...
積測度を表す記号は第3節で、実際に同時分布と結び付けるところで導入します。条件付き期待値はP3Aへ分離します。
...
を満たす測度を **積測度** といい、$P_X\otimes P_Y$ と書きます。
