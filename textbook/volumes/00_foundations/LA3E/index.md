...
$$
という二重線形な量は作れます。しかし、このような多重線形な量を毎回「引数を何個も持つ写像」として扱うだけでは、積・基底・縮約・交代化を一つの代数として整理できません。
...
LA3A では双対空間 $V^*$ を導入しました。$V$ と $V^*$ を何個ずつ掛けるかで、テンソルの型を整理できます。
...
したがって「対角成分を足す」という跡の公式は、$V$ と $V^*$ の自然な評価を行った結果です。
<!-- definition-example-end -->
...
交代形式は、基底ベクトルを互いに異なる添字で入れた値だけで決まります。その値を「どの $k$ 個の添字を選んだか」で整理すると $\binom nk$ 個あります。外積基底は、それぞれの選び方を Kronecker のデルタの行列式として読み分けます。
...
この「二段階の選択」は、最終的に $\alpha,\beta,\gamma$ へ渡す添字集合をそれぞれ $p,q,r$ 個選び、各群内の順序を保つことと同値です。従って $(p,q,r)$-シャッフルと一対一に対応します。
