...
まず外積との可換性を示します。$p\in M$ と $v_1,\dots,v_{k+\ell}\in T_pM$ を取ります。[LA3E で導入した外積](../LA3E/index.md#def-la3e-wedge-product)を置換和として展開すると、
...
したがって右辺は第一変数について $C^\infty$ 線形です。他の変数についても、関数倍を代入して同じ展開を行えば余分な微分項が相殺します。よって右辺は各変数について $C^\infty$ 線形であり、座標基底 $\partial_i$ 上で局所定義と一致するため交代性も従い、$(k+1)$ 形式です。

座標基底 $\partial_i$ 上で局所定義と一致し、両辺とも各変数について $C^\infty$ 線形なので、任意の
...
なので右辺は $dX^i$ になります。一方、左辺は $\Phi_t^*(dx^i)=d(x^i\circ\Phi_t)$ を直接 $t$ で微分して求めます。
