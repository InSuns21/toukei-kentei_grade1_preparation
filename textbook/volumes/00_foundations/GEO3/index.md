...
よって [逆関数定理](../RA6A/index.md#thm-ra6a-inverse-function) により、0の近くで \(\Phi\) は滑らかな局所微分同相写像です。新しい入力座標を
...
ははめ込みです。また \(S^1\) はコンパクト、\(\mathbb R^2\) は Hausdorff なので、連続全単射

$$
i:S^1\to i(S^1)
$$

は同相写像です。従って \(i\) は埋め込みです。
<!-- definition-example-end -->
...
従ってその微分は0です。[GEO2 の多様体上の連鎖律](../GEO2/index.md#thm-geo2-chain-rule)から
...
実 \(n\times n\) 行列全体を \(\mathbb R^{n^2}\) と同一視し、対称行列全体の線形空間を
