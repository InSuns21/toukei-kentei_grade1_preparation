...
を座標に依存しない線形写像として定義しました。本章では、その線形写像の像の次元が局所的な幾何をどこまで支配するかを調べます。まずこの量に名前を与え、その後で局所標準形へ進みます。
...
> **定義（滑らかな写像の階数）**  
> $M$ を $m$ 次元、$N$ を $n$ 次元の滑らかな多様体とし、$f:M\to N$ を滑らかな写像、$p\in M$ とする。点 $p$ における $f$ の **写像の階数**を
...
を考えると、[GEO2 の微分の座標表示](../GEO2/index.md#thm-geo2-differential-coordinate)から $df_p$ の行列表現は $DF_{x(p)}$ です。別の座標へ移れば、左右から可逆行列が掛かるだけなので写像の階数は変わりません。

### 写像の階数が一定であることは本質的な仮定
...
$y\neq0$ では写像の階数2ですが、$y=0$ では写像の階数1です。したがって原点を含むどの近傍でも写像の階数は一定ではありません。

後で示す局所標準形の定理は、写像を一つの固定した標準形へ直します。写像の階数が近傍内で変わってしまうと、一つの標準形では全点の微分を同時に表せません。ここが仮定の働く場所です。

---

## 2. 局所標準形を証明する

点 $p$ だけの写像の階数ではなく、その近くで写像の階数がずっと $r$ なら、非線形写像は座標変換によって非常に単純になります。
...
この定理の意味は、「写像の階数 $r$ の非線形写像は、局所的には $r$ 個の座標だけを残して他を捨てる写像と同じ」ということです。

### 証明の見取り図

1. まず座標表示 $F:\mathbb R^m\to\mathbb R^n$ に戻す。
2. 写像の階数 $r$ なので、ある $r\times r$ 小行列が可逆になる。
3. $F$ の最初の $r$ 成分と残りの入力変数を組み合わせた写像 $\Phi$ を作る。
4. $D\Phi$ が可逆なので [逆関数定理](../RA6A/index.md#thm-ra6a-inverse-function) で $\Phi$ を新しい入力座標にできる。
5. 新座標では最初の $r$ 出力がそのまま入力座標になる。写像の階数が $r$ を超えないため、残りの出力は残りの入力変数に依存できない。
...
$DF_0$ の写像の階数は $r$ です。したがって、行と列を必要なら並べ替えることで、左上の $r\times r$ 小行列
...
ここで元の仮定により、十分小さい近傍で $DG$ の写像の階数も常に $r$ です。$s$ 方向の $r$ 本の列は、上の $r$ 成分に $I_r$ を含むため一次独立です。
...
- $p$ で写像の階数が $r$：可逆な $r\times r$ 小行列を選ぶため。
- 近傍全体で写像の階数が $r$：新座標で $D_tH=0$ を強制するため。
- 逆関数定理：$\Phi$ を本当に局所座標へ昇格させるため。

「点 $p$ で写像の階数 $r$」だけでは、前節の $F(x,y)=(x,y^2)$ のように近傍で写像の階数が増えることがあり、最後の $D_tH=0$ が成立しません。
...
はめ込みなら写像の階数は常に $m$、沈め込みなら写像の階数は常に $n$ です。従って定数階数定理をそのまま適用できます。
...
>
> - $p\in M$ で $df_p$ が全射なら、$p$ を $f$ の **写像の正則点**という。
> - $df_p$ が全射でないなら、$p$ を **臨界点**という。
> - $q\in N$ に対して、$f^{-1}(q)$ の全ての点で微分が全射なら、$q$ を **正則値**という。$f^{-1}(q)=\varnothing$ の場合もこの条件は満たされるものとする。
...
$p\in f^{-1}(q)$ では $df_p$ が全射です。全射性はある $n\times n$ 小行列の行列式が非零という局所条件なので、$p$ の十分小さい近傍でも写像の階数 $n$ が保たれます。そこで [沈め込みの局所標準形](#cor-geo3-submersion-normal-form) を使うと、$f$ は局所的に最初の $n$ 座標を取り出す射影になります。この逆像は、その $n$ 座標を固定した座標平面です。
...
2. 第一行は常に非零です。写像の階数が1以下になるのは、第二行が第一行の実数倍になる場合です。第一行の第一成分は1ですが第二行の第一成分は0なので、実数倍になるには第二行そのものが0でなければなりません。
...
のときだけ写像の階数1で、それ以外では写像の階数2です。

3. 目標空間は2次元なので、$f$ が正則となるのは写像の階数が2の点です。したがって臨界点全体は
...
二行は一次独立なので写像の階数は2です。目標空間も2次元なので $d\pi$ は全射、従って $\pi$ は沈め込みです。
...
実数値線形写像は非零なら全射なので、$F^{-1}(c)$ の全点で $dF$ は全射です。従って $c>0$ は正則値です。
...
「逆像の全ての点で微分が全射」という条件に反する点が存在しないため、定義上は正則値です。
<!-- solution-end -->
...
なので $(x,y)\neq(0,0)$ です。従って第一行は非零で、第二行とは明らかに一次独立です。よって写像の階数は2で、$dF$ は全射です。したがって $(1,0)$ は正則値です。
