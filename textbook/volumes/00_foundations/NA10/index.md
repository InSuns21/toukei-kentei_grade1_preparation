...
よって跡と行列式はともに保存されています。

2次行列では固有値の和と積がそれぞれ跡と行列式なので、固有値1と3が保存されることとも整合します。
<!-- solution-end -->
...
次の NA11 では、非負行列のスペクトルに特有の構造へ進みます。Perron--Frobenius 理論を使うと、最大固有値と正の固有ベクトルが確率行列・PageRank の意味へ直接つながります。
