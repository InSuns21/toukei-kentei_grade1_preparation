...
Slater条件・強双対性・KKTはここではまだ使いません。まず
...
> $U\subset\mathbb R^d$ を開凸集合、$f\in C^2(U)$ とします。このとき
> $$
> \boxed{
> f\text{ が凸}
> \iff
...
<!-- formal-statement-start -->
> **定理（微分可能な凸関数の一次支持不等式）**  
> $C\subset\mathbb R^d$ を開凸集合、$f:C\to\mathbb R$ を微分可能な凸関数とします。このとき任意の $x,y\in C$ に対して
> $$
> \boxed{
> f(y)
> \ge
...
右辺は $x$ における接平面です。凸関数のグラフは、各点の接平面より下へ落ちません。
...
$t\downarrow0$ なら $z_t\to x^*$ なので、$x^*$ の任意に小さい近傍により小さい値を持つ点が存在してしまい、局所最小性に矛盾します。$\square$
<!-- proof-end -->
...
> Ax=b
> $$
> と書かれ、$f$ と各 $g_i$ が凸、等式制約が affine である問題を凸最適化問題と呼びます。
<!-- formal-statement-end -->
...
ここまでは「凸問題ならなぜ扱いやすいか」の幾何です。Lagrange双対・Slater・KKTは後段で扱います。
...
\end{pmatrix}.
$$
先頭主座小行列式は $2>0$、行列式は $20-16=4>0$ なので正定値。従って特に半正定値であり、$f$ は凸。
#### 本番答案
$\nabla^2f\succ0$ より凸。
#### 採点基準（20点）
- Hessian: 8点
...
<!-- solution-start -->
#### 詳細解答
大域的により小さい点 $y$ があると仮定し、$z_t=(1-t)x^*+ty$ を取る。凸性により任意に $x^*$ に近い改善点が生じるため局所最小性に矛盾。
#### 本番答案
...
<!-- solution-start -->
#### 詳細解答
$f'(x)=4x^3$ は狭義単調増加なので $f$ は狭義凸。一方 $f''(x)=12x^2$ より $f''(0)=0$。従って狭義凸でも Hessian が全点で正定値とは限らない。
#### 本番答案
