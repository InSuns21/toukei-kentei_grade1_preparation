...
<!-- definition-example-start: def-tsa5-α-mixing -->
**定義の確認**  
...
<a id="thm-tsa5-α-mixing-clt"></a>

<!-- formal-statement-start -->
> **定理（α-混合過程の中心極限定理：代表的十分条件）**  
...
この一般形の完全証明には、α-混合に対する共分散不等式、長いブロックと短い隙間へ分割する ブロック分割法（blocking argument）、離れたブロックを独立化して比較する誤差評価、切断による高次モーメント制御を組み合わせる必要があります。
...
また上のモーメント条件と α-混合条件からは、標準的な α-混合共分散不等式により
...
5. 弱ホワイトノイズは **二次共分散が消えるだけ**です。この例では全時点が共通尺度 $Y$ を共有しています。
   ...
   独立雑音なら異なる時点の sigma-field 自体が独立です。α-混合なら完全独立でなくても、時間間隔を広げるとその依存の上限が 0 へ減衰します。
