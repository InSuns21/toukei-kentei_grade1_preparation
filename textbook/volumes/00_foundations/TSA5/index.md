# TSA5 Encore IV 時系列解析 V：エルゴード性・混合性・従属極限定理

<!-- definition-example-audit: strict -->

TSA1--TSA4 では、定常過程を二次構造・予測・スペクトル・線形フィルタの側から調べました。しかし、観測された一本の時系列から母平均や自己共分散を推定するときには、もう一つ別の問題があります。
...
> $$
>
> を満たすとき、$T$ を測度保存変換と呼びます。時系列の標準経路空間上で
>
> $$
> (T\omega)_t=\omega_{t+1}
> $$
...
TSA1 では定常性を有限次元分布の不変性として見ました。本章では同じ事実を「経路法則を保つシフト」として読み替えます。この読み替えによって、時間平均を作用素の Cesàro 平均として扱えるようになります。

---

## 2. エルゴード性：時間を進めても残るランダム性を排除する

<a id="def-tsa5-ergodicity"></a>

<!-- formal-statement-start -->
> **定義（不変 sigma-field とエルゴード性）**  
> 測度保存シフト $T$ に対し、
>
> $$
> \mathcal I
...
<!-- formal-statement-start -->
> **定理（Hilbert 空間の平均エルゴード定理）**  
> $(\Omega,\mathcal F,P)$ 上の可逆な測度保存変換 $T$ と、
>
> $$
> Uf=f\circ T
> $$
...
最後に固定点と不変 sigma-field を同定します。$\mathcal I$-可測な $g$ は $g\circ T=g$ a.s. を満たすので $L^2(\mathcal I)\subset\operatorname{Fix}(U)$ です。
...
ここで重要なのは、平均エルゴード定理が **$L^2$ 収束**を与えることです。経路ごとの a.s. 収束は次の Birkhoff の定理が担当します。

---

## 4. Birkhoff のエルゴード定理：経路ごとの時間平均

<a id="thm-tsa5-birkhoff"></a>

<!-- formal-statement-start -->
> **定理（Birkhoff のエルゴード定理）**  
> $(\Omega,\mathcal F,P)$ 上の測度保存変換 $T$ と $f\in L^1(P)$ に対し、
>
> $$
> A_nf
> =
...
<!-- proof-start -->
### 証明について：本章での意図的な技術的入力

一般 $L^1$ 版の完全証明は、最大エルゴード不等式、正負部分の切断、極大関数を用いた a.s. 収束の議論を必要とします。これは本章の「時系列推測に必要なエルゴード性と依存極限定理」という射程を大きく越えて、一般エルゴード理論の証明技法そのものへ入ります。

そのため本章では Birkhoff の一般 $L^1$ 版を **明示的な技術的入力**として使います。一方、二乗可積分の場合の時間平均については、直前の [Hilbert 空間の平均エルゴード定理](#thm-tsa5-mean-ergodic)を完全証明しており、
...
$X_0\in L^2$ なら [Hilbert 空間の平均エルゴード定理](#thm-tsa5-mean-ergodic)も適用できるため $L^2$ 収束も従います。$\square$
<!-- proof-end -->

---

## 5. 混合性：遠く離れた時点の依存が消える
...
つまり独立列は、依存が「漸近的に小さくなる」どころか一時点離れた段階で完全に消える極端な alpha 混合過程です。
<!-- definition-example-end -->
...
ここで TSA4 の「弱ホワイトノイズ」だけでは不十分です。無相関は sigma-field の独立を意味しないため、m 依存性を結論するには駆動雑音の独立性を仮定しています。
<!-- definition-example-end -->
...
> $$
>
> 従って m 依存過程は alpha 混合的であり、エルゴード的です。
<!-- formal-statement-end -->
...
後半は [alpha 混合は混合性を含意する命題](#prop-tsa5-alpha-implies-mixing)と [混合性はエルゴード性を含意する命題](#prop-tsa5-mixing-ergodic)から従います。$\square$
<!-- proof-end -->
...
この定理は **固定した $h$** に対する一致性です。$h=h_n\to\infty$ として多数のラグを同時に扱う場合や、スペクトル密度推定のようにラグ窓を広げる場合には、一様な誤差評価や追加の混合条件が必要です。
...
従って級数は a.s. 絶対収束する版を持ち、$(X_t)$ は iid 雑音列の現在・過去の可測関数として実現できます。このため狭義定常であり、独立同分布シフトのエルゴード性を因子として受け継ぎます。
...
[F0-00P6A の iid 有限分散中心極限定理](../F0_00P6A_iid_中心極限定理/index.md)より
...
は独立雑音 $(Z_s)$ の線形結合で、その係数列は $c$ と $b$ の離散畳み込みです。

離散 Young 型評価は、$\ell^2$ の三角不等式から
...
まず $n\to\infty$ とすると、有限 $q$ の中心極限定理により第二の特性関数は
...
TSA4 では $(Z_t)$ を弱ホワイトノイズとしていましたが、ここでさらに **独立同分布**を仮定すれば本定理を適用できます。
...
時間領域の部分和、フィルタ係数の総和、0 周波数スペクトルが同じ長期分散を表しています。

---

## 9. 一般の alpha 混合中心極限定理

線形過程ではフィルタ構造を直接使って中心極限定理を証明できました。しかし非線形時系列では、線形係数 $(\psi_j)$ がありません。そこで依存の強さそのものを alpha 混合係数で制御します。

<a id="thm-tsa5-alpha-mixing-clt"></a>

<!-- formal-statement-start -->
> **定理（alpha 混合過程の中心極限定理：代表的十分条件）**  
> $(X_t)_{t\in\mathbb Z}$ を実数値・狭義定常・平均0の alpha 混合過程とします。ある $\delta>0$ が存在して
>
> $$
> E|X_0|^{2+\delta}<\infty
> $$
...
この一般形の完全証明には、alpha 混合に対する共分散不等式、長いブロックと短い隙間へ分割する blocking argument、離れたブロックを独立化して比較する誤差評価、切断による高次モーメント制御を組み合わせる必要があります。

本章ではその全技術を一般形で展開せず、**仮定と適用範囲を明示した標準的な十分条件**として使います。

ただし、従属中心極限定理の核心が完全な黒箱にならないよう、直前の [絶対可算和可能な因果線形過程の中心極限定理](#thm-tsa5-linear-process-clt)では、iid 中心極限定理への有限打切りと一様な尾部評価を用いて完全証明しました。

また上のモーメント条件と alpha 混合条件からは、標準的な alpha 混合共分散不等式により
...
と評価でき、仮定した級数収束が自己共分散の絶対可算和可能性を保証します。
<!-- proof-end -->

### 9.1 「エルゴード的なら CLT」ではない
...
1. 任意の $t_1,\dots,t_m$ と $h$ に対して
   ...
   が標本ごとに成立します。従って有限次元分布は時間移動で不変であり、狭義定常です。
...
3. $\alpha(n)\to0$ なので alpha 混合的です。[alpha 混合は混合性を含意する命題](#prop-tsa5-alpha-implies-mixing)と [混合性はエルゴード性を含意する命題](#prop-tsa5-mixing-ergodic)からエルゴード的です。

4. いいえ。弱ホワイトノイズは異時点で無相関であることしか保証しません。m 依存性の証明では、互いに素な雑音集合が生成する sigma-field の **独立性**を使いました。無相関だけではこの段階を正当化できません。
<!-- solution-end -->
...
## TSA5-B02 有限移動平均の中心極限定理を境界項から導く
...
1. $\sum_{t=1}^nX_t$ を $(1+\theta)\sum_{t=1}^nZ_t$ と境界項の和に分解せよ。
2. 境界項を $\sqrt n$ で割ると $L^2$ で 0 へ収束することを示せ。
3. iid 中心極限定理から
   $$
   \frac1{\sqrt n}\sum_{t=1}^nX_t
   $$
...
5. この例から「弱ホワイトノイズ」「独立雑音」「alpha 混合」を区別して説明せよ。

<!-- solution-start -->
### 詳細解答

1. 条件付きで $Y=y$ と固定すると $(X_t)$ は独立同分布な $N(0,y^2)$ 列です。$Y$ を混合した後も有限次元分布は時間添字の平行移動で変わらないので、狭義定常です。
...
5. 弱ホワイトノイズは **二次共分散が消えるだけ**です。この例では全時点が共通尺度 $Y$ を共有しています。
   ...
   独立雑音なら異なる時点の sigma-field 自体が独立です。alpha 混合なら完全独立でなくても、時間間隔を広げるとその依存の上限が 0 へ減衰します。

   この例の共通尺度 $Y$ はどれだけ時間を離しても消えないため、弱ホワイトノイズであることから独立性や alpha 混合性を推論してはいけません。
<!-- solution-end -->
...
2. $E|Z_0|<\infty$ と $\sum_j|\psi_j|<\infty$ から
   ...
   となる版を取れます。従って $X_t$ は iid 列の現在・過去の可測関数です。

   iid 経路シフトは混合的、従ってエルゴード的です。因果写像は時間シフトと可換なので、$X$ の不変事象を iid 経路空間へ引き戻すと iid シフトの不変事象になります。その確率は 0 または 1 です。
...
6. TSA4 の因果表示そのものは弱ホワイトノイズでも二次平均の意味で構成できます。しかし、
   ...
   - iid シフトの混合性・エルゴード性を引き継ぐ議論、
   - iid 中心極限定理を有限打切りへ適用する議論、
...
TSA6 では、観測できない状態を持つ線形 Gaussian 状態空間モデルへ進みます。予測誤差をイノベーションとして逐次更新し、Kalman フィルタ、誤差共分散の Riccati 再帰、尤度、ARMA の状態空間表現までを接続します。
