...
とします。任意の整数 $r\ge1$、時点 $t_1,\dots,t_r$、シフト $k$ に対して
```text
(X_{t_1},\dots,X_{t_r})=(Y,\dots,Y)=(X_{t_1+k},\dots,X_{t_r+k})
```
が各標本点で成り立つので、有限次元分布は時間シフトで変わりません。従ってこの過程は狭義定常です。
...
[Birkhoff のエルゴード定理](#thm-tsa5-birkhoff)を積 $X_0X_h$ に適用するには、その可積分性を確認する必要があります。Cauchy--Schwarz の不等式と定常性から
...
$X_0\in L^1$ であり、定常性から $X_h\in L^1$ かつ $E[X_h]=\mu$ です。そこで [Birkhoff のエルゴード定理](#thm-tsa5-birkhoff)をそれぞれ $X_0$ と $X_h$ に適用すると、
