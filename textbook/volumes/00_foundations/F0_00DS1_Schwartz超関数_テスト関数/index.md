...
$\varphi$ はコンパクトな台を持つので、局所可積分性だけで積分できます。

この $T_f$ を $f$ が作る正則超関数と呼びます。
...
通常の $L^1_{\mathrm{loc}}$ 関数では表せませんが、線形汎関数としては完全に意味があります。
...
各 $\varepsilon>0$ では普通の確率密度です。

しかし $\varepsilon\to0$ とすると一点へ集中し、任意のテスト関数 $\varphi$ に対して
...
点ごとの値ではなく、テスト関数への作用を観察します。

これは確率論の弱収束と雰囲気が似ていますが、対象とテスト関数のクラスは別物です。
...
そこで導関数を対象 $T$ そのものへ押し付けず、部分積分を使ってテスト関数側へ移します。
...
右辺なら $f$ 自身が微分可能でなくても意味を持つ場合があります。
...
Fourier変換は超関数へ拡張できます。

その結果、Dirac deltaと定数関数、微分と周波数の掛け算などが非常に自然に整理されます。
