...
ここで $|\theta|<1$ を使った場所は、観測過去から $Z_t$ を $L^2$ 復元する無限和の収束です。この条件を外すと「式にホワイトノイズが現れているから、それが標準イノベーション」とは言えません。Wold 分解は生成式に書かれた雑音ではなく、観測過程の過去から定まる標準イノベーションを使います。
...
次章 TSA3 では、この時間領域の分解とは別に、自己共分散の正定値性を周波数領域へ移し、Herglotz の定理と測度による周波数表現を導入します。
...
次章 TSA3 では、自己共分散列の正定値性を出発点として Herglotz の定理、周波数領域の測度表現、密度を持つ場合と線状成分、直交増分ランダム測度による表現へ進みます。
