...
前章の [過去の線形予測空間](../TSA1/index.md#def-tsa1-past-space)、[イノベーション](../TSA1/index.md#def-tsa1-innovation)、[時間移動作用素](../TSA1/index.md#lem-tsa1-unitary-shift) を正本として使います。スペクトル測度や Herglotz の定理は次章 TSA3 の内容であり、Wold 分解の証明へ逆輸入しません。
...
一段ずつ新情報を分離しても、どこまで過去へ押し戻しても消えない成分が残ることがあります。それが remote past です。
...
いま \(x\in\mathcal H_t\) を任意に取ります。上の有限直交分解から
...
この命題が Wold 分解の空間版です。あとは各 \(\mathcal I_t\) がスカラー時系列では標準イノベーション1本で生成されることを使えば、級数表示が出ます。
...
なら全ての \(\varepsilon_t=0\) in \(L^2\) です。従って全ての \(\mathcal I_t=\{0\}\) であり、過去空間の直交分解から
...
なので、\(\mathcal H_t\) は \(\{Z_s:s\le t\}\) の閉線形包と一致します。弱ホワイトノイズの remote past が \(\{0\}\) であることは3.1で証明済みなので
...
さらに \(\mathcal H_t\) は \(\{Z_s:s\le t\}\) の閉線形包と一致するため remote past は0です。したがって MA(1) の Wold 表示はそのまま
...
## TSA2-A02 弱ホワイトノイズの remote past
