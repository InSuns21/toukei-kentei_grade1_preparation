...
重要なのは、損失 $L$ の微分可能性や特定の関数形は、この有限和表示そのものには不要なことです。
...
$M:=J(0)$ と置きます。$\lambda>0$ なので
$$
m+\lambda R^2>M
$$
を満たす $R>0$ を取れます。$\|g\|_{\mathcal H}>R$ なら
$$
J(g)
\ge
m+\lambda\|g\|_{\mathcal H}^2
...
$$
へ制限できます。$S$ は有限次元なのでこの閉球はコンパクトであり、連続関数 $J|_S$ はそこで最小値を達成します。よって $\mathcal H$ 上でも最小解が存在します。
<!-- proof-end -->
