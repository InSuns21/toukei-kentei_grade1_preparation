...
## 8. いつ最小解の存在と一意性まで言えるか

表現定理は「最小解が存在するなら有限和になる」という構造を与えます。ここから少し条件を足すと、存在と一意性も確認できます。

<a id="cor-rkhs2-existence-uniqueness"></a>
...
>
> さらに $L$ が凸なら、その最小解は一意である。
<!-- formal-statement-end -->

### 証明の見取り図

表現定理により有限次元空間 $S$ だけを見れば十分です。$L$ が下に有界なら、$\lambda\|f\|^2$ が $\|f\|\to\infty$ で目的関数を $+\infty$ へ押し上げます。

有限次元では閉有界集合がコンパクトなので、連続関数は最小値を達成します。

一意性は、$\lambda\|f\|^2$ が Hilbert 空間上で狭義凸であることから従います。
...
$S$ は有限次元なので、十分大きい $R$ を取れば最小化を閉球
$$
\{g\in S:\|g\|_{\mathcal H}\le R\}
$$
の中に制限できます。この閉球はコンパクトであり、連続関数 $J|_S$ はそこで最小値を達成します。よって $\mathcal H$ 上でも最小解が存在します。

次に $L$ が凸とします。標本評価写像は線形なので
$$
f\mapsto
L\bigl(f(x_1),\dots,f(x_n)\bigr)
$$
...
一方 $f\ne g$、$0<t<1$ に対して
$$
\|tf+(1-t)g\|_{\mathcal H}^2
<
t\|f\|_{\mathcal H}^2
+
(1-t)\|g\|_{\mathcal H}^2
$$
なので $\lambda\|f\|^2$ は狭義凸です。凸関数と狭義凸関数の和は狭義凸だから $J$ は狭義凸です。

狭義凸関数が二つの異なる最小解を持つことはできないため、最小解は一意です。
<!-- proof-end -->
...
### RKHS2-B03 存在と一意性を有限次元化から示す

- Level: B
- 目安時間: 25分

$L:\mathbb R^n\to\mathbb R$ は連続、下に有界、凸とする。$\lambda>0$ に対して
$$
J(f)
=
L\bigl(f(x_1),\dots,f(x_n)\bigr)
...
2. $J|_S$ が $\|f\|\to\infty$ で $+\infty$ へ発散することを示せ。
3. 最小解の存在を示せ。
4. 最小解が一意であることを示せ。
...
前問の発散性により、十分大きい半径 $R$ を取れば最小値を探す範囲を
$$
\{g\in S:\|g\|\le R\}
$$
へ制限できます。$S$ は有限次元なのでこの閉球はコンパクトです。連続関数 $J|_S$ はコンパクト集合上で最小値を達成するため、最小解が存在します。

4. $L$ は凸で、標本評価写像は線形なので
$$
f\mapsto
L\bigl(f(x_1),\dots,f(x_n)\bigr)
$$
...
$f\mapsto\lambda\|f\|^2$ は $\lambda>0$ により狭義凸です。従って $J$ は狭義凸です。

もし異なる二つの最小解 $f\ne g$ があれば、その中点で
$$
J\left(\frac{f+g}{2}\right)
<
\frac12J(f)+\frac12J(g)
$$
となり、右辺は最小値なので矛盾です。よって最小解は一意です。
<!-- solution-end -->
