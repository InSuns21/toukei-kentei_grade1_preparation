...
そこで強圧性の代わりに、二つの安定性が必要になります。

$$
\boxed{
\text{制約を満たす方向での強圧性}
+
\text{圧力を速度試験関数から一様に検出する性質}
}
$$

です。

本章では、まず二未知量を持つ制約付き変分問題を有限次元の制約付き最小化から導入します。次に「どの非零圧力も速度側から見えなくならない」という性質を定量化し、それを使って制約を満たす速度を構成した後、制約を保つ方向で Lax--Milgram を適用して存在一意性を証明します。
...
というブロック連立方程式、離散空間の安定性条件、偽圧力モード、Taylor--Hood 要素の位置付けまで進みます。
...
まず [inf-sup 条件による有界な持ち上げ](#lem-fem5-bounded-lifting)から、ある $u_g\in V$ が存在して
