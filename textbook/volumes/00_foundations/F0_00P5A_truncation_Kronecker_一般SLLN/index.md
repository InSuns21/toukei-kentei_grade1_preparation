...
$n\to\infty$ で第1項は0へ行くので上極限は $\varepsilon$ 以下です。$\varepsilon>0$ は任意だから
...
従って $X$ は可積分です。分布は0について対称なので、正負の項を絶対収束級数として組にでき、
...
$n\to\infty$ で第1項は0へ行き、重みの総和は1以下なので上極限は $\varepsilon$ 以下です。$\varepsilon$ は任意だから
...
有限4次モーメント、有限分散、有限平均という三つの強大数則ルートがつながりました。次は分布収束と特性関数を扱う [F0-00P6 特性関数・中心極限定理](../F0_00P6_特性関数_中心極限定理/index.md) へ進みます。
