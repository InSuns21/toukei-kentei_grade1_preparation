...
を計算したいが、$f$ の原始関数は分からない。あるいは $f$ 自体が高価な数値計算の出力で、有限個の点でしか評価したくない。数値積分は、この状況で積分を
...
を満たすとします。Newton--Cotes 公式では $w\equiv1$ を使います。
...
## 2. 等間隔節点を使うと Newton--Cotes 公式になる
...
補間多項式は二次式なので、補間型求積公式の定理から少なくとも二次までは厳密です。しかし Simpson 則には対称性があり、実際には三次式まで厳密です。
...
「局所誤差の指数をそのまま大域誤差の指数だと思わない」ことは、後の ODE・差分法でも繰り返し現れます。
...
$f\in C^4([-1,1])$ なら、$-1$ を基点にした Taylor の積分形から
...
と書けます。ここで $p_3\in\mathcal P_3$ で、
...
連続関数 $f^{(4)}$ の最小値・最大値の間に、重み $-K(t)\ge0$ を使った平均値が入るので、ある $\xi\in(-1,1)$ が存在して
...
区間中心を $c$、半幅を $h$ として $x=c+hs$ と変数変換すれば
...
$r$ の次数は $n-1$ 以下です。Gauss 求積は補間型求積公式なので、補間型求積公式の定理より
...
を満たす次数 $2n-1$ 以下の Hermite 補間多項式 $H$ を取ります。

Hermite 補間の剰余公式は、[Rolle の定理](../RA3/index.md#thm-ra3-rolle)を反復する NA4 の補間誤差公式と同じ仕組みで
...
$H$ の次数は $2n-1$ 以下なので、Gauss 求積は $H$ を厳密に積分します。
...
$f^{(2n)}$ は連続なので中間の値を取ります。従ってある $\xi\in(a,b)$ が存在して
...
<a id="ex-na5-b2"></a>
#### NA5-B02 2点 Gauss--Legendre 求積をモーメントから再構成する
- Level: B
