...
- 点 $x$ を固定したとき、$S_Nf(x)$ は何へ近づくのか。
- 波形全体として最大誤差まで小さくなるのか。
- 点ごとの値ではなく、一周期の二乗誤差を測るとどうなるのか。

この三つを同じ「収束」で済ませると、跳躍点近傍に残る振動・係数の二乗和と元の二乗積分を結ぶ等式の役割が見えなくなります。本章ではまず誤差の見方を分け、その後
...
直接の前提は [FOU1 Fourier級数・係数計算](../FOU1/index.md) だけです。有限次数の Fourier 理論は FOU1 から受け取り、無限段階で必要になる「固定した点での極限」「最大誤差の極限」「二乗平均誤差の極限」は、本章で使う形をその場で式にして固定します。必要な議論は Riemann 積分と有限次数理論から組み立てます。

以後、実数値で $2\pi$ 周期の関数を「実数値周期関数」と略記します。

---

## 1. 誤差の見方を区別する

関数列 $(f_N)$ が $f$ に近づくとき、本章ではまず次の三つを分けます。
...
を、その固定した点ごとに調べます。これに対して波形全体の最大誤差を見る見方では

$$
\sup_{x\in\mathbb R}|f_N(x)-f(x)|\longrightarrow0
$$

を要求します。後者では、一つの誤差上界がすべての点で同時に使えなければなりません。

Fourier 有限和では、この二つに加えて一周期の平均的な誤差も重要です。

<a id="def-fou2-mean-square-convergence"></a>
<!-- formal-statement-start -->
> **定義（二乗平均収束）**  
> 実数値周期関数列 $(f_N)$ と実数値周期関数 $f$ を考える。各 $f_N,f$ が $[-\pi,\pi]$ 上 Riemann 積分可能で
...
<!-- definition-example-start: def-fou2-mean-square-convergence -->
**定義の確認**：
...
とします。固定した $x$ では $f_N(x)\to0$ です。さらに
...
したがって二乗平均の条件も満たします。三つの条件が同時に成立する列はありますが、測っている誤差は同じではありません。
<!-- definition-example-end -->

最大誤差が0へ行くなら、二乗平均誤差も0へ行きます。実際、$\sup_x|f_N-f|\le\delta_N$ なら
...
一方、二乗平均では有限個の点の値は見えません。積分に影響しないからです。この違いが、Fourier 級数の跳躍点で重要になります。
...
> また、$f$ を実数値周期関数とし、$[-\pi,\pi]$ 上で Riemann 積分可能とする。このとき FOU1 の Fourier 有限和は
...
Dirichlet 核の積分は常に $2\pi$ ですが、$D_N$ は正負に激しく振動します。「全質量が正しい」だけでは、平均化が安定するとは限りません。この符号変化が後の跳躍近傍の振動とつながります。
...
<!-- formal-statement-start -->
> **定義（階段関数）**  
> 実数値写像 $s:[a,b]\to\mathbb R$ が **階段関数** であるとは、ある有限分割
...
区分的 $C^1$ とは、一周期を有限個の区間に分けると各区間の内部で連続微分可能で、区分点では関数と導関数の片側極限が有限に存在する状況をいいます。周期端点 $-\pi,\pi$ は同じ点として左右を読みます。

<a id="thm-fou2-dirichlet-convergence"></a>
<!-- formal-statement-start -->
> **定理（Dirichletの点ごとの極限定理）**  
> $f$ を実数値周期関数とし、一周期上で区分的 $C^1$ とする。任意の実数 $x$ で左右極限
...
と置きます。区分的 $C^1$ 性により、導関数の左右極限

$$
f'_+(x):=\lim_{t\downarrow0}f'(x+t),
\qquad
f'_-(x):=\lim_{t\downarrow0}f'(x-t)
$$

は有限です。ここでは跳躍点での $f(x)$ を基準にした片側微分ではなく、導関数について、左右それぞれから極限を取っています。各枝を $x$ まで左右極限で連続延長し、平均値の定理を適用すると
...
$$
S_Nf(x)-m_x
=\frac1\pi\int_0^\pi g_x(t)\sin\bigl((N+\tfrac12)t\bigr)\,dt.
$$
...
Dirichlet の定理は固定した点の極限を教えます。しかし、跳躍点へ $N$ とともに近づく点を選ぶと別の現象が見えます。

<a id="prop-fou2-gibbs-square-wave"></a>
<!-- formal-statement-start -->
> **命題（標準矩形波のGibbs現象）**  
> $q$ を $2\pi$ 周期の実数値関数で、$-\pi<x<0$ では $q(x)=-1$、$0<x<\pi$ では $q(x)=1$、$x\in\pi\mathbb Z$ では $q(x)=0$ と定める。非負整数 $M$ に対して $N=2M+1$ と置き、FOU1 の係数から
...
各項に $2\sin x\cos y=\sin(y+x)-\sin(y-x)$ を適用して足し合わせると、途中項が相殺され
...
「固定した点では正しい値へ行く」と「最大誤差が0へ行く」は別です。Gibbs 現象はこの違いを最も目に見える形で示します。
...
<!-- formal-statement-start -->
> **定義（Fejér平均）**  
> $f$ を実数値周期関数で $[-\pi,\pi]$ 上 Riemann 積分可能とし、その Fourier 有限和を $S_0f,S_1f,\ldots$ とする。非負整数 $N$ に対し
...
> $f$ を実数値周期関数で $[-\pi,\pi]$ 上 Riemann 積分可能とすると、その Fejér 平均は
...
<!-- formal-statement-start -->
> **定理（Fejér平均の一様近似定理）**  
> $f$ を連続周期関数とする。このとき Fejér 平均 $\sigma_Nf$ は $f$ を一様に近似し、
...
連続周期関数 $f$ はコンパクトな一周期上で有界です。また、[RA2のHeine–Cantorの定理](../RA2/index.md#thm-ra2-heine-cantor)をコンパクト区間 $[-2\pi,2\pi]$ 上の連続な $f$ に適用すると、この区間上で一様連続です。任意の $x$ を周期移動して $[-\pi,\pi]$ に置けば、$|t|<\pi$ のとき $x$ と $x-t$ はともに拡大区間に入ります。したがって同じ $\delta$ を全実数 $x$ に共通して使えます。$M>0$ を
...
## 8. 三角多項式で連続周期関数を一様に近似する

Fejér 平均そのものが三角多項式です。したがって前節の定理は、具体的な近似多項式の存在も与えます。

<a id="thm-fou2-trigonometric-density"></a>
<!-- formal-statement-start -->
> **定理（三角多項式の一様稠密性）**  
> $f$ を連続周期関数とする。任意の $\varepsilon>0$ に対し、ある三角多項式 $T$ が存在して
...
[Fejér平均の一様近似定理](#thm-fou2-fejer-uniform)から
...
この定理が次節の橋です。区分的連続な波形をまず連続周期関数へ二乗平均で近似し、その連続周期関数を Fejér 平均で三角多項式へ近似します。
...
> $f$ を実数値周期関数とし、一周期上で有限個の不連続点しか持たず、連続な各区間では連続で、すべての片側極限が有限であるとする。FOU1 の Fourier 有限和 $S_Nf$ に対して
...
これらの短区間の外では $g=f$ とします。短区間の内部では、両端の $f$ の値を直線で結びます。周期端点をまたぐ区間も円周上で同じようにつなぎます。

こうして連続周期関数 $g$ が得られ、線形補間なので $|g|\le M$ とできます。
...
ここで Dirichlet の点ごとの極限定理は使っていません。二乗平均収束は、Fejér による近似可能性と「同じ次数なら Fourier 有限和が最良」という FOU1 の有限次数の事実から出ています。
...
> $f$ を実数値周期関数とし、一周期上で有限個の不連続点しか持たず、連続な各区間では連続で、すべての片側極限が有限であるとする。実 Fourier 係数を $a_0,a_n,b_n$ とする。このとき
...
> が成り立つ。
>
> とくに全 Fourier 係数が0なら、$f$ はすべての連続点で0である。したがって $f$ が連続なら $f\equiv0$ である。この意味で三角系はこのクラスに対して完全である。
<!-- formal-statement-end -->

### 証明の見取り図

FOU1 の[一周期積分消去則](../FOU1/index.md#thm-fou1-frequency-cancellation)により、元の二乗積分は「$S_Nf$ の二乗積分＋残差の二乗積分」へ正確に分解できます。前節で残差が0へ行くと分かったので、有限係数二乗和を極限へ送れば等号になります。

<!-- proof-start -->
### 証明

FOU1 の係数定義と[一周期積分消去則](../FOU1/index.md#thm-fou1-frequency-cancellation)から
...
さらに FOU1 の[一周期積分消去則](../FOU1/index.md#thm-fou1-frequency-cancellation)による計算から
...
矩形波 $q$ では、[Dirichletの点ごとの極限定理](#thm-fou2-dirichlet-convergence)により固定した連続点で $S_Nq(x)\to q(x)$、跳躍点では左右平均0へ収束します。しかし [Gibbs現象](#prop-fou2-gibbs-square-wave) により跳躍点近くの最大超過量は消えないので、最大誤差は0へ行きません。
...
連続周期関数 $f$ については Fejér 平均 $\sigma_Nf$ の最大誤差まで0へ行きます。本章では元の Fourier有限和 $S_Nf$ が連続関数に対して常に最大誤差まで0へ行くとは主張していません。Fejér 平均を導入する理由は、まさに一様近似を確実に作るためです。
...
1. 固定した点 $x$ で $f_N(x)\to0$ を確認してください。
2. 最大誤差 $\sup_x|f_N(x)|$ が0へ行くことを確認してください。
3. 二乗平均誤差を積分し、二乗平均収束を確認してください。
4. 最大誤差が0へ行けば二乗平均誤差も0へ行く一般の評価を書いてください。
...
を $2\pi$ 周期に延長します。[Dirichletの点ごとの極限定理](#thm-fou2-dirichlet-convergence)を使い、次の極限を求めてください。
...
#### B2. Fejér平均の一様近似を誤差分割から証明する

$f$ を連続周期関数、$|f|\le M$ とします。$\varepsilon>0$ に対し、次の順で
...
が全 $x$ で同時に成り立ちます。したがって最大誤差が0へ行きます。
<!-- solution-end -->

#### B3. 係数エネルギー等式から逆二乗和を求める
...
4. [Dirichletの点ごとの極限定理](#thm-fou2-dirichlet-convergence)から $x=\pi$ での Fourier有限和の極限を求め、係数エネルギー等式の結論と点での極限の結論が別物であることを説明してください。
...
[係数エネルギー等式](#thm-fou2-parseval)は一周期全体の二乗積分と係数二乗和の等式であり、一点 $x=\pi$ の値を主張していません。一方 Dirichlet の定理は固定した点での極限を主張します。同じ Fourier 係数から出る結果でも、測っている量が違います。
<!-- solution-end -->

#### B4. 二乗平均収束と完全性を近似から組み立てる

$f$ は実数値 $2\pi$ 周期関数で、一周期の不連続点は有限個、各連続区間の端での片側極限はすべて有限とします。$|f|\le M$ とし、$M>0$ とします。

1. 不連続点の周囲を全長 $\eta$ の短区間で覆い、区間内を直線でつないだ連続周期関数 $g$ を作って、$\int_{-\pi}^{\pi}|f-g|^2\le4M^2\eta$ を示してください。
2. Fejér 平均を使って $g$ を三角多項式 $T$ で近似し、任意の $\varepsilon>0$ に対して $\int_{-\pi}^{\pi}|f-T|^2<\varepsilon$ とできることを示してください。
3. FOU1 の固定次数最小二乗性を使い、$S_Nf$ の二乗平均収束を証明してください。
4. 全 Fourier 係数が0なら、すべての連続点で $f=0$ となることを、3の結果から示してください。さらに、連続周期関数二つの Fourier 係数がすべて等しければ、関数自体も等しいことを導いてください。
...
1. 有限個の不連続点の周囲に、端点が連続点となる互いに重ならない短区間を取ります。周期端点をまたぐ場合は、$-\pi$ と $\pi$ を同じ点として区間を取ります。短区間 $[a,b]$ の内部で

$$
g(x)=\frac{b-x}{b-a}f(a)+\frac{x-a}{b-a}f(b)
$$

とし、その外では $g=f$ とします。二つの重みは非負で和が1なので $|g|\le M$ です。補間は両端で $f$ と一致するため $g$ は連続周期関数となります。差は短区間の外で0、内部で $|f-g|\le2M$ なので

$$
\int_{-\pi}^{\pi}|f-g|^2\,dx\le(2M)^2\eta.
$$

2. $\eta<\varepsilon/(16M^2)$ とすれば、1の積分は $\varepsilon/4$ 未満です。[Fejér平均の一様近似定理](#thm-fou2-fejer-uniform)は連続周期関数 $g$ に適用できるので、十分大きい整数 $m$ に対し、三角多項式 $T=\sigma_mg$ は
...
$$
\int_{-\pi}^{\pi}|g-T|^2\,dx
<2\pi\frac{\varepsilon}{8\pi}=\frac{\varepsilon}{4}.
$$

$|u+v|^2\le2|u|^2+2|v|^2$ に $u=f-g,v=g-T$ を代入して積分すると

$$
\int_{-\pi}^{\pi}|f-T|^2\,dx
\le2\int_{-\pi}^{\pi}|f-g|^2\,dx
+2\int_{-\pi}^{\pi}|g-T|^2\,dx
<\varepsilon.
$$

3. $T=\sigma_mg$ は $m$ 次以下の三角多項式です。$N\ge m$ なら $N$ 次以下でもあるため、[FOU1の固定次数最小二乗性](../FOU1/index.md#thm-fou1-least-squares)を区分的連続な $f$ と $T$ に適用して

$$
\int_{-\pi}^{\pi}|f-S_Nf|^2\,dx
\le\int_{-\pi}^{\pi}|f-T|^2\,dx<\varepsilon
$$

を得ます。任意の $\varepsilon>0$ に対し、この評価がすべての $N\ge m$ で成り立つので二乗平均収束が示されました。

4. 全 Fourier 係数が0なら各 $S_Nf=0$ なので、3より

$$
\int_{-\pi}^{\pi}|f|^2\,dx=0.
$$

連続点 $x_0$ で $f(x_0)\ne0$ と仮定します。連続性からある $r>0$ について $|x-x_0|<r$ なら

$$
|f(x)-f(x_0)|<\frac{|f(x_0)|}{2},
\qquad |f(x)|>\frac{|f(x_0)|}{2}
$$

です。一周期内に収まる正の長さ $\ell$ の小区間をこの近傍に取れば、積分の非負性と周期性から

$$
\int_{-\pi}^{\pi}|f|^2\,dx
\ge\ell\frac{|f(x_0)|^2}{4}>0
$$

となり矛盾します。よって連続点では $f=0$ です。連続周期関数 $u,v$ の Fourier 係数が一致するなら、積分の線形性により $f=u-v$ の係数はすべて0です。$u-v$ は全点で連続なので $u-v=0$、すなわち $u=v$ が従います。
<!-- solution-end -->

### Level C

#### C1. 矩形波で点ごとの極限・Gibbs・Fejér・係数エネルギー等式を一本につなぐ
...
を導いてください。
6. $S_Nq$ は $q$ へ二乗平均収束する一方、最大誤差が0へ行かない理由を説明してください。
...
となりました。$q(x_M)=1$ なので、その点での誤差は約0.17898から0へ行きません。したがって最大誤差は0へ行きません。

二乗平均では振動が残る領域の幅が縮むことが効きます。最大誤差では一番悪い一点を見続けるので、この Gibbs 超過を無視できません。
<!-- solution-end -->
...
本章では、Riemann 積分と FOU1 の有限次数理論から Fourier 級数の無限段階を構成しました。

- 固定した点での極限・最大誤差・二乗誤差という三つの見方を分離した。
...
- Fejér 核が非負・正規化・原点集中を満たすことを証明し、連続周期関数を最大誤差まで小さく近似できることを示した。
- Fejér 平均を使って三角多項式の一様稠密性を得た。
- 区分的連続関数を連続周期関数へ局所補間し、三角多項式近似と FOU1 の最小二乗性をつないで Fourier 有限和の二乗平均収束を証明した。
