...
本章では **実数値周期関数** という語を、実数値で $2\pi$ 周期の関数の略称として使います。
...
矩形波 $q$ では、Dirichlet の定理により固定した連続点で $S_Nq(x)\to q(x)$、跳躍点では左右平均0へ収束します。しかし [Gibbs現象](#prop-fou2-gibbs-square-wave) により跳躍点近くの最大超過量は消えないので、最大誤差は0へ行きません。
