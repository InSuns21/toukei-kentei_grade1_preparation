...
直接の前提は [FOU1 Fourier級数・係数計算](../FOU1/index.md) だけです。有限次数の Fourier 理論は FOU1 から受け取り、無限段階で必要になる「固定した点での極限」「最大誤差の極限」「二乗平均誤差の極限」は、本章で使う形をその場で式にして固定します。旧 `F0_00FA1_Fourier級数_直交展開`、RA5、Lebesgue 積分、Hilbert 空間の一般論は証明依存先にしません。
...
> **定義（二乗平均収束）**  
> 実数値周期関数列 $(f_N)$ と実数値周期関数 $f$ を考える。各 $f_N,f$ の二乗が $[-\pi,\pi]$ 上 Riemann 積分可能で
...
本章では、旧 Fourier 教材や別系列の一般収束論を証明依存先にせず、FOU1 の有限次数理論から Fourier 級数の無限段階を構成しました。
