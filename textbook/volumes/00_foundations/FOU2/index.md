...
この三つを同じ「収束」で済ませると、跳躍点・Gibbs 現象・Parseval 等式の役割が見えなくなります。本章ではまず収束概念を分け、その後
...
Dirichlet 核が偶関数であることを使い、$x$ の右側と左側を組にします。左右平均を引くと、分子は $t$ の一次の大きさで0へ行きます。これを $\sin(t/2)$ で割った関数は $t=0$ でも有限値を持ち、最後は Riemann--Lebesgue 型補題で高周波を消します。
...
$$
\sigma_Nf(x):=rac1{N+1}\sum_{k=0}^{N}S_kf(x)
$$
...
\begin{aligned}
\sigma_Nf(x)
&=\frac1{N+1}\sum_{k=0}^{N}rac1{2\pi}
...
したがって Dirichlet 部分和のように $1$ を越える Gibbs 超過は起こりません。さらに $q$ は奇関数なので $\sigma_Nq(0)=0$ です。連続点 $x$ を固定すれば、上の証明で一様連続性を「その点の近傍での連続性」に置き換えることで $\sigma_Nq(x)\to q(x)$ が従います。
...
ここで Dirichlet の各点収束定理は使っていません。二乗平均収束は、Fejér による近似可能性と「同じ次数なら Fourier 有限和が最良」という FOU1 の有限次元事実から出ています。
...
FOU1 の直交性により、元の二乗積分は「$S_Nf$ の二乗積分＋残差の二乗積分」へ正確に分解できます。前節で残差が0へ行くと分かったので、有限係数二乗和を極限へ送れば等号になります。
...
次に全 Fourier 係数が0とします。Parseval 等式から
...
矩形波 $q$ では、Dirichlet の定理により固定した連続点で $S_Nq(x)\to q(x)$、跳躍点では左右平均0へ収束します。しかし Gibbs 現象により跳躍点近くの最大超過量は消えないので、一様収束はしません。
...
連続周期関数 $f$ については Fejér 平均 $\sigma_Nf$ が一様収束します。本章では ordinary partial sum $S_Nf$ が連続関数に対して常に一様収束するとは主張していません。Fejér 平均を導入する理由は、まさに一様近似を確実に作るためです。
...
を $2\pi$ 周期に延長します。Dirichlet の各点収束定理を使い、次の極限を求めてください。
...
#### B1. Riemann--Lebesgue型補題を階段関数で再構成する
...
1. Parseval 等式の左辺を計算してください。
2. 右辺を係数で書いてください。
3. $\sum_{n=1}^{\infty}1/n^2=\pi^2/6$ を導いてください。
4. Dirichlet の定理から $x=\pi$ での Fourier 部分和の極限を求め、Parseval の結論と各点値の結論が別物であることを説明してください。
...
Parseval 等式は一周期全体の二乗積分と係数二乗和の等式であり、一点 $x=\pi$ の値を主張していません。一方 Dirichlet の定理は固定した点での極限を主張します。同じ Fourier 係数から出る結果でも、測っている収束の意味が違います。
<!-- solution-end -->
...
よって Fejér 平均は値域 $[-1,1]$ を越えません。Dirichlet 部分和では核が符号を変えるため、跳躍近傍で正負の重みが作る超過が残ります。
