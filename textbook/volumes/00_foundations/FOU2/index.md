...
この三つを同じ「収束」で済ませると、跳躍点近傍に残る振動・係数の二乗和と元の二乗積分を結ぶ等式の役割が見えなくなります。本章ではまず収束様式を分け、その後
...
直接の前提は [RA5 関数列・関数級数・一様収束](../RA5/index.md) と [FOU1 Fourier級数・係数計算](../FOU1/index.md) です。RA5 から一般の各点収束・一様収束だけを使い、FOU1 から有限次数の Fourier 理論を受け取ります。旧 `F0_00FA1_Fourier級数_直交展開` は証明依存先にしません。Lebesgue 積分や Hilbert 空間の一般論も本章の証明には使わず、区分的連続関数と Riemann 積分の範囲で閉じます。

本章では、実数値で $2\pi$ 周期の関数を **実数値周期関数** と略記します。

---

## 1. 収束様式を区別する

RA5 で定義した[各点収束](../RA5/index.md#def-ra5-pointwise)と[一様収束](../RA5/index.md#def-ra5-uniform)をここでも使います。前者は点を固定してから極限を取り、後者は一つの誤差評価を全ての点で共有します。本章では、Fourier 有限和の平均的な誤差を測るため、もう一つの収束様式を加えます。
...
とします。すべての $x$ で $f_N(x)\to0$ なので RA5 の各点収束を満たします。さらに

$$
\sup_x|f_N(x)|\le\frac1N\to0
$$

なので RA5 の一様収束も満たします。また
...
したがって本章で定義した第3の条件も満たします。一つの列が三つすべてを満たすことはありますが、三つの収束様式は同じではありません。
<!-- definition-example-end -->

RA5 の一様収束は二乗平均収束を導きます。実際、$\sup_x|f_N-f|\le\delta_N$ なら
...
<!-- formal-statement-start -->
> **定理（Fejér平均の一様近似定理）**  
> $f$ を連続周期関数とする。このとき Fejér 平均 $\sigma_Nf$ は $f$ へ一様収束する。すなわち
...
この定理が次節の橋です。区分的連続関数をまず連続周期関数へ二乗平均で近似し、その連続周期関数を Fejér 平均で三角多項式へ近似します。
...
> $f$ を実数値周期関数とし、一周期上で有限個の不連続点しか持たず、各連続区間で連続で、すべての片側極限が有限であるとする。FOU1 の Fourier 有限和 $S_Nf$ に対して
...
これらの短区間の外では $g=f$ とし、各短区間の内部では両端の $f$ の値を直線で結びます。周期端点をまたぐ区間も円周上で同じようにつなぎます。こうして連続周期関数 $g$ が得られ、線形補間なので $|g|\le M$ とできます。
...
次に[三角多項式による一様近似定理](#thm-fou2-trigonometric-density)から、ある三角多項式 $T$ が存在して
...
> $f$ を実数値周期関数とし、一周期上で有限個の不連続点しか持たず、各連続区間で連続で、すべての片側極限が有限であるとする。実 Fourier 係数を $a_0,a_n,b_n$ とする。このとき
...
#### C1. 矩形波で各点収束・Gibbs・Fejér・係数エネルギー等式を一本につなぐ
...
本章では、旧 Fourier 教材を証明依存先にせず、RA5 の一般収束概念と FOU1 の有限次数理論から Fourier 級数の無限段階を構成しました。

- 点ごとの極限・最大誤差・二乗誤差という三つの見方を分離した。
...
- Fejér 核が非負・正規化・原点集中を満たすことを証明し、連続周期関数への一様近似を示した。
- Fejér 平均を使って三角多項式による一様近似を得た。
