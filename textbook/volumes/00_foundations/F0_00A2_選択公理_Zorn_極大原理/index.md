# F0-00A2 補講：選択関数・選択公理・可算選択

この講義の問いは、**「非空だと分かっている集合が大量にあるとき、全部から一斉に1個ずつ選べると言ってよいのか」**です。

Zornの補題そのものは次講F0-00A3へ分離します。

---

## 1. 「選ぶ」だけなら何が難しいのか

有限個の非空集合

$$
A_1,\dots,A_n
$$

から1個ずつ元を選ぶことには、通常問題はありません。

たとえば

$$
a_i\in A_i
$$

を順番に選べばよいからです。

ところが、添字集合 $I$ が任意の集合で、

$$
\{A_i\}_{i\in I}
$$

がすべて非空だとします。

このとき

$$
f(i)\in A_i
\qquad(i\in I)
$$

を満たす写像

$$
f:I\to\bigcup_{i\in I}A_i
$$

が存在する、と常に言ってよいでしょうか。

これを保証するのが選択公理です。

---

## 2. 選択関数

<a id="def-choice-function"></a>

<!-- formal-statement-start -->
> **定義（選択関数）**  
> 添字集合 $I$ と非空集合族 $\mathcal A=\{A_i\}_{i\in I}$ に対し、写像
$$
f:I\to\bigcup_{i\in I}A_i
$$
> が
$$
f(i)\in A_i
\qquad(i\in I)
$$
> を満たすとき、$f$ を $\mathcal A$ の **選択関数** という。
<!-- formal-statement-end -->

<!-- definition-example-start: def-choice-function -->
### 例：二つの集合から一つずつ選ぶ

**定義の確認**

$$
I=\{1,2\},\qquad A_1=\{a,b\},\qquad A_2=\{10,20\}
$$

とし、

$$
f(1)=a,\qquad f(2)=20
$$

と定めます。すると

$$
f(1)\in A_1,\qquad f(2)\in A_2
$$

なので、定義より $f$ はこの集合族の選択関数です。
<!-- definition-example-end -->

注意したいのは、各 $A_i$ に「最小の元」など自然な選び方があるなら、選択公理は要らないことです。

たとえば

$$
A_n=\{n,n+1,n+2,\dots\}
$$

なら

$$
f(n)=n
$$

と明示できます。

問題になるのは、**各集合が非空だということ以外に選び方の規則がない**場合です。

---

## 3. 選択公理

<a id="axiom-choice"></a>

<!-- formal-statement-start -->
> **公理（選択公理）**  
> 添字集合 $I$ と非空集合族 $\{A_i\}_{i\in I}$ に対して、選択関数 $f$ が存在する。すなわち
$$
\forall i\in I,\ A_i\ne\varnothing
\quad\Longrightarrow\quad
\exists f:I\to\bigcup_{i\in I}A_i
\ \text{s.t.}\ 
f(i)\in A_i\quad(i\in I)
$$
> が成り立つ。
<!-- formal-statement-end -->

この主張は有限の場合には特別な公理を必要としません。しかし「任意の添字集合」に広げると、ZFだけからは証明できません。

標準的には、ZFに選択公理を加えた体系を **ZFC** と呼びます。

---

## 4. 可算選択と完全な選択公理は違う

<a id="axiom-countable-choice"></a>

<!-- formal-statement-start -->
> **公理（可算選択公理）**  
> 非空集合列 $(A_n)_{n\in\mathbb N}$ に対して、各 $n\in\mathbb N$ で
$$
f(n)\in A_n
$$
> を満たす写像 $f:\mathbb N\to\bigcup_{n\in\mathbb N}A_n$ が存在する。
<!-- formal-statement-end -->

完全な選択公理は添字集合を任意の集合 $I$ まで許すので、可算選択公理より強い主張です。

したがって

> 無限個あるから全部選択公理が必要

ではありません。

数学では、必要な選択の強さを区別することが重要です。

本教材では、Hahn--Banachや一般の基底の存在を扱うため、標準的なZFCの立場を採ります。

---

## 5. 整列を先に定義する

選択公理の代表的な同値形を述べる前に、「整列可能」の意味を固定します。

<a id="def-well-order"></a>

<!-- formal-statement-start -->
> **定義（整列）**  
> 集合 $X$ 上の全順序 $\preceq$ が、$X$ の任意の非空部分集合 $A\subset X$ に最小元を持たせるとき、$\preceq$ を $X$ の **整列** という。
<!-- formal-statement-end -->

<!-- definition-example-start: def-well-order -->
### 例：自然数の通常の順序

**定義の確認**

$\mathbb N$ に通常の順序 $\le$ を入れます。自然数の任意の非空部分集合は最小元を持つので、$\le$ は $\mathbb N$ の整列です。

一方、整数 $\mathbb Z$ の通常の順序では、非空部分集合 $\mathbb Z$ 自身に最小元がないため整列ではありません。
<!-- definition-example-end -->

したがって「任意の集合は整列可能である」とは、任意の集合 $X$ に対して、その上にこの意味での整列を少なくとも一つ入れられるという主張です。

---

## 6. 選択公理・Zorn・整列可能定理

<a id="thm-choice-equivalences"></a>

<!-- formal-statement-start -->
> **定理（選択公理の代表的同値形）**  
> ZFの上では、次の三つは同値である。
> 1. 選択公理。
> 2. Zornの補題。
> 3. 任意の集合は整列可能である、という整列可能定理。
<!-- formal-statement-end -->

この同値性の完全な証明は集合論の一講義になります。本補講では次の意味を押さえます。

- 選択公理：任意族から一斉に元を選ぶ
- 整列可能定理：任意集合を「最初から順番に」並べられる
- Zorn：chainごとに上界があるなら極大元を得られる

見た目は違いますが、いずれも「任意に大きな集合で選択・延長を続ける」ことを保証しています。

---

## 7. どこまで選択公理を気にすべきか

統計検定1級の答案で「ここは選択公理を用いる」と書く場面はまずありません。

しかし発展理論では、

- 任意のベクトル空間の基底
- Hahn--Banach
- Tychonoffの定理
- Banach--Alaogluの一般形

など、選択原理が背景にある定理が多くあります。

本教材での目的は集合論そのものではなく、

> 「存在する」と言ったとき、何がその存在を支えているのか

を見失わないことです。

---

## 8. 演習

### F0-00A2-A01 選択公理が不要な選択

- Level: A
- 目安時間: 8分

$A_n=\{n,n+1,n+2,\ldots\}$ $(n\in\mathbb N)$ に対し、選択関数を一つ明示せよ。この例で完全な選択公理を持ち出す必要がない理由も述べよ。

<!-- solution-start -->
#### 詳細解答
$f(n)=n$ と置けば $f(n)\in A_n$ なので選択関数である。各集合に明示的な選択規則が与えられているため、存在だけを公理で保証する必要はない。

#### 本番答案
$f(n)=n$ とすればよい。選び方を具体的に構成できるため、選択公理は不要。

#### 採点基準（20点）
- 選択関数: 10点
- 所属確認: 4点
- 選択公理不要の理由: 6点
<!-- solution-end -->

### F0-00A2-B01 可算選択と完全な選択を区別する

- Level: B
- 目安時間: 10分

「非空集合 $A_n$ $(n\in\mathbb N)$ から一つずつ元を選べる」という主張と、完全な選択公理の違いを説明せよ。

<!-- solution-start -->
#### 詳細解答
前者は添字集合が $\mathbb N$ に限られた可算選択公理である。完全な選択公理は任意の添字集合 $I$ に対する非空集合族 $\{A_i\}_{i\in I}$ について選択関数の存在を主張する。したがって完全な選択公理の方が強い。

#### 本番答案
前者は可算集合族だけを対象にする可算選択。完全な選択公理は任意の添字集合に対して選択関数を保証する。

#### 採点基準（20点）
- 可算選択の対象: 7点
- 完全な選択の対象: 7点
- 強さの違い: 6点
<!-- solution-end -->

---

## 9. 次に進む

選択公理と同値な形のうち、関数解析で最も使いやすいのがZornの補題です。次講では半順序・chain・極大元から極大延長テンプレートを作ります。

**次：[F0-00A3 半順序・Zornの補題・極大延長](../F0_00A3_半順序_Zorn_極大延長/index.md#thm-zorn)**
