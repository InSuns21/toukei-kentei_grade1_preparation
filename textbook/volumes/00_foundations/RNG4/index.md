...
整数と多項式には、よく似た構造があります。体 $F$ を係数とする多項式全体 $F[x]$ では、整数の除法に対応する
...
一方、係数環が体ではなく $\mathbb Z$ や一般の一意分解整域 $R$ になると、先頭係数を自由に割れません。ここでは、まず係数全体の最大公約元を切り出し、共通因子を持たない部分へ分ける考え方が必要になります。その積の振る舞いが、後半で係数環と分数体の因数分解を結ぶ鍵になります。
...
です。最後には、有理数の根の候補を絞る方法と、係数の素因子だけから既約性を保証する方法を使って、具体的な多項式を実際に判定します。

> **この章の停止線**
>
> 本章では一変数の多項式の環構造、除法、係数の共通因子の分離、係数環と分数体の因数分解の対応、一意分解、既約判定まで扱います。体拡大、代数的元、それらを記述する既約多項式、分解体は FLD1 以降へ送ります。多変数多項式環、一般 Noether 環、局所化、Krull 次元は扱いません。
