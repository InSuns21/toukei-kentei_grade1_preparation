...
一方、係数環が体ではなく $\mathbb Z$ や一般の一意分解整域 $R$ になると、先頭係数を自由に割れません。ここで必要になるのが **content（係数の最大公約元）** と **原始多項式**、そして Gauss の補題です。
...
> **この章の停止線**
>
> 本章では一変数多項式環、除法、content、原始多項式、Gauss の補題、一意分解、既約判定まで扱います。体拡大、代数的元、それらを記述する既約多項式、分解体は FLD1 以降へ送ります。多変数多項式環、一般 Noether 環、局所化、Krull 次元は扱いません。
...
<!-- definition-example-start: def-rng4-content-primitive -->
...
<a id="thm-rng4-gauss-primitive-product"></a>
<!-- formal-statement-start -->
...
逆向きでは、$K[x]$ の因数分解から各因子の分母を払って $R[x]$ へ戻します。そのとき余分に現れる定数因子を content の乗法性で吸収し、原始部分だけの因数分解を取り出します。
...
右辺では Gauss の補題により $G_0H_0$ は原始なので
...
Gauss の補題では、係数の共通因子を既約元へ分解し、その既約元を素元として使います。
