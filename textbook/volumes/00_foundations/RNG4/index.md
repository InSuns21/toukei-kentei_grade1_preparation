...
> **この章の停止線**
>
> 本章では一変数多項式環、除法、content、原始多項式、Gauss の補題、一意分解、既約判定まで扱います。体拡大、代数的元、最小多項式、分解体は FLD1 以降へ送ります。多変数多項式環、一般 Noether 環、局所化、Krull 次元は扱いません。
...
$$
>
> 全体を $R[x]$ と書き、係数ごとの加法と畳み込みによる乗法で環とする。
>
> $f\ne0$ に対し、$a_n\ne0$ で $a_k=0\ (k>n)$ なら
>
$$
...
$\gcd(p,q)=1$ なので $\gcd(p,q^n)=1$ です。よって Euclid の補題から
...
一般の整域では既約元が素元とは限らないことを RNG3 で見ました。この機構が失われると、原始多項式の積に関する素因子追跡がそのままでは使えません。
...
#### RNG4-C01 平行移動と Eisenstein 判定を組み合わせる
...
自己同型は単元を単元へ写し、次数も平行移動では変わらないので、これは $T(f)$ の非自明な因数分解です。
...
次の抽象代数主線では **MOD1「加群・部分加群・商加群・自由加群」** へ進みます。体論側では RNG4 を prerequisite として FLD1「体拡大・代数的元・最小多項式」へ分岐できます。
