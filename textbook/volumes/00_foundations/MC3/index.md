...
を作り、独立な複製 $Z_{c,1},\ldots,Z_{c,N}$ の平均で $I$ を推定する方法を**対照変量法**という。
...
## 3. 層化抽出：母集団を分けて各部分を必ず観測する

通常 Monte Carlo では、標本が偶然ある領域へ偏ることがあります。

層化抽出では標本空間をいくつかの部分へ分け、各部分から標本を取ります。
...
## 4. 重点サンプリング：よく効く領域を多く引き、重みで補正する
...
従って、期待値が既知というだけでは不十分で、$Y$ と相関する対照変量を選ぶ必要があります。
<!-- solution-end -->
...
3. $X=U$、$E[X]=1/2$ を対照変量としたときの最適係数と一標本分散を求めよ。
...
- 対照変量法：期待値が既知で対象と相関する変数を使い、**既知平均からのずれを差し引く**。
- 層化抽出：空間を分けて各層を必ず観測し、**層内分散だけを標本誤差として残す**。
- 重点サンプリング：寄与の大きい領域を多く引くよう**提案密度を変え、尤度比で期待値を補正する**。
