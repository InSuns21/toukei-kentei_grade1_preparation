...
[GEO13](../GEO13/index.md) では、Riemann 計量から Levi-Civita 接続を一意に構成し、曲線に沿う共変微分と平行移動まで整えました。本章では、その接続に対して「速度ベクトル自身が平行である曲線」を選びます。

これが **測地線**です。

Euclid 空間では測地線は直線です。しかし一般の Riemann 多様体では、測地線は
...
という流れで、接空間の線形構造を多様体へ移します。

最後に凸正規近傍を導入します。GEO15 の Hopf--Rinow の定理では、本章の局所結果を大域へ押し広げます。
...
従って [逆関数定理](../RA6A/index.md)により、$0\in T_pM$ の十分小さい開近傍 $U_0$ と $p\in M$ の開近傍 $U$ を取って
...
## 9. Gauss の補題：放射方向と角方向は直交する

正規座標を局所距離へ結び付ける核心が Gauss の補題です。
...
さらに Gauss の補題を $v=\beta=ru$ と $w=u$ に適用すると
...
しかし十分短い放射測地線なら、正規球から外へ出る競合曲線の方がむしろ長くなります。

<a id="cor-geo14-local-minimizing"></a>
<!-- formal-statement-start -->
> **系（測地線の局所最短性）**  
> 任意の測地線 $\gamma:I\to M$ と $t_0\in I$ に対し、ある $\delta>0$ が存在して、
>
$$
t_0-\delta<s<t<t_0+\delta
$$
>
> なら
>
...
$\alpha$ が $U$ 内に留まるなら、前節から

$$
L(\alpha)\ge|v|.
$$

$\alpha$ が $U$ を出るなら、最初に $U$ を出る直前までの部分曲線を考えます。
...
が $0$ から $R$ へ近づかなければなりません。

前節と同じ評価から、その部分だけで長さは少なくとも $R$ です。
...
一般の十分近い $s<t$ については、基点を $\gamma(s)$ に取り直します。

測地線の初期値への滑らかな依存と $\gamma([t_0-\delta,t_0+\delta])$ のコンパクト性を使って $\delta$ を必要なら縮めると、各 $\gamma(s)$ のまわりで同じ議論を適用できる一様な小ささを確保できます。
...
>
> $q=\exp_p(v)$ が十分小さい半径領域にあり、前節の局所最短性が適用できるとする。
>
> このとき
>
$$
...
GEO15 では、近傍内の任意の二点を直接結びたいので、もう一段強い概念を使います。
...
$$
>
> が存在し、しかもその測地線が $x$ と $y$ の間の距離を実現することをいう。
<!-- formal-statement-end -->
...
完全な主張は「基点を固定した $\exp_p$」だけでなく、基点 $x$ 自身も動かして指数写像を同時に扱うことで得られます。
...
$$
E:TM\supset\mathcal U\to M\times M,
$$
...
ここで $\pi:TM\to M$ は束射影です。

測地線の初期値への滑らかな依存から、$\mathcal U$ を十分小さく取れば $E$ は滑らかです。
...
第二成分について、基点を $\xi$ だけ動かすと時刻0の位置が同じだけ動くので $\xi$ が現れ、初期速度を $\eta$ だけ動かすと
...
逆関数定理より、$0_p$ の近傍 $\mathcal W\subset TM$ と $(p,p)$ の近傍 $\mathcal O\subset M\times M$ を取って
...
従って $(x,y)\in\mathcal O$ に対して一意な小さい接ベクトル
...
が $x$ から $y$ への一意な短い測地線です。

残る問題は、この測地線を一つの共通近傍の中に閉じ込めることです。
...
$[0,1]$ はコンパクトなので、$(x,y)$ を $(p,p)$ に十分近く制限すれば、全ての $0\le t\le1$ について
...
さらに GEO12 で Riemann 距離が多様体位相を誘導することを証明済みなので、十分小さい距離球
...
$$
\overline U\subset U_1
$$

とできます。

$U$ を必要ならさらに縮め、任意の $x,y\in U$ について上の短い測地線の長さが

$$
L(\gamma_{x,y})<\rho
$$

となるようにします。これは $V(x,y)\to0$ と初期値への滑らかな依存から可能です。

もし $\gamma_{x,y}$ が $U$ の外へ出るなら、ある時刻 $t_*$ で

$$
d(p,\gamma_{x,y}(t_*))=\rho
$$
...
$x\in U$ なので $d(p,x)<\rho$ ですが、ここだけでは矛盾にはなりません。
...
もし $\gamma_{x,y}$ が $B_d(p,\rho)$ の外へ出るなら
...
従って、$U$ の二点を結ぶ短い測地線は少なくとも共通の正規領域 $B_d(p,\rho)$ 内に留まります。

最後に $U$ を、上の端点写像 $E$ の一意性領域と各点中心の局所最短性領域に同時に入るようさらに縮めます。

すると任意の $x,y\in U$ に対して

$$
\gamma_{x,y}(t)=\exp_x(tV(x,y))
$$

は $U$ 内に取ることができ、Gauss の補題による局所最短性から距離を実現します。

また別の $U$ 内測地線があれば、$E$ の局所一意性に反します。
...
2. 接空間内で
   ...
   Gauss の補題より、$u$ 方向と $r\dot u$ 方向の像は直交します。
...
さらに Gauss の補題から放射方向と角方向の直交性を導き、
