...
> $$
> と $f(x)<\infty$ を満たす点 $x$ を考えます。$p\in\mathbb R^n$ が
> $$
> f(y)\ge f(x)+p^{\mathsf T}(y-x)
> \qquad(\forall y\in\mathbb R^n)
> $$
> を満たすとき、$p$ を $x$ における **劣勾配** といいます。劣勾配全体を
> $$
> \partial f(x)
> $$
> と書き、$x$ における **劣微分** といいます。
...
なので $p_1+p_2\in\partial(f+g)(0)$ です。
...
と置きます。有限値凸関数は有限次元で連続なので $A$ は開凸集合、$B$ は凸集合です。また $p\in\partial(f+g)(0)$ より $A\cap B=\varnothing$ です。
...
> **注意**  
> 拡張実数値凸関数の和則では定義域の境界で退化を防ぐ条件が必要です。代表例は
> $$
...
の support function です。$p\in\partial h(x)$ なら全方向で
...
$$
\boxed{\partial h(0)=\operatorname{conv}\{-1,1\}=[-1,1]}.
$$
#### 本番答案
...
### F0-02C4-B02 normal coneで制約付き最適性
