...
ただし、デジタルネットは実数の通常の加法ではなく、**$b$ 進桁を有限体上で足す構造**から作られています。そのため、QMC3 の Fourier モードに対応する自然な基底が **Walsh 関数**です。
...
さらに、Walsh 周波数の「粗さ」を測る Niederreiter--Rosenbloom--Tsfasman 型重み、以下 **NRT 重み**、を導入すると、QMC4 の $t$ 値が
...
## 1. Walsh 関数は「桁ごとの有限 Fourier 解析」である
...
最後に三角不等式を使えば絶対値上界が従います。
<!-- proof-end -->

---

## 8. NRT 重み：Walsh 周波数の「最初の非零桁」を測る
