...
[RKHS の表現定理](../RKHS2/index.md#thm-rkhs2-representer)の射影論証により、任意の $f\in\mathcal H$ から標本部分空間の外の直交成分を捨てても訓練値は変わらず、目的値は増えません。従って探索を

$$
S=\operatorname{span}\{K(x_1,\cdot),\dots,K(x_n,\cdot)\}
$$
...
です。ここで $G$ の正半定値性を使いました。よって $\alpha_\lambda$ は標本部分空間上の最小係数です。先に確認した $\inf_{f\in\mathcal H}J_\lambda(f)=\inf_{f\in S}J_\lambda(f)$ から、この係数が表す関数は $\mathcal H$ 全体でも最小解であり、存在も同時に確認できました。
