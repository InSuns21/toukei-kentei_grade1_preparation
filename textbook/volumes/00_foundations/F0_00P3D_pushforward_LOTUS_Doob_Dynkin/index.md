...
$m$ は一般に $P_Y$ に関してほとんど至る所（almost everywhere; a.e.）の意味でしか一意ではありません。これは条件付き期待値自体が $P$-a.s. 一意だからです。
...
<!-- solution-start -->
指示関数では押し出し測度の定義。単関数へ線形性で拡張。非負可測関数は単関数単調近似と単調収束定理（Monotone Convergence Theorem; MCT）。一般可積分関数は正負部分へ分解する。
<!-- solution-end -->
