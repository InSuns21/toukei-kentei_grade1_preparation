...
有限過去予測の最遠ラグ係数は、中間のラグを線形予測で取り除いた後に残る端点間の線形関係を表します。これを次で定義します。
...
[TSA5](../TSA5/index.md) では、定常性だけでは標本平均や標本自己共分散の収束が保証されない問題へ進みます。測度保存シフト、エルゴード性、平均エルゴード定理、混合性、従属系列の大数則・中心極限定理、標本自己共分散の一致性を扱います。
