...
TSA3 では、二次定常過程を [スペクトル表現](../TSA3/index.md#thm-tsa3-spectral-representation)
...
この一本の対応から、ARMA の因果性・可逆性、スペクトル密度、ARIMA の差分、季節差分、自己相関関数と偏自己相関関数の切れ方までを一つの言葉で整理します。
...
従って係数が絶対可算和可能なら部分和列は $L^2$ Cauchy 列です。$L^2$ の完備性により極限が存在します。

<a id="prop-tsa4-filter-stationary"></a>
...
<a id="thm-tsa4-filter-spectrum"></a>
...
係数は多項式次数 $m_\nu-1$ の増加しか持たず、$|r_\nu|^{-n}$ は指数減衰するので絶対可算和可能です。有限個の絶対可算和可能列の畳み込みも絶対可算和可能です。従って $1/a(z)$ の係数列 $(c_j)$ は
...
は $L^2$ で定義できます。絶対可算和可能な係数列の畳み込み則から
...
[TSA3 の弱ホワイトノイズのスペクトル](../TSA3/index.md#def-tsa3-spectral-density)は
...
[線形フィルタによるスペクトル測度の変換](index.md#thm-tsa4-filter-spectrum)を適用すると
...
> $$
>
> とします。対応する共分散行列が正定値で係数が一意に定まるとき、
>
> $$
> \boxed{
> \alpha(k):=a_{k,k}
...
>    $$
>
>    で $Z_t$ が標準イノベーション、有限過去の共分散行列が正定値とします。このとき
>
>    $$
>    \alpha(k)=0
>    \qquad(k>p).
...
で $\lambda=0$ に特異点を持ちます。

この発散は「低周波成分を無限に増幅する」ことを表し、通常の定常スペクトル密度が存在しないことと整合します。
...
ただし最後の二つは単純な Fourier 変換だけで相互に変換される主張ではありません。ACF/PACF の打切りは有限次数モデルの予測構造、スペクトルの有理形はフィルタ構造を表し、**同じ ARMA モデルを異なる角度から特徴付ける**ものです。

---

# 演習

## TSA4-A01 二点移動平均の自己共分散とスペクトル
...
2. 伝達関数を求めよ。
3. スペクトル密度を求めよ。
4. $\phi>0$ と $\phi<0$ でスペクトルの最大位置がどう変わるか説明せよ。
...
1. 係数列を $(h_j)$、$(g_j)$ とします。すると
   ...
   絶対可算和可能性により和の順序を交換でき、合成係数は畳み込み
...
## TSA4-C01 ARIMA・季節差分・有理スペクトルを統合する
...
4. $X_t$ から $Y_t$ への差分フィルタの伝達関数は
   ...
   TSA3 のスペクトル測度は有限測度でなければならず、その全質量は分散に等しいので、この発散は $X_t$ が二次定常でないことと整合します。従って非定常 $X$ へ通常のスペクトル密度を機械的に割り当ててはいけません。
...
- 線形フィルタは時間領域の畳み込みと周波数領域の乗算を結ぶ。
- ARMA の因果性は AR 多項式 $\phi$ の単位円外根条件で決まる。
- ARMA の可逆性は MA 多項式 $\theta$ の単位円外根条件で決まる。
- 因果 ARMA のスペクトルは
  $$
  \frac{\sigma^2}{2\pi}
  \frac{|\theta|^2}{|\phi|^2}
...
TSA5 では、定常性だけでは標本平均や標本自己共分散の収束が保証されない問題へ進みます。定常シフト、エルゴード性、平均エルゴード定理、mixing の入口、従属系列の大数則・中心極限定理、標本自己共分散の一致性を扱います。
