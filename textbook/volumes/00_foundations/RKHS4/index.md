...
Slater 条件が成り立つので最適 KKT 組が存在します。$w^*\ne0$ なので、停留条件
...
で正例と負例がともに存在するため、正例側にも負例側にも正の双対変数を持つ点が少なくとも一つずつ存在します。

正例側で $\alpha_i^*>0$ の点を $x_+$、負例側で $\alpha_j^*>0$ の点を $x_-$ とします。
...
であり、$w^*$ と正の双対変数を持つ正例が与えられれば $b^*$ は一意です。

別の最適切片 $\widetilde b$ が存在するとしても、同じ $w^*$ に対する最適双対解の相補性から同じ等式を満たすため、
...
線形分離可能で正負両クラスがあるため $w^*\ne0$ です。従って $\alpha^*$ は零ベクトルではありません。
...
係数は非負なので、正例側に正の $\alpha_i^*$ があるなら負例側にも正の $\alpha_j^*$ が必要です。逆も同様です。したがって両クラスに少なくとも1点ずつ正の双対変数を持つ点があります。

正例側で $\alpha_i^*>0$ なら、相補性から
