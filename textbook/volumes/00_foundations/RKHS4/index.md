...
[RKHS3](../RKHS3/index.md) では無限次元の関数空間が有限個の標本へ落ちる構造を見ました。本章はいったん有限次元の超平面分類へ戻り、SVM 固有の最適化構造を分離して理解します。核を用いた非線形分類への拡張は次章で、この構造をそのまま特徴空間へ持ち上げます。
...
線形分離可能性から実行可能点 $(\bar w,\bar b)$ を一つ取れます。目的値がこの点以下の実行可能点だけを見れば
...
正例を一つ $x_+$、負例を一つ $x_-$ 固定します。実行可能性から

$$
b\ge 1-w^{\mathsf T}x_+,
\qquad
b\le -1-w^{\mathsf T}x_-.
$$

Cauchy--Schwarz の不等式と $\|w\|_2\le R$ により
...
従って、この目的値以下の実行可能集合では $w$ も $b$ も有界です。制約は連続関数の閉半空間で定まるため、この集合は閉でもあります。有限次元では閉有界集合はコンパクトなので、連続な目的関数

$$
(w,b)\longmapsto \frac12\|w\|_2^2
$$

は最小値を達成します。

したがって、以後の「最適解」「KKT 組」は単なる下限ではなく、実際に存在する点として扱えます。
...
これが **サポートベクトル（support vector）** という名前の代数的な意味です。
...
$$
>
> と置けば、$(w_0,b_0)$ はハードマージン主問題の最適解を与える。
<!-- formal-statement-end -->
...
ここで構成した $(w_0,b_0)$ が最適解であることまでは、この節だけで示せています。第12節では、最適な $w,b$ がそれぞれ一つに定まることを証明します。その結果と合わせれば、任意の最適解 $(w^*,b^*)$ はこの $(w_0,b_0)$ と一致します。
<!-- proof-end -->
...
逆向きも成り立ちます。任意の $p\in C_+$ と $q\in C_-$ は、それぞれ正例・負例の凸結合として書けます。その凸結合係数を各側で $\theta_i^+,\theta_i^-$ とし、任意の $\rho>0$ に対して正例側を $\alpha_i=\rho\theta_i^+$、負例側を $\alpha_i=\rho\theta_i^-$ と置けば、$\alpha_i\ge0$ と $\sum_i\alpha_i y_i=0$ を満たし、同じ $p,q,\rho$ を再現できます。

したがって、非零の双対実行可能点を $(\rho,p,q)$ で表す操作は情報を失っていません。
...
[線形分離可能性と凸包非交差の同値性](#thm-rkhs4-convex-hull-separation)より、この標本は線形分離可能ではありません。
...
次章では制約違反を許す変数を導入し、この実行不可能性を **ソフトマージン**へ変換します。
...
次章では、線形分離不能データに対して **スラック変数**と **ヒンジ損失**を導入します。その後、内積

$$
x_i^{\mathsf T}x_j
$$

を核値へ置き換えることで、ここで得た双対構造を RKHS 上の **カーネル SVM** へ持ち上げます。
...
逆に任意の $p\in C_+$、$q\in C_-$ を凸結合で表す係数を $\theta_i^+,\theta_i^-$ とすれば、任意の $\rho>0$ に対し各側の係数を $\rho\theta_i^+,\rho\theta_i^-$ と置くことで双対実行可能な $\alpha$ を構成できます。従って $(\rho,p,q)$ による書き換えは双対実行可能集合を失いません。
...
[最大マージンと凸包間最短距離の対応](#thm-rkhs4-convex-hull-distance)より
