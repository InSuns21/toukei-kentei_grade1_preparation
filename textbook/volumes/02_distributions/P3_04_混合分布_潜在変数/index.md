...
混合分布では、観測値 $X$ の背後に「どの成分から発生したか」という見えない変数を置く。成分を表す確率変数を $Z$ とすると、$Z$ を固定した条件付き分布は単純でも、$Z$ を観測しないと複数の分布が混ざった周辺分布になる。

典型的には次の順で計算する。

1. 成分ごとの条件付き分布を書く。
2. 成分を足し合わせる、または潜在変数を積分して周辺分布を求める。
3. 全期待値・全分散で平均と分散を求める。
4. 観測後はベイズの定理で成分確率を更新する。
5. 潜在成分が見える場合と見えない場合で尤度の形がどう変わるかを確認する。
