...
主要な連続分布を密度や特性値の暗記表としてではなく、生成機構・台・母数・正規化・モーメントを一つにつなげて学びます。待ち時間、割合、寿命、対数変換など、問題設定から分布を選ぶことを出発点にします。
...
演習では、**確率密度関数・累積分布関数・生存関数などが計算の出発点として必要で、それ自体を導出させる問題でない場合は、問題文に具体式を与えます。** 一方、その式の導出自体や分布同定が採点対象なら、答えを先に与えないため問題文には置きません。
