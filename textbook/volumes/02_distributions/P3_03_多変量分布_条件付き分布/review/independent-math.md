...
- 分散共分散行列について、対称性と $\boldsymbol a^{\mathsf T}\Sigma\boldsymbol a=\operatorname{Var}(\boldsymbol a^{\mathsf T}\boldsymbol X)\geq0$ を確認した。特異共分散を許す多変量正規の定義と、正定値の場合だけLebesgue密度を持つという区別は正しい。
- アフィン変換の平均 $A\boldsymbol\mu+\boldsymbol b$、共分散 $A\Sigma A^{\mathsf T}$、多変量正規モーメント母関数、周辺分布を再導出した。
...
- 根拠・独立計算: P3-02で得た標準正規モーメント母関数 $M_Z(t)=e^{t^2/2}$ を4回微分して $M_Z^{(4)}(0)=3$、または積分漸化式から $E[Z^4]=3E[Z^2]=3$ と得る。従って $\operatorname{Var}(Z_i^2)=3-1=2$、独立和の分散は $2p$ となる。
- 修正案: 詳細解答にモーメント母関数微分または正規積分の漸化式を1〜2行追加して $E[Z_i^4]=3$ を導く。本番答案は圧縮形のままでよい。
...
- P3M-MATH-004: **解消**。P3M-C04詳細解答に、標準正規モーメント母関数 $M_Z(t)=e^{t^2/2}$ の4階微分から $E[Z_i^4]=M_Z^{(4)}(0)=3$ を得る根拠が追加され、$\operatorname{Var}(Z_i^2)=2$、独立和の分散 $2p$ まで接続された。

### 全文再計算結果

- 特異共分散を許す多変量正規の定義と正定値の場合の密度、密度正規化、モーメント母関数、アフィン変換、周辺分布を再確認し、新規指摘はない。
...
- 章全文、全14演習・答案・P3M-DRILL-01を再計算した。多変量正規密度、特異/正定値の区別、条件付き公式、Schur補、白色化、Mahalanobis二次形式は改訂後も正しい。Lebesgue/a.e./jointly normalの平易化も数学的内容を維持している。

### ENTRY-MATH-P3M-001 — minor

- 場所: `00_overview.md` 前提知識チェック。
- 根拠: モーメント母関数が日本語正式名・英語・定義式より先に略語だけで現れ、正式導入は `03_theorems.md` まで後置される。
- 修正案: overviewを「モーメント母関数」へ置換するか、その場で正式名・英語・略語・定義式を示す。
...
- ENTRY-MATH-P3M-001: **解消**。overviewは「モーメント母関数」とし、03で英語・略語・定義式を導入する順序になった。
- P3M-DRILL-01には$N_2$の平均次元・正定値共分散・密度式が追加され、与えた行列の主小行列式$4,32$も再計算と一致する。
- 多変量正規の正規化、特異/正定値、条件付き公式、Schur補、白色化、全14演習・答案・ドリルを再確認し、新規指摘なし。
- 最終件数: `fatal: 0 / major: 0 / minor: 0`。**承認**。

### validator行単位契約強化後の影響確認

- `/root/f0_math_review`、2026-08-11T11:16:39+09:00。共通索引・単変量表の検査強化は多変量正規の定義・条件付き公式・ドリルを変えない。P3M固有03/06/08契約も成功し、新規指摘なし。最終0/0/0、承認維持。
