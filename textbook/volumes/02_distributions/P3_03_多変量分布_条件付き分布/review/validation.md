...
- [x] `二乗可積分`・`精度行列`など未修語を前提にせず、F0-01で既習の`正定値`・`半正定値`・`Schur補`は前提章リンク付きで使用する構成へ変更した
...
- 当初D01・30分ドリルから`Schur補`や正定値性の名称を外したが、その後F0-01で既習であることと統計数理100で継続利用していることを確認し、名称を使える方針へ再調整した。名称暗記だけを採点対象にはしない。
- 通常教材スタイルガイドは、数学監査を理由に未修の専門語を主線へ追加しない一方、前提章で既習の専門語は通常語彙として利用する規則へ再調整した。
- 最終読解監査で、$A\Sigma A^{\mathsf T}$ の説明にあった「行列式」の誤記を「行列による表記」へ修正し、偏相関の共分散・分散展開を1項ずつ明示した。
...
- 本節の最終記録コミットを通常CIとGitHub Pages公開確認の対象とする。
