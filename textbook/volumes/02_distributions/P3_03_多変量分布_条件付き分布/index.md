...
二変量の具体例から平均ベクトル・分散共分散行列・条件付き分布の意味をつかみ、その後に多変量の一般式へ広げます。線形変換、条件付き正規分布、偏相関、二次形式を同じ行列表記でつなげます。
...
$$
<a id="def-p3-03-correlation-matrix"></a>
...
独立なら共分散0になりますが、逆は一般には成り立ちません。本教材では、確率変数の独立を $X\perp Y$、$Z$ を与えた下での条件付き独立を $X\perp Y\mid Z$ と書きます。
...
演習では、**初見でも記号や分布公式を推測せず立式できるようにします。**分布の確率密度関数や条件付き公式が計算の出発点で、それ自体を導出させない場合は問題文付近に与えます。逆に、公式の導出や分布同定そのものが採点対象なら先に答えを与えません。各大問の採点基準は20点満点です。
...
$R_1=X_1-0.5X_3$, $R_2=X_2-0.6X_3$。残差共分散は$0.3-0.5\cdot0.6=0$、残差分散は$0.75,0.64$なので$\rho_{12\cdot3}=0$。三変量正規なら条件付き分布も正規で交差共分散0ゆえ、$X_1\perp X_2\mid X_3$。
