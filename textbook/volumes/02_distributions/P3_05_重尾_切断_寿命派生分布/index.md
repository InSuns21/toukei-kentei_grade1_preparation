...
この章では、裾の減衰が遅いPareto分布、左右対称なLaplace分布、Rayleigh分布を扱う。さらに、寿命データで重要な切断・打切り、生存関数、ハザード、累積ハザード、平均残存寿命を一つの流れとして整理する。

重要なのは、分布名ごとの公式を並べることではない。次の対応を計算から使えるようにすることである。

1. 尾確率からモーメントの存在条件を調べる。
2. 切断では条件付き分布として密度を正規化する。
3. 打切りでは「故障した」と「その時刻まで故障しなかった」を別の尤度寄与として扱う。
4. 生存関数から密度、ハザード、累積ハザード、平均残存寿命を導く。
...
1. 台と母数範囲を確認する。
2. Pareto分布では尾確率を先に求め、べき積分の収束条件を見る。
...
確率変数 $X$ は $x_m=2,\alpha=3$ のPareto分布に従い、
...
$x_m>0,\alpha>0$ のPareto分布に従う。平均が有限となる条件と、分散が有限となる条件をそれぞれ求めよ。
...
$x_m>0,\alpha>0$ のPareto分布に従う。$r>0$ とする。
...
$x_m>0$ のPareto分布に従う。$\alpha>0$ は既知、$x_m$ は未知とし、
...
よって $M$ はPareto$(x_m,n\alpha)$ 型であり、$n\alpha>1$ のとき
...
### P3T-D01 Weibull寿命と右打切り

$X_1,\ldots,X_n$ は独立で、形状母数 $c>0$ が既知、尺度母数 $\eta>0$ が未知のWeibull分布に従う。生存関数と密度は
...
## P3T-DRILL-01 Weibull分布と右打切り

寿命 $X_i$ は形状母数 $c=2$、尺度母数 $\eta>0$ のWeibull分布に従い、
...
寿命分布では、生存関数・ハザードを求めるだけで終わらず、右打切り尤度や尺度母数の推定へ続く形が重要である。またPareto分布では、最小値の分布と台に依存する尤度を組み合わせる問題につながる。
