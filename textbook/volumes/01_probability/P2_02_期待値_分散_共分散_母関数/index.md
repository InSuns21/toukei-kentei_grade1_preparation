...
$H$ を $P(H)>0$ を満たす事象とします。$X$ の条件付き分布 $P(X\in A\mid H)$ に関する期待値を $E[X\mid H]$ と書きます。条件付き二次モーメントが有限なら
...
と定義します。本章では有限分割 $H_1,\ldots,H_m$ の各事象を条件とする数値として扱います。
...
$X$ を可積分な実数値確率変数とします。$H_1,\ldots,H_m$ を、互いに排反で全事象を分割し、各 $P(H_i)>0$ を満たす事象とします。このとき
...
$X$ を $E[X^2]<\infty$ を満たす実数値確率変数とし、$H_1,\ldots,H_m$ を互いに排反で全事象を分割する事象、各 $P(H_i)>0$ とします。
