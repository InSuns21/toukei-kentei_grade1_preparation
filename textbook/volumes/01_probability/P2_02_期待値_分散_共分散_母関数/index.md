...
期待値・分散・共分散を定義から計算し、条件付きモーメントを使って周辺の量へ戻す方法を学びます。確率母関数・モーメント母関数も完成形から始めず、分布の定義式へ代入して作ります。
...
また、実際の統計モデルでは「群・状態・潜在変数などを知ったときには簡単な分布になるが、それを平均して周辺分布を見ると複雑になる」という状況が頻繁に現れます。例えば理工80のポアソン混合では、工程率 $\Lambda$ を固定すれば $Y\mid\Lambda$ は単純なポアソン分布ですが、$\Lambda$ 自身が変動するため周辺分散には工程間変動が上乗せされます。このような計算の共通言語が、条件付き期待値・条件付き分散・条件付き共分散です。
...
平均と分散だけでは、分布の左右非対称性や裾の重さ、単位に依存しない相対的なばらつきは分かりません。また中央値を一般化すると、パーセント点・四分位数・四分位範囲を同じ累積分布関数から定義できます。

<a id="def-p2-02-shape-characteristics"></a>

<!-- formal-statement-start -->
> **定義（歪度・尖度・変動係数）**  
> $E[X]=\mu$、$\operatorname{Var}(X)=\sigma^2$、$0<\sigma<\infty$ とする。三次・四次中心モーメントが有限なら、歪度と尖度を
>
> $\gamma_1=E[(X-\mu)^3]/\sigma^3$、
> $\beta_2=E[(X-\mu)^4]/\sigma^4$
>
> とする。正規分布の尖度は3なので、$\beta_2-3$ を超過尖度と呼ぶ。正の尺度で平均 $\mu>0$ をもつ量では、変動係数を $CV=\sigma/\mu$ とする。
<!-- formal-statement-end -->

<!-- definition-example-start: def-p2-02-shape-characteristics -->
**定義の確認**  
$X\sim N(\mu,\sigma^2)$ では対称性から歪度は0、尖度は3、超過尖度は0です。平均100、標準偏差15の正の測定量なら変動係数は $15/100=0.15$ で、単位を同じ倍率で変えても値は変わりません。
<!-- definition-example-end -->

<a id="def-p2-02-quantiles"></a>
...
**定義の確認**  
標準正規分布では $Q_2=0$、$Q_1\approx-0.674$、$Q_3\approx0.674$ なので、四分位範囲は約1.349です。離散分布でも $\inf\{x:F(x)\ge p\}$ を使えば分位点を一意の規則で選べます。
<!-- definition-example-end -->
...
平均10・標準偏差2の測定量Aと、平均100・標準偏差2の測定量Bでは標準偏差は同じですが、変動係数はそれぞれ0.20と0.02です。変動係数は「平均の大きさに対して、ばらつきが何割か」を比較したい場面で使います。
...
<a id="ex-p2-02-syllabus-characteristics"></a>
### P202-B05 歪度・尖度・変動係数と四分位範囲

- Level: B
- 目安時間: 12分
- 主題: 分布の特徴量

ある正の測定量は平均40、標準偏差8である。また、その第1四分位数と第3四分位数は32と46である。

1. 変動係数を求めよ。
2. 四分位範囲を求めよ。
3. 歪度が正であることは、平均と中央値の大小関係を必ず一意に決めるか述べよ。
4. 正規分布の尖度と超過尖度を答えよ。
...
1. $CV=8/40=0.20$。
2. $IQR=46-32=14$。
3. 決めない。歪度は標準化された三次中心モーメントであり、平均と中央値の大小だけから一般に符号を完全には決められない。
4. 尖度は3、超過尖度は0。

##### 本番答案

$CV=0.20$、$IQR=14$。歪度の符号だけで平均と中央値の大小は一般には決まらない。正規分布の尖度は3、超過尖度は0。

##### 採点基準

変動係数5点、四分位範囲5点、歪度の解釈5点、尖度5点。計20点。
