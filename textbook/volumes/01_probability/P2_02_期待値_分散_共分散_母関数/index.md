...
この章では、期待値・分散・共分散を**定義から計算する力**と、確率母関数・モーメント母関数を**分布から自分で作って使う力**を身につけます。母関数の公式を暗記して代入するのではなく、定義式から和・積分を立て、有限となる範囲を確認してから微分します。
...
です。正の部分と負の部分がともに無限大となる場合に、形式的な相殺で有限値を作ってはいけません。

一般の関数 $g$ についても、$E[|g(X)|]<\infty$ なら $E[g(X)]$ を同様に定義します。

### 2.2 条件付き期待値・条件付き分散

$P(H)>0$ の事象 $H$ に対し、条件付き分布 $P(X\in A\mid H)$ に関する期待値を $E[X\mid H]$ と書きます。条件付き二次モーメントが有限なら

$$
\operatorname{Var}(X\mid H)
=E\left[(X-E[X\mid H])^2\mid H\right]
$$

です。本章では有限分割 $H_1,\ldots,H_m$ の各事象を条件とする数値として扱います。
...
$X\in\mathbb N_0=\{0,1,2,\ldots\}$ に対し
...
を確率母関数と呼びます。$|s|\leq1$ では $|s|^X\leq1$ なので必ず有限で、特に

$$
G_X(1)=\sum_{k=0}^{\infty}P(X=k)=1
$$
...
$E[|g(X)|]<\infty$ なら、$g(X)$ の分布を先に求めなくても
...
$\mu=E[X]$, $\nu=E[Y]$ とします。二次モーメントが有限なら $2|XY|\leq X^2+Y^2$ から $E[|XY|]<\infty$ です。よって
...
特に $X,Y$ が独立なら共分散項が消えます。独立な $X_1,\ldots,X_n$ について
...
$H_1,\ldots,H_m$ を $P(H_i)>0$ の有限分割とします。各群で期待値が存在するとき

$$
\boxed{
E[X]=\sum_{i=1}^m E[X\mid H_i]P(H_i)
}.
$$

これは各群での平均を、その群へ入る確率で重み付けしたものです。

### 3.6 有限分割に対する全分散

二次モーメントが有限とします。$\mu_i=E[X\mid H_i]$, $\mu=E[X]$ とおくと、$H_i$ の下で
...
E[(X-\mu)^2\mid H_i]
&=E[(X-\mu_i)^2\mid H_i]\\
&\quad+2(\mu_i-\mu)E[X-\mu_i\mid H_i]\\
&\quad+(\mu_i-\mu)^2.
\end{aligned}
$$
...
なので交差項が消えます。各群を $P(H_i)$ で重み付けして足すと
...
モーメント母関数が0を含む開区間で有限なら、その区間で微分でき
...
$P(H_1)=P(H_2)=1/2$、群平均が0と2、群内分散がともに1とします。まず
...
(3) $H=0$ なら $I=0$ なので $XI=0$、$H=1$ なら $I=1$ なので $XI=X$ です。したがって
...
P2-DRILL-02では、全期待値・全分散で得た量を群指標との共分散へ再利用します。復習時には混合比を未知数 $p$ として、$E[X]$ から $p$ を解くモーメント推定までつなげます。
...
3. 全分散の群内項と群間項がどこから出るか。
4. $G_X'(1-)$ が平均、$G_X''(1-)$ が階乗モーメントになる理由。
5. モーメント母関数を、分布の定義から自力で作れること。
6. 独立和の母関数が積になる箇所で、どこに独立性を使うか。
7. 広義積分からモーメントの存在範囲を判定できること。
8. 相関係数の絶対値が1以下となることを分散の非負性から示せること。
