...
| 公式範囲 | 本章の内容 |
|---|---|
| 確率の計算 | 補集合、差、加法公式、有限等確率モデル、数え上げ |
| 包除原理 | 二事象・三事象、ちょうど何個起こるか |

統計的独立、条件付き確率、Bayesの定理は P1-02 で扱います。

## 前提知識チェック

数学の記号や集合演算に不安がある場合は [F0-00 統計検定1級のための数学速習](../../00_foundations/F0_00_統計検定1級のための数学速習/index.md) を参照します。

1. $A\Longrightarrow B$ と $A\Longleftrightarrow B$ の違いを述べられる。
2. 有限和 $\sum_{k=1}^n k$ を計算できる。
3. 単調増加する有界数列が収束することを理解している。
4. 「全ての」と「少なくとも一つ」の否定を書ける。
